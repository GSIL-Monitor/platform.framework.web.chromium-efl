...
using resolution_type = std::underlying_type<base::TimeDelta::Resolution>::type;

void ParamTraits<base::TimeDelta>::Write(base::Pickle* m, const param_type& p) {
  ParamTraits<int64_t>::Write(m, p.ToInternalValue());
  ParamTraits<resolution_type>::Write(
      m, static_cast<resolution_type>(p.GetResolution()));
}
...
bool ParamTraits<base::TimeDelta>::Read(const base::Pickle* m,
  ...
  ret &= ParamTraits<resolution_type>::Read(m, iter, &resolution);
  if (ret) {
    *r = base::TimeDelta::FromInternalValue(value);
    r->SetResolution(static_cast<base::TimeDelta::Resolution>(resolution));
  }
...
void ParamTraits<base::TimeDelta>::Log(const param_type& p, std::string* l) {
  ParamTraits<int64_t>::Log(p.ToInternalValue(), l);
  ParamTraits<resolution_type>::Log(
      static_cast<resolution_type>(p.GetResolution()), l);
}
