...
def _CommonChecks(input_api, output_api):
  ...
  class ScopedSysPathHijacker(object):
    ...
    def __enter__(self):
      import copy
      self._saved_sys_path = copy.copy(sys.path)
      sys.path = sys.path + [self.dir]

    def __exit__(self, type, value, traceback):
      sys.path = self._saved_sys_path

  srcdir = input_api.PresubmitLocalPath()
  with ScopedSysPathHijacker(input_api.os_path.join(
      srcdir, 'tools', 'python', 'samsung')):
    import internal_presubmit

  results = []
  results.extend(internal_presubmit.CommonChecks(
      input_api, output_api,
      excluded_paths=_EXCLUDED_PATHS))
  results.extend(_CommonChromiumChecks(input_api, output_api))
  return results


def _CommonChromiumChecks(input_api, output_api):
  results = []
  results.extend(
...
def CheckChangeOnCommit(input_api, output_api):
  # Same as CheckChangeOnUpload because we do not want to run dcommit hooks.
