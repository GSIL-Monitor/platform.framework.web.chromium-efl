 ...
 * UDF #0x10: trap.
 *
 * When ARM processor is not running in debug mode then using BKPT
 * instruction causes "Unhandled prefetch abort: debug event (0x002)"
 * and SIGBUS generation instead of SIGTRAP. So change breakpoint
 * instruction to one which is used by gdb on ARM.
 */
