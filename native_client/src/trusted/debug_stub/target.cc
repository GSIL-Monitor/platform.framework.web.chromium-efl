...
bool Target::IsUserBreakpoint(uint32_t user_address, uint32_t instruction) {
#if defined(OS_TIZEN_TV_PRODUCT)
  // It seems that currently whenever we put a breakpoint on bundle in gdb,
  // it is being removed and added again in every step (on gdb request).
  //
  // It causes the following situation:
  //   * We need to add breakpoint for next step to simulate step debugging.
  //   * bundle_head is a breakpoint.
  //   * nacl_arm_dec::IsBreakPointAndConstantPoolHead check returns true.
  //
  // The above situation doesn't happen if we remove breakpoint
  // for bundle head in gdb, but it doesn't seem to be universal solution.
  //
  // Following check was added the to ensure, that even when we face the
  // breakpoint on bundle_head, we should also check if breakpoint for
  // next address exists and if there is none,
  // we should continue instead of returning with error.
  return (nacl_arm_dec::Instruction(instruction).Bits(31, 0) ==
          NACL_INSTR_ARM_BREAKPOINT) &&
         (breakpoint_map_.find(user_address) == breakpoint_map_.end());
#else
  UNREFERENCED_PARAMETER(user_address);
...
bool Target::AddBreakpoint(uint32_t user_address) {
  ...
  if (nacl_arm_dec::IsBreakPointAndConstantPoolHead(bundle_head) &&
      !IsUserBreakpoint(user_address, bundle_head)) {
    NaClLog(LOG_ERROR,
...
void Target::AddStepBreakPoint(port::Thread* thread) {
  if (step_breakpoint_addr_ != 0) {
    NaClLog(LOG_ERROR, "Step breakpoint already exists\n");
    return;
  }

  uint32_t next_pc = NaClNextInstructionAddr(thread);
  if (next_pc == 0) {
    NaClLog(LOG_WARNING, "Next PC is 0\n");
    return;
  }

  uint32_t user_next_pc = NaClSysToUser(thread->GetAppThread()->nap, next_pc);

  if (AddBreakpoint(user_next_pc)) {
    step_breakpoint_addr_ = user_next_pc;
  } else {
    NaClLog(
        LOG_WARNING,
        "Adding step breakpoint failed - maybe breakpoint already exists?\n");
  }
}

void Target::RemoveStepBreakPoint() {
  if (step_breakpoint_addr_ != 0) {
    if (!RemoveBreakpoint(step_breakpoint_addr_)) {
      NaClLog(LOG_FATAL, "RemoveStepBreakPoint: Failed to remove breakpoint\n");
    }
    step_breakpoint_addr_ = 0;
  }
}
...
bool Target::ProcessPacket(Packet *pktIn, Packet *pktOut) {
  ...
  switch (cmd) {
    ...
    case 's': {
      ...
      if (thread) {
        if (!thread->SetStep(true))
          AddStepBreakPoint(thread);
      }
      return true;
    }
      ...
      if (strncmp(str, "Cont", 4) == 0) {
        ...
        if (strncmp(subcommand, ";s:", 3) == 0) {
          ...
          if (*end == 0) {
            ...
            // TODO(a.bujalski) Investigate why this doesn't work on ARM
            if (NACL_ARCH(NACL_BUILD_ARCH) != NACL_arm)
          ...
          if (!it->second->SetStep(true))
            AddStepBreakPoint(it->second);
