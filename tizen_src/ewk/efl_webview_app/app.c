...
static void __auth_challenge_cb(Evas_Object* ewk_view, Ewk_Auth_Challenge* auth_challenge, void* user_data);
...
static void on_key_down(void *data, Evas *e, Evas_Object *obj, void *event_info)
{
    ...
    if (strcmp(ev->key, "XF86Back") == 0 || strcmp(ev->key, "XF86Stop") == 0)
    {
    ...
    else if (!strcmp(ev->key, "BackSpace") || !strcmp(ev->key, "XF86Back") ||
        !strcmp(ev->key, "XF86Stop")) {
        ewk_view_back(data);
    }
}
...
int main(int argc, char** argv)
{
...
#if defined(OS_TIZEN)
  // we should always go fullscreen on device.
  ecore_evas_fullscreen_set(ee, EINA_TRUE);
#else
  if (fullscreen_enabled) {
    // When in fullscreen, we don't need to manually specify the dimensions of
    // EwkView.
    ecore_evas_fullscreen_set(ee, EINA_TRUE);
  } else {
  ...
  ewk_view_authentication_callback_set(view, __auth_challenge_cb, NULL);
...
void __load_finished_cb(void* data, Evas_Object* obj, void* event_info) {
  printf("APP.C callback called __load_finsihed_cb \n");
  ...
  if (test_cookie) {
    ...
    ewk_cookie_manager_accept_policy_async_get(
        ...
        __ewk_cookie_manager_async_policy_get_cb, NULL);
  }
...
void __auth_challenge_cb(Evas_Object* ewk_view, Ewk_Auth_Challenge* auth_challenge, void* user_data)
{
...
{
  printf("new download start callback is triggered:%s\n", download_url);
}
...
Eina_Bool __mime_override_cb(const char* url, const char *mime, char **new_mime, void* user_data)
{
  printf("APP.C callback %s, url: %s, mime: %s\n", __func__, url, mime);
  /* Example:
...
void request_application_permission(const char* privilege) {
  ...
  switch (result) {
    case PRIVACY_PRIVILEGE_MANAGER_CHECK_RESULT_ALLOW:
      printf("%s privilege was allowed.\n", privilege);
      break;
    case PRIVACY_PRIVILEGE_MANAGER_CHECK_RESULT_DENY:
      printf("%s privilege was denied.\n", privilege);
      break;
...
void __app_request_response_cb(ppm_call_cause_e cause,
  ...
  switch (result) {
    case PRIVACY_PRIVILEGE_MANAGER_REQUEST_RESULT_ALLOW_FOREVER:
      printf("%s privilege was allowed forever.\n", privilege);
      break;
    case PRIVACY_PRIVILEGE_MANAGER_REQUEST_RESULT_DENY_FOREVER:
      printf("%s privilege was denied forever.\n", privilege);
      break;
