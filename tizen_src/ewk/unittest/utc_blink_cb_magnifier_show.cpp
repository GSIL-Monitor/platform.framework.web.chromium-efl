...
public:
 ...
 void LoadFinished(Evas_Object* webview) override {
   EventLoopStop(utc_blink_ewk_base::Success);
    ...
    {
      utc_blink_cb_magnifier_show* owner =
        static_cast<utc_blink_cb_magnifier_show*>(data);
      ecore_timer_add(1.0, do_text_selection_down, owner);
    }

 protected:
  utc_blink_cb_magnifier_show()
...
TEST_F(utc_blink_cb_magnifier_show, callback)
{
    ...
    SetTestJob(utc_blink_cb_magnifier_show::job_do_text_selection_down);
    ASSERT_EQ(Success, EventLoopStart());
}
