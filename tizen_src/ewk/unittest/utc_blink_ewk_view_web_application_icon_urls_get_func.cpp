...
protected:
 ...
 static void webAppUrlsGetCallback(
     Eina_List* urls,
     utc_blink_ewk_view_web_application_icon_urls_get* owner) {
   ...
   EINA_LIST_FOREACH(urls, l, d) {
     const char* size = ewk_web_application_icon_data_size_get(
         static_cast<Ewk_Web_App_Icon_Data*>(d));
     ASSERT_TRUE(NULL != size);
     const char* url = ewk_web_application_icon_data_url_get(
         static_cast<Ewk_Web_App_Icon_Data*>(d));
