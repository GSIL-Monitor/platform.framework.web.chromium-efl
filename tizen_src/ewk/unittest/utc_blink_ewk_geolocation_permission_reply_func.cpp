...
protected:
 void PostSetUp() override {
   ...
   evas_object_smart_callback_add(
       GetEwkWebView(), "geolocation,permission,request",
  ...
  void ConsoleMessage(Evas_Object*, const Ewk_Console_Message* msg) override {
...
TEST_F(utc_blink_ewk_geolocation_permission_reply, True)
{
  ...
  ASSERT_TRUE(title);
  EXPECT_STRNE("EWK Geolocation TIMEOUT", title)
      << "\e[43mIs GPS enabled and receives a signal?\e[0m";
#ifdef OS_TIZEN
  ASSERT_STREQ("EWK Geolocation SUCCESS", title);
#else
  ASSERT_STREQ("EWK Geolocation POSITION_UNAVAILABLE", title);
#endif
}
