...
TEST_F(utc_blink_ewk_context_web_storage_delete_all, POS_TEST)
{
  ...
  // Local storage item may be set in renderer cache but setting it in physical
  // memory may be delayed. ewk_context_web_storage_delete_all removes all data
  // of origins present in physical memory. In case of no line below, item may
  // not be saved in physical memory before ewk_context_web_storage_delete_all
  // is called.
  EventLoopWait(6.0);
