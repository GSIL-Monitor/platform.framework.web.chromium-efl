...
protected:
 void PostSetUp() override {
   evas_object_smart_callback_add(GetEwkWebView(), "url,changed", url_changed,
                                  this);
  }
...
TEST_F(utc_blink_cb_url_changed, NoRefresh)
{
  ...
  EXPECT_EQ(Success,
            EventLoopStart());  // Wait for "url,changed" to .../sample.html
  ASSERT_TRUE(ewk_view_url_set( GetEwkWebView(), GetResourceUrl("common/sample_1.html").c_str()));
  EXPECT_EQ(Timeout,
            EventLoopStart(5));  // Check for not redirect to .../sample.html
}
