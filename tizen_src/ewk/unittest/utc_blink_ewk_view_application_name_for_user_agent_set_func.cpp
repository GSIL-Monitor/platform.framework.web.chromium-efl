...
TEST_F(utc_blink_ewk_view_application_name_for_user_agent_set, EmptyString)
{
  ...
  // if set empty string then assign PRODUCT_NAME (version_info_efl.h)
  EXPECT_STREQ("efl-webengine", ewk_view_application_name_for_user_agent_get(GetEwkWebView()));
}

TEST_F(utc_blink_ewk_view_application_name_for_user_agent_set, NullString)
{
  EXPECT_EQ(EINA_TRUE, ewk_view_application_name_for_user_agent_set(GetEwkWebView(), NULL));
  // if set NULL as string then assign PRODUCT_NAME (version_info_efl.h)
  EXPECT_STREQ("efl-webengine", ewk_view_application_name_for_user_agent_get(GetEwkWebView()));
}
