...
TEST_F(utc_blink_ewk_view_scale_get, POS_TEST)
{
  ...
  ASSERT_GT(std::abs(maxScale - minScale), precision);
  ...
  ASSERT_EQ(Success, EventLoopStart());
  currentScale = ewk_view_scale_get(GetEwkWebView());
  ASSERT_LT(std::abs(minScale - currentScale), precision);
}
...
TEST_F(utc_blink_ewk_view_scale_get, NEG_TEST)
{
  ...
  ASSERT_LT(std::abs(getScaleFactor + 1), precision);
}
