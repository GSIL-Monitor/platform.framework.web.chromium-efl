...
public class ApplicationStatus {
    private static class ActivityInfo {
        ...
        public ActivityInfo(int taskId) {
            mTaskId = taskId;
        }

        /**
         * @return The task id of the activity.
         */
    ...
    public static void initialize(BaseChromiumApplication application) {
        Log.i(TAG, "initialize application (" + application + ")");
    ...
    private static void onStateChange(Activity activity, @ActivityState int newState) {
        Log.i(TAG, "onStateChange activity = " + activity + " newState = " + newState);
        ...
        if (newState == ActivityState.CREATED) {
            ...
            sActivityInfo.put(activity, new ActivityInfo(activity.getTaskId()));
            Log.i(TAG, "onStateChange put Activity Info");
        }
        ...
        if (newState == ActivityState.DESTROYED) {
            ...
            Log.i(TAG, "onStateChange remove Activity Info");
    ...
    /**
     * Query the task id for a given activity.
     *
     * @param activity The activity whose task id is to be returned.
     * @return The task id of the specified activity.
     */
    public static int getTaskIdForActivity(Activity activity) {
        ActivityInfo info = sActivityInfo.get(activity);
        return info != null ? info.getTaskId() : activity.getTaskId();
    }
    ...
    public static void registerStateListenerForActivity(ActivityStateListener listener,
        ...
        if (BuildInfo.isAtLeastO() && info == null && !activity.isDestroyed()) {
            info = new ActivityInfo(activity.getTaskId());
            sActivityInfo.put(activity, info);
        }
        assert info != null && info.getStatus() != ActivityState.DESTROYED;
        if (info == null) {
            Log.i(TAG, "registerStateListenerForActivity info is null!!");
            return;
        }
