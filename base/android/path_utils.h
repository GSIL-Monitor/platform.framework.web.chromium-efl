...
// Retrieves the path to the native JNI local libraries via
// ApplicationInfo.nativeLibraryDir on the Java side. The result is placed in
// the FilePath pointed to by 'result'.
BASE_EXPORT bool GetNativeLocalLibraryDirectory(base::FilePath* result);

// Retrieves the path to the apk directory via
// ApplicationInfo.sourceDir on the Java side. The result is placed in
// the FilePath pointed to by 'result'.
BASE_EXPORT bool GetLibraryApkPath(base::FilePath* result);
