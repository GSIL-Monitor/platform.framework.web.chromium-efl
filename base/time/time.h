...
class BASE_EXPORT TimeDelta {
 public:
  TimeDelta() : delta_(0), resolution_(Resolution::MICROSECONDS) {}

  enum class Resolution { MILLISECONDS = 1000, MICROSECONDS = 1 };

  using ComparableDeltas = std::pair<int64_t, int64_t>;
  ...
  TimeDelta RoundedToMilliseconds() const;
  ...
  TimeDelta operator+(TimeDelta other) const {
    return TimeDelta(time_internal::SaturatedAdd(*this, other.delta_),
                     std::max(resolution_, other.resolution_));
  }
  TimeDelta operator-(TimeDelta other) const {
  ...
  TimeDelta operator*(T a) const {
    ...
    rv *= a;
    if (rv.IsValid())
      return TimeDelta(rv.ValueOrDie(), resolution_);
    // Matched sign overflows. Mismatched sign underflows.
    if ((delta_ < 0) ^ (a < 0))
      return TimeDelta(std::numeric_limits<int64_t>::min(), resolution_);
    return TimeDelta(std::numeric_limits<int64_t>::max(), resolution_);
  }
  template<typename T>
  TimeDelta operator/(T a) const {
    CheckedNumeric<int64_t> rv(delta_);
  ...
  TimeDelta operator%(TimeDelta a) const {
    return TimeDelta(delta_ % a.delta_, resolution_);
  }

  // Comparison operators.
  bool operator==(TimeDelta other) const {
    ComparableDeltas deltas = GetComparableDeltas(other);
 ...
 private:
  ...
  constexpr explicit TimeDelta(int64_t delta_us,
      ...
      : delta_(delta_us), resolution_(resolution) {}
  ...
  TimeDelta RoundedTo(int64_t timescale) const;

  constexpr TimeDelta FlooredTo(int64_t timescale) const;

  TimeDelta::ComparableDeltas GetComparableDeltas(TimeDelta& other) const;

  constexpr TimeDelta FloorToResolution() const;
