...
TimeDelta TimeDelta::RoundedTo(int64_t timescale) const {
  if (is_max()) {
    // Preserve max to prevent overflow.
    return *this;
  }

  return TimeDelta::FromMicroseconds((delta_ / timescale) * timescale);
}

TimeDelta TimeDelta::RoundedToMilliseconds() const {
  return RoundedTo(Time::kMicrosecondsPerMillisecond);
}

constexpr TimeDelta TimeDelta::FlooredTo(int64_t timescale) const {
  return (is_max()
              ? *this
              : TimeDelta::FromMicroseconds((delta_ / timescale) * timescale));
}

TimeDelta::ComparableDeltas TimeDelta::GetComparableDeltas(
    TimeDelta& other) const {
  ComparableDeltas new_deltas(delta_, other.delta_);
  if (resolution_ != other.resolution_) {
    Resolution comparable_resolution = std::max(resolution_, other.resolution_);
    new_deltas.first = FlooredTo(static_cast<
                       std::underlying_type<base::TimeDelta::Resolution>::type>(
                       comparable_resolution)).delta_;
    new_deltas.second = other.FlooredTo(static_cast<
                        std::underlying_type<base::TimeDelta::Resolution>::type>
                        (comparable_resolution)).delta_;
  }
  return new_deltas;
...
constexpr TimeDelta TimeDelta::FloorToResolution() const {
  return FlooredTo(
      static_cast<std::underlying_type<base::TimeDelta::Resolution>::type>(
          resolution_));
}
