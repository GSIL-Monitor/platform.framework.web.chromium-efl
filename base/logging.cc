...
#if defined(OS_TIZEN) && !defined(OS_NACL)
void LogMessage::DlogLogMessage(LogSeverity severity, const char* message) {
  ...
  dlog_print(priority, LOG_TAG, "%s", message);
}
#endif  // defined(OS_TIZEN) && !defined(OS_NACL)
...
LogMessage::~LogMessage() {
  size_t stack_start = stream_.tellp();
    ...
    !defined(OS_AIX) && !defined(OS_TIZEN_PUBLIC)
...
#elif defined(OS_TIZEN) && !defined(OS_NACL)
    DlogLogMessage(severity_, str_newline.c_str());
#elif defined(OS_TIZEN_TV_PRODUCT) && defined(OS_NACL) && \
    !defined(OS_NACL_NONSFI)
    NaClLogMessageHandler(severity_, str_newline);
#else
