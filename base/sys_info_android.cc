...
std::string GetAndroidBuildPDA() {
  static char os_build_pda_str[PROP_VALUE_MAX];
  if (!os_build_pda_str[0]){
    __system_property_get("ro.build.PDA", os_build_pda_str);
  }
  return std::string(os_build_pda_str);
}
int getGetAndroidBuildPDASubstringIndex(const char* model){
  int model_length = strlen(model);
  int last_dash_index = -1;
  for (int i = model_length - 1; i >= 0; i--) {
    if (model[i] == '-') {
      last_dash_index = i + 1;
      break;
    }
  ...
  if (last_dash_index != -1) {
    return model_length - last_dash_index;
  }
  return model_length;
}
std::string GetCustomModelName(const char* op_code, const char* model) {
  std::string tmp_model = std::string(model);
  std::string prefix = "SAMSUNG ";
  std::string postfix = "";
  if ((strcmp(op_code, "KTT") == 0 )
          || (strcmp(op_code, "KTC") == 0 )
          || (strcmp(op_code, "TMO") == 0 )) {
    StringAppendF(&postfix, "/%s",
        GetAndroidBuildPDA().substr(
        getGetAndroidBuildPDASubstringIndex(model)).c_str());
  } else if (strcmp(op_code, "VODA") == 0) {
    StringAppendF(&postfix, "/%s", GetAndroidBuildPDA().c_str());
  } else if (strcmp(op_code, "KDO") == 0) {
    postfix = "-parrot";
  } else if (strcmp(op_code, "JPN") == 0 || (strcmp(op_code, "JP") == 0)) {
    prefix = "";
  } else if( (strcmp(op_code, "ATT") == 0)
          || (strcmp(op_code, "AIO") == 0)) {
    if (tmp_model.find("SAMSUNG-") != std::string::npos){
      tmp_model = tmp_model.substr(8);
    }
  } else if (strcmp(op_code, "CTC") == 0) {
    prefix = "SAMSUNG-";
  }
  return prefix + tmp_model + postfix;
}
...
std::string SysInfo::HardwareModelName() {
  ...
  // For GED devices, only return the original model name.
  if (cmd->HasSwitch(switches::kUseGEDFeature))
    return std::string(device_model_str);
  std::string custom_ua;
  if (cmd->HasSwitch(switches::kCscFeatureUserAgent)) {
      custom_ua =
          cmd->GetSwitchValueASCII(switches::kCscFeatureUserAgent);
  }
  if (custom_ua.c_str() == nullptr || custom_ua == "" ) {
    char countryiso_code[PROP_VALUE_MAX];
    __system_property_get("ro.csc.countryiso_code", countryiso_code);
    if (strcmp(countryiso_code, "JP") == 0) {
      return GetCustomModelName(countryiso_code, device_model_str);
    } else {
      char sales_code[PROP_VALUE_MAX];
      __system_property_get("ro.csc.sales_code", sales_code);
      return GetCustomModelName(sales_code, device_model_str);
    }
  } else {
    return GetCustomModelName(custom_ua.c_str(), device_model_str);
  }
#else
  return std::string(device_model_str);
#endif
