...
void MD5Hmac(unsigned char* key,
             int key_len,
             unsigned char* text,
             int text_len,
             MD5Digest* digest) {
  ...
  if (key_len > 64) {
    ...
    MD5Init(&tctx);
    MD5Update(&tctx, StringPiece(reinterpret_cast<const char*>(key), key_len));
    MD5Final(&tk, &tctx);
  ...
  MD5Init(&context); /* init context for 1st
                    * pass */
  MD5Update(&context, StringPiece(reinterpret_cast<const char*>(k_ipad),
                                  64)); /* start with inner pad */
  MD5Update(&context, StringPiece(reinterpret_cast<const char*>(text),
  ...
  MD5Init(&context);                          /* init context for 2nd
                                             * pass */
  MD5Update(&context, StringPiece(reinterpret_cast<const char*>(k_opad),
                                  64)); /* start with outer pad */
  MD5Update(&context, StringPiece(reinterpret_cast<const char*>(digest),
