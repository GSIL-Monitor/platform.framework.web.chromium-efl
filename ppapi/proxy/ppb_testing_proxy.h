...
class PPB_Testing_Samsung_Proxy : public InterfaceProxy {
 public:
  explicit PPB_Testing_Samsung_Proxy(Dispatcher* dispatcher);

  static const PPB_Testing_Samsung* GetProxyInterface();
 ...
 private:
  // Message handlers.
  void OnMsgLoseContext(PP_Instance instance);

  // When this proxy is in the host side, this value caches the interface
  // pointer so we don't have to retrieve it from the dispatcher each time.
  // In the plugin, this value is always NULL.
  const PPB_Testing_Samsung* ppb_testing_samsung_impl_;

  DISALLOW_COPY_AND_ASSIGN(PPB_Testing_Samsung_Proxy);
};
