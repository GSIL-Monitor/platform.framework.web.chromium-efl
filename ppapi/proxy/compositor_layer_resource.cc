...
int32_t CompositorLayerResource::SetColor(float red,
                                          ...
                                          const PP_Size* size) {
  if (!compositor_) {
    LOG(ERROR) << "Compositor is NULL";
    return PP_ERROR_BADRESOURCE;
  }

  if (compositor_->IsInProgress()) {
    LOG(ERROR) << "Drawing is in progress";
    return PP_ERROR_INPROGRESS;
  }

  if (!SetType(TYPE_COLOR)) {
    LOG(ERROR) << "SetType failed";
...
int32_t CompositorLayerResource::SetTexture(
  ...
  if (enter.failed()) {
    LOG(ERROR) << "Enter PPB_Graphics3D_API failed";
    return PP_ERROR_BADRESOURCE;
  }
  ...
  if (!size) {
    ...
    LOG(ERROR) << "Wrong size: " << size->width << "x" << size->height;
    return PP_ERROR_BADARGUMENT;
  }
...
int32_t CompositorLayerResource::SetImage(
  ...
  if (enter.failed()) {
    LOG(ERROR) << "Enter PPB_ImageData_API failed";
    return PP_ERROR_BADRESOURCE;
  }
  ...
  if (desc.size.width * 4 != desc.stride) {
    LOG(ERROR) << "Unsupported images which width * 4 != stride";
    return PP_ERROR_BADARGUMENT;
  }
...
#else
  if (desc.format != PP_IMAGEDATAFORMAT_BGRA_PREMUL) {
#endif
...
#if defined(TIZEN_PEPPER_EXTENSIONS)
int32_t CompositorLayerResource::SetBackgroundPlane(const PP_Size* size) {
  if (!compositor_) {
    LOG(ERROR) << "Compositor is NULL";
    return PP_ERROR_BADRESOURCE;
  }
  ...
  if (!SetType(TYPE_BACKGROUND_PLANE)) {
    LOG(ERROR) << "Invalid background plane type";
    return PP_ERROR_BADARGUMENT;
  }
...
int32_t CompositorLayerResource::SetPremultipliedAlpha(PP_Bool premult) {
  ...
  LOG(ERROR) << "SetPremultipliedAlpha can be called only for texture target";
  return PP_ERROR_BADARGUMENT;
}
...
#if defined(TIZEN_PEPPER_EXTENSIONS)
  if (type == TYPE_BACKGROUND_PLANE) {
    if (data_.is_null())
      data_.background_plane.reset(
          new CompositorLayerData::BackgroundPlaneLayer());
    return (data_.background_plane != nullptr);
  }
#endif
...
int32_t CompositorLayerResource::CheckForSetTextureAndImage(
  ...
  if (!release_callback_.is_null()) {
    LOG(ERROR) << "The layer's image has been set and it is not committed";
    return PP_ERROR_INPROGRESS;
  }
  ...
  if (release_callback->is_blocking()) {
    LOG(ERROR)
        << "It's not allowed using a block callback as a release callback";
    return PP_ERROR_BADARGUMENT;
  }
