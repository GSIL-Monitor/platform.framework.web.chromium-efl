...
#include "ppapi/c/samsung/ppb_testing_samsung.h"
#endif
...
void LoseContext(PP_Instance instance) {
  ...
  dispatcher->Send(new PpapiHostMsg_PPBTestingSamsung_LoseContext(
      API_ID_PPB_TESTING_SAMSUNG, instance));
}

const PPB_Testing_Samsung samsung_testing_interface = {
    &ReadImageData,
    &RunMessageLoop,
    &QuitMessageLoop,
    &GetLiveObjectsForInstance,
...
PPB_Testing_Samsung_Proxy::PPB_Testing_Samsung_Proxy(Dispatcher* dispatcher)
    : InterfaceProxy(dispatcher), ppb_testing_samsung_impl_(NULL) {
  if (!dispatcher->IsPlugin()) {
    ppb_testing_samsung_impl_ = static_cast<const PPB_Testing_Samsung*>(
        dispatcher->local_get_interface()(PPB_TESTING_SAMSUNG_INTERFACE));
  }
}

// static
const PPB_Testing_Samsung* PPB_Testing_Samsung_Proxy::GetProxyInterface() {
  return &samsung_testing_interface;
}

void PPB_Testing_Samsung_Proxy::OnMsgLoseContext(PP_Instance instance) {
  if (ppb_testing_samsung_impl_)
    ppb_testing_samsung_impl_->LoseContext(instance);
}

bool PPB_Testing_Samsung_Proxy::OnMessageReceived(const IPC::Message& msg) {
  if (!dispatcher()->permissions().HasPermission(PERMISSION_TESTING))
    return false;

  bool handled = true;
  IPC_BEGIN_MESSAGE_MAP(PPB_Testing_Samsung_Proxy, msg)
  IPC_MESSAGE_HANDLER(PpapiHostMsg_PPBTestingSamsung_LoseContext,
                      OnMsgLoseContext)
  IPC_MESSAGE_UNHANDLED(handled = false)
  IPC_END_MESSAGE_MAP()
  return handled;
