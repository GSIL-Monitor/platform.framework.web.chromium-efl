...
int32_t FlashFileResource::CreateTemporaryFile(PP_Instance /*instance*/,
                                               PP_FileHandle* file) {
  if (!file) {
    LOG(ERROR) << "Memory for file handle not allocated";
  ...
  if (!reply_params.TakeFileHandleAtIndex(0, &transit_file)) {
    LOG(ERROR) << "Cannot take file handle at index 0";
...
int32_t FlashFileResource::OpenFileHelper(const std::string& path,
                                          ...
                                          PP_FileHandle* file) {
  if (path.empty() ||
      !ppapi::PepperFileOpenFlagsToPlatformFileFlags(mode, NULL) || !file) {
    LOG(ERROR);
    return PP_ERROR_BADARGUMENT;
  }
...
int32_t FlashFileResource::QueryFileHelper(const std::string& path,
                                           ...
                                           PP_FileInfo* info) {
  if (path.empty() || !info) {
    LOG(ERROR) << "Path is empty or not allocated memory for PP_FileInfo";
    return PP_ERROR_BADARGUMENT;
  }
