...
PP_Resource ResourceCreationProxy::CreateMediaPlayer(
    PP_Instance instance,
    PP_MediaPlayerMode player_mode,
    PP_BindToInstanceMode bind_mode) {
  return MediaPlayerResource::Create(GetConnection(), instance, player_mode,
                                     bind_mode);
}

PP_Resource ResourceCreationProxy::CreateURLDataSource(PP_Instance instance,
                                                       const char* url) {
  return (new URLDataSourceResource(GetConnection(), instance, url))
      ->GetReference();
}

PP_Resource ResourceCreationProxy::CreateESDataSource(PP_Instance instance) {
  return (new ESDataSourceResource(GetConnection(), instance))->GetReference();
}

#endif  // defined(TIZEN_PEPPER_PLAYER)

#if defined(TIZEN_PEPPER_TEEC)
PP_Resource ResourceCreationProxy::CreateTEECContext(PP_Instance instance) {
  return (new TEECContextResource(GetConnection(), instance))->GetReference();
}

PP_Resource ResourceCreationProxy::CreateTEECSession(PP_Instance instance,
                                                     PP_Resource context) {
  return (new TEECSessionResource(GetConnection(), instance, context))
      ->GetReference();
}

PP_Resource ResourceCreationProxy::CreateTEECSharedMemory(PP_Instance instance,
                                                          PP_Resource context,
                                                          uint32_t flags) {
  return (new TEECSharedMemoryResource(GetConnection(), instance, context,
                                       flags))
      ->GetReference();
}
