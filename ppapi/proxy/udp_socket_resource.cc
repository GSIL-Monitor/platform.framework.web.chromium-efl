...
int32_t UDPSocketResource::Bind(PP_Resource addr,
  ...
  if (enter.failed()) {
    LOG(ERROR) << "Incorrect PP_Resource";
...
int32_t UDPSocketResource::SendTo(const char* buffer,
  ...
  EnterNetAddressNoLock enter(addr, true);
  if (enter.failed()) {
    LOG(ERROR) << "Enter address failed";
    return PP_ERROR_BADARGUMENT;
  }
...
int32_t UDPSocketResource::SetOption1_0(
  ...
  if (name > PP_UDPSOCKET_OPTION_RECV_BUFFER_SIZE) {
    LOG(ERROR) << "Wrong option name = " << static_cast<int>(name);
    return PP_ERROR_BADARGUMENT;
  }
...
int32_t UDPSocketResource::GetOption(PP_UDPSocket_Option name,
                                     PP_Var* value,
                                     scoped_refptr<TrackedCallback> callback) {
  return GetOptionImpl(name, value, callback);
}
#endif
