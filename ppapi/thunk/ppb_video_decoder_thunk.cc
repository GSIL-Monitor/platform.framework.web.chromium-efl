...
#if defined(TIZEN_PEPPER_EXTENSIONS)
int32_t SetLatencyMode(PP_Resource video_decoder, PP_VideoLatencyMode mode) {
  VLOG(4) << "PPB_VideoDecoderSamsung::SetLowLatencyMode()";
  EnterResource<PPB_VideoDecoder_API> enter(video_decoder, true);
  if (enter.failed())
    return enter.retval();
  return enter.object()->SetLatencyMode(mode);
}
#endif
...
#if defined(TIZEN_PEPPER_EXTENSIONS)
const PPB_VideoDecoder_Samsung_1_0 g_ppb_videodecoder_samsung_thunk_1_0 = {
    &IsVideoDecoder, &SetLatencyMode};
#endif
...
#if defined(TIZEN_PEPPER_EXTENSIONS)
PPAPI_THUNK_EXPORT const PPB_VideoDecoder_Samsung_1_0*
GetPPB_VideoDecoder_Samsung_1_0_Thunk() {
  return &g_ppb_videodecoder_samsung_thunk_1_0;
}
#endif
