...
struct PPAPI_SHARED_EXPORT CompositorLayerData {
  ...
  struct BackgroundPlaneLayer {
    BackgroundPlaneLayer() {}
  };
#endif
  ...
  bool is_null() const {
#if defined(TIZEN_PEPPER_EXTENSIONS)
    return !(color || texture || image || background_plane);
#else
...
#if defined(TIZEN_PEPPER_EXTENSIONS)
  std::unique_ptr<BackgroundPlaneLayer> background_plane;
#endif
};
