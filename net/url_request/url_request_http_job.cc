...
URLRequestJob* URLRequestHttpJob::Factory(URLRequest* request,
  ...
  if (!url.SchemeIsCryptographic()) {
    ...
    if (hsts && hsts->ShouldUpgradeToSSL(url.host())) {
      ...
      LOG(INFO) << "LoaderLog  MaybeInternallyRedirect"
          << "  url.scheme() = " << url.scheme().data()
          << "  ResponseCode : 307_TEMPORARY_REDIRECT"
          << "  redirect_reason : HSTS";
...
#ifdef TIZEN_DLP
  // Enabling DLP before first request will guarantee that faster response for
  // first requests
  static bool dlp_first_run = true;
