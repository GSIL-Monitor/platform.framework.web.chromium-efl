...
#ifdef TIZEN_DLP
  // Send data to Tizen DLP to verify request headers
  privacy_guard_dlp_check_leak(request_->url.HostNoBrackets().c_str(),
                               request_headers_.get()->data(),
                               bytes_remaining);
#endif
...
int HttpStreamParser::DoSendBody() {
  if (request_body_send_buf_->BytesRemaining() > 0) {
    ...
    privacy_guard_dlp_check_leak(request_->url.HostNoBrackets().c_str(),
                                 request_body_send_buf_.get()->data(),
