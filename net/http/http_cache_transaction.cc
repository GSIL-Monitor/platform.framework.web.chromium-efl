...
#if defined(USE_TTRACE)
    traceMark(TTRACE_TAG_WEB,
              "HttpCache::Transaction::DoCacheReadDataComplete %s",
              key().substr(0, 128).c_str());
#endif
...
int HttpCache::Transaction::WriteResponseInfoToEntry(bool truncated) {
  ...
  bool is_stream = mime_type.find("video") == 0 || mime_type.find("audio") == 0;
  if ((response_.headers->HasHeaderValue("cache-control", "no-store")) ||
      IsCertStatusError(response_.ssl_info.cert_status) || is_stream) {
#else
  if ((response_.headers->HasHeaderValue("cache-control", "no-store")) ||
