...
public class HttpNegotiateAuthenticator {
    ...
    private static HandlerThread sThread = new HandlerThread("HttpNegotiateAuthenticator");

    /**
     * This interface is used to handler kerbero authentication.
     */
    public interface KerberosAuthenticator {
         ...
         */
        String getNegotiateToken(final String principal, String authToken, boolean canDelegate);
    }

    private static KerberosAuthenticator sKerberosAuthenticator = null;

    public static void setKerberosAuthenticator(KerberosAuthenticator authenticator) {
        sKerberosAuthenticator = authenticator;
    }
    ...
    void getNextAuthToken(final long nativeResultObject, final String principal, String authToken,
        ...
        if (sKerberosAuthenticator != null) {
            final String tmpAuthToken = authToken;
            final boolean tmpCanDelegate = canDelegate;
            Handler handler = new Handler(sThread.getLooper());
            handler.post(new Runnable() {
                @Override
                public void run() {
                    String token = sKerberosAuthenticator.getNegotiateToken(
                            principal, tmpAuthToken, tmpCanDelegate);
                    nativeSetResult(nativeResultObject, (token != null)
                            ? 0 : NetError.ERR_UNSUPPORTED_AUTH_SCHEME, (token != null)
                            ? token : String.valueOf(NetError.ERR_UNSUPPORTED_AUTH_SCHEME));
                }
            });
            return;
        }
