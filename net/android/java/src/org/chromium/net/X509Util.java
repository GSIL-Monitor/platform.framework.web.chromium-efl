...
public class X509Util {
    ...
    /**
     * This interface checks for certificate
     */
    public interface CertificateErrorChecker {
        /*
         * return a value in CertVerifyStatusAndroid.java
         */
        int check(CertificateException e);
    }
    ...
    private static CertificateErrorChecker sCertificateErrorChecker;
    private static Class<? extends CertificateErrorChecker> sCertificateErrorCheckerType;

    private static CertificateErrorChecker getOrCreateCertificateErrorChecker() {
        if (sCertificateErrorChecker != null) return sCertificateErrorChecker;

        try {
            sCertificateErrorChecker = sCertificateErrorCheckerType.newInstance();
        } catch (NullPointerException | InstantiationException | IllegalAccessException e) {
            Log.e(TAG, "Failed to init certificate error checker: " + e.getMessage());
            return null;
        }

        return sCertificateErrorChecker;
    }

    public static void setCertificateErrorCheckerType(
            Class<? extends CertificateErrorChecker> checkerType) {
        sCertificateErrorCheckerType = checkerType;
    }
        ...
        synchronized (sLock) {
            ...
            try {
                ...
                try {
                    ...
                    // SAMSUNG : certificate_revocation start >>
                    if (getOrCreateCertificateErrorChecker() != null) {
                        return new AndroidCertVerifyResult(
                                sCertificateErrorChecker.check(eDefaultManager));
                    }
                    // SAMSUNG : certificate_revocation end <<
                    return new AndroidCertVerifyResult(
