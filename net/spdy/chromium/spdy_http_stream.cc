...
#include "dlp/privacy-guard-dlp.h"
#endif
...
#ifdef TIZEN_DLP
  std::string hostname = request_info_->url.HostNoBrackets();
  std::string text_dlp = request_info_->url.spec();

  // Tizen DLP checks request path
  privacy_guard_dlp_check_leak(hostname.c_str(),
                               (char *)text_dlp.c_str(),
                               text_dlp.length());

  // Tizen DLP checks request headers
  text_dlp = request_headers.ToString();
  privacy_guard_dlp_check_leak(hostname.c_str(),
                               (char *)text_dlp.c_str(),
                               text_dlp.length());
#endif
...
#ifdef TIZEN_DLP
  // Send data to Tizen DLP to verify request body
  privacy_guard_dlp_check_leak(request_info_->url.HostNoBrackets().c_str(),
                               (char *)request_body_buf_.get()->data(),
                               status);
#endif
