...
CookieMonster::CookieMonster(PersistentCookieStore* store,
    ...
    : initialized_(false),
      ...
      cookies_changed_callback_(nullptr),
      cookies_changed_callback_data_(nullptr),
#endif
...
#if defined(OS_TIZEN_TV_PRODUCT) || defined(TIZEN_DESKTOP_SUPPORT)
  if (initialized_ && store_.get() && cookies_changed_callback_) {
    LOG(INFO) << "Notify cookies changed, sync_to_store: " << sync_to_store
              << ", persistent session: " << persist_session_cookies_;
    store_->Flush(base::Closure());
    (*cookies_changed_callback_)(cookies_changed_callback_data_);
  }
#endif
...
#if defined(OS_TIZEN_TV_PRODUCT) || defined(TIZEN_DESKTOP_SUPPORT)
void CookieMonster::RegisterCookiesChangedCallback(
    Ewk_Cookie_Manager_Changes_Watch_Cb callback,
    void* data) {
  cookies_changed_callback_ = callback;
  cookies_changed_callback_data_ = data;
}
#endif
