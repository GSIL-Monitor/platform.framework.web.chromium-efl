...
bool CTLogVerifier::Init(const base::StringPiece& public_key) {
  ...
  if (!public_key_ || CBS_len(&cbs) != 0) {
    if (!public_key_) {
      LOG(ERROR) << "Init fail to parse public key";
    }

    if (CBS_len(&cbs) != 0) {
      LOG(ERROR) << "Init fail - have cbs remain len";
    }
    ...
    if (ERR_peek_error() != 0) {
      ...
      // const tracked_objects::Location& location = FROM_HERE;
  ...
  switch (EVP_PKEY_type(public_key_->type)) {
    ...
    case EVP_PKEY_EC:
      ...
      break;
    default:
      LOG(ERROR) << "Init fail - Unsupported key type: "
          << EVP_PKEY_type(public_key_->type);
      if (ERR_peek_error() != 0) {
        std::string message;
      // FIXME: Commented the code during dev/m63_3239 rebase
      // It needs to be re-looked by author.
  ...
  if (signature_algorithm_ == ct::DigitallySigned::SIG_ALGO_RSA &&
    ...
    LOG(ERROR) << "Init fail - Too small a public key.";
    if (ERR_peek_error() != 0) {
      std::string message;
      // FIXME: Commented the code during dev/m63_3239 rebase
      // It needs to be re-looked by author.
