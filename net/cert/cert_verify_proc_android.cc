...
bool VerifyFromAndroidTrustManager(
  ...
  if (status != android::CERT_VERIFY_STATUS_ANDROID_OK)
    LOG(INFO) << "LoaderLog CertVerifyStatusAndroid error:" << status;
  ...
  switch (status) {
    ...
    case android::CERT_VERIFY_STATUS_ANDROID_UNABLE_TO_CHECK_REVOCATION:
      verify_result->cert_status |= CERT_STATUS_UNABLE_TO_CHECK_REVOCATION;
      break;
    case android::CERT_VERIFY_STATUS_ANDROID_REVOKED:
...
int CertVerifyProcAndroid::VerifyInternal(
  ...
  if (IsCertStatusError(verify_result->cert_status)) {
    // SAMSUNG : send_cert_error_type_to_mdm start >>
    android::NotifyCertificateError(verify_result->cert_status,
                                  cert[0].HasExpired());
    // SAMSUNG : send_cert_error_type_to_mdm end <<
