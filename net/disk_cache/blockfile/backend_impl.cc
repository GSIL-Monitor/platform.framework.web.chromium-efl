...
void BackendImpl::UpdateMaxCacheSize() {
  int used_size = CalculateWebAppCacheSize(path_, cache_type_);
...
#if defined(OS_TIZEN_TV_PRODUCT)
  if (max_size_) {
    if (base::SysInfo::AmountOfFreeDiskSpace(path_) <
...
#if defined(OS_TIZEN_TV_PRODUCT)
  // if zero set cache size to zero not use the default.
  if (max_size_ >= 0)
    return;
#else
  if (max_size_)
