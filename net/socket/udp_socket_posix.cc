...
int UDPSocketPosix::GetReceiveBufferSize(int32_t* out_size) {
  DCHECK_NE(socket_, kInvalidSocket);
  DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
  int32_t size = 0;
  socklen_t option_size = sizeof(size);
  int rv = getsockopt(socket_, SOL_SOCKET, SO_RCVBUF,
                      reinterpret_cast<char*>(&size), &option_size);
  if (rv == 0) {
    *out_size = size;
    return OK;
  }
...
int UDPSocketPosix::GetSendBufferSize(int32_t* out_size) {
  DCHECK_NE(socket_, kInvalidSocket);
  DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
  int32_t size = 0;
  socklen_t option_size = sizeof(size);
...
int UDPSocketPosix::GetAllowAddressReuse(bool* out_flag) {
  DCHECK_NE(socket_, kInvalidSocket);
  DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
  bool flag = 0;
  socklen_t flag_size = sizeof(flag);
  int rv = getsockopt(socket_, SOL_SOCKET, SO_REUSEADDR,
                      reinterpret_cast<char*>(&flag), &flag_size);
  if (rv == 0) {
    *out_flag = flag;
    return OK;
  }
...
int UDPSocketPosix::GetBroadcast(bool* out_flag) {
  DCHECK_NE(socket_, kInvalidSocket);
  DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
  bool flag = 0;
  socklen_t flag_size = sizeof(flag);
  socklen_t rv = getsockopt(socket_, SOL_SOCKET, SO_BROADCAST,
                      reinterpret_cast<char*>(&flag), &flag_size);
  if (rv == 0) {
    *out_flag = flag;
    return OK;
...
int UDPSocketPosix::GetMulticastTimeToLive(int* out_ttl) {
  DCHECK_NE(socket_, kInvalidSocket);
  DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
  ...
  if (addr_family_ == AF_INET) {
    ...
    int rv = getsockopt(socket_, IPPROTO_IP, IP_MULTICAST_TTL,
                    &ttl, &ttl_size);
    if (rv == 0) {
      *out_ttl = ttl;
      return OK;
    ...
    int ttl = -1;
    socklen_t ttl_size = sizeof(ttl);
    int rv = getsockopt(socket_, IPPROTO_IPV6, IPV6_MULTICAST_HOPS,
                    &ttl, &ttl_size);
    if (rv == 0) {
      *out_ttl = ttl;
      return OK;
...
int UDPSocketPosix::GetMulticastLoopbackMode(bool* loopback) {
  DCHECK_NE(socket_, kInvalidSocket);
  DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);

  if (addr_family_ == AF_INET) {
    u_char loop = 0;
    socklen_t loop_size = sizeof(loop);
    int rv = getsockopt(socket_, IPPROTO_IP, IP_MULTICAST_LOOP,
                    &loop, &loop_size);
    if (rv == 0) {
      *loopback = (loop == 1);
      return OK;
