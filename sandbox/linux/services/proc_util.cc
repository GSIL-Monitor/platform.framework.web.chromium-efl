...
#if defined(OS_TIZEN)
    const std::string kernelLogFile("/dev/kmsg");
    std::string filePath("/proc/self/fd/");
    filePath += de->d_name;
    char targetPath[PATH_MAX];
    if (readlink(filePath.c_str(), targetPath, PATH_MAX) > 0 &&
        std::string(targetPath, kernelLogFile.length()) == kernelLogFile)
      continue;
#endif
