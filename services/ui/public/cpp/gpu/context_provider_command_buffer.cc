...
bool ContextProviderCommandBuffer::BindToCurrentThread() {
  ...
  // this context has been added to the list.
  {
    ...
    command_buffer_ = gpu::CommandBufferProxyImpl::Create(
        ...
        stream_priority_, attributes_, active_url_, task_runner, context_type_);
#else
...
#if defined(DIRECT_CANVAS)
    gles2_stub_.reset(
        new gpu::gles2::DirectCanvasGLES2Interface(gles2_impl_.get()));
#endif
...
#if defined(DIRECT_CANVAS)
gpu::gles2::GLES2Interface* ContextProviderCommandBuffer::DirectCanvasGL() {
  return gles2_stub_.get();
}
#endif
