...
#include "tizen_src/ewk/efl_integration/wrt/hbbtv_widget_host.h"
...
TimeZoneMonitor::TimeZoneMonitor() {
  ...
  HbbtvWidgetHost::Get().SetTimeZoneMonitor(this);
#endif
}
...
void TimeZoneMonitor::NotifyTimeZoneChange(std::string time_zone) {
  clients_.ForAllPtrs(
      [&time_zone](device::mojom::TimeZoneMonitorClient* client) {
        client->OnTimeZoneChange(time_zone);
      });
}
...
void TimeZoneMonitor::NotifyClients() {
  ...
  NotifyTimeZoneChange(zone_id_str);
}
