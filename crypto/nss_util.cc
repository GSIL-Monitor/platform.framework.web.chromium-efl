    ...
    if (!database_dir.empty()) {
      ...
      if (status != SECSuccess) {
        LOG(ERROR) << "Error initializing NSS with a persistent "
                   ...
                   << "): " << GetNSSErrorMessage();
#if defined(OS_TIZEN_TV_PRODUCT)
        LOG(ERROR) << "Try init NssDB again after delete all related files";
        if (DeleteFilesInDirectory(database_dir))
          status = NSS_InitReadWrite(nss_config_dir.c_str());
#endif
      }
    }
    if (status != SECSuccess) {
      LOG(ERROR) << "Initializing NSS without a persistent database.";
...
#if defined(OS_TIZEN_TV_PRODUCT)
  bool DeleteFilesInDirectory(base::FilePath& dir_path) {
    base::FileEnumerator traversal(dir_path, true,
    ...
    for (base::FilePath current = traversal.Next(); !current.empty();
      ...
      if (!(unlink(current.value().c_str()) == 0)) {
        LOG(ERROR) << "Fail to unlink. path : " << current.value().c_str();
        return false;
      } else
        LOG(INFO) << "Deleted files. path : " << current.value().c_str();
    }
    return true;
  }
#endif
