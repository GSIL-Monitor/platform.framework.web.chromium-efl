...
int WebMStreamParser::ParseInfoAndTracks(const uint8_t* data, int size) {
  ...
  if (!framerate_set_cb_.is_null()) {
    ...
    if (tracks_parser.GetVideoDefaultDuration(timecode_scale_in_us) !=
      ...
      stream_scale_time_ = info_parser.timecode_scale();
...
int WebMStreamParser::ParseCluster(const uint8_t* data, int size) {
  ...
  if (!framerate_set_cb_.is_null() && !is_framerate_set_ &&
    ...
    const int cluster_block_count = cluster_parser_->block_count();
    const int cluster_block_time_code = cluster_parser_->block_time_code();
