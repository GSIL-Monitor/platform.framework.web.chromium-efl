...
class MEDIA_EXPORT MP4StreamParser : public StreamParser {
 ...
 private:
  ...
  size_t trun_sample_size_;
  int64_t trun_total_duration_;
  size_t try_count_;
#endif
  // Tracks the number of MEDIA_LOGS for skipping empty trun samples.
  int num_empty_samples_skipped_;

  // Injected before a moof, these will be used instead of the sinf or seig
  // values
  std::string key_id_override_;
  std::string key_iv_override_;
