...
bool MP4StreamParser::ParseBox(bool* err) {
  ...
  if (reader->type() == FOURCC_MOOV) {
    ...
    *err = !ParseMoof(reader.get());
    ...
    return !(*err);
  } else if (reader->type() == FOURCC_UUID) {
    std::vector<uint8_t> uuid;
    if (reader->ReadVec(&uuid, 16)) {
      std::vector<uint8_t> tenc_uuid = { 0x36, 0x73, 0x8d, 0x38, 0x8c, 0x70, 0x11, 0xe7, 0x8f, 0x2a, 0xac, 0xde, 0x48, 0x00, 0x11, 0x22 };
      if (uuid == tenc_uuid) {
        TrackEncryption tenc_override;
        if (reader->ScanChildren() && reader->MaybeReadChild(&tenc_override)) {
          key_id_override_ = std::string(reinterpret_cast<const char*>(&tenc_override.default_kid[0]), tenc_override.default_kid.size());
#if BUILDFLAG(ENABLE_CBCS_ENCRYPTION_SCHEME)
          key_iv_override_ = std::string(reinterpret_cast<const char*>(tenc_override.default_constant_iv), tenc_override.default_constant_iv_size);
#endif
        }
      }
    }
...
#if defined(OS_TIZEN_TV_PRODUCT)
  if (!runs_->is_audio() && !is_framerate_set_ &&
      !framerate_set_cb_.is_null()) {
    // Min Trun sample count for calculate fps.
    constexpr const size_t kMinTrunSampleCount = 10;
    // Max try count.
    constexpr const size_t kMaxTryCount = 2;

    const TrackRunSampleInfo& trun_sample_info = runs_->GetTrunSampleInfo();
    trun_sample_size_ += trun_sample_info.trun_sample_size;
    trun_total_duration_ += trun_sample_info.trun_duration;
    try_count_++;

    StreamFramerate::Framerate framerate;
    if (trun_sample_size_ > kMinTrunSampleCount) {
      ...
      StreamFramerate sft(kMp4FramerateBase * trun_sample_info.time_scale *
                              trun_sample_size_ / trun_total_duration_,
                          kMp4FramerateBase);
      ...
      trun_sample_size_ = 0;
      trun_total_duration_ = 0;
    } else if (try_count_ >= kMaxTryCount) {
      framerate.num = 30;
      framerate.den = 1;
      framerate_set_cb_.Run(framerate);
      is_framerate_set_ = true;
...
bool MP4StreamParser::EnqueueSample(BufferQueueMap* buffers, bool* err) {
  ...
  if (video) {
    if (runs_->video_description().video_codec == kCodecH264 ||
      ...
      bool need_subsamples = false;
      if (runs_->is_encrypted()) {
        EncryptionScheme encryption_scheme =
            GetEncryptionScheme(runs_->video_description().sinf);
        need_subsamples =
            subsamples.empty() &&
            encryption_scheme.mode() == EncryptionScheme::CIPHER_MODE_AES_CBC &&
            encryption_scheme.pattern().IsInEffect();
      }
      if (!runs_->video_description().frame_bitstream_converter->ConvertFrame(
              &frame_buf, runs_->is_keyframe(), &subsamples, need_subsamples)) {
        MEDIA_LOG(ERROR, media_log_)
