...
class MEDIA_EXPORT MediaPlayerAndroid {
 public:
  ...
  // NOTE: Keep these values in sync with SwContentVideoView.java.
  enum ClosedCaptionStaus {
    CLOSED_CAPTION_DISABLED,
    CLOSED_CAPTION_VISIBLE,
    CLOSED_CAPTION_INVISIBLE
  };
#endif
...
#if defined(S_TERRACE_SUPPORT)
  virtual const std::string GetCookies();
#endif
  ...
  void DetachListener();
  void SetPlayed() { played_ = true; }
  bool IsPlayed() { return played_; }

#if defined(S_TERRACE_SUPPORT)
  void PlayMedia();
  void PauseMedia();
  void SetClosedCaptionUrl(const std::string& url);
  std::string GetClosedCaptionUrl() { return closed_caption_url_; }
  void SetClosedCaptionStatus(int status);
  int GetClosedCaptionStatus();
  bool HasClosedCaption();
  void SetDimension(const std::string& type) { dimension_ = type; }
  std::string GetDimension() { return dimension_; }
  void SetIsMediaControlDisplayed(bool is_displayed);
  bool IsMediaControlDisplayed() { return is_media_control_displayed_; }
#endif
 ...
 private:
  ...
  // Flag for app logging
  bool played_;

#if defined(S_TERRACE_SUPPORT)
  std::string closed_caption_url_;
  ClosedCaptionStaus closed_caption_status_;
  std::string dimension_;
  bool is_media_control_displayed_;
#endif
