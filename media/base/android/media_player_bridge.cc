...
bool MediaPlayerBridge::HasAudio() const {
  DCHECK(prepared_);
  JNIEnv* env = base::android::AttachCurrentThread();
  return MediaPlayerAndroid::GetEffectiveVolume() > 0 &&
         Java_MediaPlayerBridge_hasAudio(env, j_media_player_bridge_);
}
...
void MediaPlayerBridge::StartInternal() {
  ...
  // call below function for app logging
  SetPlayed();
}
...
const std::string MediaPlayerBridge::GetCookies() {
  return cookies_;
}
#endif

void MediaPlayerBridge::SetRate(double rate) {
  if (j_media_player_bridge_.is_null())
    return;
  ...
  Java_MediaPlayerBridge_setRate(env, j_media_player_bridge_, rate);
}
