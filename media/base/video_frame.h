...
class MEDIA_EXPORT VideoFrame : public base::RefCountedThreadSafe<VideoFrame> {
 public:
...
#if defined(TIZEN_TBM_SUPPORT)
    STORAGE_TBM_SURFACE = 8,
    STORAGE_LAST = STORAGE_TBM_SURFACE,
#else
  ...
  // Wraps encoded frame. Stores data in the first plane.
  static scoped_refptr<VideoFrame> WrapExternalEncodedData(
      const gfx::Size& size,
      uint8_t* data,
      int32_t data_size,
      base::TimeDelta timestamp);
  ...
  static scoped_refptr<VideoFrame> WrapTBMSurface(
      const gfx::Size& size,
      base::TimeDelta timestamp,
  ...
  static bool GetScalerIndex(bool is_kantm2, uint32_t* scaler_value);
  static bool GetScalerNumber(uint32_t* scaler_value);
  static bool LockScalerDuringNextFrame(bool is_kantm2,
