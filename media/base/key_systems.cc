...
const char kAppleFairPlaySystem[] = "com.apple.fps";
...
static bool IsPotentiallySupportedKeySystem(const std::string& key_system) {
  ...
  if (key_system == kClearKeyKeySystem)
    return true;
  if( key_system == kAppleFairPlaySystem)
    return true;
...
bool IsAppleFairPlay(const std::string& key_system) {
  return kAppleFairPlaySystem == key_system;
}
