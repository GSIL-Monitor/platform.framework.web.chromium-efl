...
#if defined(OS_TIZEN_TV_PRODUCT) && !defined(TIZEN_VD_DISABLE_GPU)
#include <dp_reg_ctrl.h>
#endif
#endif
...
bool VideoFrame::GetScalerIndex(bool is_kantm2, uint32_t* scaler_value) {
#if !defined(TIZEN_VD_DISABLE_GPU)
  uint32_t u32Addr = 0xCB0500;  // physical address
  if (is_kantm2)
    u32Addr = 0xC12BFC;
  return (sdp_dpreg_read(u32Addr, scaler_value) == 0);
#else
  return false;
#endif
}

bool VideoFrame::GetScalerNumber(uint32_t* scaler_value) {
#if !defined(TIZEN_VD_DISABLE_GPU)
  const uint32_t u32Addr = 0xC10C1C;  // physical address
  return (sdp_dpreg_read(u32Addr, scaler_value) == 0);
#else
  return false;
#endif
...
bool VideoFrame::LockScalerDuringNextFrame(bool is_kantm2,
                                           uint32_t scaler_physical_id,
                                           bool enable) {
#if !defined(TIZEN_VD_DISABLE_GPU)
  // Reduce lock usage of the GPU.
  // The scaler frame index has 0 ~ 4 numbers.
  // It counts decreased.
  uint32_t u32Addr = 0xCB0608;  // physical address
  if (is_kantm2)
    u32Addr = 0xC12BF8;

  const uint32_t max_scaler_num = 15;  // kantm = 5ea.
  if (scaler_physical_id > max_scaler_num) {
    LOG(ERROR) << "Not enough scaler buffer bits";
    return false;
  }

  LOG(INFO) << "lock scaler! (lock current) : " << scaler_physical_id
            << " enable: " << enable;
  if (enable)
    scaler_physical_id |= (1 << 4);

  LOG(INFO) << "bitset : " << scaler_physical_id;
  return sdp_dpreg_write(u32Addr, scaler_physical_id);
#else
  return false;
#endif
}
...
scoped_refptr<VideoFrame> VideoFrame::WrapTBMSurface(
  ...
  if (!IsValidConfig(format, storage, size, visible_rect, size)) {
    DLOG(ERROR) << __FUNCTION__ << " WrapTBMSurface Invalid config."
                << ConfigToString(format, storage, size, visible_rect, size);
    return nullptr;
  }
  scoped_refptr<VideoFrame> frame(
      new VideoFrame(format, storage, size, gfx::Rect(size), size, timestamp));
...
// static
scoped_refptr<VideoFrame> VideoFrame::WrapExternalEncodedData(
    const gfx::Size& size,
    uint8_t* data,
    int32_t data_size,
    base::TimeDelta timestamp) {
  return WrapExternalYuvData(PIXEL_FORMAT_ENCODED, size, gfx::Rect(size), size,
                             data_size, 0, 0, data, nullptr, nullptr,
                             timestamp);
}
...
int VideoFrame::BitsPerChannel(VideoPixelFormat format) {
  ...
  switch (format) {
    case media::PIXEL_FORMAT_ENCODED:
    case media::PIXEL_FORMAT_UNKNOWN:
#if defined(TIZEN_TBM_SUPPORT)
    case media::PIXEL_FORMAT_TBM_SURFACE:
#endif
      NOTREACHED();
    // Fall through!
...
unsigned VideoFrame::CreateTbmTextureIfNeeded(gpu::gles2::GLES2Interface* gl) {
  ...
  mailbox_holders_release_cb_ =
      base::Bind(ReleaseTbmTexture, base::ThreadTaskRunnerHandle::Get(), gl,
                 texture_id_, image);
...
void VideoFrame::GenTbmTextureMailbox() {
  DCHECK(gl_ && texture_id_);
  gpu::Mailbox mailbox;
  gl_->GenMailboxCHROMIUM(mailbox.name);
  gl_->ProduceTextureDirectCHROMIUM(texture_id_, GL_TEXTURE_EXTERNAL_OES,
                                    mailbox.name);
