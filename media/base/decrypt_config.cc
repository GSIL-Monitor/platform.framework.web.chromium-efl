...
std::unique_ptr<DecryptConfig> DecryptConfig::CreateCencConfig(
  ...
  return base::MakeUnique<DecryptConfig>(EncryptionMode::kCenc, key_id, iv,
...
std::unique_ptr<DecryptConfig> DecryptConfig::CreateCbcsConfig(
  ...
  return base::MakeUnique<DecryptConfig>(EncryptionMode::kCbcs, key_id, iv,
                                         subsamples,
                                         std::move(encryption_pattern));
}

DecryptConfig::DecryptConfig(
    const EncryptionMode &encryption_mode, const std::string &key_id,
    const std::string &iv, const std::vector<SubsampleEntry> &subsamples,
    base::Optional<EncryptionPattern> encryption_pattern)
    : encryption_mode_(encryption_mode), key_id_(key_id), iv_(iv),
