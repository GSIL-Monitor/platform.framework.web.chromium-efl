...
SourceBufferStream<RangeClass>::SourceBufferStream(
  ...
  memory_limit_ = GetSourceBufferVideoMemoryLimit(video_config);
  if (memory_limit_ == kSourceBufferVideoMemoryLimitForUHD) {
    LOG(INFO) << "Source buffer memory limit is for UHD video.";
  }
#else
...
#if defined(OS_TIZEN_TV_PRODUCT)
template <typename RangeClass>
base::TimeDelta SourceBufferStream<RangeClass>::KeyframeBeforeTimestamp(
    const base::TimeDelta timestamp) {
  DecodeTimestamp timestampTime =
      DecodeTimestamp::FromPresentationTime(timestamp);
  DecodeTimestamp returnTimestamp = FindKeyframeBeforeTimestamp(timestampTime);

  if (returnTimestamp != kNoDecodeTimestamp())
    return returnTimestamp.ToPresentationTime().RoundedToMilliseconds();

  return timestamp;
}

template <typename RangeClass>
bool SourceBufferStream<RangeClass>::CanSeekTo(
    const base::TimeDelta timestamp) {
  const auto dts = DecodeTimestamp::FromPresentationTime(timestamp);
  typename RangeList::iterator itr = ranges_.end();
  for (itr = ranges_.begin(); itr != ranges_.end(); ++itr) {
    if (RangeCanSeekTo(itr->get(), dts))
      return true;
  }
...
#if defined(OS_TIZEN_TV_PRODUCT)
  memory_limit_ = GetSourceBufferVideoMemoryLimit(config);
  if (memory_limit_ == kSourceBufferVideoMemoryLimitForUHD) {
    LOG(INFO) << "Source buffer memory limit is for UHD video.";
  }
#endif
...
#if defined(OS_TIZEN_TV_PRODUCT)
template <typename RangeClass>
DecodeTimestamp SourceBufferStream<RangeClass>::FindKeyframeBeforeTimestamp(
    const DecodeTimestamp timestamp) {
  ...
  return RangeKeyframeBeforeTimestamp(itr->get(), timestamp);
}
#endif
...
#if defined(OS_TIZEN_TV_PRODUCT)
template <typename RangeClass>
size_t SourceBufferStream<RangeClass>::GetSourceBufferVideoMemoryLimit(
    const VideoDecoderConfig& video_config) const {
  const int kFHDWidth = 1920;
  const int kFHDHeight = 1080;
  if (video_config.coded_size().width() > kFHDWidth ||
      video_config.coded_size().height() > kFHDHeight) {
    return kSourceBufferVideoMemoryLimitForUHD;
  }

  return kSourceBufferVideoMemoryLimitForFHD;
}
#endif

template <>
bool SourceBufferStream<SourceBufferRangeByDts>::BufferingByPts() {
  return false;
}

template <>
bool SourceBufferStream<SourceBufferRangeByPts>::BufferingByPts() {
  return true;
}
