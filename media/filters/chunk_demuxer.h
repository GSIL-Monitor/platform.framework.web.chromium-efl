...
using RelevantSeekValues = std::pair<base::TimeDelta, base::TimeDelta>;
#endif
...
class MEDIA_EXPORT ChunkDemuxerStream : public DemuxerStream {
 public:
  ...
  bool CanSeekTo(const base::TimeDelta timestamp);
#endif
...
#if defined(OS_TIZEN_TV_PRODUCT)
  RelevantSeekValues GetRelevantSeekValues(const base::TimeDelta timestamp);
#endif
...
#if defined(OS_TIZEN_TV_PRODUCT)
  const StreamFramerate::Framerate& GetFramerate();
...
#if defined(OS_TIZEN_TV_PRODUCT)
  bool waiting_for_call_to_seek_{false};
#endif
...
#if defined(OS_TIZEN_TV_PRODUCT)
  StreamFramerate::Framerate framerate_ = {0, 0};
