...
base::TimeDelta ChunkDemuxerStream::KeyframeBeforeTimestamp(
    const base::TimeDelta timestamp) {
  return SBSTREAM_OP(KeyframeBeforeTimestamp(timestamp));
}

bool ChunkDemuxerStream::CanSeekTo(const base::TimeDelta timestamp) {
  base::AutoLock auto_lock(lock_);
  return SBSTREAM_OP(CanSeekTo(timestamp));
}
#endif
...
void ChunkDemuxer::Seek(TimeDelta time, const SeekCB& seek_cb) {
...
#if defined(OS_TIZEN_TV_PRODUCT)
  waiting_for_call_to_seek_ = false;
#endif
  seek_cb_ = BindToCurrentLoop(base::Bind(seek_cb, time));
...
void ChunkDemuxer::StartWaitingForSeek(TimeDelta seek_time) {
  ...
  cancel_next_seek_ = false;
#if defined(OS_TIZEN_TV_PRODUCT)
  waiting_for_call_to_seek_ = true;
#endif
}
...
#if defined(OS_TIZEN_TV_PRODUCT)
  currentMediaTime = std::get<0>(GetRelevantSeekValues(currentMediaTime));
  DVLOG(1) << "Moving media_time to key frame: "
           << currentMediaTime.InSecondsF();
#endif
  base::AutoLock auto_lock(lock_);
...
void ChunkDemuxer::MarkEndOfStream(PipelineStatus status) {
  ...
  Ranges<TimeDelta> ranges;
  {
    ...
    // In reality marking a MediaSource as 'ended' changes buffered. Buffered
    // calculation algorithm: "If readyState is "ended", then set the end time
    // on the last range in source ranges to highest end time."
    ranges = GetBufferedRanges_Locked();
  }
  host_->OnBufferedTimeRangesChanged(ranges);
}
...
#if defined(OS_TIZEN_TV_PRODUCT)
void ChunkDemuxer::SetFramerateCallback(
    const StreamParser::FramerateSetCB& framerate_set_cb) {
  framerate_set_cb_ = BindToCurrentLoop(framerate_set_cb);
}
...
const StreamFramerate::Framerate& ChunkDemuxer::GetFramerate() {
  return framerate_;
}
#endif
...
#if defined(OS_TIZEN_TV_PRODUCT)
RelevantSeekValues ChunkDemuxer::GetRelevantSeekValues(
    const base::TimeDelta timestamp) {
  base::TimeDelta keyFrameTimestamp;
  auto video_it =
      std::find_if(video_streams_.begin(), video_streams_.end(),
                   [](const decltype(video_streams_)::value_type& stream) {
                     return stream->IsEnabled();
                   });
  auto audio_it =
      std::find_if(audio_streams_.begin(), audio_streams_.end(),
                   [](const decltype(audio_streams_)::value_type& stream) {
                     return stream->IsEnabled();
                   });

  if (video_it != video_streams_.end() && *video_it) {
    keyFrameTimestamp = (*video_it)->KeyframeBeforeTimestamp(timestamp);
    bool can_audio_seek =
        audio_it == audio_streams_.end() ||
        (audio_it != audio_streams_.end() &&
         ((*audio_it && (*audio_it)->CanSeekTo(keyFrameTimestamp)) ||
          *audio_it == NULL));
    return (can_audio_seek
                ? RelevantSeekValues{keyFrameTimestamp, keyFrameTimestamp}
                : RelevantSeekValues{timestamp, keyFrameTimestamp});
  }
  return RelevantSeekValues{timestamp, media::kNoTimestamp};
}
#endif
