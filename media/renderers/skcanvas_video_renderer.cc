...
sk_sp<SkImage> NewSkImageFromVideoFrameNative(VideoFrame* video_frame,
  ...
  DCHECK(PIXEL_FORMAT_ARGB == video_frame->format() ||
         ...
         PIXEL_FORMAT_TBM_SURFACE == video_frame->format() ||
#endif
...
void SkCanvasVideoRenderer::Paint(const scoped_refptr<VideoFrame>& video_frame,
                                  ...
                                  const Context3D& context_3d) {
  TRACE_EVENT0("media", "SkCanvasVideoRenderer::Paint");
...
void SkCanvasVideoRenderer::CopyVideoFrameSingleTextureToGLTexture(
  ...
  DCHECK(video_frame->HasTextures());
#else
  bool is_mailbox = !video_frame->IsTBMBackend();
  uint32_t source_texture = 0;
  if (!is_mailbox) {
    source_texture = video_frame->CreateTbmTextureIfNeeded(gl);
    if (!source_texture) {
      video_frame->GenTbmTextureMailbox();
      is_mailbox = true;
    }
  }
#endif
  ...
  DCHECK(mailbox_holder.texture_target == GL_TEXTURE_2D ||
         mailbox_holder.texture_target == GL_TEXTURE_RECTANGLE_ARB ||
#if defined(TIZEN_TBM_SUPPORT)
         (video_frame->IsTBMBackend() && !mailbox_holder.texture_target) ||
#endif
         mailbox_holder.texture_target == GL_TEXTURE_EXTERNAL_OES)
      << mailbox_holder.texture_target;
...
#if defined(TIZEN_TBM_SUPPORT)
    if (video_frame->IsTBMBackend() || video_frame->HasTextures()) {
#else
    if (video_frame->HasTextures()) {
#endif
