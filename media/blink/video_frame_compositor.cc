...
scoped_refptr<VideoFrame> VideoFrameCompositor::GetCurrentFrame() {
  ...
  TTRACE_WEB("VideoFrameCompositor::GetCurrentFrame");
#endif
  DCHECK(task_runner_->BelongsToCurrentThread());
...
bool VideoFrameCompositor::ProcessNewFrame(
    ...
    bool repaint_duplicate_frame) {
#if defined(USE_TTRACE)
  TTRACE_WEB("VideoFrameCompositor::ProcessNewFrame");
#endif
  DCHECK(task_runner_->BelongsToCurrentThread());
...
#endif
  DCHECK(compositor_task_runner->BelongsToCurrentThread());
...
void VideoFrameCompositor::SetStartedOESTexture(bool enabled) {
  if (is_started_oes_frame_ != enabled) {
    is_started_oes_frame_ = enabled;
    LOG(INFO) << "Started OES = " << enabled;
  }
}

void VideoFrameCompositor::CreateTbmTextureIfNeeded(
    gpu::gles2::GLES2Interface* gl,
    unsigned textureId) {
  // TBM texture has been created, it is not neccessary to
  // create it again.
  if (!current_frame_ || current_frame_->GetTbmTexture()) {
    LOG(INFO) << "current frame is null or tbm texture has been created.";
    return;
  }

  if (!current_frame_->GetTbmBuffer().tbm_surface) {
    LOG(ERROR) << "Fail: TBM is empty!";
    return;
  }

  uint32_t image = gl->CreateTizenImageCHROMIUM(
      current_frame_->GetTbmBuffer(), current_frame_->visible_rect().width(),
      current_frame_->visible_rect().height(), GL_RGBA);
  if (!image) {
    LOG(ERROR) << "Fail: CreateTizenImageCHROMIUM id = " << image;
    current_frame_->SetImageID(0);
    return;
  }

  const uint32_t target = GL_TEXTURE_EXTERNAL_OES;

  gl->BindTexImage2DCHROMIUM(target, image);
  ...
  current_frame_->SetTbmTexture(textureId);
  current_frame_->SetImageID(image);
  current_frame_->SetReleaseMailboxCB(
      base::Bind(ReleaseTbmWithoutTexture, task_runner_, gl, textureId, image));

#if defined(_DEBUG_TBM_VIDEO_RENDERING) && _DEBUG_TBM_VIDEO_RENDERING
  LOG(INFO) << "VideoFrame > --CreateTbmTexture OES >"
            << ", img:" << image << ", txt:" << textureId;
#endif
}
#endif
}  // namespace media
