...
#endif
#if defined(OS_TIZEN)
#if defined(TIZEN_MFC_TEXTURE_VIDEO_DECODER)
#include <iterator>
#include "ui/gl/gl_context.h"
#include "ui/gl/gl_surface_egl.h"  // NOLINT(build/include)
#endif  // defined(TIZEN_OMX_MFC_VIDEO_DEC)
#endif
...
media::VideoDecodeAccelerator::SupportedProfiles GetSupportedTizenMfcProfiles();
#endif
}  // namespace content
...
// resolutions and other supported profile parameters.
  ...
  if (capabilities.supported_profiles.empty()) {
    capabilities.supported_profiles = content::GetSupportedTizenMfcProfiles();
  } else {
    // Extend supported profiles list on Tizen platform
    auto supported_profiles = content::GetSupportedTizenMfcProfiles();
    capabilities.supported_profiles.reserve(
        capabilities.supported_profiles.size() + supported_profiles.size());
    std::move(supported_profiles.begin(), supported_profiles.end(),
              std::back_inserter(capabilities.supported_profiles));
  }
#endif  // defined(TIZEN_MFC_TEXTURE_VIDEO_DECODER)
#if defined(TIZEN_WEBRTC)
  capabilities.supported_profiles = content::GetSupportedTizenProfiles();
#endif
#endif
...
#endif
#if defined(TIZEN_MFC_TEXTURE_VIDEO_DECODER)
    &GpuVideoDecodeAcceleratorFactory::CreateTizenMfcVDA,
#endif
#if defined(TIZEN_WEBRTC)
    &GpuVideoDecodeAcceleratorFactory::CreateTizenVDA,
#endif
  };
...
#if defined(TIZEN_RESOURCE_MANAGER)
    vda->SetResourceManager(content::TizenResourceManagerImpl::GetInstance());
#endif
...
GpuVideoDecodeAcceleratorFactory::CreateTizenMfcVDA(
    ...
    const gpu::GpuPreferences& gpu_preferences) const {
  std::unique_ptr<VideoDecodeAccelerator> decoder;
  decoder.reset(content::CreateTizenMfcVideoDecodeAccelerator(
      gl::GLSurfaceEGL::GetHardwareDisplay(),
      get_gl_context_cb_.Run()->GetHandle(), make_context_current_cb_));
  return decoder;
}
#endif
...
GpuVideoDecodeAcceleratorFactory::CreateTizenVDA(
    ...
    const gpu::GpuPreferences& gpu_preferences) const {
  std::unique_ptr<VideoDecodeAccelerator> decoder;
  decoder.reset(content::CreateTizenVideoDecodeAccelerator());
  return decoder;
}
#endif
