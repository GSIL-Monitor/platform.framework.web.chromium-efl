...
void GpuVideoDecodeAccelerator::NotifyDecodeDone(
    base::SharedMemoryHandle shared_memory_handle,
    uint32_t bitstream_buffer_id,
    uint32_t buffer_size,
    gfx::Rect frame_size) {
  if (!Send(new AcceleratedVideoDecoderHostMsg_NotifyDecodeDone(
          host_route_id_, shared_memory_handle, bitstream_buffer_id,
          buffer_size, frame_size))) {
    DLOG(ERROR)
        << "Send(AcceleratedVideoDecoderHostMsg_NotifyDecodeDone) failed";
  }
}
#endif
