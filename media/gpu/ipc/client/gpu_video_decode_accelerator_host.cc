...
bool GpuVideoDecodeAcceleratorHost::OnMessageReceived(const IPC::Message& msg) {
  ...
  IPC_BEGIN_MESSAGE_MAP(GpuVideoDecodeAcceleratorHost, msg)
    ...
    IPC_MESSAGE_HANDLER(AcceleratedVideoDecoderHostMsg_NotifyDecodeDone,
                        OnNotifyDecodeDone)
#endif
...
#if defined(TIZEN_WEBRTC)
void GpuVideoDecodeAcceleratorHost::OnNotifyDecodeDone(
    base::SharedMemoryHandle shared_memory_handle,
    uint32_t bitstream_buffer_id,
    uint32_t buffer_size,
    gfx::Rect frame_size) {
  if (client_)
    client_->NotifyDecodeDone(shared_memory_handle, bitstream_buffer_id,
