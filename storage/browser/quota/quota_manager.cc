 ...
 protected:
  void Run() override {
    ...
    if (is_unlimited_) {
      ...
      SetDesiredHostQuota(barrier, kQuotaStatusOk, 2 * 1024 * kMBytes);
#else
...
#if defined(OS_TIZEN)
void QuotaManager::GetTizenUnlimitedStoragePrivilege(
    const GURL& origin,
    QuotaClient::ID id,
    ...
    int64_t usage,
    int64_t quota) {
  if (id == QuotaClient::kIndexedDatabase &&
      status == QuotaStatusCode::kQuotaStatusOk &&
      transaction_size + usage > quota) {
    if (special_storage_policy_.get() &&
        special_storage_policy_->RequestUnlimitedStoragePolicy(
            origin, quota,
            base::Bind(&QuotaManager::DidGetTizenUnlimitedStoragePrivilege,
                       weak_factory_.GetWeakPtr(), origin, callback, status,
                       usage, quota))) {
...
void QuotaManager::DidGetTizenUnlimitedStoragePrivilege(
    const GURL& origin,
    const UsageAndQuotaCallback& callback,
    QuotaStatusCode status,
    int64_t usage,
    ...
    bool policy_changed) {
  if (policy_changed)
    GetUsageAndQuota(origin, kStorageTypeTemporary, callback);
  else
    callback.Run(status, usage, quota);
}
#endif
...
void QuotaManager::UpdatePath(const base::FilePath& path) {
  profile_path_ = path;
}
#endif
