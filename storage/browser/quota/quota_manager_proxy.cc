...
void QuotaManagerProxy::GetUsageAndQuota(
  ...
  if (!io_thread_->BelongsToCurrentThread()) {
    io_thread_->PostTask(
        FROM_HERE, base::Bind(&QuotaManagerProxy::GetUsageAndQuota, this,
                              ...
                              type, callback, id, transaction_size));
    return;
  }
  if (!manager_) {
  ...
  const UsageAndQuotaCallback& wrapped_callback =
#if defined(OS_TIZEN)
      (type == kStorageTypeTemporary)
          ? base::Bind(&QuotaManager::GetTizenUnlimitedStoragePrivilege,
                       manager_, origin, id, transaction_size, callback)
          :
#endif
          callback;
