...
void PluginPrivateFileSystemBackend::UpdatePath(const base::FilePath& path) {
  base::FilePath base_path =
      path.Append(kFileSystemDirectory).Append(kPluginPrivateDirectory);
  base_path_ = base_path;
  static_cast<ObfuscatedFileUtil*>(
      static_cast<AsyncFileUtilAdapter*>(file_util_.get())->sync_file_util())
      ->UpdatePath(base_path_);
}
#endif
