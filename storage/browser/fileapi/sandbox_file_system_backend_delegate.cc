...
void SandboxFileSystemBackendDelegate::UpdatePath(const base::FilePath& path) {
  static_cast<ObfuscatedFileUtil*>(
      static_cast<AsyncFileUtilAdapter*>(sandbox_file_util_.get())
          ->sync_file_util())
      ->UpdatePath(path.Append(kFileSystemDirectory));
}
#endif
