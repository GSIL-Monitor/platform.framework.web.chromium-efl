...
class WebLocalFrame : public WebFrame {
 public:
  ...
  virtual void MoveRangeSelectionExtent(const WebPoint&,
                                        bool isLastTouchPoint = false) = 0;
  ...
  // Returns the center points of the matched bounding boxes in visual
  // viewport coordinates.
  virtual std::vector<WebPoint> FindMatchPoints() = 0;

  // Returns the urls of the matched bounding boxes.
  virtual std::vector<WebURL> FindMatchUrls() = 0;
#endif
  ...
  // If set to false, do not allow scrolling on this frame's view.
