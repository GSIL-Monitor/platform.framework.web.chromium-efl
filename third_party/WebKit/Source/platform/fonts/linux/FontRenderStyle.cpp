...
#if defined(OS_TIZEN)
  // Tizen 2.x Webkit, Font hint style was disabled by default with
  // "TIZEN_FONT_HINT_NONE" build macro.
#if defined(OS_TIZEN_TV_PRODUCT)
  // FIXME : Launching time is delayed about 100ms if Hinting is enabled on TV webapp.
  if (content::IsTIZENWRT())
#else
  if (RuntimeEnabledFeatures::TizenCompatibilityModeEnabled())
#endif
  {
    if (result.use_hinting == FontRenderStyle::kNoPreference)
