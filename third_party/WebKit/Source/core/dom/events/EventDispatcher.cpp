...
#if defined(TIZEN_ATK_FEATURE_VD)
  // Now node_i might be null - in most cases it returns 'this', but
  // for SliderThumbElement it may return null.
  const Node* node_i = node_.Get()->FocusDelegate();
  if (IsWebBrowser() && node_i) {
    LocalFrame* frame = node_i->GetDocument().GetFrame();
    if (frame && frame->GetSettings() &&
        frame->GetSettings()->GetAccessibilityEnabled() &&
        event_->IsKeyboardEvent() &&
        event_->type() == EventTypeNames::keydown) {
      KeyboardEvent* kevent = ToKeyboardEvent(&GetEvent());
      if (!kevent->key().IsNull() &&
          kWebFocusTypeNone != FocusDirectionForKey(kevent) &&
          event_->defaultPrevented()) {
        // If current arrow has been prevented but not actually had action on
        // the object, reset the default prevent to trigger spatial navigation
        LOG(INFO) << "Current arrow key has been prevented";
        AXObjectCache* cache = node_i->GetDocument().ExistingAXObjectCache();
        if (cache && !cache->GetActiveObjectChangedFlag()) {
          LOG(INFO) << "Prevented arrow key has no actual action, reset to "
                       "navigation";
          kevent->SetDefaultPrevented(false);
        }
      }
    }
  }
