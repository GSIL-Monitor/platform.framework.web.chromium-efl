...
bool Document::kZawgyiEnabled = false;
...
Document::Document(const DocumentInit& initializer,
  ...
  if (frame_) {
    ...
    Document::kZawgyiEnabled = settings ? settings->GetSupportMyanmarFonts() : false;
...
#if defined(S_TERRACE_SUPPORT)
Image* Document::SearchCandidateImage(const int resolution,
                                      const float aspectRatio,
                                      const IntPoint base,
                                      const IntPoint centerBase,
                                      int& square,
                                      int& distance) {
  if (!GetPage())
    return nullptr;

  IntRect visibleRect = IntRect((GetPage()->GetVisualViewport()).VisibleRect());
  visibleRect.MoveBy(View()->VisibleContentRect().Location());

  Image* image = nullptr;
  IntPoint center;

  if (centerBase == IntPoint())
    center = visibleRect.Center();
  else if (base == IntPoint())
    center = centerBase + visibleRect.Location();
  else
    center = centerBase;

  visibleRect.MoveBy(base);

  distance = visibleRect.MinXMinYCorner().DistanceSquaredToPoint(
      visibleRect.MaxXMaxYCorner());
  square = 0;

  for (HTMLElement* child = Traversal<HTMLElement>::FirstChild(*body()); child;
       child = Traversal<HTMLElement>::Next(*child)) {
    LayoutObject* object = child->GetLayoutObject();

    if (!object)
      continue;

    IntRect layoutRect = object->AbsoluteBoundingBoxRect();
    if (layoutRect.IsEmpty())
      continue;

    // 1. find an image from the given layout object.
    Image* currImage = nullptr;

    // Find an image inside the frame if the given object is either frame or
    // iframe.
    if (object->IsFrame() || object->IsLayoutIFrame()) {
      Document* document = ToHTMLFrameOwnerElement(child)->contentDocument();

      IntPoint baseForFrame =
          (object->IsFrame())
              ? IntPoint()
              : visibleRect.Location() + (-layoutRect.Location());
      IntPoint centerBaseForFrame = center + (-layoutRect.Location());

      int maxSquare = 0;
      int minDistance = 0;
      currImage = document->SearchCandidateImage(
          resolution, aspectRatio, baseForFrame, centerBaseForFrame, maxSquare,
          minDistance);
      ...
      if ((minDistance < distance) ||
          (minDistance == distance && maxSquare > square)) {
    ...
    if (object->IsImage()) {
      currImage = child->ImageContents();
    } else if (object->Style()->HasBackgroundImage()) {
      for (const FillLayer* layer = &object->Style()->BackgroundLayers(); layer;
           layer = layer->Next()) {
        if (layer->GetImage()->IsImageResource()) {
          if (layer->GetImage()->CachedImage())
            currImage = layer->GetImage()->CachedImage()->GetImage();
          break;
        }
      }
    }
    ...
    // 2-1. check if the size of the object in content is big enough.
    IntRect visualRect = IntRect(object->VisualRect());
    const int relativeSize = resolution / 3;
    if (visualRect.Width() < relativeSize || visualRect.Height() < relativeSize)
      continue;

    // 2-2. check if the natural size is larger than the given.
    const float width = currImage->width();
    const float height = currImage->height();
    if (width < resolution || height < resolution)
      continue;

    // 2-3. check if the aspect ratio is supportable.
    float curRatio = (width > height) ? width / height : height / width;
    if (curRatio > aspectRatio)
      continue;

    // 3-1. check if the intersected region exists.
    layoutRect.Intersect(visibleRect);
    if (layoutRect.IsEmpty())
      continue;

    // 3-2. check if intersected region covers at least 70% of the whole image.
    if ((layoutRect.Width() * layoutRect.Height()) <
        (visualRect.Width() * visualRect.Height() / 10 * 7))
      continue;

    // 4. (1) the closer the image to the center, the higher the priority.
    //    (2) the bigger the image, the higher the priority when the distances
    //    are exactly same.
    const int currSquare = layoutRect.Width() * layoutRect.Height();
    const int currDistance = center.DistanceSquaredToPoint(layoutRect.Center());
    if ((currDistance < distance) ||
        (currDistance == distance && currSquare > square)) {
      distance = currDistance;
      square = currSquare;
      image = currImage;
    }

    // TODO: additional rules should be added to select the most proper image.
  }
...
bool Document::IsVideoPlaying() {
  return ExecutionContext::IsVideoPlaying();
}

#if defined(OS_TIZEN_TV_PRODUCT)
bool Document::SetTranslatedURL(const String& url) {
  return ExecutionContext::SetTranslatedURL(url);
}

void Document::SetPreferTextLang(const String& lang) {
  ExecutionContext::SetPreferTextLang(lang);
}

void Document::SetParentalRatingResult(const String& url, bool is_pass) {
  ExecutionContext::SetParentalRatingResult(url, is_pass);
}
#endif
