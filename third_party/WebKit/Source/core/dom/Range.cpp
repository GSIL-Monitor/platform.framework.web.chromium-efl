...
void Range::TextBoundingBox(Vector<FloatQuad>& quads) const
{
    Node* startContainer = &start_.Container();
    Node* endContainer = &end_.Container();
    ...
    if (startContainer && endContainer) {
        ...
        for (Node* node = FirstNode(); node != stopNode; node = NodeTraversal::Next(*node)) {
            ...
            if (r && r->IsText()) {
                int startOffset = (node == startContainer) ? start_.Offset() : 0;
                int endOffset = (node == endContainer) ? end_.Offset() : std::numeric_limits<int>::max();
                LayoutText* renderText = ToLayoutText(r);
                renderText->AbsoluteQuadsForRange(quads, startOffset, endOffset);
            }
        }
    }
}

void Range::TextAndImagesBoundingBox(Vector<FloatQuad>& quads) const
{
    Node* startContainer = &start_.Container();
    Node* endContainer = &end_.Container();
    ...
    Node* stopNode = PastLastNode();
    for (Node* node = FirstNode(); node != stopNode; node = NodeTraversal::Next(*node)) {
        LayoutObject* renderer = node->GetLayoutObject();
        if (!renderer)
            continue;
        if (renderer->IsText()) {
            int startOffset = (node == startContainer) ? start_.Offset() : 0;
            int endOffset = (node == endContainer) ? end_.Offset() : std::numeric_limits<int>::max();
            ToLayoutText(renderer)->AbsoluteQuadsForRange(quads, startOffset, endOffset);
        } else if (renderer->IsImage()) {
            quads.push_back(ToLayoutImage(renderer)->AbsoluteBoundingBoxRect());
        }
    }
}
