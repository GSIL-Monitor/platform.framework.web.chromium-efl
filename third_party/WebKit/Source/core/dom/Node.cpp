...
#if defined(TIZEN_ATK_FEATURE_VD)
bool Node::IsAXObject() {
  if (!CheckCSSVisibility())
    return false;

  if (AXObjectCache* cache = GetDocument().ExistingAXObjectCache()) {
    AXObject* obj = cache->GetAXObject(this);
    if (obj && !obj->AccessibilityIsIgnored()) {
      if (obj->IsWidget() || obj->IsComposite())
        return true;

      // <div> <p> ..., elements with role isTextObjectType or role isSection
      // and have (name or description or value string or help attr) in itself
      if ((obj->IsTextObjectType() || obj->IsSection()) && obj->HasText())
        return true;

      // <div> <p> ..., elements without (name, description,value string, help
      // attr) in itself but only have isTextObjectType children, and there is
      // speech content in children
      // this is for focus in article (to satisfy B2G with the US government)
      if (IsWebBrowser() && (obj->IsTextObjectType() || obj->IsSection()) &&
          obj->HasOnlyTextChildren() && obj->HasTextInChildren())
        return true;
    }
  }
  return false;
...
bool Node::CheckCSSVisibility() {
  if (IsElementNode()) {
    Element& element = ToElement(*this);
    const ComputedStyle* style = element.EnsureComputedStyle();
    if (style && !style->IsOverflowVisible() &&
        style->ClipLeft().IntValue() == 0 &&
        style->ClipRight().IntValue() == 0 &&
        style->ClipTop().IntValue() == 0 &&
        style->ClipBottom().IntValue() == 0 && !style->HasAutoClip())
      return false;
  }
  return true;
}
