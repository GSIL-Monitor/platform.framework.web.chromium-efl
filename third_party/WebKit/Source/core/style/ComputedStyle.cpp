...
Length ComputedStyle::LineHeight() const {
  ...
  if (lh.IsFixed()) {
    ...
    // Only applying the scalefactor when the text size is changed
    // and the text doesn't have the multiplier value
    if (desc.TextScaleFactor() != 1.0f && multiplier < desc.TextScaleFactor())
      multiplier = desc.TextScaleFactor();
#endif
    return Length(
...
#if defined(S_TERRACE_SUPPORT)
  FontDescription desc(GetFontDescription());
  // Don't apply multiplier values when the scalefactor is changed.
  if (multiplier == 1.0f && desc.TextScaleFactor() != 1.0f)
    return;
#endif
