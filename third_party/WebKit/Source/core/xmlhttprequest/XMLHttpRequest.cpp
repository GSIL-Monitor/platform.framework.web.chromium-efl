...
XMLHttpRequest::XMLHttpRequest(
    ...
    : SuspendableObject(context),
      ...
      send_flag_(false),
#if defined(OS_TIZEN)
      async_start_timer_(this, &XMLHttpRequest::AsyncStartTimerFired),
#endif
...
#if defined(OS_TIZEN)
void XMLHttpRequest::AsyncStartTimerFired(TimerBase*) {
  DCHECK(!loader_);
  loader_ =
      ThreadableLoader::Create(*GetExecutionContext(), this, async_options_,
                               async_resource_loader_options_);
  loader_->Start(async_request_);
}
#endif
...
void XMLHttpRequest::CreateRequest(RefPtr<EncodedFormData> http_body,
...
#if defined(USE_EFL)
  unsigned long timeout_milliseconds_global = 0;
  if (this->GetExecutionContext()->IsDocument() && GetDocument()
      && GetDocument()->GetFrame() && GetDocument()->GetFrame()->GetPage())
    timeout_milliseconds_global = GetDocument()->GetFrame()->
        GetPage()->GetLongPollingGlobalTimeout() * 1000;

  unsigned long timeout_to_set = timeout_milliseconds_;
  if (timeout_milliseconds_global > 0 && timeout_milliseconds_global < timeout_to_set)
    timeout_to_set = timeout_milliseconds_global;

  options.timeout_milliseconds = timeout_to_set == 0 ?
  ...
  if (async_) {
    ...
    if (IsMobileProfile() &&
      ...
      async_request_ = request;
      async_options_ = options;
      async_resource_loader_options_ = resource_loader_options;
      async_start_timer_.StartOneShot(0, FROM_HERE);
      return;
    }
#endif
...
void XMLHttpRequest::DidReceiveData(const char* data, unsigned len) {
  ...
  if (response_type_code_ == kResponseTypeDocument && ResponseIsHTML()) {
    ...
    if (RuntimeEnabledFeatures::TizenCompatibilityModeEnabled()) {
      // NULL is not a valid symbol in JSON so it should be removed.
      text = text.Replace('\0', "");
    }
#endif
    if (!text.IsEmpty() && !response_text_overflow_) {
