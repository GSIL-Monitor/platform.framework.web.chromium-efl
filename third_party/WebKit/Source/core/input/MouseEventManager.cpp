...
void MouseEventManager::EnterDragState(
  ...
  if (frame_->GetSettings() &&
      frame_->GetSettings()->GetTouchDragDropEnabled() && frame_->View()) {
    mouse_down_ = WebMouseEvent(WebInputEvent::kMouseDown, last_gesture_event,
                                WebPointerProperties::Button::kLeft, 1,
                                WebInputEvent::Modifiers::kLeftButtonDown,
                                TimeTicks::Now().InSeconds());

    WebMouseEvent mouse_drag_event(WebInputEvent::kMouseMove,
                                   last_gesture_event,
                                   WebPointerProperties::Button::kLeft, 1,
                                   WebInputEvent::Modifiers::kLeftButtonDown,
                                   TimeTicks::Now().InSeconds());
...
#if defined(OS_TIZEN_TV_PRODUCT)
  // Disable the DND(drop & drag) feature for TV WebBrowser Requirement.
  if (IsWebBrowser() && frame_->GetSettings() &&
      !frame_->GetSettings()->GetDragDropEnabled()) {
