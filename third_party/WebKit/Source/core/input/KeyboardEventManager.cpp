...
#if defined(TIZEN_ATK_FEATURE_VD)
  if (event->key() == "ArrowDown" || event->keyCode() == VKEY_DOWN)
    ret_val = kWebFocusTypeDown;
  else if (event->key() == "ArrowUp" || event->keyCode() == VKEY_UP)
    ret_val = kWebFocusTypeUp;
  else if (event->key() == "ArrowLeft" || event->keyCode() == VKEY_LEFT)
    ret_val = kWebFocusTypeLeft;
  else if (event->key() == "ArrowRight" || event->keyCode() == VKEY_RIGHT)
...
#if defined(TIZEN_ATK_FEATURE_VD)
bool KeyboardEventManager::ShouldFakeTabKeyEvent(KeyboardEvent* event,
                                                 Node* node) {
  AXObjectCache* cache = node->GetDocument().ExistingAXObjectCache();
  if (cache && cache->ShouldFakeTabKeyEvent(event, node))
    return true;

  // For media element, Usually use LEFT/RIGHT to control progress bar and use
  // UP/DOWN to control volume and use TAB to navigation, but there is not TAB
  // in TV remote control, consider there are volume key on remote control, so
  // leave LEFT/RIGHT for JS and change UP/DOWN to TAB/SHIFT TAB
  WebFocusType type = FocusDirectionForKey(event);
  if (kWebFocusTypeUp != type && kWebFocusTypeDown != type)
    return false;

  bool is_in_media_player = false;
  const Node* current = node;
  while (current) {
    const Element* cur_element =
        current->IsElementNode() ? ToElement(current) : nullptr;
    if (current->IsMediaControls() || current->IsMediaControlElement() ||
        (cur_element && IsHTMLMediaElement(cur_element))) {
      is_in_media_player = true;
      break;
    }

    // Special case: the media control element of youtube.com is just a DIV, and
    // don't has Media Control Ancestor. use their class to distinguish them
    if (cur_element) {
      const AtomicString& class_attr =
          cur_element->getAttribute(HTMLNames::classAttr);
      if (class_attr.Contains("html5-video-player")) {
        is_in_media_player = true;
        break;
      }
    }
  ...
  if (is_in_media_player) {
    LOG(INFO) << "For media element, convert DOWN/UP to TAB/SHIFT TAB";
    return true;
  }
...
WebKeyboardEvent KeyboardEventManager::CreateFakeTabKeyDownEvent(
    KeyboardEvent* event) {
  WebKeyboardEvent web_event(
      WebInputEvent::kRawKeyDown, WebInputEvent::kNoModifiers,
      ui::EventTimeStampToSeconds(base::TimeTicks::Now()));

  WebFocusType type = FocusDirectionForKey(event);
  if (kWebFocusTypeUp == type || kWebFocusTypeLeft == type)
    web_event.SetModifiers(WebInputEvent::kShiftKey);

  web_event.text[0] = VKEY_TAB;
  web_event.windows_key_code = VKEY_TAB;
  web_event.dom_key = Platform::Current()->DomKeyEnumFromString("Tab");
  return web_event;
}
#endif
...
#if defined(TIZEN_ATK_FEATURE_VD)
  if (IsWebBrowser() && frame_->GetSettings() &&
      frame_->GetSettings()->GetAccessibilityEnabled() &&
      FocusDirectionForKey(keydown) != kWebFocusTypeNone) {
    if (ShouldFakeTabKeyEvent(keydown, node))
      keydown = KeyboardEvent::Create(CreateFakeTabKeyDownEvent(keydown),
...
void KeyboardEventManager::DefaultArrowEventHandler(
  ...
  if (type != kWebFocusTypeNone && IsSpatialNavigationEnabled(frame_) &&
    ...
    if (page->GetFocusController().AdvanceCSSNavigationFocus(type)) {
      event->SetDefaultHandled();
      return;
    }
#endif
