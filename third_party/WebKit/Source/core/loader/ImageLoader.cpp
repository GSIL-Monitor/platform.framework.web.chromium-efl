...
#if defined(OS_TIZEN_TV_PRODUCT)
    // Currently youtube.com appbar red icons are shown after a LoadFinished
    // callback. This changes the url to load the icons early using the data
    // thumb attribute in the WebProduct lib.
    if (IsWebBrowser()) {
      url = KURL(
          kParsedURLString,
          wp_tv_get_replaced_url_for_image_load_earlier_if_needed(
              element_->GetDocument().BaseURL().GetString().Utf8().data(),
              url.GetString().Utf8().data(),
              element_->getAttribute("data-thumb").GetString().Utf8().data())
              .c_str());
    }
#endif
