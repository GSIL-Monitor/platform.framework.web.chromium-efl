...
inline float ParentTextZoomFactor(LocalFrame* frame) {
  ...
  if (!parent || !parent->IsLocalFrame()) {
    ...
    return static_cast<float>(
        frame->GetPage()->GetSettings().GetAccessibilityFontScaleFactor());
#else
    return 1;
#endif
  }
...
#if defined(USE_EFL)
    has_visible_scrollbars_ = View()->HasVisibleScrollbars();
#endif
  }
...
#if defined(USE_EFL)
    frame_view->ChangeScrollbarVisibility(has_visible_scrollbars_);
#endif
  } else {
...
void LocalFrame::NotifyUrlForPlayingVideo(const KURL& url) {
  if (page_)
    page_->NotifyUrlForPlayingVideo(url);
}
