...
void LocalFrameView::ScrollbarSizeDidChange() {
  UpdateScrollbarGeometry();
  UpdateScrollCorner();
  PositionScrollbarLayers();
}
...
void LocalFrameView::ChangeScrollbarVisibility(bool visible) {
  ...
  if (visible && visual_viewport.LayerForContainer()) {
    ScrollbarThemeOverlay& theme = ScrollbarThemeOverlay::MobileTheme();

    const int scrollbar_thickness = theme.ScrollbarThickness(kRegularScrollbar);
    horizontal_scrollbar_width =
        visual_viewport.LayerForContainer()->Size().Width() -
        scrollbar_thickness;
    horizontal_scrollbar_height = scrollbar_thickness;

    vertical_scrollbar_width = scrollbar_thickness;
    vertical_scrollbar_height =
        visual_viewport.LayerForContainer()->Size().Height() -
        scrollbar_thickness;
  }
  ...
  if (scrollbar_graphics_layer)
    scrollbar_graphics_layer->SetContentsRect(
        IntRect(0, 0, horizontal_scrollbar_width, horizontal_scrollbar_height));

  scrollbar_graphics_layer = visual_viewport.LayerForVerticalScrollbar();
  if (scrollbar_graphics_layer)
    scrollbar_graphics_layer->SetContentsRect(
        IntRect(0, 0, vertical_scrollbar_width, vertical_scrollbar_height));
}
#endif
...
/*        if (frame_->IsMainFrame() && GetFrame().Loader().Client()->IsLocalFrameClientImpl()) {
          LOG(INFO) << "LoaderLog firstLayout";
        }
*/
        first_layout_ = false;
...
Scrollbar* LocalFrameView::ScrollbarAtFramePoint(
  ...
  // scrollbar rect with corner size. It will prevent deliverng the mouse
  // event to content's element when mouse works on the scroll corner area.
  if (VerticalScrollbar() &&
      VerticalScrollbar()->ShouldParticipateInHitTesting()) {
    IntRect frame_rect = VerticalScrollbar()->FrameRect();
    frame_rect.SetHeight(frame_rect.Height() +
                         VerticalScrollbar()->CornerHeight());
    if (frame_rect.Contains(point_in_frame))
...
#if defined(USE_EFL)
  if (!has_visible_scrollbars_) {
    if (LayerForHorizontalScrollbar()) {
      LayerForHorizontalScrollbar()->SetContentsRect(IntRect(0, 0, 0, 0));
    }
    if (LayerForVerticalScrollbar()) {
      LayerForVerticalScrollbar()->SetContentsRect(IntRect(0, 0, 0, 0));
    }
  }
#endif
}
...
#if defined(TIZEN_VD_NATIVE_SCROLLBARS)
  if (HasOverlayScrollbars() && !UseNativeScrollbars())
    return corner_rect;
#else
  if (HasOverlayScrollbars())
    return corner_rect;
...
void LocalFrameView::ThumbPartFocusChanged(ScrollbarOrientation orientation,
                                           bool focused) {
  if (Page* page = GetPage())
    return page->GetChromeClient().ThumbPartFocusChanged(orientation, focused);
}
#endif
