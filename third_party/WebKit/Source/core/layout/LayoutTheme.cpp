...
static String FormatChromiumMediaControlsTime(float time,
...
#if defined(OS_TIZEN)
  if (IsMobileProfile()) {
    int durationHours = durationSecs / kSecondsPerHour;
    durationMins %= 60;
    minutes %= 60;
    int hours = seconds / kSecondsPerHour;
  ...
  minutes %= 60;
  if (durationHours || hours)
    return String::Format("%s%s%01d:%02d:%02d", separator,
...
#if defined(OS_TIZEN_TV_PRODUCT)
    if (IsWebBrowser() && settings && settings->GetAccessibilityEnabled())
      should_draw = true;
#elif defined(TIZEN_ATK_SUPPORT)
    if (IsMobileProfile() && settings &&
        (settings->HwKeyboardConnected() ||
         settings->GetAccessibilityEnabled()))
      should_draw = true;
#endif  // OS_TIZEN_TV_PRODUCT
  }
  if (!should_draw && ThemeDrawsFocusRing(style))
    return false;
#else  // OS_TIZEN
...
void LayoutTheme::SetCaretBlinkInterval(double interval) {
  ...
  // Changing caret blinking interval
  // from the default 0.5s to 1s for better power consumption.
  if (caret_blink_interval_ != 1.0)
    caret_blink_interval_ = 1.0;
}
