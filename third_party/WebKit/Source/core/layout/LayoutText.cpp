...
static String ConvertToZawgyiEncoding(String text) {
  StringBuilder result;
  result.ReserveCapacity(text.length());
  for (size_t i = 0; i < text.length(); i++) {
      if (0x1000 <= text[i] && text[i] <= 0x109F)
          result.Append(text[i] | 0xF100);
      else
...
LayoutText::LayoutText(Node* node, RefPtr<StringImpl> str)
  ...
  if (Document::kZawgyiEnabled)
    text_ = ConvertToZawgyiEncoding(text_);
