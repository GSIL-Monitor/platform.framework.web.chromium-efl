...
HTMLFormElement::HTMLFormElement(Document& document)
    : HTMLElement(formTag, document),
      ...
      ,
      autologin_timer(this, &HTMLFormElement::AutologinTimerFired)
#endif
{
}
...
#if defined(S_TERRACE_SUPPORT)
void HTMLFormElement::AutologinTimerFired(TimerBase*) {
  LocalFrame* frame = GetDocument().GetFrame();
  // If form is already submitted, there is no need to send again
  if (frame && !WasSubmitted()) {
    frame->Client()->DispatchPasswordFormWasVisible(
        this, frame->IsLoading());
  }
}
#endif
...
bool HTMLFormElement::WasSubmitted() const {
  return was_submitted;
}

#if defined(S_TERRACE_SUPPORT)
void HTMLFormElement::PasswordFieldWasRendered() {
  if (!autologin_timer.IsActive())
    autologin_timer.StartOneShot(0, BLINK_FROM_HERE);
}
#endif
