...
bool AllowedToRequestFullscreen(Document& document) {
  ...
  // Samsung video assistant should work enter fullscreen without user gesture.
...
bool FullscreenElementReady(const Element& element) {
  ...
  if (RuntimeEnabledFeatures::UseSamsungMediaControlsEnabled()) {
     // Samsung video player has a case to make fullscreen for video element forcely.
     if (!IsHTMLVideoElement(element) &&
         !AllowedToUseFullscreen(element.GetDocument().GetFrame()))
       return false;
  } else {
