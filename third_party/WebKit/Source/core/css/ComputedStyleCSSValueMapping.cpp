...
static CSSValue* ValueForNavigationDirection(const ComputedStyle& style,
                                             CSSPropertyID property_id) {
  StyleNavigationValue nav_value;

  switch (property_id) {
    case CSSPropertyNavDown:
      nav_value = style.NavDown();
      break;
    case CSSPropertyNavUp:
      nav_value = style.NavUp();
      break;
    case CSSPropertyNavLeft:
      nav_value = style.NavLeft();
      break;
    case CSSPropertyNavRight:
      nav_value = style.NavRight();
      break;
    default:
      NOTREACHED();
      break;
  ...
  selector_text.append(nav_value.Id());
  CSSValue* id_sel = CSSStringValue::Create(selector_text);
  CSSValue* target = nullptr;
  if (nav_value.NavigationTarget() == TargetName)
    target = CSSStringValue::Create(nav_value.GetTargetName());
  else if (nav_value.NavigationTarget() == Root)
    target = CSSIdentifierValue::Create(CSSValueRoot);
  else if (nav_value.NavigationTarget() == Current)
...
const CSSValue* ComputedStyleCSSValueMapping::Get(
  ...
  switch (property_id) {
    ...
    case CSSPropertyNavRight:
    case CSSPropertyNavUp:
      return ValueForNavigationDirection(style, property_id);
