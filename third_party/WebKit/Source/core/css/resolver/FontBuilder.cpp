...
float FontBuilder::GetComputedSizeFromSpecifiedSize(
  ...
  font_description.SetTextScaleFactor(zoom_factor);
#endif
...
void FontBuilder::UpdateComputedSize(FontDescription& font_description,
  ...
  // Don't apply multiplier values when the text size is changed.
  if (font_description.TextScaleFactor() != 1.0f &&
      multiplier < font_description.TextScaleFactor()) {
    computed_size =
        TextAutosizer::ComputeAutosizedFontSize(computed_size, 1.0f);
