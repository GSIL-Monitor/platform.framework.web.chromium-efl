...
#if defined(USE_EFL)
      view_mode_(ViewModeInvalid),
#endif
      m_hasOverviewModeChanged(false),
#if defined(USE_EFL)
      long_polling_global_timeout_(0),
      defers_loading_(false),
#endif
#if defined(OS_TIZEN_TV_PRODUCT)
      is_floating_page_(false),
      is_view_resumed_by_tab_switching_(false),
#endif
#if defined(OS_TIZEN)
      m_isSuspended(false),
#endif
...
#if defined(USE_EFL)
void Page::SetViewMode(ViewMode mode) {
  DCHECK(mode != ViewModeInvalid);
  DCHECK(main_frame_);
...
void Page::SettingsChanged(SettingsDelegate::ChangeType change_type) {
  switch (change_type) {
    ...
    case SettingsDelegate::kViewportDescriptionChange:
      setHasOverviewModeChanged(true);
      if (MainFrame() && MainFrame()->IsLocalFrame()) {
...
#if defined(USE_EFL)
    case SettingsDelegate::kTizenVersionChange: {
      RuntimeEnabledFeatures::SetTizenCompatibilityModeEnabled(
...
void Page::NotifyUrlForPlayingVideo(const KURL& url) {
  GetChromeClient().NotifyUrlForPlayingVideo(url);
}

#if defined(USE_EFL)
void Page::SetLongPollingGlobalTimeout(unsigned long timeout) {
  long_polling_global_timeout_ = timeout;
}

unsigned long Page::GetLongPollingGlobalTimeout() {
...
#if defined(OS_TIZEN)
void Page::SetIsSuspended(bool isSuspended) {
