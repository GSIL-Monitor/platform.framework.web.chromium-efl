...
bool PaintLayerScrollableArea::HitTestOverflowControls(
  ...
  if (HasVerticalScrollbar() &&
    ...
    v_bar_rect.SetHeight(v_bar_rect.Height() +
...
void PaintLayerScrollableArea::ScrollbarSizeDidChange() {
  PositionOverflowControls();
  SetScrollCornerNeedsPaintInvalidation();
  layer_.UpdateLayerPositionsAfterScrollbarSizeChange();
}
#endif
...
void PaintLayerScrollableArea::ThumbPartFocusChanged(
  ...
  if (LocalFrame* frame = Box().GetFrame()) {
    if (Page* page = frame->GetPage())
      page->GetChromeClient().ThumbPartFocusChanged(orientation, focused);
  }
}
#endif
