...
void NavigatorContentUtilsClient::registerContentHandler(const String& mimeType, const KURL& baseURL, const KURL& url, const String& title)
{
    web_frame_->Client()->RegisterProtocolHandler(mimeType, url, title);
}

NavigatorContentUtilsClient::CustomHandlersState NavigatorContentUtilsClient::isContentHandlerRegistered(const String& mimeType, const KURL& baseURL, const KURL& url)
{
    return static_cast<NavigatorContentUtilsClient::CustomHandlersState>(web_frame_->Client()->isContentHandlerRegistered(mimeType, baseURL, url));
}

void NavigatorContentUtilsClient::unregisterContentHandler(const String& mimeType, const KURL& baseURL, const KURL& url)
{
    web_frame_->Client()->unregisterContentHandler(mimeType, baseURL, url);
}
