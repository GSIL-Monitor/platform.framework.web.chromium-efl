...
void FetchManager::Loader::DidReceiveResponse(
  ...
  if (request_->Url().IsLocalFile()) {
    if (response.HttpStatusCode() == 0)  // No error
      response_data->SetStatus(200);
    else
      response_data->SetStatus(500);
  } else {
...
void FetchManager::Loader::PerformSchemeFetch() {
  ...
  if (SchemeRegistry::ShouldTreatURLSchemeAsSupportingFetchAPI(
    ...
    PerformDataFetch();
  } else if (request_->Url().IsLocalFile()) {
    PerformFileFetch();
  } else {
...
void FetchManager::Loader::PerformFileFetch() {
  DCHECK(request_->Url().IsLocalFile());
