...
void DoHost(const CHAR* spec,
  ...
  Component scheme;
  ExtractScheme(spec, host.begin, &scheme);
#endif

  if (DoHostSubstring<CHAR, UCHAR>(spec, host, output)) {
#if defined(OS_TIZEN_TV_PRODUCT)
    if (CompareSchemeComponent(spec, scheme, url::kDvbScheme)) {
      // The above host checking code can think that the DVB locator is an
      // IP address. So we skip the IP Address check if it starts with DVB.
      // This allows the later code to check the validity by access.
      // TODO: Validate the DVB Locator.
      LOG(INFO) << "This is a DVB Locator " << spec;
    } else if (CompareSchemeComponent(spec, scheme, url::kTVKeyScheme)) {
      // TVKeys address starts with an hexadecimal number so could be
      // confused with an IP Address. Also an IP Address would be a
      // security hole.
      LOG(INFO) << "This is a TVKeys Url " << spec;
      host_info->family = CanonHostInfo::NEUTRAL;
    } else if (CompareSchemeComponent(spec, scheme, url::kOpAppScheme)) {
      // The above host checking code can think that the OpApp locator is an
      // IP address. So we skip the IP Address check if it starts with
      // 'hbbtv-package'.
      // This allows the later code to check the validity by access.
      LOG(INFO) << "This is an OpApp Locator " << spec;
      host_info->family = CanonHostInfo::NEUTRAL;
    } else if (CompareSchemeComponent(spec, scheme, url::kHbbTVCarouselScheme)) {
      // HbbTV Carousel URLs have a pattern that could be mistaken for
      // a IP address with a port (a single integer). So again we need to
      // skip the IP check.
      //
      ...
      //
      // Ref: HbbTV 2.0.1 Specification (errata 2) section 6.3.2
      LOG(INFO) << "This is a HbbTV Carousel Url " << spec;
    } else if (CompareSchemeComponent(spec, scheme, url::kCIScheme)) {
      // The above host checking code can think that the CI locator is an
      // IP address. So we skip the IP Address check if it starts with CI.
      // This allows the later code to check the validity by access.
      LOG(INFO) << "This is a HbbTV CI Locator " << spec;
      host_info->family = CanonHostInfo::NEUTRAL;
    } else
#endif
    {
