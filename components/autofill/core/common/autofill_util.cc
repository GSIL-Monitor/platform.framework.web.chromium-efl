...
bool IsUpiEnabled() {
  // If the switch |enable-upi-vpa| is present and enabled, parsing is done
  // to identify upi fields.
  if (base::CommandLine::ForCurrentProcess()->HasSwitch("enable-upi-vpa")) {
    std::string value =
        base::CommandLine::ForCurrentProcess()->GetSwitchValueASCII(
            "enable-upi-vpa");
    if (value == kUpiVpaEnabled)
      return true;
  }
  return false;
}
