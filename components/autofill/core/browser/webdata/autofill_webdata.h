...
class AutofillWebData {
 public:
  ...
  // Schedules a task to add upi data to the web database.
  virtual void AddUpiData(const UPIName& upi_data) = 0;

  // Schedules a task to update upi data in the web database.
  virtual void UpdateUpiData(const UPIName& upi_data) = 0;
  ...
  // Schedules a task to remove a upi data from the web database.
  // |guid| is identifier of the upi data to remove.
  virtual void RemoveUpiData(const std::string& guid) = 0;
  ...
  virtual WebDataServiceBase::Handle GetUpiData(
      WebDataServiceConsumer* consumer) = 0;
