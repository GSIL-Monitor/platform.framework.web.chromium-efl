...
class PersonalDataManager : public KeyedService,
 ...
 public:
  ...
  // Saves |imported_upi_data| to the WebDB if it exists. Returns the guid of
  // of the new or updated upi field, or the empty string if no upi was saved.
  std::string SaveImportedUpiData(const UPIName& imported_upi_data);
  ...
  // Returns the upi data with the specified |guid|, or NULL if there is
  // no upi with the specified |guid|.
  virtual UPIName* GetUPINameByGUID(const std::string& guid);
  ...
  // Adds |upi_name| to the web database.
  void AddUPIName(const UPIName& upi_name);

  // Updates |upi_name| which already exists in the web database.
  virtual void UpdateUPIName(const UPIName& upi_name);
  ...
  // Returns all UPI Names.
  virtual const std::vector<UPIName*>& GetUPINames() const;
  ...
  // Returns the upi data to suggest to the user.
  const std::vector<UPIName*> GetUpiDataToSuggest() const;
  ...
  // Gets upi data that can suggest data for |type|. See
  // GetProfileSuggestions for argument descriptions.
  std::vector<Suggestion> GetUpiSuggestions(
      const AutofillType& type,
      const base::string16& field_contents);

  bool IsUPIGuidLocallyExists(const UPIName& upi_name);
 ...
 protected:
  ...
  // Sets |upi_data_| to the contents of |upi_data| and updates the web
  // database by adding, updating and removing credit cards.
  void SetUpiData(std::vector<UPIName>& upi_data);
  ...
  // Loads the saved upi dara from the web database.
  virtual void LoadUpiData();
  ...
  // Cached versions of the upi datas.
  std::vector<std::unique_ptr<UPIName>> upi_data_;

  mutable std::vector<UPIName*> upi_names_;
  ...
  WebDataServiceBase::Handle pending_upidata_query_;
 ...
 public:
  bool ImportCreditCard(
      ...
      bool ignore_merging = false,
      // FIXME: m60_3112 bringup, needs to be checked by author
      // https://codereview.chromium.org/2855943002
      bool* imported_credit_card_matches_masked_server_credit_card = nullptr);

  bool ImportUpiVpa(const FormStructure& form,
