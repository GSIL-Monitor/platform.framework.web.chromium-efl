...
AutofillExternalDelegate::AutofillExternalDelegate(AutofillManager* manager,
    ...
    : manager_(manager),
      ...
      should_show_credit_card_helper_(false),
      should_show_upi_helper_(false),
#endif
...
#if defined(S_TERRACE_SUPPORT)
  should_show_credit_card_helper_ =
      manager_->ShouldShowCreditCardHelper(query_form_, query_field_);
  should_show_upi_helper_ =
      manager_->ShouldShowUpiHelper(query_form_, query_field_);

  // This should be called !before! ShowAutofillPopup
  manager_->client()->UseCreditCardHelper(should_show_credit_card_helper_);
  manager_->client()->UseUpiHelper(should_show_upi_helper_);
#endif
}
...
#if defined(S_TERRACE_SUPPORT)
  if (suggestions.empty() && !should_show_credit_card_helper_ &&
      !should_show_upi_helper_) {
    // No suggestions, any popup currently showing is obsolete.
    manager_->client()->HideAutofillPopup();
    manager_->client()->DestroySuggestionPopup();
    return;
  }
#else
  if (suggestions.empty()) {
...
#if defined(S_TERRACE_SUPPORT)
  } else if (identifier == POPUP_ITEM_ID_POPUP_HELPER) {
    FormStructure form_structure(query_form_);
    form_structure.ParseFieldTypesFromAutocompleteAttributes();
    if (form_structure.ShouldBeParsed())
      form_structure.DetermineHeuristicTypes(nullptr);

    if (should_show_upi_helper_) {
      std::unique_ptr<UPIName> upi_name;
      std::string upi_vpa_registered =
          manager_->client()->GetRegisteredUpiVpa();
      if (!upi_vpa_registered.empty()) {
        bool saved = manager_->client()->GetPersonalDataManager()->ImportUpiVpa(
            form_structure, &upi_name, base::UTF8ToUTF16(upi_vpa_registered));
        if (saved)
          AutofillUpiVpa(*upi_name);
      }
    } else {
      std::unique_ptr<CreditCard> credit_card;
      manager_->client()->GetPersonalDataManager()->ImportCreditCard(
          form_structure, true /* should_return_local_card */, &credit_card,
          true /* ignore_validation */, true /* ignore_merging */);

      manager_->client()->OpenCreditCardHelper(
          credit_card ? *credit_card : CreditCard(),
          base::Bind(&AutofillExternalDelegate::DidOpenCreditCardHelper,
                     GetWeakPtr()));
    }
#endif
  } else {
...
#if defined(S_TERRACE_SUPPORT)
void AutofillExternalDelegate::UpdateFormData(const FormData& form) {
  if (query_form_.SameFormAs(form))
    query_form_ = form;
}

void AutofillExternalDelegate::DidChangeScrollOffset(
    const gfx::RectF& element_bounds) {
  if (should_show_credit_card_helper_ || should_show_upi_helper_) {
    manager_->client()->DidChangeScrollOffset(element_bounds);
  }
}

void AutofillExternalDelegate::DidOpenCreditCardHelper(
    const CreditCard& credit_card) {
  manager_->FillCreditCardForm(query_id_, query_form_, query_field_,
                               credit_card, base::string16());
}

void AutofillExternalDelegate::AutofillUpiVpa(const UPIName& upi_name) {
  manager_->FillUpiForm(query_id_, query_form_, query_field_, upi_name);
}

void AutofillExternalDelegate::HideUPISuggestionPopupFullMode(
    const gfx::RectF& element_bounds) {
  if (should_show_upi_helper_)
    manager_->client()->HideUPISuggestionPopupFullMode(element_bounds);
}
#endif
