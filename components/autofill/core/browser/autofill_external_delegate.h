...
class AutofillExternalDelegate : public AutofillPopupDelegate {
 public:
  ...
  void DidChangeScrollOffset(const gfx::RectF& element_bounds);

  void HideUPISuggestionPopupFullMode(const gfx::RectF& element_bounds);

  // Called when a credit card object is passed from credit card helper.
  void DidOpenCreditCardHelper(const CreditCard& credit_card);

  void AutofillUpiVpa(const UPIName& upi_name);
#endif
...
#if defined(S_TERRACE_SUPPORT)
  // Whether the credit card helper should be shown to the user
  bool should_show_credit_card_helper_;
  bool should_show_upi_helper_;
#endif
