...
enum UpiEventTypes {
  EVENT_UPI_FORM_AUTOFILL_NONPINGPAY = 0,
  EVENT_UPI_FORM_AUTOFILL_PINGPAY,
  EVENT_UPI_FORM_AUTOFILL_HELPERPOPUP_CREATED
};
#endif
namespace {
...
#if defined(S_TERRACE_SUPPORT)
bool AutofillManager::ShouldShowCreditCardHelper(const FormData& form,
                                                 const FormFieldData& field) {
  AutofillField* autofill_field = GetAutofillField(form, field);
  if (!autofill_field || autofill_field->Type().group() != CREDIT_CARD)
    return false;
  ...
  ServerFieldType storable_type = autofill_field->Type().GetStorableType();
  if (storable_type != CREDIT_CARD_NAME_FULL &&
      storable_type != CREDIT_CARD_NUMBER &&
      storable_type != CREDIT_CARD_EXP_MONTH &&
      storable_type != CREDIT_CARD_EXP_2_DIGIT_YEAR &&
      storable_type != CREDIT_CARD_EXP_4_DIGIT_YEAR &&
      storable_type != CREDIT_CARD_EXP_DATE_2_DIGIT_YEAR &&
      storable_type != CREDIT_CARD_EXP_DATE_4_DIGIT_YEAR) {
    return false;
  }
...
bool AutofillManager::ShouldShowUpiHelper(const FormData& form,
                                          const FormFieldData& field) {
  AutofillField* autofill_field = GetAutofillField(form, field);
  if (!autofill_field || autofill_field->Type().group() != UPI_DATA)
    return false;
...
bool AutofillManager::OnFormSubmitted(const FormData& form) {
  // TODO(ashlin.j): Remove the log once the upi feature testing phase is done.
  LOG(INFO) << "AutofillManager::OnFormSubmitted";
...
#if defined(S_TERRACE_SUPPORT)
void AutofillManager::ImportUpiData(const FormStructure& form) {
  UPIName candidate_upi_name;
  candidate_upi_name.set_origin(form.source_url().spec());
  for (const auto& field : form) {
    ...
    if (field_type.group() != UPI_DATA)
      continue;

    bool saved = candidate_upi_name.SetInfo(field_type, value, app_locale_);

    if (saved && candidate_upi_name.IsValid(value))
      client_->ConfirmSaveUPILocally(
          candidate_upi_name,
          base::Bind(
              base::IgnoreResult(&PersonalDataManager::SaveImportedUpiData),
              base::Unretained(personal_data_), candidate_upi_name));
  }
}
#endif
...
#if defined(S_TERRACE_SUPPORT)
  ProcessPossibleCreditCardForm(form);
#endif
}

#if defined(S_TERRACE_SUPPORT)
void AutofillManager::ProcessPossibleCreditCardForm(const FormData& form) {
  std::unique_ptr<FormStructure> possible_creditcard_form =
      ValidateSubmittedForm(form);
  if (!possible_creditcard_form)
    return;

  CheckAndReplaceCreditCardIfMasked(*possible_creditcard_form);
  if (possible_creditcard_form->IsAutofillable()) {
    ImportCreditCardData(*possible_creditcard_form);
  }
}

void AutofillManager::CheckAndReplaceCreditCardIfMasked(
    const FormStructure& submitted_form) {
  for (const auto& field : submitted_form) {
    ServerFieldType server_field_type = field->Type().GetStorableType();
    if (server_field_type == CREDIT_CARD_NUMBER) {
      base::string16 value;
      base::TrimWhitespace(field->value, base::TRIM_ALL, &value);
      std::string checkstring = base::UTF16ToUTF8(value);
      if ((checkstring.find('*') == std::string::npos) &&
          (checkstring.find('x') == std::string::npos) && (!(value.empty())))
        credit_card_number_observed_ = value;
      else if (((checkstring.find('*') != std::string::npos) ||
                (checkstring.find('x') != std::string::npos)) &&
               (!(credit_card_number_observed_.empty())))
        field->value = credit_card_number_observed_;
    }
  }
}
#endif
...
void AutofillManager::OnQueryFormFieldAutofillImpl(
  ...
  if (is_autofill_possible && driver()->RendererIsAvailable() &&
    ...
    bool is_filing_upi_data = (autofill_field->Type().group() == UPI_DATA);
    if (is_filing_upi_data) {
      suggestions = GetUpiSuggestions(field, autofill_field->Type());
    } else if (is_filling_credit_card) {
  ...
  if (is_autofill_possible && driver()->RendererIsAvailable()
    && got_autofillable_form
    && autofill_field->Type().group() == CREDIT_CARD && !suggestions.empty()) {
    Suggestion has_credit_card;
    has_credit_card.frontend_id = POPUP_ITEM_ID_HAS_CREDIT_CARD;
    suggestions.push_back(has_credit_card);
  }
#endif
...
#if defined(S_TERRACE_SUPPORT)
    client_->AuthenticateCreditCardAutofill(this, query_id, form,
                                            field, credit_card);
    return;
#endif
  }
  FillOrPreviewDataModelForm(
      action, query_id, form, field, credit_card, CREDITCARD,
...
void AutofillManager::FillOrPreviewProfileForm(
  ...
  FillOrPreviewDataModelForm(
      action, query_id, form, field, profile, PROFILES,
      base::string16() /* cvc */, form_structure, autofill_field);
}
...
void AutofillManager::FillOrPreviewForm(
  ...
  const UPIName* upi = nullptr;
  AutofillField* autofill_field = GetAutofillField(form, field);
  bool is_filling_upi_data =
      (autofill_field && autofill_field->Type().group() == UPI_DATA);
  if (is_filling_upi_data && GetUpiData(unique_id, &upi)) {
    FillOrPreviewDataModelForm(action, query_id, form, field, *upi, UPI,
                               base::string16() /* cvc */);
#if defined(S_TERRACE_SUPPORT)
    upi->IsPingpayID()
        ? client_->SALoggingUPIAutofill(EVENT_UPI_FORM_AUTOFILL_PINGPAY)
        : client_->SALoggingUPIAutofill(EVENT_UPI_FORM_AUTOFILL_NONPINGPAY);
#endif
  } else if (GetCreditCard(unique_id, &credit_card))
...
void AutofillManager::FillCreditCardForm(int query_id,
  ...
  FillOrPreviewDataModelForm(AutofillDriver::FORM_DATA_ACTION_FILL, query_id,
                             form, field, credit_card, CREDITCARD, cvc);
}

void AutofillManager::FillUpiForm(int& query_id,
                                  const FormData& form,
                                  const FormFieldData& field,
                                  const UPIName& upi_name) {

// FIXME: Commented the code in order to fix the build during dev/m63_3239
...
#if defined(S_TERRACE_SUPPORT)
  client_->SALoggingUPIAutofill(EVENT_UPI_FORM_AUTOFILL_HELPERPOPUP_CREATED);
#endif
  FillOrPreviewDataModelForm(AutofillDriver::FORM_DATA_ACTION_FILL, query_id,
                             form, field, upi_name, UPI, base::string16());
#endif
}
...
#if defined(S_TERRACE_SUPPORT)
void AutofillManager::OnListBoxDidChangeItem(const FormData& form,
                                             const FormFieldData& field) {
  ProcessPossibleCreditCardForm(form);
  ...
  external_delegate_->UpdateFormData(form);
}

void AutofillManager::OnDidCommitRedirection() {
  if (!validated_credit_card_data_)
    return;

  client_->ConfirmSaveCreditCardLocally(
      *validated_credit_card_data_,
      base::Bind(
          base::IgnoreResult(&PersonalDataManager::SaveImportedCreditCard),
          base::Unretained(personal_data_), *validated_credit_card_data_));

  validated_credit_card_data_.reset();
}
#endif
...
bool AutofillManager::GetDeletionConfirmationText(const base::string16& value,
  ...
  const AutofillProfile* profile = nullptr;
  const UPIName* upi = nullptr;
  if (GetUpiData(identifier, &upi)) {
    return true;
  } else if (GetCreditCard(identifier, &credit_card)) {
...
#if defined(S_TERRACE_SUPPORT)
void AutofillManager::DidChangeScrollOffset(const gfx::RectF& bounding_box) {
  external_delegate_->DidChangeScrollOffset(bounding_box);
}

void AutofillManager::HideUPISuggestionPopupFullMode(
    const gfx::RectF& bounding_box) {
  external_delegate_->HideUPISuggestionPopupFullMode(bounding_box);
}
#endif
...
bool AutofillManager::IsCreditCardUploadEnabled() {
  ...
  return false;
#endif
  IdentityProvider* identity = GetIdentityProvider();
  if (!identity)
    return false;

  return ::autofill::IsCreditCardUploadEnabled(
      client_->GetPrefs(), client_->GetSyncService(),
      identity->GetActiveUsername());
}
...
#if defined(S_TERRACE_SUPPORT)
void AutofillManager::ImportCreditCardData(const FormStructure& form) {
  std::unique_ptr<CreditCard> imported_credit_card;
  bool imported_credit_card_matches_masked_server_credit_card;
  if (!personal_data_->ImportCreditCard(
          form, false /* should_return_local_card */, &imported_credit_card,
          false /* ignore_validation */, true /* ignore_merging */,
          &imported_credit_card_matches_masked_server_credit_card)) {
    return;
  }

  // FIXME: m63_3239 bringup
  if (imported_credit_card ||
      imported_credit_card_matches_masked_server_credit_card) {
    validated_credit_card_data_ = std::move(imported_credit_card);
  }
}
#endif
...
void AutofillManager::ImportFormData(const FormStructure& submitted_form) {
...
#if defined(S_TERRACE_SUPPORT)
  if(!imported_credit_card) {
    if (personal_data_->ImportCreditCard(submitted_form,
                                         true /* should_return_local_card */,
                                         &imported_credit_card)) {
      client_->ConfirmSaveCreditCardToSamsungPay(*imported_credit_card);
    }
    return;
  }
#else
  ...
  if (!IsCreditCardUploadEnabled() ||
    ...
    // In opensource save credit card infobar is shwon after the validation
    // of the credit card form submitted. This fails for many web contents
    // as details are not enetered in a form. In the new logic implemented
    // we save a copy of validated credit card and save credit card infobar
    // is invoked on next redirection. Hence commenting the below part.
    #if defined(S_TERRACE_SUPPORT)
      validated_credit_card_data_ = std::move(imported_credit_card);
    #else
    client_->ConfirmSaveCreditCardLocally(
...
bool AutofillManager::RefreshDataModels() {
  ...
  const std::vector<CreditCard*>& credit_cards =
      personal_data_->GetCreditCards();
  const std::vector<UPIName*>& upi_data = personal_data_->GetUPINames();
  ...
  if (profiles.empty() && credit_cards.empty() && upi_data.empty())
    return false;
...
bool AutofillManager::GetUpiData(int unique_id, const UPIName** upi_data) {
  // Unpack the |unique_id| into component parts.
  std::string upi_id;
  SplitFrontendIDUpi(unique_id, &upi_id);
  *upi_data = NULL;
  if (base::IsValidGUID(upi_id))
    *upi_data = personal_data_->GetUPINameByGUID(upi_id);
  return !!*upi_data;
}
...
void AutofillManager::FillOrPreviewDataModelForm(
    ...
    AutofillModelType type_of_data,
    const base::string16& cvc) {
  FormStructure* form_structure = NULL;
  AutofillField* autofill_field = NULL;
  if (!GetCachedFormAndField(form, field, &form_structure, &autofill_field))
  ...
  FillOrPreviewDataModelForm(action, query_id, form, field, data_model,
                             type_of_data, cvc, form_structure,
                             autofill_field);
}
...
void AutofillManager::FillOrPreviewDataModelForm(
    ...
    for (FormFieldData& iter : result.fields) {
      if (iter.SameFieldAs(field)) {
        ...
        if (AutofillField::FillFormField(*autofill_field, value,
          ...
          if (type_of_data == PROFILES && !value.empty())
  ...
  for (size_t i = 0; i < form_structure->field_count(); ++i) {
    ...
    if (field_group_type == NO_GROUP)
      continue;
    if (type_of_data == UPI && field_group_type != UPI_DATA)
      continue;

    base::string16 value =
        data_model.GetInfo(cached_field->Type(), app_locale_);
    if (type_of_data == CREDITCARD && cached_field->Type().GetStorableType() ==
                              CREDIT_CARD_VERIFICATION_CODE) {
      value = cvc;
    } else if (type_of_data == CREDITCARD &&
    ...
    // still be autofilled).
    bool should_notify = type_of_data == PROFILES && !value.empty() &&
...
std::vector<Suggestion> AutofillManager::GetUpiSuggestions(
    const FormFieldData& field,
    const AutofillType& type) const {
  std::vector<Suggestion> suggestions =
      personal_data_->GetUpiSuggestions(type, field.value);
  for (size_t i = 0; i < suggestions.size(); i++) {
    suggestions[i].frontend_id = MakeFrontendIDUpi(suggestions[i].backend_id);
  }
  return suggestions;
}
...
int AutofillManager::BackendIDToIntUpi(const std::string& backend_id) const {
  if (!base::IsValidGUID(backend_id))
    return 0;

  const auto found = backend_to_int_map_upi_.find(backend_id);
  if (found == backend_to_int_map_upi_.end()) {
    // Unknown one, make a new entry.
    int int_id = backend_to_int_map_upi_.size() + 1;
    backend_to_int_map_upi_[backend_id] = int_id;
    int_to_backend_map_upi_[int_id] = backend_id;
    return int_id;
  }
  return found->second;
}

std::string AutofillManager::IntToBackendIDUpi(int int_id) const {
  if (int_id == 0)
    return std::string();

  const auto found = int_to_backend_map_upi_.find(int_id);
  if (found == int_to_backend_map_upi_.end())
    return std::string();

  return found->second;
}

int AutofillManager::MakeFrontendIDUpi(
    const std::string& upi_backend_id) const {
  return BackendIDToIntUpi(upi_backend_id);
}

void AutofillManager::SplitFrontendIDUpi(int frontend_id,
                                         std::string* upi_backend_id) const {
  *upi_backend_id = IntToBackendIDUpi(frontend_id);
}
