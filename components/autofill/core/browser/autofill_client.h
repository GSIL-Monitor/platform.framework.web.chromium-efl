...
class AutofillClient : public RiskDataLoader {
 public:
  ...
  virtual void AuthenticateCreditCardAutofill(AutofillManager*,
      int query_id,
      const FormData& form,
      const FormFieldData& field,
      const CreditCard& credit_card) = 0;

  virtual void ConfirmSaveCreditCardToSamsungPay(const CreditCard& card) = 0;

  virtual void ConfirmSaveUPILocally(const UPIName& card,
                                     const base::Closure& callback) = 0;

  typedef base::Callback<void(const CreditCard&)> CreditCardHelperCallback;
  virtual void OpenCreditCardHelper(const CreditCard& credit_card,
                                    const CreditCardHelperCallback&) = 0;

  virtual void UseCreditCardHelper(bool use_credit_card_helper) = 0;

  virtual void UseUpiHelper(bool use_upi_helper) = 0;

  virtual void HideUPISuggestionPopupFullMode(
      const gfx::RectF& bounding_box) = 0;

  virtual void DestroySuggestionPopup() = 0;

  virtual void DidChangeScrollOffset(const gfx::RectF& bounding_box) = 0;

  // Get the UpiVpa registered with Spay by clicking on Upi helper popup
  virtual std::string GetRegisteredUpiVpa() const = 0;

  virtual void SALoggingUPIAutofill(const unsigned int&) = 0;
#endif
