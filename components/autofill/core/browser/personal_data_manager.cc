...
PersonalDataManager::~PersonalDataManager() {
  ...
  CancelPendingQuery(&pending_upidata_query_);
...
void PersonalDataManager::OnWebDataServiceRequestDone(
  ...
  if (!result) {
    ...
    if (h == pending_creditcards_query_)
      ...
      pending_server_profiles_query_ = 0;
    else if (h == pending_upidata_query_)
      pending_upidata_query_ = 0;
  } else {
    switch (result->GetType()) {
      ...
      case AUTOFILL_CREDITCARDS_RESULT:
        ...
        break;
    case AUTOFILL_UPIDATA_RESULT:
      ReceiveLoadedDbValues(h, result.get(), &pending_upidata_query_,
                            &upi_data_);
  ...
  if (pending_profiles_query_ == 0 && pending_creditcards_query_ == 0 &&
      pending_server_profiles_query_ == 0 &&
      pending_server_creditcards_query_ == 0 && pending_upidata_query_ == 0) {
...
UPIName* PersonalDataManager::GetUPINameByGUID(const std::string& guid) {
  const std::vector<UPIName*>& upi_names = GetUPINames();
  std::vector<UPIName*>::const_iterator iter =
      FindElementByGUID<UPIName>(upi_names, guid);
  return (iter != upi_names.end()) ? *iter : NULL;
}

void PersonalDataManager::AddUPIName(const UPIName& upi_name) {
  if (is_off_the_record_)
    return;

  if (upi_name.IsEmpty(app_locale_))
    return;

  if (FindByGUID<UPIName>(upi_data_, upi_name.guid()))
    return;
  ...
  // Don't add a duplicate.
  if (FindByContents(upi_data_, upi_name))
    return;

  // Add the new UPI Name to the web database.
  database_->AddUpiData(upi_name);

  // Refresh our local cache and send notifications to observers.
  Refresh();
}

void PersonalDataManager::UpdateUPIName(const UPIName& upi_name) {
  if (is_off_the_record_)
    return;

  UPIName* existing_upi_name = GetUPINameByGUID(upi_name.guid());
  if (!existing_upi_name)
    return;

  // Don't overwrite the origin for a UPI Name that is already stored.
  if (existing_upi_name->Compare(upi_name) == 0)
    return;

  if (upi_name.IsEmpty(app_locale_)) {
    RemoveByGUID(upi_name.guid());
    return;
  }

  // Update the cached version.
  *existing_upi_name = upi_name;

  if (!database_.get())
    return;

  // Make the update.
  database_->UpdateUpiData(upi_name);
...
void PersonalDataManager::RemoveByGUID(const std::string& guid) {
  ...
  bool is_upi_data = FindByGUID<UPIName>(upi_data_, guid);
  if (!is_credit_card && !is_profile && !is_upi_data)
    return;
...
bool PersonalDataManager::IsUPIGuidLocallyExists(const UPIName& upi_name) {
  if (is_off_the_record_)
    return true;
...
const std::vector<UPIName*>& PersonalDataManager::GetUPINames() const {
  upi_names_.clear();
  for (const auto& upi : upi_data_)
    upi_names_.push_back(upi.get());
  return upi_names_;
}
...
std::vector<Suggestion> PersonalDataManager::GetUpiSuggestions(
    const AutofillType& type,
    const base::string16& field_contents) {
  if (IsInAutofillSuggestionsDisabledExperiment())
    return std::vector<Suggestion>();

  std::vector<UPIName*> upi_data_to_suggest = GetUPINames();

  // Rank the suggestions by frecency (see AutofillDataModel for details).
  base::Time comparison_time = base::Time::Now();
  std::sort(upi_data_to_suggest.begin(), upi_data_to_suggest.end(),
            [comparison_time](const AutofillDataModel* a,
                              const AutofillDataModel* b) {
              return a->CompareFrecency(b, comparison_time);
            });
  ...
  for (const UPIName* upi_data : upi_data_to_suggest) {
    base::string16 upi_field_value = upi_data->GetInfo(type, app_locale_);
    if (upi_field_value.empty())
      continue;
    base::string16 upi_field_lower = base::i18n::ToLower(upi_field_value);
    bool prefix_matched_suggestion;
    if (IsValidSuggestionForFieldContents(
            upi_field_lower, field_contents_lower, type,
            /* is_masked_server_card= */ false, &prefix_matched_suggestion)) {
      // Make a new suggestion.
      suggestions.push_back(Suggestion());
      Suggestion* suggestion = &suggestions.back();

      suggestion->value = upi_data->GetInfo(type, app_locale_);
      suggestion->backend_id = upi_data->guid();
...
void PersonalDataManager::SetUpiData(std::vector<UPIName>& upi_fields) {
  if (is_off_the_record_)
    return;

  // Remove empty remove upi datas from input.
  upi_fields.erase(std::remove_if(upi_fields.begin(), upi_fields.end(),
                                  IsEmptyFunctor<UPIName>(app_locale_)),
                   upi_fields.end());

  if (!database_.get())
    return;

  // Any upi data that are not in the new upi data list should be
  // removed.
  for (const auto& upi : upi_data_) {
    if (!FindByGUID<UPIName>(upi_fields, upi->guid()))
      database_->RemoveUpiData(upi->guid());
  }

  // Update the web database with the existing upi datas.
  for (const UPIName& upi : upi_fields) {
    if (FindByGUID<UPIName>(upi_data_, upi.guid()))
      database_->UpdateUpiData(upi);
  }

  // Add the new upi to the web database.  Don't add a duplicate.
  for (const UPIName& upi : upi_fields) {
    if (!FindByGUID<UPIName>(upi_data_, upi.guid()) &&
        !FindByContents(upi_data_, upi))
      database_->AddUpiData(upi);
  }

  // Copy in the new upi data.
  upi_data_.clear();
  for (const UPIName& upi : upi_fields)
    upi_data_.push_back(base::MakeUnique<UPIName>(upi));
...
void PersonalDataManager::LoadUpiData() {
  if (!database_.get()) {
    NOTREACHED();
    return;
  }
  ...
  CancelPendingQuery(&pending_upidata_query_);
  pending_upidata_query_ = database_->GetUpiData(this);
}
...
std::string PersonalDataManager::SaveImportedUpiData(
    const UPIName& imported_upi_data) {
  DCHECK(!imported_upi_data.GetUPIName().empty());
  if (is_off_the_record_)
    return std::string();

  std::string guid = imported_upi_data.guid();
  std::vector<UPIName> upi_fields;
  for (auto& upi : upi_data_)
    upi_fields.push_back(*upi);

  upi_fields.push_back(imported_upi_data);
  SetUpiData(upi_fields);
  return guid;
}
...
bool PersonalDataManager::ImportUpiVpa(
    const FormStructure& form,
    std::unique_ptr<UPIName>* imported_upi_name,
    const base::string16& upi_vpa) {
  UPIName candidate_upi_name;
  candidate_upi_name.set_origin(form.source_url().spec());

  for (const auto& field : form) {
    if (!field->IsFieldFillable() || !field->is_focusable)
      continue;

    if (field->Type().group() != UPI_DATA)
      continue;

    ServerFieldType server_field_type = field->Type().GetStorableType();
    if (server_field_type == UPI_FIELD) {
      candidate_upi_name.SetRawInfo(server_field_type, upi_vpa);
      imported_upi_name->reset(new UPIName(candidate_upi_name));
      return true;
    }
  }
  return false;
...
bool PersonalDataManager::ImportCreditCard(
  ...
  *imported_credit_card_matches_masked_server_credit_card = false;
...
#if defined(S_TERRACE_SUPPORT)
  if (!ignore_validation && !candidate_credit_card.IsValid())
#else
  ...
  if (!ignore_merging) {
#endif
    // Attempt to merge with an existing credit card. Don't present a prompt if
    // we have already saved this card number, unless |should_return_local_card|
    // is true which indicates that upload is enabled. In this case, it's useful
    // to present the upload prompt to the user to promote the card from a local
    // card to a synced server card.
    for (const auto& card : local_credit_cards_) {
