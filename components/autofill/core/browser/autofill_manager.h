...
class AutofillManager : public AutofillHandler,
 ...
 public:
  ...
  // Whether the |field| should show an entry to open credit card helper.
  virtual bool ShouldShowCreditCardHelper(const FormData& form,
                                          const FormFieldData& field);
  virtual bool ShouldShowUpiHelper(const FormData& form,
                                   const FormFieldData& field);
#endif
  ...
  virtual void FillUpiForm(int& query_id,
                           const FormData& form,
                           const FormFieldData& field,
  ...
  void DidShowSuggestions(bool is_new_popup,
                          ...
                          const FormFieldData& field);
#if defined(S_TERRACE_SUPPORT)
  void OnListBoxDidChangeItem(const FormData& form, const FormFieldData& field);
  void OnDidCommitRedirection();
  void ProcessPossibleCreditCardForm(const FormData& form);
  void ImportCreditCardData(const FormStructure& form);
#endif
  ...
  void Reset() override;
#if defined(S_TERRACE_SUPPORT)
  void HideUPISuggestionPopupFullMode(const gfx::RectF& bounding_box);
  void DidChangeScrollOffset(const gfx::RectF& bounding_box);
#endif
 ...
 protected:
...
#if defined(S_TERRACE_SUPPORT)
  // Imports the upi data, submitted by the user, into |personal_data_|.
  void ImportUpiData(const FormStructure& form);
#endif

  // Maps suggestion backend ID to and from an integer identifying it
  // for upi fields.
  virtual int BackendIDToIntUpi(const std::string& backend_id) const;
  virtual std::string IntToBackendIDUpi(int int_id) const;
  // AutofillHandler:
  ...
  // Methods for packing and unpacking upi IDs for sending
  // and receiving to and from the renderer process.
  int MakeFrontendIDUpi(const std::string& upi_backend_id) const;
  void SplitFrontendIDUpi(int frontend_id, std::string* upi_backend_id) const;

  // Gets the upi data referred by |unique_id|. Returns true if the credit
  // card exists.
  bool GetUpiData(int unique_id, const UPIName** upi_data);
...
private:
  ...
  // Returns a list of values from the stored upi data that match |type| and
  // the value of |field| and returns the labels of the matching upi data.
  std::vector<Suggestion> GetUpiSuggestions(const FormFieldData& field,
                                            const AutofillType& type) const;
...
#if defined(S_TERRACE_SUPPORT)
  // If the credit number picked from the form is masked it is replaced
  // with actual number for credit card saving to profiles.
  void CheckAndReplaceCreditCardIfMasked(const FormStructure& submitted_form);
#endif
...
#if defined(S_TERRACE_SUPPORT)
  // A copy of any valid credit card entered by the user.
  std::unique_ptr<CreditCard> validated_credit_card_data_;

  // Saves the credit card number temporarily and it replaces the credit card
  // number picked from the form if it is masked by the content provider.
  base::string16 credit_card_number_observed_;
#endif
  ...
  // Suggestion backend ID to ID mapping. We keep two maps to convert back and
  // forth. These should be used only by BackendIDToIntUpi and
  // IntToBackendIDUpi. Note that the integers are not frontend IDs.
  mutable std::map<std::string, int> backend_to_int_map_upi_;
  mutable std::map<int, std::string> int_to_backend_map_upi_;
