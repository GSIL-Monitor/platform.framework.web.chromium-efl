...
bool IsWebElementInsideBody(const blink::WebElement& element) {
  // TODO: We are checking only bottom position for P170818-00673.
  // If we need to check top, left, right, should check them also,
  // or if it makes a side effect, remove this patch
  int element_bottom_in_window =
      element.BoundsInViewport().y + element.BoundsInViewport().height;
  int body_top_in_window = element.GetDocument().Body().BoundsInViewport().y;
  bool is_inside_body = (element_bottom_in_window - body_top_in_window) >= 0;
  if (!is_inside_body) {
    CR_DEFINE_STATIC_LOCAL(WebString, kName, ("name"));
    CR_DEFINE_STATIC_LOCAL(WebString, kId, ("id"));
    base::string16 element_name = element.GetAttribute(kName).Utf16();
    base::string16 element_id = element.GetAttribute(kId).Utf16();
    LOG(ERROR) << __FUNCTION__ << " element_name: " << element_name
               << " / element_id: " << element_id
               << " / element_bottom: " << element_bottom_in_window
               << " / body_top: " << body_top_in_window;
  }
  return is_inside_body;
}
#endif
...
bool IsWebElementVisible(const blink::WebElement& element) {
  ...
  bool res = g_prevent_layout ? element.HasNonEmptyLayoutSize()
                              : element.IsFocusable();
#if defined(S_TERRACE_SUPPORT)
  return res && IsWebElementInsideBody(element);
#else
  return res;
#endif
}
