...
bool FillUserNameAndPassword(
  ...
  if (fill_data.autologin_required) {
...
bool FillFormOnPasswordReceived(
  ...
  while (cur_frame->Parent() && !fill_data.autologin_required) {
...
#if defined(S_TERRACE_SUPPORT)
  if (fill_data.autologin_required && form_has_fillable_username &&
      !fill_data.selected_user.empty()) {
    username_element.SetValue(blink::WebString::FromUTF16(
        fill_data.selected_user), true);
  }
#endif
...
PasswordAutofillAgent::PasswordAutofillAgent(
    ...
    : content::RenderFrameObserver(render_frame),
      password_autologin_agent_(new PasswordAutologinAgent(render_frame)),
...
PasswordAutofillAgent::~PasswordAutofillAgent() {
  delete password_autologin_agent_;
...
#if defined(S_TERRACE_SUPPORT)
    // Password field of the modified form is cleared.
    // As per latest Open Source the password field is not cleared even
    // if other elements in the form are cleared. Thus, putting change
    // under the S_TERRACE_SUPPORT flag.
    blink::WebInputElement password = iter->second.password_field;
    if (password.IsAutofilled()) {
      password.SetValue(blink::WebString::FromUTF16(base::string16()), true);
...
#if defined(S_TERRACE_SUPPORT)
void PasswordAutofillAgent::PasswordFormWasVisible(
    const blink::WebFormElement& form,
    const bool is_loading) {
  // Send password forms to  browser.
  std::unique_ptr<RendererSavePasswordProgressLogger> logger;
  std::vector<PasswordForm> password_forms;
  std::unique_ptr<PasswordForm> password_form(
      CreatePasswordFormFromWebForm(form, nullptr, &form_predictions_));
  ...
  if (logging_state_active_) {
    logger.reset(new RendererSavePasswordProgressLogger(
        GetPasswordManagerDriver().get()));
    logger->LogMessage(Logger::STRING_PASSWORD_FORM_WAS_VISIBLE_METHOD);
    LogHTMLForm(logger.get(), Logger::STRING_FORM_FOUND_ON_PAGE, form);
  ...
  if (password_form.get()) {
    ...
    GetPasswordManagerDriver()->PasswordFormsParsed(password_forms);
    if (!is_loading) {
      GetPasswordManagerDriver()->PasswordFormsWasRendered(password_forms);
    }
  }
}
#endif
...
void PasswordAutofillAgent::FillPasswordForm(
  ...
  if ((form_data.wait_for_username && !form_data.autologin_required)
      || form_data.manual_autofill)
    return;
  ...
  for (auto element : elements) {
    ...
    if (form_data.autologin_required && password_element.IsAutofilled())
      password_autologin_agent_->SubmitFormForAutoLogin(password_element);
  }
}
