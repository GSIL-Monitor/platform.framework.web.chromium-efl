...
void AutofillAgent::DidCommitRedirection() {
  GetAutofillDriver()->DidCommitRedirection();
}
#endif
...
void AutofillAgent::DidCommitCompositorFrame() {
  WebLocalFrame* frame = render_frame()->GetWebFrame();
  if (!element_.IsNull() && frame && !frame->IsLoading() && is_scrolled_) {
    GetAutofillDriver()->DidChangeScrollOffset(
...
void AutofillAgent::TextFieldDidChangeImpl(
  ...
  if (form_util::FindFormAndFieldForFormControlElement(element, &form,
                                                       &field)) {
    if (!element_.IsNull() && render_frame()->GetWebFrame() &&
        !(element_.Value().IsEmpty())) {
      GetAutofillDriver()->HideUPISuggestionPopupFullMode(
          render_frame()->GetRenderView()->ElementBoundsInWindow(element_));
    }
...
void AutofillAgent::ClearForm() {
  ...
  if (form_util::FindFormAndFieldForFormControlElement(element_, &form,
                                                       &field)) {
    GetAutofillDriver()->ListBoxDidChangeItem(form, field);
  }
#endif
}
...
bool AutofillAgent::IsUsernameOrPasswordForm(
    const blink::WebInputElement& element) {
  return password_autofill_agent_->IsUsernameOrPasswordField(element);
}
#endif
...
#if defined(S_TERRACE_SUPPORT)
void AutofillAgent::listBoxDidChangeItem(
