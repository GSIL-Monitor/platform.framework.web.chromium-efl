...
void ContentAutofillDriver::ListBoxDidChangeItem(const FormData& form,
                                                 const FormFieldData& field) {
#if defined(S_TERRACE_SUPPORT)
  autofill_manager_->OnListBoxDidChangeItem(form, field);
#endif
}
...
void ContentAutofillDriver::DidCommitRedirection() {
#if defined(S_TERRACE_SUPPORT)
  autofill_manager_->OnDidCommitRedirection();
#endif
}
...
void ContentAutofillDriver::DidChangeScrollOffset(
    const gfx::RectF& bounding_box) {
#if defined(S_TERRACE_SUPPORT)
  autofill_manager_->DidChangeScrollOffset(bounding_box);
#else
  (void)bounding_box;
#endif
}

void ContentAutofillDriver::HideUPISuggestionPopupFullMode(
    const gfx::RectF& bounding_box) {
#if defined(S_TERRACE_SUPPORT)
  autofill_manager_->HideUPISuggestionPopupFullMode(bounding_box);
#else
  (void)(bounding_box);
#endif
}
