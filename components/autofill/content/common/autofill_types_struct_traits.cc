...
bool StructTraits<autofill::mojom::PasswordFormFillDataDataView,
  ...
  if (!data.ReadName(&out->name) || !data.ReadOrigin(&out->origin) ||
      ...
      !data.ReadSelectedUser(&out->selected_user) ||
      !data.ReadUsernameList(&out->username_list))
    return false;

  out->wait_for_username = data.wait_for_username();
  out->is_possible_change_password_form =
      data.is_possible_change_password_form();
  out->username_element_readonly = data.username_element_readonly();
  out->manual_autofill = data.manual_autofill();
  out->autologin_required = data.autologin_required();
