...
void ContentPasswordManagerDriver::PasswordFormsWasRendered(
    const std::vector<autofill::PasswordForm>& forms) {
#if defined(S_TERRACE_SUPPORT)
  for (const auto& form : forms)
    if (!CheckChildProcessSecurityPolicy(
            form.origin, BadMessageReason::CPMD_BAD_ORIGIN_FORMS_WAS_RENDERED))
      return;

  GetPasswordManager()->OnPasswordFormsWasRendered();
#else
  return;
#endif
}
