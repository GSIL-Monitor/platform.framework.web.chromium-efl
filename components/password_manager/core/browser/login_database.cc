...
#if defined(S_TERRACE_SUPPORT)
const int kMinVersionNumberOfWBSSupported = 5;
#endif
...
void BindAddStatement(const PasswordForm& form,
  ...
  s->BindBlob(COLUMN_ENCRYPTED_GENERATED_KEY, &ct.front(), ct.size());
...
#if defined(S_TERRACE_SUPPORT)
  builder->AddColumn("encrypted_generated_key", "BLOB");
  builder->AddColumn("stored_iv", "VARCHAR");
#endif
  version = builder->SealVersion();
...
#if defined(S_TERRACE_SUPPORT)
  if (!DropLoginsTableIfNecessary()) {
    LOG(ERROR) << "Unable to DropLoginsTableIfNecessary.";
    db_.Close();
    return false;
  }
#endif
...
#if defined(S_TERRACE_SUPPORT)
bool LoginDatabase::DropLoginsTableIfNecessary() {
  if (meta_table_.GetVersionNumber() < kMinVersionNumberOfWBSSupported) {
    if (!db_.Execute("DROP TABLE IF EXISTS logins")) {
      return false;
    }
    meta_table_.SetVersionNumber(kCurrentVersionNumber);
  }
...
#if defined(S_TERRACE_SUPPORT)
  std::string wbs_encrypted_password;
  std::string iv;
  std::vector<unsigned char> encrypted_key;
  OSCrypt::WBSEncrypt(
      encrypted_password, &wbs_encrypted_password, &iv, &encrypted_key);
#endif
...
#if defined(S_TERRACE_SUPPORT)
  BindAddStatement(form, wbs_encrypted_password, &s, encrypted_key, iv);
#else
...
PasswordStoreChangeList LoginDatabase::UpdateLogin(const PasswordForm& form) {
  ...
  std::string iv;
  std::vector<unsigned char> encrypted_key;
  OSCrypt::WBSEncrypt(password, &encrypted_password, &iv, &encrypted_key);
#endif
...
#if defined(S_TERRACE_SUPPORT)
  s.BindInt(next_param++, form.use_additional_authentication);
  s.BindBlob(next_param++, &encrypted_key.front(), encrypted_key.size());
  s.BindString(next_param++, iv);
#endif
...
#if defined(S_TERRACE_SUPPORT)
void LoginDatabase::RemoveBiometricLogins() {
  sql::Statement s(db_.GetCachedStatement(SQL_FROM_HERE,
      "DELETE FROM logins WHERE "
      "use_additional_auth = ?"));
  s.BindInt(0, 1);
...
#if defined(S_TERRACE_SUPPORT)
  std::vector<unsigned char> encrypted_key;
  s.ColumnBlobAsVector(COLUMN_ENCRYPTED_GENERATED_KEY, &encrypted_key);
  std::string iv = s.ColumnString(COLUMN_STORED_PLATFORM_IV);
  std::string wbs_encrypted_password(encrypted_password);
  OSCrypt::WBSDecrypt(
      wbs_encrypted_password, iv, encrypted_key, &encrypted_password);
#endif
  base::string16 decrypted_password;
...
#if defined(S_TERRACE_SUPPORT)
    if (content::TerraceContentBridge::Get()
        .IsActivatedAsEasySigninGetLogins()) {
      const std::string sql_query_for_spass =
          "SELECT * FROM logins "
          "WHERE (signon_realm == ? OR signon_realm REGEXP ?) "
          "AND use_additional_auth == ?";
      sql::Statement s_for_spass(
          db_.GetUniqueStatement(sql_query_for_spass.c_str()));
      s_for_spass.BindString(0, form.signon_realm);
      s_for_spass.BindString(1, regexp);
      s_for_spass.BindInt(2, 0);
      bool success = StatementToForms(&s_for_spass, &form, forms);
      if (success)
        return true;
      forms->clear();
      return false;
...
#if defined(S_TERRACE_SUPPORT)
  } else if (content::TerraceContentBridge::Get()
                 .IsActivatedAsEasySigninGetLogins()) {
    const std::string sql_query_for_spass =
        "SELECT * FROM logins "
        "WHERE signon_realm == ? AND use_additional_auth == ?";
    sql::Statement s_for_spass(
        db_.GetUniqueStatement(sql_query_for_spass.c_str()));
    s_for_spass.BindString(0, form.signon_realm);
    s_for_spass.BindInt(1, 0);
    bool success = StatementToForms(&s_for_spass, nullptr, forms);
    if (success)
      return true;
    forms->clear();
    return false;
...
#if defined(S_TERRACE_SUPPORT)
bool LoginDatabase::GetLoginsWithBio(
    std::vector<std::unique_ptr<PasswordForm>>* forms) {
  DCHECK(forms);
  const std::string sql_query =
      "SELECT * FROM logins WHERE use_additional_auth = ?";
  sql::Statement s;
...
std::string LoginDatabase::GetRegexpForPSLDomainMatch(
    const PasswordStore::FormDigest& form) {
  const GURL signon_realm(form.signon_realm);
  std::string registered_domain = GetRegistryControlledDomain(signon_realm);
  const bool should_PSL_matching_apply =
...
void LoginDatabase::InitializeStatementStrings(const SQLTableBuilder& builder) {
  ...
  blacklisted_statement_ =
      ...
      " ORDER BY use_additional_auth, origin_url";
#else
