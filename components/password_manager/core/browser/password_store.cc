...
#if !defined(OS_ANDROID) && !defined(OS_IOS) && !defined(OS_TIZEN)
...
void PasswordStore::AddLogin(const PasswordForm& form) {
  ...
  if (content::TerraceContentBridge::Get().IsCachedActivatedAsEasySignin() &&
    ...
    content::TerraceContentBridge::Get().OnAddLoginAsEasySignin(form);
  } else {
...
#if defined(S_TERRACE_SUPPORT)
  if (content::TerraceContentBridge::Get().IsCachedActivatedAsEasySignin() &&
      new_form.use_additional_authentication) {
    content::TerraceContentBridge::Get().OnAddLoginAsEasySignin(new_form);
  } else {
...
#if defined(S_TERRACE_SUPPORT)
void PasswordStore::RemoveLoginForUIManager(const PasswordForm& form) {
  ScheduleTask(base::Bind(&PasswordStore::RemoveLoginInternal, this, form));
}
#endif
...
#if defined(S_TERRACE_SUPPORT)
void PasswordStore::RemoveBiometricLogins(const base::Closure& completion) {
  ScheduleTask(base::Bind(
      &PasswordStore::RemoveBiometricLoginsInternal, this, completion));
}
#endif
...
#if defined(S_TERRACE_SUPPORT)
  std::vector<std::unique_ptr<PasswordForm>> forms_of_local_db(
      FillMatchingLogins(form));
  if (content::TerraceContentBridge::Get().IsCachedActivatedAsEasySignin()) {
    std::vector<std::unique_ptr<PasswordForm>> forms_of_easy_signin(
        content::TerraceContentBridge::Get().GetLoginsAsEasySignin(form));

    forms_of_local_db.reserve(
        forms_of_local_db.size() + forms_of_easy_signin.size());
    forms_of_local_db.insert(forms_of_local_db.end(),
        std::make_move_iterator(forms_of_easy_signin.begin()),
            std::make_move_iterator(forms_of_easy_signin.end()));

    forms_of_easy_signin.clear();
  }
  request->NotifyConsumerWithResults(std::move(forms_of_local_db));
#else
...
#if defined(S_TERRACE_SUPPORT)
void PasswordStore::RemoveBiometricLoginsInternal(
    const base::Closure& completion) {
  RemoveBiometricLoginsImpl();
  if (!completion.is_null())
    main_task_runner_->PostTask(FROM_HERE, completion);
}
#endif
