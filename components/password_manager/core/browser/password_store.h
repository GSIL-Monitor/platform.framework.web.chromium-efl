...
class PasswordStore : protected PasswordStoreSync,
 ...
 public:
  ...
  // Init() without google sync service dependencies.
  virtual bool Init();

  // Gets a list of credentials matching |form| and only biometrics infomation.
  // e.g. |use_additional_authentication| of |PasswordForm| is true.
  virtual bool GetLoginsWithBio(
      std::vector<std::unique_ptr<autofill::PasswordForm>>* forms) = 0;
  // Gets current savedpasswords data.
  virtual bool GetSavedPasswordData(
      std::vector<std::unique_ptr<autofill::PasswordForm>>* forms) = 0;
  // Gets a regular expression for Public Suffix Domain match.
  virtual std::string GetRegexpForPSLDomainMatch(
      const PasswordStore::FormDigest& form) = 0;
  // Removes the matching PasswordForm from the secure password store (async).
  // This api didn't check whether Samsung Pass is activated or not.
  void RemoveLoginForUIManager(const autofill::PasswordForm& form);
#endif
...
#if defined(S_TERRACE_SUPPORT)
  // Remove all biometric related login data from Login DB.
  virtual void RemoveBiometricLogins(const base::Closure& completion);
#endif
...
#if defined(S_TERRACE_SUPPORT)
  virtual void RemoveBiometricLoginsImpl() = 0;
#endif
...
#if defined(S_TERRACE_SUPPORT)
  void RemoveBiometricLoginsInternal(const base::Closure& completion);
#endif
