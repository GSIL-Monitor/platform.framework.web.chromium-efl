...
void PasswordFormManager::ScoreMatches(
  ...
  if (observed_form_.username_element_readonly &&
      observed_form_.signon_realm.find("accounts.google.com") !=
          std::string::npos) {
    bool is_matched = false;
    base::string16 current_username = observed_form_.username_value;
    base::string16 partial_username;
    std::size_t position =
        current_username.find(base::UTF8ToUTF16("@gmail.com"));
    if (position == std::string::npos)
      position = current_username.find(base::UTF8ToUTF16("@googlemail.com"));

    if (position != std::string::npos)
      partial_username.assign(current_username, 0, position);

    for (const auto& key_value : best_matches_) {
      if (key_value.first == current_username ||
          key_value.first == partial_username) {
        preferred_match_ = key_value.second;
        is_matched = true;
        if (key_value.second->use_additional_authentication)
          break;
      }
    }
    if (!is_matched)
...
#if defined(S_TERRACE_SUPPORT)
  std::size_t found = username.find(base::UTF8ToUTF16("@gmail.com"));
  if (found == std::string::npos) {
    found = username.find(base::UTF8ToUTF16("@googlemail.com"));
    if (found == std::string::npos)
      return false;
  }

  base::string16 partial_username;
  partial_username.assign(username, 0, found);
  auto it = best_matches_.find(partial_username);
  if (it != best_matches_.end()) {
    autofill::PasswordForm found_form = *it->second;
    if (found_form.signon_realm.find("accounts.google.com") !=
        std::string::npos &&
        found_form.signon_realm ==
        submitted_form_->signon_realm &&
        found_form.password_element ==
        submitted_form_->password_element &&
        found_form.username_element ==
        submitted_form_->username_element) {
      pending_credentials_ = found_form;
      return true;
    }
  }
#endif
...
#if defined(S_TERRACE_SUPPORT)
  if (score > 0 && candidate.use_additional_authentication)
    score += 1u << 8;
#endif
