...
bool PasswordStoreDefault::Init() {
  ...
  // https://chromium-review.googlesource.com/c/chromium/src/+/600207
  //ScheduleTask(base::Bind(&PasswordStoreDefault::InitOnDBThread, this));
  return true;
}

bool PasswordStoreDefault::GetLoginsWithBio(
    std::vector<std::unique_ptr<PasswordForm>>* forms) {
  DCHECK(background_task_runner()->RunsTasksInCurrentSequence());
  return login_db_ && login_db_->GetLoginsWithBio(forms);
}

bool PasswordStoreDefault::GetSavedPasswordData(
    std::vector<std::unique_ptr<PasswordForm>>* forms) {
  DCHECK(background_task_runner()->RunsTasksInCurrentSequence());
  return login_db_ && login_db_->GetAutofillableLogins(forms);
}

std::string PasswordStoreDefault::GetRegexpForPSLDomainMatch(
    const PasswordStore::FormDigest& form) {
  DCHECK(background_task_runner()->RunsTasksInCurrentSequence());
  if (login_db_)
    return login_db_->GetRegexpForPSLDomainMatch(form);
  return std::string();
}
#endif
...
#if defined(S_TERRACE_SUPPORT)
void PasswordStoreDefault::RemoveBiometricLoginsImpl() {
  if (login_db_)
    login_db_->RemoveBiometricLogins();
}
#endif
