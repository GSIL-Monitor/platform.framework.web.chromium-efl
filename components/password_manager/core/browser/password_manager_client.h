...
class PasswordManagerClient {
 public:
  ...
  virtual bool IsWebLoginEnabled();
#endif
  ...
  // If authentication is unsuccessful, it passes all normal remembered
  // credentials fo autofill.
  virtual void AuthenticateAutofillAndFillForm(
      password_manager::PasswordManagerDriver* driver_,
      const autofill::PasswordFormFillData& fill_data,
      const autofill::PasswordFormFillData& manual_fill_data) = 0;

  // |is_password_forms_rendered_| set to true in terrace side.
  // This flag is used for prevent call AuthticateAutofillAndFillForm twice.
  virtual void OnPasswordFormsRendered() = 0;
#endif
