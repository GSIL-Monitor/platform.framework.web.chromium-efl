...
bool TestPasswordStore::GetLoginsWithBio(
    std::vector<std::unique_ptr<autofill::PasswordForm>>* forms) {
  return false;
}

bool TestPasswordStore::GetSavedPasswordData(
    std::vector<std::unique_ptr<autofill::PasswordForm>>* forms) {
  return false;
}

std::string TestPasswordStore::GetRegexpForPSLDomainMatch(
    const PasswordStore::FormDigest& form) {
  return std::string();
}

void TestPasswordStore::RemoveBiometricLogins(
    const base::Closure& completion) {}

void TestPasswordStore::RemoveBiometricLoginsImpl() {}
#endif
