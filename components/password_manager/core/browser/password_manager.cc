...
PasswordManager::PasswordManager(PasswordManagerClient* client)
    : client_(client)
#if defined(S_TERRACE_SUPPORT)
    , sent_password_forms_rendered_(false)
#endif
     {
  DCHECK(client_);
}
...
#if defined(S_TERRACE_SUPPORT)
void PasswordManager::OnPasswordFormsWasRendered() {
  LOG(INFO) << __FUNCTION__;
  if (!sent_password_forms_rendered_) {
    sent_password_forms_rendered_ = true;
    client_->OnPasswordFormsRendered();
  }
}
#endif
...
#if defined(S_TERRACE_SUPPORT)
  // Make it true so that other possible usernames are also checked before
  // saving the credentials to db while form submission. This is set to false
  // by the https://codereview.chromium.org/1047673004 in upstream.
  return true;
#else
  return false;
#endif
...
bool PasswordManager::CanProvisionalManagerSave() {
  ...
  if (!provisional_save_manager_) {
    ...
    LOG(ERROR) << __FUNCTION__ << " No provisional save manager";
    return false;
  }
  ...
  if (provisional_save_manager_->form_fetcher()->GetState() ==
    ...
    LOG(ERROR) << __FUNCTION__ << " No form manager has completed matching";
    provisional_save_manager_.reset();
    return false;
  }
  return true;
...
void PasswordManager::OnPasswordFormsRendered(
...
#if defined(S_TERRACE_SUPPORT)
  if (!sent_password_forms_rendered_ && visible_forms.size() > 0) {
    sent_password_forms_rendered_ = true;
    client_->OnPasswordFormsRendered();
  }
#endif
  ...
  if (client_->WasLastNavigationHTTPError()) {
    LOG(ERROR) << __FUNCTION__ << " WasLastNavigationHTTPError";
    if (logger)
...
void PasswordManager::OnLoginSuccessful() {
  ...
  if (base::FeatureList::IsEnabled(features::kDropSyncCredential)) {
    ...
    (defined(OS_LINUX) && !defined(OS_CHROMEOS)) && !defined(OS_TIZEN)
...
#if defined(S_TERRACE_SUPPORT)
    if (empty_password) {
      LOG(ERROR) << __FUNCTION__ << " username_value is empty";
      return;
    }
#endif
    if (logger)
...
void PasswordManager::Autofill(
  ...
  if (form_for_autofill.username_element_readonly) {
    ...
    for (std::map<base::string16,
        const PasswordForm*>::const_iterator it = best_matches.begin();
  ...
  bool is_websign_enabled = client_->IsWebLoginEnabled();
#endif
  ...
  InitPasswordFormFillData(form_for_autofill, best_matches, &preferred_match,
                           ...
                           manual_autofill,
#endif
                           OtherPossibleUsernamesEnabled(), &fill_data);
  if (logger)
...
#if defined(S_TERRACE_SUPPORT)
  fill_data.autologin_required = authentication_required;

  autofill::PasswordFormFillData manual_fill_data;

  if (fill_data.additional_logins.size() + 1 > fill_data.username_list.size()) {
    manual_autofill = true;
    // We call InitPasswordFormFillData for Non-FP remembered credential
    // with preferred_match_manaul which contains a Non-FP remembered
    // credential.
    const PasswordForm *preferred_match_manual = &preferred_match;
    if (preferred_match.use_additional_authentication) {
      for (std::map<base::string16, const PasswordForm*>::const_iterator iter =
          best_matches.begin();
              iter != best_matches.end(); iter++) {
        if (!iter->second->use_additional_authentication) {
          preferred_match_manual = iter->second;
          break;
        }
      }
    }
    InitPasswordFormFillData(form_for_autofill, best_matches,
                             preferred_match_manual, wait_for_username,
                             manual_autofill, OtherPossibleUsernamesEnabled(),
                             &manual_fill_data);
    manual_fill_data.autologin_required = false;
  }

  if (authentication_required && is_websign_enabled) {
    if (preferred_match.username_element !=
            form_for_autofill.username_element ||
        (!preferred_match.password_element.empty() &&
         preferred_match.password_element !=
             form_for_autofill.password_element)) {
      LOG(INFO) << __FUNCTION__
                << ": no matched username_element or password_element";
      return;
    }
    client_->AuthenticateAutofillAndFillForm(driver, fill_data,
                                             manual_fill_data);
  } else {
    // Either WebSign-In is off or we don't have any FP registered account
    // In that case, we should directly autofill.
    manual_fill_data.manual_autofill = false;
    driver->FillPasswordForm(manual_fill_data);
    if (content::TerraceContentBridge::Get().IsCachedActivatedAsEasySignin()) {
      content::TerraceContentBridge::Get().ShowMoveToEasySignin();
    }
  }
#else
...
void PasswordManager::ShowInitialPasswordAccountSuggestions(
  ...
  autofill::PasswordFormFillData fill_data;
// TODO: m53_2785 bringup
// This is newly added.
// It has to be revisited by Author.
#if defined(S_TERRACE_SUPPORT)
  InitPasswordFormFillData(form_for_autofill, best_matches, &preferred_match,
