...
#if defined(USE_EFL)
    // Initially scissor rect of frame buffer is set as (0, 0, 0, 0). If
    // |scissor_rect_| is set after calling |BindFramebufferToTexture| this
    // value does not affect to scissor rect of frame buffer.
    // In this situation, if this function is called after calling
    // |BindFramebufferToOutputSurface|, scissor rect is not set because of
    // |scissor_rect == scissor_rect_| condition. Eventually scissor rect of
    // frame buffer is remained as (0, 0, 0, 0).
    GLint current_scissor_rect[4];
    gl_->GetIntegerv(GL_SCISSOR_BOX, current_scissor_rect);
    if (scissor_rect.width() == current_scissor_rect[2] &&
        scissor_rect.height() == current_scissor_rect[3])
#endif
    return;
  }
