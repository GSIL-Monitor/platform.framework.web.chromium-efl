...
NaClForkDelegate::NaClForkDelegate(bool nonsfi_mode)
    : nonsfi_mode_(nonsfi_mode),
      status_(kNaClHelperUnused),
#if defined(OS_TIZEN_TV_PRODUCT)
      sandbox_descriptor_(-1),
#endif
      fd_(-1) {
}
...
void NaClForkDelegate::Init(const int sandboxdesc,
...
#if defined(OS_TIZEN_TV_PRODUCT)
  // Aim of this check is to prevent preloading of NaCl process.
  // On the first run, Init method is called only to preload
  // NaCl process. Below feature enables running NaCl process
  // only when it is requested in Fork method.
  // Here sandbox descriptor is saved to allow launching
  // the process later.
  if (sandbox_descriptor_ == -1 &&
      !base::CommandLine::ForCurrentProcess()->HasSwitch(
          switches::kEnableNaClZygotePreloading)) {
    sandbox_descriptor_ = sandboxdesc;
    return;
  }
#endif
  ...
  LOG(INFO) << "Starting NaCl Process in " << (nonsfi_mode_ ? "Non-SFI" : "SFI")
            << " mode";
  ...
  const bool using_namespace_sandbox =
      ...
      !base::CommandLine::ForCurrentProcess()->HasSwitch(
          switches::kDisableNamespaceSandbox);
...
#if defined(OS_TIZEN_TV_PRODUCT)
  CustomTizenFD::AddCustomTizenFD(&(options.fds_to_remap));
#endif
...
#if defined(OS_TIZEN_TV_PRODUCT)
      // We append enabling switch only when dlog FD are opened and it`s
      // possible to properly initialize them in NaCl Zygote.
      if (CustomTizenFD::IsDlogFDInitialized())
        cmd_line.AppendSwitch(switches::kEnableNaClDlog);
#endif
...
#if defined(OS_TIZEN_TV_PRODUCT) && !defined(TIZEN_EMULATOR_SUPPORT)
      if (sandbox_path.value().empty()) {
        LOG(FATAL) << "Requested NaCl Process to start in setuid sandbox, but"
                   << " chrome-sandbox binary is not available.";
      }
#endif  // defined(OS_TIZEN_TV_PRODUCT) && !defined(TIZEN_EMULATOR_SUPPORT)
...
#if defined(OS_TIZEN_TV_PRODUCT)
    // Some FD won`t be used anymore after initializing NaCl Zygote. So
    // it`s safe to close them after spawning process.
    nacl::CustomTizenFD::CleanupTizenFD();
#endif
...
pid_t NaClForkDelegate::Fork(const std::string& process_type,
  ...
  if (status_ == kNaClHelperUnused &&
      !base::CommandLine::ForCurrentProcess()->HasSwitch(
          switches::kEnableNaClZygotePreloading)) {
    Init(sandbox_descriptor_, false);
  }
#endif
