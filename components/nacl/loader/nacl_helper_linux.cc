...
bool HandleZygoteRequest(int zygote_ipc_fd,
  ...
  if (!nacl_sandbox->layer_one_enabled()) {
    ...
    LOG(INFO) << "NaCl helper process running with a sandbox!";
#endif
  }
...
#if defined(OS_TIZEN_TV_PRODUCT) && !defined(OS_NACL)
  // This is crucial to be called as fast as possible, to ensure that dlog is
  // initialized before we miss any important error logs messages.
  if (base::CommandLine::ForCurrentProcess()->HasSwitch(
          switches::kEnableNaClDlog)) {
    int fd_conf = nacl::CustomTizenFD::kLogConfDesc;
    int fd_devices[] = {
        nacl::CustomTizenFD::kLogMainDesc, nacl::CustomTizenFD::kLogRadioDesc,
        nacl::CustomTizenFD::kLogSystemDesc,
        // The file "/dev/log_apps" doesn`t exists on TV board, thus we pass
        // invalid FD to let dlog know, that it lacks this device.
        // The dlog will use this information to initialize it with "log_main",
        // so logging from APP will be redirected to main logging channel (thus
        // visible via command "dlogutil CHROMIUM")
        -1,
    };
    dlog_init_with_fd(fd_conf, fd_devices);
    logging::SetNaClLoggingEnabled(true);
  }
#endif
