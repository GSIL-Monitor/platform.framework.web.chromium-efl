...
void NaClSandbox::CheckForExpectedNumberOfOpenFds() {
  ...
  // On Tizen TVs there are 4 more FDs for dlog system.
  expected_num_fds += 4;
#endif

  LOG_IF(INFO,
         expected_num_fds != sandbox::ProcUtil::CountOpenFds(proc_fd_.get()))
      << "Wrong number of opened FDs!. Expected " << expected_num_fds
      << ", got " << sandbox::ProcUtil::CountOpenFds(proc_fd_.get());
}
