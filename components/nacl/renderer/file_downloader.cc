...
void FileDownloader::DidReceiveResponse(const blink::WebURLResponse& response) {
  ...
  if (http_status_code_ != 200) {
    ...
    // On Tizen TV, we receive response with status code 0,
    // when NaCl module is loaded from file:// protocol.
    // We can ignore error code, because in case of any errors,
    // didFail will be called.
    GURL url = response.Url();
    if (url.SchemeIsFile()) {
      if (http_status_code_ != 0)
        status_ = FAILED;
    } else {
      status_ = FAILED;
    }
