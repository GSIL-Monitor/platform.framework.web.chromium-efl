...
class TopSites : public RefcountedKeyedService {
 public:
  ...
  virtual void SyncWithHistory(bool is_previous) = 0;

  // For restore legacy most visited sites, need to clear top site cache
  // that made while first lunch after app upgrade.
  virtual void ClearThreadSafeCache() = 0;
#else
