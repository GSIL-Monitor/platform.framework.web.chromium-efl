...
//                    visit.)
//   dominant_color   Dominant color of icon.
...
bool InitTables(sql::Connection* db) {
  ...
  const char kFaviconBitmapsSql[] =
      ...
      "dominant_color INTEGER DEFAULT 0,"
      "image_data BLOB,"
...
bool ThumbnailDatabase::GetFaviconBitmap(
    ...
    gfx::Size* pixel_size,
    unsigned int* dominant_color) {
  DCHECK(bitmap_id);
  sql::Statement statement(db_.GetCachedStatement(SQL_FROM_HERE,
      "SELECT last_updated, image_data, width, height, last_requested, "
      "dominant_color FROM favicon_bitmaps WHERE id=?"));
  ...
  if (dominant_color)
    *dominant_color = statement.ColumnInt(5);
...
void ThumbnailDatabase::UpdateDominantColor(const GURL& icon_url,
    unsigned int dominant_color) {
  sql::Statement statement1(db_.GetCachedStatement(SQL_FROM_HERE,
      "SELECT id "
      "FROM favicons "
      "WHERE favicons.url=? "));
  statement1.BindString(0, URLDatabase::GURLToDatabaseURL(icon_url));
  if (!statement1.Step()) {
      LOG(WARNING) << "UpdateDominantColor() query1 run failed for URL: "
              << URLDatabase::GURLToDatabaseURL(icon_url);
      return;
  }
  favicon_base::FaviconID icon_id = statement1.ColumnInt64(0);
  sql::Statement statement2(db_.GetCachedStatement(SQL_FROM_HERE,
      "UPDATE favicon_bitmaps "
      "SET dominant_color=? "
      "WHERE favicon_bitmaps.icon_id=? "));
  statement2.BindInt(0, dominant_color);
  statement2.BindInt64(1, icon_id);
  if (!statement2.Run()) {
      LOG(WARNING) << "UpdateDominantColor() query2 run failed for URL: "
              << URLDatabase::GURLToDatabaseURL(icon_url);
  }
}
...
bool ThumbnailDatabase::UpgradeToVersion9() {
  // Add the dominant_color column to the favicon_bitmaps table.
  const char kFaviconBitmapsDominantColorSql[] =
      "ALTER TABLE favicon_bitmaps ADD COLUMN dominant_color INTEGER DEFAULT 0";
  if (!db_.Execute(kFaviconBitmapsDominantColorSql))
    return false;

  meta_table_.SetVersionNumber(9);
  meta_table_.SetCompatibleVersionNumber(std::min(9, kCompatibleVersionNumber));
  return true;
}
