...
#if defined(S_TERRACE_SUPPORT)
void TopSitesImpl::ClearThreadSafeCache() {
  base::AutoLock lock(lock_);
  thread_safe_cache_->SetTopSites(std::vector<MostVisitedURL> ());
}

void TopSitesImpl::SyncWithHistory(bool is_previous) {
  DCHECK(thread_checker_.CalledOnValidThread());
  if (is_previous)
    StartQueryForMostVisited(is_previous);
  else if (loaded_) {
    timer_.Stop();
...
#if defined(S_TERRACE_SUPPORT)
void TopSitesImpl::StartQueryForMostVisited(bool is_previous) {
  if (!is_previous)
    DCHECK(loaded_);
  if (!history_service_)
    return;
  ...
  history_service_->QueryMostVisitedURLs(
      ...
      base::Bind(&TopSitesImpl::OnTopSitesAvailableFromHistory,
                 base::Unretained(this)),
      &cancelable_task_tracker_, is_previous);
}

void TopSitesImpl::StartQueryForMostVisited() {
  StartQueryForMostVisited(false);
}
#else
void TopSitesImpl::StartQueryForMostVisited() {
  DCHECK(loaded_);
...
void TopSitesImpl::SetTopSites(const MostVisitedURLList& new_top_sites,
  ...
  else if (location == CALL_LOCATION_FROM_OTHER_PLACES)
    NotifyTopSitesChanged(TopSitesObserver::ChangeReason::MOST_VISITED, true);
  else
...
#if defined(S_TERRACE_SUPPORT)
  if (already_mostvisited_request_) {
    // During app update, crash can occured
    // if History DB's segment table are all lost
    // and Top Sites DB's thumbnails table are remain.
    size_t min_size =
        std::min(thumbnails->most_visited.size(), cache_->top_sites().size());
    for (size_t i=0; i < min_size; i++)
      thumbnails->most_visited[i].score = cache_->top_sites()[i].score;
  }
#endif
...
#if defined(S_TERRACE_SUPPORT)
  already_mostvisited_request_ = true;
#endif
}
