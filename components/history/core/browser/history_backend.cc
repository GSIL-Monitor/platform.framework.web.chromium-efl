...
SegmentID HistoryBackend::UpdateSegments(const GURL& url,
  ...
  if ((ui::PageTransitionCoreTypeIs(transition_type,
    ...
    // For avoid different url create same segment name, using url content
    std::string segment_name = url.spec();
#else
  if ((ui::PageTransitionCoreTypeIs(transition_type,
                                    ui::PAGE_TRANSITION_TYPED) ||
       ui::PageTransitionCoreTypeIs(transition_type,
...
void HistoryBackend::GetLargestFaviconForURL(
  ...
  if (!thumbnail_db_->GetFaviconBitmap(
          ...
          &bitmap_result.bitmap_data, &bitmap_result.pixel_size, &bitmap_result.dominant_color)) {
    return;
  }
...
void HistoryBackend::UpdateDominantColor(const GURL& icon_url,
    unsigned int dominant_color) {
  if (!thumbnail_db_) return;
  thumbnail_db_->UpdateDominantColor(icon_url, dominant_color);
}
...
bool HistoryBackend::IsFaviconBitmapDataEqual(
  ...
  thumbnail_db_->GetFaviconBitmap(bitmap_id, nullptr, nullptr,
                                  &original_bitmap_data, nullptr, nullptr);
