...
void HistoryService::UpdateDominantColor(const GURL& icon_url,
    unsigned int dominant_color) {
  DCHECK(thread_) << "History service being called after cleanup";
  DCHECK(thread_checker_.CalledOnValidThread());

  ScheduleTask(PRIORITY_NORMAL, base::Bind(&HistoryBackend::UpdateDominantColor,
        history_backend_.get(), icon_url, dominant_color));
}
...
base::CancelableTaskTracker::TaskId HistoryService::QueryMostVisitedURLs(
  ...
  return tracker->PostTaskAndReply(
      ...
      base::Bind(
          &HistoryBackend::QueryMostVisitedURLs, history_backend_.get(),
          result_count, days_back, base::Unretained(result), is_previous),
      base::Bind(callback, base::Owned(result)));
}
#else
base::CancelableTaskTracker::TaskId HistoryService::QueryMostVisitedURLs(
