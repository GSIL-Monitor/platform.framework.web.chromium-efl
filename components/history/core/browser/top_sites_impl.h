...
class TopSitesImpl : public TopSites, public HistoryServiceObserver {
 public:
  ...
  void ClearThreadSafeCache() override;
  void SyncWithHistory(bool is_previous = false) override;
#else
...
#if defined(S_TERRACE_SUPPORT)
  void StartQueryForMostVisited(bool is_previous);
#endif
...
#if defined(S_TERRACE_SUPPORT)
  // Are we need to add score value?
  bool already_mostvisited_request_ = false;
#endif
