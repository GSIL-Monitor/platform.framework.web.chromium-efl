...
VisitSegmentDatabase::QuerySegmentUsage(
  ...
  while (statement.Step()) {
    ...
    if (segment_id != previous_segment_id) {
      if ((is_previous && previous_segment_id) ||
          ((day_visit >= min_day_visit) &&
          (sum_visit_count >= min_visit_count))) {
        segments.push_back(
            base::MakeUnique<PageUsageData>(previous_segment_id));
        segments.back()->SetScore(score);
      }
    ...
    day_visit++;
    sum_visit_count += visit_count;
  }
  if ((is_previous && previous_segment_id) || ((day_visit >= min_day_visit) &&
      (sum_visit_count >= min_visit_count))) {
    segments.push_back(
        base::WrapUnique(new PageUsageData(previous_segment_id)));
    segments.back()->SetScore(score);
  }
#else
  while (statement.Step()) {
