...
#if defined(S_TERRACE_SUPPORT)
void TranslateScript::SetAccessToken(const std::string& token) {
  if (token_.compare(token) == 0) return;

  if (!data_.empty()) {
    std::string old_token =
        base::StringPrintf("var translateAccessToken = '%s'", token_.c_str());
    std::string new_token =
        base::StringPrintf("var translateAccessToken = '%s'", token.c_str());

    size_t pos = data_.find(old_token);
    if (pos != std::string::npos) {
      data_.replace(pos, old_token.length(), new_token);
      token_ = token;
    }
  } else {
    token_ = token;
...
#if defined(S_TERRACE_SUPPORT)
  // For CBT, using local JS file for translation.
  // After CBT, we'll upload the javascript file to server and get it.
  OnScriptFetchComplete(0, true, "");
#else
...
void TranslateScript::OnScriptFetchComplete(
  ...
  if (success) {
    ...
    base::StringPiece str = ResourceBundle::GetSharedInstance().
        GetRawDataResource(IDR_TRANSLATE_JS_SAMSUNG);
    data_ = base::StringPrintf("var translateAccessToken = '%s';\n",
        token_.c_str());
