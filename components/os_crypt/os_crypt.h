...
class OSCrypt {
 public:
...
#if defined(S_TERRACE_SUPPORT)
  // Get the key to encrypt/decrypt password using AES256.
  static std::string GetKey256(std::string password);
  ...
  static void WBSEncrypt(const std::string& raw_data,
                         std::string* out_encrypted_data,
                         std::string* out_iv,
                         std::vector<unsigned char>* out_encrypted_key);

  static void WBSDecrypt(const std::string& encrypted_data,
                         const std::string& iv,
