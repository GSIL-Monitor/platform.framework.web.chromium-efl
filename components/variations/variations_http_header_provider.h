...
class VariationsHttpHeaderProvider : public base::FieldTrialList::Observer,
 ...
 public:
  ...
  // Sets Search Widget Experiment Token for Google's Data Collection about
  // GSA (Google Search App) status.
  void SetSearchWidgetExperimentToken(std::string token);

  // Returns the token string of Search Widget Experiment that has been set.
  // It will be appended on the X-Client-Data request header.
  std::string GetSearchWidgetExperimentToken();
 ...
 private:
  ...
  // Search Widget Experiment Token
  std::string token_;
