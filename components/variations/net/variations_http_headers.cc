...
void AppendSearchWidgetExperimentToken(const GURL& url,
  ...
  std::string token = VariationsHttpHeaderProvider::GetInstance()->
      GetSearchWidgetExperimentToken();

  if (token.size() == 0 || incognito || !google_util::IsGoogleSearchUrl(url))
    return;

  headers->SetHeaderIfMissing(kClientData, token);
}
