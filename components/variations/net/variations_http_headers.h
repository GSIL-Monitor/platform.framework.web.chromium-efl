...
// Appends experiment token for Search Widget Experiment, which is requested
// from Google, to |headers|. The token from GSA(Google Search App) is set on
// 'X-Client-Data' header field, if the reuqested url is Google Search domain.
// It won't be applied on |incognito| mode.
void AppendSearchWidgetExperimentToken(const GURL& url,
                                       bool incognito,
