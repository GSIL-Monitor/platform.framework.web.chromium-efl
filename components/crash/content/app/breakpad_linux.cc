...
#include "components/crash/content/app/libtombstone/public/tombstone.h"
...
#if defined(S_TERRACE_SUPPORT) && !defined(ARCH_CPU_X86_FAMILY)
bool g_prevent_system_popup = false;
#endif
...
#if defined(S_TERRACE_SUPPORT) && !defined(ARCH_CPU_X86_FAMILY)
// crash handler for apps using Terrace
#define _LOG(fmt, ...) \
  __android_log_print(ANDROID_LOG_ERROR, "tombstone", fmt, ##__VA_ARGS__)

// TODO(kiwon): Crash keys are not used actually. Disable this until this is
// required. Or delete this on 8.0.
//void PrintCrashKey() {
//  // print crash keys ( url, memory usage , etc )
//  if (g_crash_keys) {
//    CrashKeyStorage::Iterator crash_key_iterator(*g_crash_keys);
//    const CrashKeyStorage::Entry* entry;
//    _LOG("--- --- --- --- --- --- --- --- --- --- --- --- --- --- --- ---\n");
//    _LOG("crash keys:\n");
//    while ((entry = crash_key_iterator.Next())) {
//      // kActiveURL is defined in Terrace layer, so we can't refer it here.
//      // [TODO] need to define filter funtions in base::debug::
//      // "url-chunk" should be same as kActiveURL
//      if (strstr(entry->key, "url-chunk") != NULL)
//        continue;
...
void* StartLogging(void* data) {
  std::string issue_type = "native crash";
  std::string source;
  std::string proces_type = g_process_type;
  if (proces_type == kBrowserProcessType) {
    source = "browser";
  } else if (proces_type == kGpuProcessType) {
    source = "gpu";
  } else {
    _LOG("[ERROR] StartLogging: undefined process type!!");
    return nullptr;
  }

  std::string crash_info;
  if (data != nullptr) {
    crash_info = *(static_cast<std::string*>(data));
  }
  content::TerraceContentBridge::Get().StartLogging(issue_type, source,
                                                    crash_info);
  return nullptr;
}

void ReportCrash(std::string& process_type,
                 ExceptionHandler::CrashContext* context,
                 base::android::BuildInfo* build_info) {
  // Browser and GPU processes only have the permission to launch the logging
  // service.
  if (process_type != kBrowserProcessType && process_type != kGpuProcessType)
    return;

  // NOTE: Don't use pthread_join(). This process might get killed before
  // returns from the reporting thread.
  pthread_t thread_t;
  if (pthread_create(&thread_t, nullptr, StartLogging, nullptr) == 0) {
    if (process_type == kGpuProcessType) {
      // GPU process blocks system popup, so this process gets killed
      // right after returns from this handler. So suspend for a second
      // util launch the logging service.
      struct timespec ts;
      ts.tv_sec = 0;
...
bool TerraceCrashCallback(const void* crash_context,
                          size_t crash_context_size,
                          void* data) {
  std::string process_type = g_process_type;

  ExceptionHandler::CrashContext* context = NULL;
  context = (ExceptionHandler::CrashContext*)crash_context;

  // print version information
  base::android::BuildInfo* build_info =
      base::android::BuildInfo::GetInstance();
  _LOG("Fatal signal %d, code %d, fault addr %p, Process Type : %s\n",
       context->siginfo.si_signo, context->siginfo.si_code,
       context->siginfo.si_addr, process_type.c_str());

  if (build_info) {
    _LOG("SBrowser version = %s\n", build_info->package_version_name());
    _LOG("SBrowser version code = %s\n", build_info->package_version_code());
  }

  if (g_crash_keys != nullptr) {
    const char* vrplugin_isofficial =
        g_crash_keys->GetValueForKey("vrplugin_isofficial");
    if (vrplugin_isofficial != nullptr) {
      _LOG("vrplugin isofficial = %s\n", vrplugin_isofficial);
    }

    const char* vrplugin_version =
        g_crash_keys->GetValueForKey("vrplugin_version");
    if (vrplugin_version != nullptr) {
      _LOG("vrplugin version = %s\n", vrplugin_version);
    }

    const char* vrruntime_version =
        g_crash_keys->GetValueForKey("vrruntime_version");
    if (vrruntime_version != nullptr) {
      _LOG("vrruntime version = %s\n", vrruntime_version);
    }
  }

  // print tombstone
// FIXME: m62_3202 bringup, needs to be checked by author
/*  TombStone tombstone(g_pid, context->tid);
  tombstone.signal_ = context->siginfo.si_signo;
  tombstone.siginfo_ = &context->siginfo;
  if (process_type != kBrowserProcessType && process_type != kGpuProcessType) {
    // Sandboxed processes cannot call alarm().
    // See "## The `seccomp-bpf` sandbox" in src/docs/linux_sandboxing.md.
    tombstone.timeout_ = 0;
  }
  tombstone.ucontext_ = &context->context;
  tombstone.dump_siblings_backtrace_ = true;
  tombstone.dump_memory_map_ = true;
  tombstone.engrave();*/

  ReportCrash(process_type, context, build_info);

  // Return value meanings of chrome ExceptionHandler callback
  // true : call linux default signal handler ( ex. seem to kill process )
  // false : call old signal handler ( ex. android system crash handler )
  //          if return false, the system crash popup will be executed.
  if (process_type == kBrowserProcessType && !g_prevent_system_popup) {
    return false;
  } else {
    // Don't know the reason but it takes long time that this process is really
    // killed. So exit.
    exit(1);
    return true;
  }
...
void DumpTombstoneWithoutCrashingFunction() {
  if (g_breakpad == nullptr) {
    LOG(ERROR) << "Tombstone is not initialized\n";
    return;
  }

  // Several places in upstream call this but this is only for Renderer
  // Unresponsive. Hence drop any requests except renderer.
  std::string process_type = g_process_type;
  if (process_type != "renderer")
    return;

  // pause all threads of current pid and print backtrace
// FIXME: m62_3202 bringup, needs to be checked by author
/*  TombStone tombstone;
  tombstone.dump_siblings_backtrace_ = true;
  tombstone.timeout_ = 0;
...
void InitTombstone(const std::string& process_type) {
  // TODO(kiwon): Remove this condition.
  // Disable tombstone for renderer temporarily because tombstone hangs.
  if (process_type == "renderer")
    return;
  ...
  if (process_type.empty())
    process_name = kBrowserProcessType;
  else
    process_name = process_type;

  // Save the process type (it is leaked).
  const size_t process_name_len = process_name.size() + 1;
  g_process_type = new char[process_name_len];
  strncpy(g_process_type, process_name.c_str(), process_name_len);

  // collect informations from several modules ( ex. current url )
  InitCrashKeys();

  // Uninitialized mode causes UpdatePath() and assertion occurs.
  // Microdumpt works on the cloned process but we use in-process,
  // hence minidump_descriptor dose not work eventually.
  MinidumpDescriptor minidump_descriptor(
      MinidumpDescriptor::kMicrodumpOnConsole);

  // ExceptonHandler has good logic for sigaction handling
  // (ex. stack overflow, etc)
  // so let's reuse it instead of implementing our sigaction code.
  g_breakpad =
      new ExceptionHandler(minidump_descriptor, nullptr,
                           nullptr,  // minidump handler is not required
                                     // because we will use custom handler
                           nullptr,
                           true,  // Install handlers.
                           -1);   // Server file descriptor. -1 for in-process.

  // ignore chrimium's default minidump
  // use owr own callback to print tomstone information to logcat
  g_breakpad->set_crash_handler(TerraceCrashCallback);
  if (process_name == kBrowserProcessType || process_name == kGpuProcessType) {
    content::TerraceContentBridge& terrace =
        content::TerraceContentBridge::Get();
    // Init CustomLogger to call the startLogging java function.
    terrace.InitCustomLogger();
    g_prevent_system_popup = terrace.PreventSystemPopup();
  }

  // set dump function to base::debug for any module can call this function
  // this is required because breakpad_linux is component/app layer
  base::debug::SetDumpWithoutCrashingFunction(
      DumpTombstoneWithoutCrashingFunction);
}
