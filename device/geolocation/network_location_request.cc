...
// TV Product uses maxmind server, not googleapis.
const char kNetworkLocationBaseUrl[] =
    "https://112627:pi92dZGcGKGE@geoip.maxmind.com/geoip/v2.1/city/me?pretty";
const char kLatitudeString[] = "latitude";
const char kLongitudeString[] = "longitude";
const char kAccuracyString[] = "accuracy_radius";
#else
const char kNetworkLocationBaseUrl[] =
...
#if defined(OS_TIZEN_TV_PRODUCT)
  // TV product donot use FormRequestURL(), because this is only for googleapis.
  const GURL request_url(kNetworkLocationBaseUrl);
...
#if defined(OS_TIZEN_TV_PRODUCT)
  // Fixup! Add authorization header for MaxMind.
  // The value converts from Base64Encoded(<UserName>:<Password>).
  // UserName: bitpark@samsung.com
  // Password: webengine!
  const char* req_auth_header =
      "Authorization: Basic MTM1MDQzOldId2I1ZWh4cENCTw==";
  url_fetcher_->AddExtraRequestHeader(req_auth_header);
#endif
...
#if defined(OS_TIZEN_TV_PRODUCT)
  // maxmind server have accuracy in location_object.
