...
class NetworkLocationProvider : public LocationProvider {
 ...
 private:
...
#if defined(OS_TIZEN_TV_PRODUCT)
  bool GetGeographicalPositionFromDatabase();
  void SetGeographicalPositionToDatabase();
  bool OpenDatabase();
  bool AddToDatabase();
  ...
  void CheckMaxRows();
  bool IsOpen() const {
    return geo_database_.get() ? geo_database_->is_open() : false;
  }
#endif
...
#if defined(OS_TIZEN_TV_PRODUCT)
  std::unique_ptr<sql::Connection> geo_database_;
  base::FilePath geo_database_directory_;
  base::FilePath geo_database_path_;
  bool is_open_failed_;
  bool is_recreated_;
#endif
