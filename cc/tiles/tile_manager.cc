...
class RasterTaskImpl : public TileTask {
 public:
  ...
  TileResolution GetTileResolution() const override { return tile_resolution_; }
...
#if defined(OS_TIZEN)
  if (!did_check_for_completed_tasks_since_last_schedule_tasks_ && visible) {
#else
...
#if defined(OS_TIZEN)
void TileManager::SetClearTilesOnHide(bool enable) {
...
void TileManager::FreeResourcesForTile(Tile* tile) {
  ...
  if (resource) {
#if defined(OS_TIZEN)
    if (!visibility_ && hide_enabled_)
      resource_pool_->ReleaseResourceImmediately(resource);
    else
...
void TileManager::OnRasterTaskCompleted(
  ...
  if (tile && tile->raster_task_.get()) {
    TileResolution tr = tile->raster_task_->GetTileResolution();
    if (tr == LOW_RESOLUTION) {
