...
bool Viewport::ShouldBrowserControlsConsumeScroll(
  ...
  if (TotalScrollOffset().y() < MaxTotalScrollOffset().y())
    return true;
  // Force the top controls to consume if contents is taller than viewport
  // but smaller than viewport + top controls height
  // The gmail log-in page has this condition
  float ratio = host_impl_->browser_controls_manager()->TopControlsShownRatio();
