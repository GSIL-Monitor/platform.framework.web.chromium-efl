...
// scale during pinch.
//
// Increased from 2.0 for Samsung's browser.
const float kMaxScaleRatioDuringPinch = 4.0f;

// When creating a new tiling during pinch, snap to an existing
// tiling's scale if the desired scale is within this ratio.
//
// Increased from 1.2 for Samsung's browser.
const float kSnapToExistingTilingRatio = 1.44f;
...
void PictureLayerImpl::UpdateViewportRectForTilePriorityInContentSpace() {
  ...
  if (layer_tree_impl()->IsPendingTree()) {
    LayerImpl* active_tree_layer =
        layer_tree_impl()->FindActiveTreeLayerById(this->id());
    LayerImpl* pending_clipping_layer =
        layer_tree_impl()->OuterViewportContainerLayer();
    LayerImpl* active_clipping_layer =
        (active_tree_layer && active_tree_layer->layer_tree_impl()) ?
        active_tree_layer->layer_tree_impl()->OuterViewportContainerLayer() :
        nullptr;
    if (active_tree_layer && pending_clipping_layer && active_clipping_layer &&
        pending_clipping_layer->bounds() != active_clipping_layer->bounds() &&
        active_tree_layer->visible_layer_rect().Contains(
            visible_layer_rect())) {
      visible_rect_in_content_space = active_tree_layer->visible_layer_rect();
    }
  }
#endif
...
#if defined(S_TERRACE_SUPPORT)
      CheckScrollIntersectionArea(&visible_rect_in_content_space);
#endif
    }
  }
...
#if defined(S_TERRACE_SUPPORT)
void PictureLayerImpl::CheckScrollIntersectionArea(
...
#if defined(OS_TIZEN)
void PictureLayerImpl::SetTileCoverAreaMultiplier(float cover_area_multiplier) {
  if (tilings_)
    tilings_->SetTileCoverAreaMultiplier(cover_area_multiplier);
}
#endif
