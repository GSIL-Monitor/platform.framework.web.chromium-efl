...
void SolidColorScrollbarLayerImpl::AppendQuads(
  ...
  // SAMSUNG: Draw the track areas if the scrollbar is not overlay.
  if (!is_overlay_scrollbar()) {
    gfx::Rect track_quad_rect(bounds());
    gfx::Rect visible_track_quad_rect =
        draw_properties().occlusion_in_content_space.GetUnoccludedContentRect(
            track_quad_rect);
    viz::SolidColorDrawQuad* track_quad =
        render_pass->CreateAndAppendDrawQuad<viz::SolidColorDrawQuad>();
    track_quad->SetNew(shared_quad_state, track_quad_rect,
                       visible_track_quad_rect, SkColorSetRGB(241, 241, 241),
                       false);
  }
