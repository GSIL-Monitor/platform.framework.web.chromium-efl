...
Layer::Inputs::Inputs(int layer_id)
    : layer_id(layer_id),
      ...
      scroll_corner_layer_id(INVALID_ID),
#endif
...
void Layer::SetLayerInverted(bool inverted) {
  ...
  for (size_t i = 0; i < inputs_.children.size(); ++i)
    inputs_.children[i]->SetFilters(filters);
}
#endif
...
#if defined(TIZEN_VD_NATIVE_SCROLLBARS)
  layer->SetScrollCornerLayerId(inputs_.scroll_corner_layer_id);
#endif
...
#if defined(TIZEN_VD_NATIVE_SCROLLBARS)
void Layer::SetScrollCornerLayerId(int scroll_corner_layer_id) {
  if (inputs_.scroll_corner_layer_id == scroll_corner_layer_id)
    return;

  inputs_.scroll_corner_layer_id = scroll_corner_layer_id;
  SetNeedsCommit();
}
#endif
}  // namespace cc
