...
const float kCanScrollEpsilon = 0.01f;
...
#if defined(TIZEN_VD_NATIVE_SCROLLBARS)
  layer->ToScrollbarLayer()->SetIsHovered(is_hovered_);
#endif
...
bool ScrollbarLayerImplBase::CanScrollOrientation() const {
  ...
  return !MathUtil::IsFloatNearlyTheSame(clip_layer_length(),
                                         scroll_layer_length()) &&
         kCanScrollEpsilon < scroll_layer_length() - clip_layer_length();
}
...
void ScrollbarLayerImplBase::SetIsHovered(bool is_hovered) {
  ...
  layer_tree_impl()->UpdateScrollbarAnimationIfNeeded(scroll_element_id_);
}
#endif
}  // namespace cc
