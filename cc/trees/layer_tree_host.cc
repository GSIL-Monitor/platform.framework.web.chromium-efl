...
#if defined(OS_TIZEN)
void LayerTreeHost::DidChangeLoadState(bool is_loading) {
  proxy_->DidChangeLoadState(is_loading);
}
#endif
...
void LayerTreeHost::SetRootLayer(scoped_refptr<Layer> root_layer) {
  ...
  if (root_layer_.get()) {
    ...
    if (layer_invert_failed_) {
      root_layer_->SetLayerInverted(layer_inverted_);
      layer_invert_failed_ = false;
    }
#endif
  }
...
void LayerTreeHost::SetFastScrollerEnabled(bool enabled) {
  if (fast_scroller_enabled_ == enabled)
    return;
  fast_scroller_enabled_ = enabled;
  SetNeedsCommit();
}
#endif
...
#if defined(OS_TIZEN_TV_PRODUCT)
void LayerTreeHost::SetLayerInverted(bool inverted) {
  if (root_layer_.get()) {
    root_layer_->SetLayerInverted(inverted);
  } else {
    LOG(INFO) << "Root Layer not yet ready, we store the invert value to apply "
                 "it later.";
    layer_inverted_ = inverted;
    layer_invert_failed_ = true;
  }
}
#endif
...
void LayerTreeHost::PushLayerTreePropertiesTo(LayerTreeImpl* tree_impl) {
  ...
  tree_impl->RegisterSelection(selection_, selection_rect_);
...
#if defined(S_TERRACE_SUPPORT)
  tree_impl->set_fast_scroller_enabled(fast_scroller_enabled_);
#endif
