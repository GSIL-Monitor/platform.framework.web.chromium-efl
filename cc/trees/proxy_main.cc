...
#if defined(OS_TIZEN)
// For throttling BeginMainFrame
const int kBeginMainFrameDelay = 100;
#endif
...
#if defined(OS_TIZEN)
      first_begin_main_frame_(true),
      need_to_throttle_(false),
      begin_main_frame_timer_(new base::OneShotTimer),
#endif
...
#if defined(OS_TIZEN)
  // Skip throttling for the first BeginMainFrame to improve launching
  // performance.
  if (need_to_throttle_ && !first_begin_main_frame_) {
    DCHECK(!begin_main_frame_timer_->IsRunning());
    begin_main_frame_callback_ =
        base::Bind(&ProxyMain::DoBeginMainFrame, weak_factory_.GetWeakPtr(),
                   base::Passed(&begin_main_frame_state));
    begin_main_frame_timer_->Start(
        FROM_HERE, base::TimeDelta::FromMilliseconds(kBeginMainFrameDelay),
        begin_main_frame_callback_);
  } else {
    begin_main_frame_timer_->Stop();
    DoBeginMainFrame(std::move(begin_main_frame_state));
  }
}

void ProxyMain::DoBeginMainFrame(
    std::unique_ptr<BeginMainFrameAndCommitState> begin_main_frame_state) {
  first_begin_main_frame_ = false;
  begin_main_frame_timer_.reset(new base::OneShotTimer);
#endif
...
#if defined(OS_TIZEN)
void ProxyMain::SetTileCoverAreaMultiplier(float cover_area_multiplier) {
  ImplThreadTaskRunner()->PostTask(
      FROM_HERE, base::BindOnce(&ProxyImpl::SetTileCoverAreaMultiplierOnImpl,
                                base::Unretained(proxy_impl_.get()),
                                cover_area_multiplier));
}

void ProxyMain::SetClearTilesOnHide(bool enable) {
  ImplThreadTaskRunner()->PostTask(
      FROM_HERE, base::BindOnce(&ProxyImpl::SetClearTilesOnHideOnImpl,
                                base::Unretained(proxy_impl_.get()), enable));
}

void ProxyMain::DidChangeLoadState(bool is_loading) {
  DCHECK(IsMainThread());
  if (need_to_throttle_ == is_loading)
    return;

  need_to_throttle_ = is_loading;

  // Force to call BeginMainFrame immediately if loading is finished.
  if (!need_to_throttle_ && begin_main_frame_timer_->IsRunning() &&
      !begin_main_frame_callback_.is_null()) {
    begin_main_frame_callback_.Run();
  }
  begin_main_frame_callback_.Reset();
}
#endif
