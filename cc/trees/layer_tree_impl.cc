...
void LayerTreeImpl::PushPropertiesTo(LayerTreeImpl* target_tree) {
  ...
  target_tree->RegisterSelection(selection_, selection_rect_);

#if defined(S_TERRACE_SUPPORT)
  target_tree->set_fast_scroller_enabled(fast_scroller_enabled_);
#endif
...
#if defined(OS_TIZEN)
void LayerTreeImpl::SetTileCoverAreaMultiplier(float cover_area_multiplier) {
  for (PictureLayerImpl* layer : picture_layers_)
    layer->SetTileCoverAreaMultiplier(cover_area_multiplier);
}
#endif
...
#if defined(S_TERRACE_SUPPORT)
void LayerTreeImpl::set_fast_scroller_enabled(bool fast_scroller_enabled) {
  if (fast_scroller_enabled_ == fast_scroller_enabled)
    return;
  fast_scroller_enabled_ = fast_scroller_enabled;
}
#endif
...
void LayerTreeImpl::DidUpdatePageScale() {
  LOG(INFO) << "DidUpdatePageScale IsActiveTree: "
      << IsActiveTree() << " ScaleFactor: " << current_page_scale_factor();
...
void LayerTreeImpl::RegisterSelection(const LayerSelection& selection,
  ...
  if (selection_ == selection && selection_rect_ == selection_rect)
    return;
...
void LayerTreeImpl::GetSelectionRect(gfx::Rect* rect) {
...
void LayerTreeImpl::UpdateScrollbarAnimationIfNeeded(
  ...
  LayerImpl* scroll_layer = LayerByElementId(scroll_element_id);
  ScrollbarAnimationController* controller =
  ...
  if (!settings().use_native_scrollbars ||
      ...
      !scroll_layer || !scroll_layer->scrollable() || !controller)
    return;

  // We don't want scrollbar fade animation when scrollbar is hovered.
  bool should_stop_animation = false;
  for (ScrollbarLayerImplBase* scrollbar : ScrollbarsFor(scroll_element_id)) {
