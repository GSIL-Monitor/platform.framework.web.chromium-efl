...
LayerTreeHostImpl::LayerTreeHostImpl(
    ...
    : client_(client),
      ...
      big_sub_scrolling_layer_(false),
      fast_scroll_disabled_by_wheel_(false),
      target_viewport_scrolling_(false),
#endif
...
bool LayerTreeHostImpl::PrepareTiles() {
  ...
  bool did_prepare_tiles =
      tile_manager_.PrepareTiles(global_tile_state_, visible_);
#else
...
DrawResult LayerTreeHostImpl::CalculateRenderPasses(FrameData* frame) {
  ...
  if (num_missing_tiles) {
    UMA_HISTOGRAM_COUNTS_100("LayerTreeHostImpl.NumMissingTiles", num_missing_tiles);
  }
...
#if defined(OS_TIZEN)
  if (resource_provider_ && !visible_ && hide_enabled_)
...
viz::CompositorFrameMetadata LayerTreeHostImpl::MakeCompositorFrameMetadata()
  ...
  active_tree_->GetSelectionRect(&metadata.selection_rect);
...
#if defined(OS_TIZEN)
void LayerTreeHostImpl::SetTileCoverAreaMultiplier(
    float cover_area_multiplier) {
  active_tree_->SetTileCoverAreaMultiplier(cover_area_multiplier);
}

void LayerTreeHostImpl::SetClearTilesOnHide(bool enable) {
  hide_enabled_ = enable;
  tile_manager_.SetClearTilesOnHide(enable);
}
#endif
...
#if defined(S_TERRACE_SUPPORT)
void LayerTreeHostImpl::NotifyScrollbarState(bool requestToDisable) {
  bool nativeScrollbarVisible = !active_tree_->get_fast_scroller_enabled();
  if (input_handler_client_) {
    input_handler_client_->NotifyScrollbarState(
        requestToDisable, nativeScrollbarVisible);
  }
}

void LayerTreeHostImpl::CheckBigSubScrollingLayer(LayerImpl* scroll_layer,
    InputHandler::ScrollInputType type) {

  if (type == WHEEL) {
    NotifyScrollbarState(true);
    fast_scroll_disabled_by_wheel_ = true;
    return;
  }

  if (fast_scroll_disabled_by_wheel_) {
    fast_scroll_disabled_by_wheel_ = false;
  }

  if (scroll_layer == InnerViewportScrollLayer() ||
    scroll_layer == OuterViewportScrollLayer()) {
    NotifyScrollbarState(false);
    big_sub_scrolling_layer_ = false;
    return;
  }

  gfx::Rect scrolling_layer_bounds(
      (int) scroll_layer->position().x(), (int) scroll_layer->position().y(),
      scroll_layer->bounds().width(), scroll_layer->bounds().height());

  gfx::Rect outer_scrolling_layer_bounds(
      (int) OuterViewportScrollLayer()->position().x(),
      (int) OuterViewportScrollLayer()->position().y(),
      OuterViewportScrollLayer()->bounds().width(),
      OuterViewportScrollLayer()->bounds().height());

  if (scrolling_layer_bounds.Contains(outer_scrolling_layer_bounds)) {
    big_sub_scrolling_layer_ = true;
    NotifyScrollbarState(true);
  } else {
    big_sub_scrolling_layer_ = false;
  }
...
#if defined(S_TERRACE_SUPPORT)
  target_viewport_scrolling_ = true;
#endif
...
InputHandlerScrollResult LayerTreeHostImpl::ScrollBy(
  ...
  TRACE_EVENT2("cc", "LayerTreeHostImpl::ScrollBy"
      ,"delta_x", scroll_state->delta_x()
      ,"delta_y", scroll_state->delta_y());
  ScrollTree& scroll_tree = active_tree_->property_trees()->scroll_tree;
  ScrollNode* scroll_node = scroll_tree.CurrentlyScrollingNode();
