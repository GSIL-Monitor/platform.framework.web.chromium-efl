...
#if defined(OS_TIZEN)
  void SetClearTilesOnHide(bool enable) override;
#endif

#if defined(OS_TIZEN)
  void SetTileCoverAreaMultiplier(float cover_area_multiplier) override;
  void DidChangeLoadState(bool is_loading) override;
  void DoBeginMainFrame(
      std::unique_ptr<BeginMainFrameAndCommitState> begin_main_frame_state);
#endif
...
#if defined(OS_TIZEN)
  bool first_begin_main_frame_;
  bool need_to_throttle_;
  base::Closure begin_main_frame_callback_;
  std::unique_ptr<base::OneShotTimer> begin_main_frame_timer_;
#endif
