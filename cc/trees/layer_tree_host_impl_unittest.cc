...
TEST_F(LayerTreeHostImplTest, DISABLED_SelectionBoundsPassedToCompositorFrameMetadata) {
  int root_layer_id = 1;
  std::unique_ptr<SolidColorLayerImpl> root =
  ...
  const gfx::Rect selection_rect =
      fake_layer_tree_frame_sink->last_sent_frame()->metadata.selection_rect;
...
// FIXME Commented the test case during dev/m60_3112 rebaseline.
// It needs a re-visit by Author.
/*
TEST_F(LayerTreeHostImplTest,
       SelectionRectDefaultBoundsPassedToCompositorFrameMetadata) {
  int root_layer_id = 1;
  std::unique_ptr<SolidColorLayerImpl> root =
      SolidColorLayerImpl::Create(host_impl_->active_tree(), root_layer_id);
  root->SetPosition(gfx::PointF());
  ...
  // Ensure the selection rect default bounds are propagated to the
  // frame metadata when rect was not explicitly passed
  const gfx::Rect selection_rect =
      fake_compositor_frame_sink->last_sent_frame()->metadata.selection_rect;
  EXPECT_EQ(gfx::Rect(), selection_rect);
}
*/
