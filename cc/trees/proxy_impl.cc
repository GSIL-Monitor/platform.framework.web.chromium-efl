...
#if defined(OS_TIZEN)
void ProxyImpl::SetTileCoverAreaMultiplierOnImpl(float cover_area_multiplier) {
  TRACE_EVENT1("cc", "ProxyImpl::SetTileCoverAreaMultiplierOnImplThread",
               "cover_area_multiplier", cover_area_multiplier);
  DCHECK(IsImplThread());
  host_impl_->SetTileCoverAreaMultiplier(cover_area_multiplier);
}

void ProxyImpl::SetClearTilesOnHideOnImpl(bool enable) {
  TRACE_EVENT1("cc", "ProxyImpl::SetClearTilesOnHideOnImplThread", "enable",
               enable);
  DCHECK(IsImplThread());
  host_impl_->SetClearTilesOnHide(enable);
}
#endif
