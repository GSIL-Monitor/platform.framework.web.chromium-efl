...
void Scheduler::BeginImplFrameWithDeadline(const viz::BeginFrameArgs& args) {
  ...
  if (ShouldRecoverMainLatency(adjusted_args, can_activate_before_deadline)) {
    ...
    // It may skip BeginImplFrameDeadline, so that it causes frame drop.
    // Thus, commenting out below part.
/*
...
void Scheduler::ScheduleBeginImplFrameDeadline() {
  ...
  if (client_is_single_thread_proxy_ && state_machine_.visible() &&
      begin_impl_frame_deadline_mode_ ==
          SchedulerStateMachine::BEGIN_IMPL_FRAME_DEADLINE_MODE_IMMEDIATE) {
...
void Scheduler::SetDeferCommits(bool defer_commits) {
  TRACE_EVENT1("cc", "Scheduler::SetDeferCommits", "defer_commits",
               defer_commits);
  LOG(INFO) << "LoaderLog  SetDeferCommits  defer_commits = " << defer_commits;
