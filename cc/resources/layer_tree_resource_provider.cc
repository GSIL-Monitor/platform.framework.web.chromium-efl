...
viz::ResourceId LayerTreeResourceProvider::CreateResourceFromExternalTexture(
  ...
  Resource* resource = InsertResource(
      id, Resource(texture_id, gfx::Size(), Resource::EXTERNAL, texture_target,
                   GL_LINEAR, TEXTURE_HINT_DEFAULT, RESOURCE_TYPE_GL_TEXTURE,
                   viz::RGBA_8888));

  resource->release_callback =
      base::Bind(&viz::SingleReleaseCallback::Run,
                 base::Owned(release_callback.release()));
  resource->tbm_video = true;
  resource->allocated = true;
  return id;
}
#endif
...
#if defined(OS_TIZEN)
    resource->last_used_frame_ack_index = current_frame_ack_count();
#endif
