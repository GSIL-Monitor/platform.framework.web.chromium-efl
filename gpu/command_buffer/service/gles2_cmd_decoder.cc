...
void GLES2DecoderImpl::DoBindTexture(GLenum target, GLuint client_id) {
  ...
  TTRACE_WEB("GLES2DecoderImpl::DoBindTexture");
#endif
...
void GLES2DecoderImpl::DoViewport(GLint x, GLint y, GLsizei width,
  ...
  if (base::CommandLine::ForCurrentProcess()->HasSwitch(
    ...
    state_.viewport_width = std::min(surface_size.width(), viewport_max_width_);
    state_.viewport_height =
        std::min(surface_size.height(), viewport_max_height_);
    glViewport(x, y, state_.viewport_width, state_.viewport_height);
  } else {
...
error::Error GLES2DecoderImpl::HandleTexImage2D(uint32_t immediate_data_size,
  ...
  if (base::CommandLine::ForCurrentProcess()->HasSwitch(
    ...
    bool rgb_emulation = (internal_format == GL_RGB) && (alpha_bits > 0);

    if (rgb_emulation) {
      back_buffer_color_format_ = internal_format;
...
void GLES2DecoderImpl::DoCopyTexImage2D(
    ...
    GLint border) {
#if defined(USE_TTRACE)
  TTRACE_WEB("GLES2DecoderImpl::DoCopyTexImage2D");
#endif
...
void GLES2DecoderImpl::DoCopyTexSubImage2D(
    ...
    GLsizei height) {
#if defined(USE_TTRACE)
  TTRACE_WEB("GLES2DecoderImpl::DoCopyTexSubImage2D");
#endif
...
#if defined(USE_TTRACE)
  TTRACE_WEB("GLES2DecoderImpl::DoCopyTextureCHROMIUM");
#endif
...
#if defined(USE_TTRACE)
  TTRACE_WEB("GLES2DecoderImpl::DoBindTexImage2DCHROMIUM");
#endif
