...
void ContextState::InitState(const ContextState* prev_state) const {
  if (prev_state) {
    ...
    glPixelStorei(GL_UNPACK_ALIGNMENT, unpack_alignment);
    glPolygonOffset(polygon_offset_factor, polygon_offset_units);
    if (!GetEnabled(GL_POLYGON_OFFSET_FILL) &&
        feature_info_->workarounds().use_virtualized_gl_contexts) {
      // glPolygonOffset throws GL_INVALID_FRAMEBUFFER_OPERATION
      // on some mali drivers when called before a texture is attached
      // to FBO. This seems specific to cases when use_virtualized_gl_contexts
      // driver workaround is enabled. As it causes TCT failures, the error
      // is supressed temporarily.
      glGetError();
    }
    glSampleCoverage(sample_coverage_value, sample_coverage_invert);
    glScissor(scissor_x, scissor_y, scissor_width, scissor_height);
