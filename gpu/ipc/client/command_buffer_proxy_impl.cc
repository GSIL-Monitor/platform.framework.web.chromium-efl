...
CommandBufferProxyImpl::CommandBufferProxyImpl(int channel_id,
    ...
    : command_buffer_id_(CommandBufferProxyID(channel_id, route_id)),
      ...
      use_direct_canvas_(base::CommandLine::ForCurrentProcess()->HasSwitch(
...
#if defined(DIRECT_CANVAS)
  init_params.context_type = context_type;
#endif
...
#if defined(DIRECT_CANVAS)
  bool sent = false;
  if (use_direct_canvas_ &&
      (config.context_type ==
       ui::command_buffer_metrics::OFFSCREEN_CONTEXT_FOR_WEBGL))
    sent = channel->Send(new GpuChannelMsg_CreateDirectCanvasCommandBuffer(
...
#if defined(DIRECT_CANVAS)
  context_type_ = config.context_type;
#endif
...
gpu::CommandBuffer::State CommandBufferProxyImpl::WaitForGetOffsetInRange(
  ...
  base::AutoLock lock(last_state_lock_);
#if defined(USE_TTRACE)
  TTRACE(TTRACE_TAG_WEB, "CommandBufferProxyImpl::WaitForGetOffsetInRange");
#endif
...
int32_t CommandBufferProxyImpl::CreateEGLImage(
  ...
  params.internal_format = internalformat;

  gfx::GpuMemoryBufferHandle handle;
  handle.type = gfx::NATIVE_PIXMAP;
  handle.tbm_surface = reinterpret_cast<intptr_t>(buffer_handle.tbm_surface);
  handle.media_packet = reinterpret_cast<intptr_t>(buffer_handle.media_packet);
  params.gpu_memory_buffer = handle;
...
void CommandBufferProxyImpl::DestroyEGLImage(int32_t image_id) {
  ...
  Send(new GpuCommandBufferMsg_DestroyImage(route_id_, image_id));
}
#endif
...
#if defined(USE_TTRACE)
  TTRACE(TTRACE_TAG_WEB, "CommandBufferProxyImpl::Send");
#endif
...
void CommandBufferProxyImpl::DisconnectChannel() {
  ...
  if (use_direct_canvas_ &&
      (context_type_ ==
       ui::command_buffer_metrics::OFFSCREEN_CONTEXT_FOR_WEBGL))
    channel_->Send(
        new GpuChannelMsg_DestroyDirectCanvasCommandBuffer(route_id_));
  else
#endif
