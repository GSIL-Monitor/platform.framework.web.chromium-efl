...
class InProcessGpuServiceHolder {
 public:
  InProcessGpuServiceHolder() : direct_canvas_enabled(false) {}
  ~InProcessGpuServiceHolder() {}

  void SetGpuThreadService(gpu::gles2::MailboxManager* mailbox_manager,
                           scoped_refptr<gl::GLShareGroup> share_group) {
    gpu_thread_service_ = new gpu::DirectCanvasInProcessThreadService(
        new gpu::SyncPointManager(), mailbox_manager, share_group,
        gpu_feature_info_);
  }
  ...
  const scoped_refptr<base::SingleThreadTaskRunner>& GetMainThreadTaskRunner() {
    return task_runner_;
  }

  void SetDirectCanvasEnabled() { direct_canvas_enabled = true; }
  bool GetDirectCanvasEnabled() { return direct_canvas_enabled; }

  void Release() {
    gpu_thread_service_ = nullptr;
 ...
 private:
  ...
  bool direct_canvas_enabled;
};

base::LazyInstance<InProcessGpuServiceHolder>::DestructorAtExit g_gpu_service =
    LAZY_INSTANCE_INITIALIZER;

scoped_refptr<InProcessCommandBuffer::Service> GetGpuService() {
  return g_gpu_service.Get().GetGpuThreadService();
}

void InProcessGpuService::SetGpuService(
    gpu::gles2::MailboxManager* mailbox_manager,
    scoped_refptr<gl::GLShareGroup> share_group) {
  base::ThreadTaskRunnerHandle::IsSet();
  return g_gpu_service.Get().SetGpuThreadService(mailbox_manager, share_group);
}

void InProcessGpuService::SetMainThread(
    scoped_refptr<base::SingleThreadTaskRunner> task_runner) {
  g_gpu_service.Get().SetMainThreadTaskRunner(task_runner);
}

const scoped_refptr<base::SingleThreadTaskRunner>&
InProcessGpuService::GetMainThread() {
  return g_gpu_service.Get().GetMainThreadTaskRunner();
}

void InProcessGpuService::SetDirectCanvasEnabled() {
  g_gpu_service.Get().SetDirectCanvasEnabled();
}

bool InProcessGpuService::GetDirectCanvasEnabled() {
  return g_gpu_service.Get().GetDirectCanvasEnabled();
}

void InProcessGpuService::ReleaseGpuService() {
  return g_gpu_service.Get().Release();
}
#endif
namespace {
...
#else
  surface_ = GLSharedEvasGL::GetSurface();
#endif
...
#else
    context_ = GLSharedEvasGL::GetContext();
#endif
    if (context_.get()) {
      service_->gpu_feature_info().ApplyToGLContext(context_.get());
    }
...
int32_t InProcessCommandBuffer::CreateEGLImage(
...
void InProcessCommandBuffer::DestroyEGLImage(int32_t image_id) {
  NOTREACHED();
}
#endif
