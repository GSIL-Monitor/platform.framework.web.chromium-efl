...
class UI_TOUCH_SELECTION_EXPORT TouchSelectionController
 ...
 public:
  ...
  virtual bool WillHandleTouchEvent(const MotionEvent& event);
  ...
  virtual void HandleLongPressEvent(base::TimeTicks event_time,
  ...
  virtual bool Animate(base::TimeTicks animate_time);
  ...
  virtual gfx::RectF GetRectBetweenBounds() const;
 ...
 protected:
  const gfx::SelectionBound& GetStartSelectionBound() const;
  const gfx::SelectionBound& GetEndSelectionBound() const;

  TouchHandleOrientation GetStartHandleOrientation() const;
  TouchHandleOrientation GetEndHandleOrientation() const;
  void SetStartHandleOrientation(TouchHandleOrientation orientation);
  void SetEndHandleOrientation(TouchHandleOrientation orientation);
  void RefreshHandleOrientation();

  const TouchHandle* GetStartSelectionHandle() const;
  const TouchHandle* GetEndSelectionHandle() const;
  ...
  void CreateStartSelectionHandle();
  void CreateEndSelectionHandle();
  virtual bool ShouldUpdateSelectionBounds(const gfx::SelectionBound& start,
                                           const gfx::SelectionBound& end);

  // TODO(avi) : Find a way to keep this under private
  // Bug: http://web.sec.samsung.com/bugzilla/show_bug.cgi?id=10785
  virtual void OnSelectionChanged();
  virtual void DeactivateSelection();

  // Returns true if selection mode was newly (re)activated.
  virtual bool ActivateSelectionIfNecessary();
 ...
 protected:
  ...
  InputEventType GetResponsePendingInputEvent() {
    return response_pending_input_event_;
  }
 ...
 private:
  ...
  void OnHandleDragEnd(const TouchHandle& handle);
 ...
 protected:
  virtual bool GetStartVisible() const;
  virtual bool GetEndVisible() const;
