...
class UI_TOUCH_SELECTION_EXPORT TouchHandleDrawable {
 public:
  ...
  virtual void SetScale(float alpha) = 0;
  virtual void SetOriginForScale() = 0;
#endif
...
class UI_TOUCH_SELECTION_EXPORT TouchHandle : public TouchSelectionDraggable {
...
#if defined(S_TERRACE_SUPPORT)
  // Store focus points for the handles. The handle will be positioned
  // while finger drag at the selection rect, either |top| or |bottom|
  // based on the mirror parameters.
  void StoreFocus(const gfx::PointF& top, const gfx::PointF& bottom);
  void PositionUpdatedHandle();
  void SetDeferredOrientation();
  void RemoveImmediate();
  bool IsHideAnimationInProgress();
  bool is_dragging() const { return is_dragging_; }
  void StartHandleZoomIn();
  void StartHandleZoomOut();
  void BeginAnimateAndSetFocus(const gfx::PointF& top,
 ...
 private:
  ...
  void EndScale();
  void SetOriginForScale();
  void BeginAnimationOnHide();
  void EndHideAnimation();
  bool AnimateOnPath(base::TimeTicks frame_time);
  void CalculateHandleAnimationPath();
  void EndAnimationOnPath();
#endif
...
#if defined(S_TERRACE_SUPPORT)
  float scale_;
  base::TimeTicks scale_end_time_;
  base::TimeTicks scale_bounce_end_time_;
  base::TimeTicks drag_animation_end_time_;
  bool is_hide_animation_in_progress_;
  bool is_orientation_changed_;
  bool is_handle_moving_with_touch_;
  bool is_path_animation_in_progress_;
  gfx::PointF final_top_;
  gfx::PointF final_bottom_;
  gfx::RectF touch_down_drawable_bounds_;
  gfx::PointF bound_focus_top_;
  gfx::PointF bound_focus_bottom_;
  gfx::PointF last_touch_move_position_;
  float x_speed_top_bound_;
  float x_speed_bottom_bound_;
  float y_speed_bottom_bound_;
  float y_speed_top_bound_;
  enum HandleZoomState {
    TOUCH_HANDLE_STATE_UNDEFINED, // Initial state (Scale 0-1).
    TOUCH_HANDLE_STATE_ZOOM_IN, // handle in zoom-in animation.
    TOUCH_HANDLE_STATE_ZOOM_OUT // handle in zoom-out & path animation.
  };
  int touch_handle_state_;
#endif
