...
InputHandlerProxy::EventDisposition InputHandlerProxy::HandleGestureScrollBegin(
  ...
  switch (scroll_status.thread) {
    ...
    case cc::InputHandler::SCROLL_UNKNOWN:
    case cc::InputHandler::SCROLL_ON_MAIN_THREAD:
      LOG(WARNING) << "SBRTOUCHLOG  HandleGestureScrollBegin scroll status"
      << " thread = " << scroll_status.thread << " main thread scrolling"
      << " reason = " << scroll_status.main_thread_scrolling_reasons;
      result = DID_NOT_HANDLE;
      break;
    case cc::InputHandler::SCROLL_IGNORED:
  ...
  LOG(WARNING) << "SBRTOUCHLOG  HandleGestureScrollBegin  result = " << result;
  ...
  for (size_t i = 0; i < touch_event.touches_length; ++i) {
    ...
    if (event_listener_type !=
      ...
      *is_touching_scrolling_layer =
      ...
      LOG(WARNING) << "SBRTOUCHLOG : TouchStart - Forwarding to render main";
      result = DID_NOT_HANDLE;
      break;
    }
  }
  LOG(WARNING) << "SBRTOUCHLOG : TouchStart";
  ...
  switch (gesture_event.GetType()) {
    ...
    case WebInputEvent::kGestureScrollUpdate: {
      ...
      if (ShouldSuppressScrollForFlingBoosting(current_fling_velocity_,
        ...
        // Handle scroll and boosted fling at the same time.
        return false;
#else
        return true;
#endif
...
#if defined(S_TERRACE_SUPPORT)
void InputHandlerProxy::NotifyScrollbarState(bool disable, bool visible) {
  client_->NotifyScrollbarState(disable, visible);
}
#endif
