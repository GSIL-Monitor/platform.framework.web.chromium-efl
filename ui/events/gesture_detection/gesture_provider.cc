...
// Approaching speed to current touch point.
const float kScrollCompensationSpeed = 0.1f;

// Samsung - Compensation scroll
// Scroll count for the enhanced compensation scroll
// Twice touch slop distances are added to total scroll distance
// from 3rd Scroll to 5th Scroll
const int kEnhancedScrollCompensationCount = 2;
const int kMaxScrollCompensationValue = 30;

// Maximum count of the compensation scroll
// After 5th scroll, compensation scroll is not applied any more
const int kScrollCompensationMaxCount = 5;
const int kAndroidDefaultTouchSlop = 8;
...
GestureProvider::Config::Config()
  ...
  if (command_line.HasSwitch(switches::kFlingVelocityMultiplier)) {
    std::string string_value =
        command_line.GetSwitchValueASCII(switches::kFlingVelocityMultiplier);
    base::StringToDouble(string_value, &multiplier);
  }
  fling_velocity_multiplier = static_cast<float>(multiplier);
}
...
class GestureProvider::GestureListenerImpl : public ScaleGestureListener,
 ...
 public:
  GestureListenerImpl(const GestureProvider::Config& config,
    ...
    gesture_detector_.SetDefaultFlingMultiplier(
        config_.fling_velocity_multiplier);
  }
  ...
  void OnTouchEvent(const MotionEvent& event) {
    ...
    if (IsDoubleTapEnabled() && is_pointer_down_) {
  ...
  bool OnScale(const ScaleGestureDetector& detector,
               const MotionEvent& e) override {
    TRACE_EVENT1("input",
                 "GestureProvider::GestureListenerImpl::OnScale",
                 "action",
                 GetMotionEventActionName(e.GetAction()));
  ...
  bool OnScroll(const MotionEvent& e1,
    ...
    TRACE_EVENT2("input",
                 "GestureProvider::GestureListenerImpl::OnScroll",
                 "action1",
                 GetMotionEventActionName(e1.GetAction()),
                 "action2",
                 GetMotionEventActionName(e2.GetAction()));
    ...
    if (!scroll_event_sent_ && e2.GetPointerCount() < 3) {
      ...
      // Samsung - Compensation scroll
      compensation_x_ = distance_x - delta.x();
      compensation_y_ = distance_y - delta.y();
      compensation_x_in_this_round_ = compensation_x_;
      compensation_y_in_this_round_ = compensation_y_;
      distance_x = delta.x();
      distance_y = delta.y();
    }
    ...
    // Samsung - Compensation scroll
    AdjustScrollDistance(&distance_x, &compensation_x_, &last_distance_x_,
                                      &compensation_x_in_this_round_);
    AdjustScrollDistance(&distance_y, &compensation_y_, &last_distance_y_,
                                      &compensation_y_in_this_round_);
  ...
  bool OnFling(const MotionEvent& e1,
    ...
    TRACE_EVENT2("input",
                 "GestureProvider::GestureListenerImpl::OnFling",
                 "action1",
                 GetMotionEventActionName(e1.GetAction()),
                 "action2",
                 GetMotionEventActionName(e2.GetAction()));
    ...
    velocity_x *= config_.fling_velocity_multiplier;
    velocity_y *= config_.fling_velocity_multiplier;
  ...
  bool OnSwipe(const MotionEvent& e1,
               ...
               float velocity_y) override {
    TRACE_EVENT2("input",
                 "GestureProvider::GestureListenerImpl::OnSwipe",
                 "action1",
                 GetMotionEventActionName(e1.GetAction()),
                 "action2",
                 GetMotionEventActionName(e2.GetAction()));
  ...
  bool OnDoubleTap(const MotionEvent& e) override {
    TRACE_EVENT1("input",
                 "GestureProvider::GestureListenerImpl::OnDoubleTap",
                 "action",
                 GetMotionEventActionName(e.GetAction()));
  ...
  bool OnDoubleTapEvent(const MotionEvent& e) override {
    switch (e.GetAction()) {
      case MotionEvent::ACTION_DOWN:
        ...
        // SAMSUNG: DoubleTap Perf
        // Do double tap zooming on action down, to run it faster.
...
#if defined(S_TERRACE_SUPPORT)
          // SAMSUNG: DoubleTap Perf
          // Disable anchored scaling. Samsung does not support it from 4.x.
          scale_gesture_detector_.ResetScaleWithSpan(0);
#endif
          return true;
        }
        break;
 ...
 private:
  ...
  // Samsung - Compensation scroll
  void AdjustScrollDistance(float* distance, float* compensation,
                            float* last_distance,
                            float* compensation_in_this_round) {
    if (*distance != 0.f && *compensation_in_this_round != 0.f) {
      float distance_value = *distance;
      float last_distance_value = *last_distance;
      if (last_distance_value * distance_value < 0.f) {
        compensation_count_ ++;
        if (*compensation == 0.f &&
            compensation_count_ <= kScrollCompensationMaxCount) {
          *compensation = *compensation_in_this_round *= -1;
          // For scroll initial response, touch_slop is changed
          // to android default one.
          // The amount of scroll compensation becomes smaller than before,
          // To fix the stickness issue, it needs to adjust the compensation.
          int touch_slop = config_.gesture_detector_config.touch_slop;
          if (touch_slop == kAndroidDefaultTouchSlop) {
            if (compensation_count_ > kEnhancedScrollCompensationCount) {
              if (compensation_count_ == kScrollCompensationMaxCount) {
                *compensation += *compensation + *compensation;
              } else {
                *compensation += *compensation;
              }
            }
            if (*compensation > kMaxScrollCompensationValue) {
              *compensation = kMaxScrollCompensationValue;
            } else if (*compensation < -kMaxScrollCompensationValue) {
              *compensation = -kMaxScrollCompensationValue;
            }
          } else {
            if (compensation_count_ > kEnhancedScrollCompensationCount) {
          ...
          *compensation = 0.f;
          *compensation_in_this_round = 0;
        }
      } else if (*compensation != 0.f) {
        float compensation_value = CalculateCompensationValue(*distance);
        if (compensation_count_ > kEnhancedScrollCompensationCount)
          compensation_value += compensation_value;
        if (compensation_value > std::abs(*compensation)) {
          *distance += *compensation;
          *compensation = 0.f;
        } else {
          *distance +=
            (*distance > 0.f ? compensation_value : -compensation_value);
          std::string device_name = base::SysInfo::HardwareModelName();
          if (device_name.find("SM-A800") != std::string::npos)
            *distance += (*distance > 0.f ? 2 * compensation_value
                                          : -2 * compensation_value);
          *compensation +=
            (*distance < 0.f ? compensation_value : -compensation_value);
        }
      }
  ...
  float CalculateCompensationValue(float distance) {
    return std::abs(distance) * kScrollCompensationSpeed;
  }

  void ResetCompensationScroll() {
    compensation_count_ = 1;
    compensation_x_ = 0.f;
    compensation_y_ = 0.f;
    compensation_x_in_this_round_ = 0.f;
    compensation_y_in_this_round_ = 0.f;
    last_distance_x_ = 0.f;
    last_distance_y_ = 0.f;
  }

  int compensation_count_;
  float compensation_x_;
  float compensation_y_;
  float compensation_x_in_this_round_;
  float compensation_y_in_this_round_;
  float last_distance_x_;
  float last_distance_y_;
...
bool GestureProvider::OnTouchEvent(const MotionEvent& event) {
  ...
  if (!CanHandle(event)) {
    LOG(WARNING) << "SBRTOUCHLOG !CanHandle(event)"
              << " current_down_event() = " << current_down_event()
