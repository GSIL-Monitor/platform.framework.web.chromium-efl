...
#define EGL_SEC_CACHEABLE_TEXTURE      0x3FFE
...
bool GLContextEGL::Initialize(GLSurface* compatible_surface,
  ...
  if (base::CommandLine::ForCurrentProcess()->HasSwitch(
          switches::kUseCacheableTextureExtension)) {
    isExynos = true;
  }

  const EGLint kContextAttributes_exynos[] = {
    EGL_CONTEXT_CLIENT_VERSION, context_client_major_version,
    EGL_SEC_CACHEABLE_TEXTURE, EGL_TRUE,
    EGL_NONE
  };
  const EGLint kContextRobustnessAttributes_exynos[] = {
    EGL_CONTEXT_CLIENT_VERSION, context_client_major_version,
  ...
  const EGLint* context_attributes_exynos = nullptr;
  ...
  if (GLSurfaceEGL::IsCreateContextRobustnessSupported()) {
    ...
    context_attributes_exynos = kContextRobustnessAttributes_exynos;
  } else {
    ...
    context_attributes_exynos = kContextAttributes_exynos;
  }
  ...
  if (isExynos) {
    context_ = eglCreateContext(
        ...
        context_attributes_exynos);

    if (context_) {
      DVLOG(1) << "eglCreateContext created for exynos";
      return true;
    }

    EGLint error = eglGetError();
    // eglCreateContext is called with normal attrib list
    // when error is EGL_BAD_ATTRIBUTE
    //(that is, EGL_SEC_CACHEABLE_TEXTURE doesn't exist)
    if (EGL_BAD_ATTRIBUTE != error) {
      LOG(ERROR) << "eglCreateContext failed with error for exynos"
                 << GetLastEGLErrorString();
      return false;
    }
  }
