...
bool GLSurfaceEGL::InitializeOneOff(EGLNativeDisplayType native_display) {
...
#if defined(OS_TIZEN)
  // There is no api to set this extension for evas context. To match shared
  // context property, don't use "EGL_EXT_create_context_robustness".
  ...
  if (g_egl_surfaceless_context_supported) {
    // This is used to restore a context that is not created or owned by
    // Chromium native code, but created by Tizen platform itself.
#if defined(USE_EFL)
