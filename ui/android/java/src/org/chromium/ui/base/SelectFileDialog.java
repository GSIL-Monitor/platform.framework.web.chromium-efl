...
public class SelectFileDialog implements WindowAndroid.IntentCallback,
    ...
    private static final String APP_MEDIA_URI = "app://media";
    ...
    /**
     * Delegate interface for getting DLPManager functionality.
     */
    public interface DLPManagerDelegate {
        /**
         * Checks whether the given list of files is allowed to be uploaded in the given domain.
         * @param uri Array of URI for files to be uploaded.
         * @param domainName The domain where the files are being uploaded.
         * @return array of boolean values implying whether the file is allowed for upload.
         */
        boolean[] isFileUploadAllowed(Uri[] uri, String domainName);
    }

    private static DLPManagerDelegate sDLPManagerDelegate;

    /**
     * Set the delegate object for DLPManager functionality.
     * @param delegate object
     */
    public static void setDLPManagerDelegate(DLPManagerDelegate delegate) {
        sDLPManagerDelegate = delegate;
    }

    private boolean[] isUploadAllowed(Uri[] uris) {
        boolean[] allowed = new boolean[uris.length];
        Arrays.fill(allowed, true);
        if (sDLPManagerDelegate == null) return allowed;
        return sDLPManagerDelegate.isFileUploadAllowed(uris, mRequestorUrl);
    }

    private String mRequestorUrl;

    @CalledByNative
    private void setRequestorUrl(String requestorUrl) {
    ...
    private void launchSelectFileWithCameraIntent(boolean hasCameraPermission, Intent camera) {
        ...
        /**
         * In cases where the file URI is in browser sandbox, Knox DLP APIs are
         * not able to get the file's properties to verify if it is DLP file or
         * not. Proposed solution is to block the intent to launch such apps in
         * the first place.
         */
        Uri[] appMediaUri = {Uri.parse(APP_MEDIA_URI)};
        boolean isAllowed[] = isUploadAllowed(appMediaUri);
        ...
        if (isAllowed[0]) {
            if (captureCamera() && camera != null) {
                ...
                if (mWindowAndroid.showIntent(soundRecorder, this, R.string.low_memory_error)) {
                    return;
                }
            }
        }
        ...
        if (extraIntents.isEmpty()) {
            ...
            // Add Myfiles extraIntents
            extraIntents.add(new Intent("com.sec.android.app.myfiles.PICK_DATA"));
        }
    ...
    public void onIntentCompleted(WindowAndroid window, int resultCode, Intent results) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR2
            ...
            Uri[] allowedFilePathArray = filterFilesForUpload(filePathArray);
            if (allowedFilePathArray == null) {
                Log.w(TAG, "Files selected are not allowed to be uploaded.");
                onFileNotSelected();
                return;
            }
            ...
            GetDisplayNameTask task =
                    new GetDisplayNameTask(ContextUtils.getApplicationContext(), true);
            task.executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR, allowedFilePathArray);
            return;
        }

        Uri[] filePathArray = {results.getData()};
        if (ContentResolver.SCHEME_FILE.equals(filePathArray[0].getScheme())) {
            filePathArray = filterFilesForUpload(filePathArray);
            if (filePathArray == null) {
                onFileNotSelected();
                return;
            }
            onFileSelected(mNativeSelectFileDialog, filePathArray[0].getSchemeSpecificPart(), "");
            return;
        }
    ...
    private Uri[] filterFilesForUpload(final Uri[] filePathArray) {
        final int numUris = filePathArray.length;
        int numAllowedFiles = 0;
        boolean[] isAllowed = isUploadAllowed(filePathArray);
        for (int k = 0; k < numUris; k++) {
            if (isAllowed[k]) numAllowedFiles++;
        }

        if (numAllowedFiles == 0) {
            return null;
        }

        Uri[] allowedFilePathArray = new Uri[numAllowedFiles];
        int j = 0;
        for (int i = 0; i < numUris; i++) {
            if (isAllowed[i]) {
                allowedFilePathArray[j++] = filePathArray[i];
            }
        }

        return allowedFilePathArray;
    }
