...
WindowAndroid::~WindowAndroid() {
  ...
  LOG(WARNING) << "TAB-STATUS window destroyed";
#endif
}
...
void WindowAndroid::AddObserver(WindowAndroidObserver* observer) {
  ...
  if (!observer_list_.HasObserver(observer)) {
    ...
    LOG(WARNING) << "TAB-STATUS observer added";
  } else {
    LOG(WARNING) << "TAB-STATUS same observer detected";
  }
#else
  if (!observer_list_.HasObserver(observer))
    observer_list_.AddObserver(observer);
...
#if defined(S_TERRACE_SUPPORT)
  LOG(WARNING) << "TAB-STATUS observer removed";
#endif
}
