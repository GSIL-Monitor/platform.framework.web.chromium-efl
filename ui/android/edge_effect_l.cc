...
const float kEdgeHeightAdjustment = 0.37f;
...
const SkScalar kDefaultIntensityAdjust = 1.2f;
}  // namespace
...
EdgeEffectL::EdgeEffectL(ui::ResourceManager* resource_manager)
    : resource_manager_(resource_manager),
      ...
      decay_clear_(false),
#endif
      glow_intensity_adjust_(kDefaultIntensityAdjust) {
...
#if defined(S_TERRACE_SUPPORT)
  if (state_ == STATE_RECEDE || state_ == STATE_PULL_DECAY) {
    return;
  }
#else
...
#if defined(S_TERRACE_SUPPORT)
  // Set the start time only when EdgeEffectL::Pull called in other state.
  if(state_ != STATE_PULL) {
...
#if defined(S_TERRACE_SUPPORT)
  // Call Finish if it is in STATE_PULL_DECAY.
  // Otherwise set flag to call Finish immediately.
  if (state_ == STATE_PULL_DECAY)
    Finish();
  else
    decay_clear_ = true;
#else
...
bool EdgeEffectL::Update(base::TimeTicks current_time) {
  ...
  if (t >= 1.f - kEpsilon) {
    switch (state_) {
      ...
      case STATE_PULL:
#if defined(S_TERRACE_SUPPORT)
        // Original: PULL -> PULL_DECAY -> RECEDE
        // Modified: PULL -> RECEDE -> PULL_DECAY
        state_ = STATE_RECEDE;
        start_time_ = current_time;
#else
      ...
      case STATE_RECEDE:
#if defined(S_TERRACE_SUPPORT)
        // Call Finish immediately if the flag is true.
        // Otherwise set state as STATE_PULL_DECAY.
        if (decay_clear_)
          Finish();
        else
          state_ = STATE_PULL_DECAY;
...
void EdgeEffectL::ApplyToLayers(Edge edge,
  ...
  const float h = (r - y) * kEdgeHeightAdjustment;
  const float o_r = size.height() * 0.75f / kSin;
  const float o_y = kCos * o_r;
  const float o_h = (o_r - o_y) * kEdgeHeightAdjustment;
...
void EdgeEffectL::SetEdgeEffectProperties(
  const SkColor theme_color,
  const SkScalar edge_intensity_factor) {
  glow_intensity_adjust_ = edge_intensity_factor;
}
