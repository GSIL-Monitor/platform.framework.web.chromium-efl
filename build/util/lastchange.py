...
def FetchGitRevision(directory, upstream):
  """
  ...
  git_args = ['log', '-1', '--format=%H',
              '--author=chrome-tpm@google.com' if upstream else ""]
...
def FetchVersionInfo(directory=None,
  ...
  version_info = FetchGitRevision(directory, upstream)
  if not version_info:
...
def main(argv=None):
  ...
  parser.add_option("--upstream", action="store_true",
                    help="use an author(chrome-tpm@google.com) in git log " +
                    "for getting upstream revision")
