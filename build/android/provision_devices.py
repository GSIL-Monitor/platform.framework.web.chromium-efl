...
from pylib import filtered_devices
...
def DisablePredictiveKeyboard(device):
  automator_jar = "samsung_scripts/TextPredictorDisabler.jar"
  host_path = os.path.abspath(os.path.join(os.path.dirname(__file__),
                                           automator_jar))
  device_path = "/data/local/tmp/TextPredictorDisabler.jar"
  jar_contents = open(host_path, "rb").read()
  device.WriteFile(device_path, jar_contents)
  cmd = "uiautomator runtest TextPredictorDisabler.jar " + \
        "-c com.samsung.partner.reaktor.TextPredictorDisabler"
...
def ProvisionDevices(args):
  ...
  devices = filtered_devices.FilterAttachedDevices(devices)\
            if args.quickbuild else devices
  if not devices:
...
def main():
  ...
  parser.add_argument('--quickbuild', action='store_true',
                      help='If set, this script will run on quickbuild.')
  ...
  parser.add_argument('--force-battery-status', action='store_true',
                    default=False, help="Force battery status")
  parser.add_argument('--disable-selinux', action='store_true',
                    default=False, help="Disable SELinux")
  ...
  parser.add_argument('--disable-predictive-keyboard', action='store_true', default=False,
                    help="Disables Samsung predictive keyboard")
