...
from pylib.local.device.emulator import reboot_emulator
from pylib.local.device.emulator import is_emualtor
from pylib.local.device.test_crash_logger import print_crash_dump
from pylib.local.device.test_crash_logger import print_unknown_error
...
class LocalDeviceTestRun(test_run.TestRun):
  ...
  def __init__(self, env, test_instance):
    ...
    self._tools = {}
    self._quickbuild = env._quickbuild
  ...
  def RunTests(self):
          ...
          try:
            if self._ShouldShard():
              ...
              self._env.parallel_devices.pMap(
                  run_tests_on_device, tc, try_results, self._quickbuild).pGet(None)
            else:
              self._env.parallel_devices.pMap(
                  run_tests_on_device, tests, try_results, self._quickbuild).pGet(None)
    ...
    print_crash_dump(self._env.devices, self._quickbuild)
    return results
