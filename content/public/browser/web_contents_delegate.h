...
class CONTENT_EXPORT WebContentsDelegate {
 public:
  ...
  // Called when the renderer puts a tab out of video element's fullscreen mode.
  virtual void SetFullscreenModeForVideo(WebContents*, bool enter) {}
#endif
  ...
  // |user_gesture| is true if the registration is made in the context of a user
  // gesture.
  virtual void RegisterContentHandler(WebContents* web_contents,
                                      const std::string& mime_type,
                                      const GURL& url,
                                      bool user_gesture) {}

  // Unregister the registered handler for URL requests with the given
  // mime_type. |user_gesture| is true if the registration is made in the
  // context of a user gesture.
  virtual void UnregisterContentHandler(WebContents* web_contents,
                                        const std::string& mime_type,
                                        const GURL& url,
                                        bool user_gesture) {}
  ...
  virtual void DidRenderRotatedFrame(int rotation,
                                     int frame_data_output_rect_width) {}
  virtual void QueryNumberFieldAttributes() {}
  virtual void WillImePanelShow() {}
  virtual void DidTopControlsMove(int offset) {}
#endif
#if defined(TIZEN_TBM_SUPPORT)
  virtual bool OffscreenRenderingEnabled() { return false; }
  virtual void SetOffscreenRendering(bool enable) {}
...
#if defined(S_TERRACE_SUPPORT)
  virtual void OpenWebLinkMatchInfosReply(WebContents* web_contents,
                                          int version,
  ...
  virtual void PageSavedAs(const base::FilePath::StringType& file_name) {}
  virtual void ReceivedBitmapFromCache(const SkBitmap& bitmap,
                                       const int request_id) {}
  virtual void PopulateSoftwareBitmapFinished(const SkBitmap& bitmap,
                                              const int request_id) {}
  virtual void OnRecognizeArticleResult(bool is_article,
                                const base::string16& page_url) {}

  virtual void OnNumberOfBlockedElements(int count) {}
...
#if defined(S_TERRACE_SUPPORT)
   virtual void ReceivedCandidateImage(const SkBitmap& bitmap) {}
  ...
  virtual const std::string& GetActiveDRM() {
