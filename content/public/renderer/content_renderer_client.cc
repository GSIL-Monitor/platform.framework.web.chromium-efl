...
bool ContentRendererClient::ShouldUseMediaPlayerForURL(const GURL& url) {
  ...
  // Terrace must use legacy webmediaplayer_android
  // instead of webmediaplayerimpl.
  return true;
#else
  return false;
#endif
...
#if defined(OS_TIZEN_TV_PRODUCT)
bool ContentRendererClient::GetFileDecryptedDataBuffer(
    const GURL& url,
...
#if defined(USE_EFL)
bool ContentRendererClient::GetWrtParsedUrl(const GURL& url, GURL& parsed_url) {
  return false;
}
#endif
