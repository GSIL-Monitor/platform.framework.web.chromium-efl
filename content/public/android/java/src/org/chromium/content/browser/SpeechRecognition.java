...
public class SpeechRecognition {
    ...
    // Remember if there is no service error.
    private boolean mIsNoServiceError;
    ...
    private void startRecognition(String language, boolean continuous, boolean interimResults) {
        ...
        // SAMSUNG KNOX - If there are no selected voice recognition services,
        // SpeechRecognizer.startListening will be returned without sending intent.
        // User doesn't know about that, so We should show the error pages.

        // FIXME: Commented the code during dev/m60_3112 rebaseline
        // It needs a re-visit by author.
        /*  String serviceComponent = Settings.Secure.getString(ContextUtils.getContentResolver(),
                "voice_recognition_service");
        if (TextUtils.isEmpty(serviceComponent)) {
            nativeOnRecognitionError(mNativeSpeechRecognizerImplAndroid,
                    SpeechRecognitionErrorCode.NOT_ALLOWED);
            mIsNoServiceError = true;
            return;
        }*/
    ...
    private void abortRecognition() {
        ...
        // SAMSUNG KNOX - There are some crash issues, when the mContext.unbindService
        // is called even there are no binded services.
        if (mIsNoServiceError) {
            mIsNoServiceError = false;
            return;
        }
