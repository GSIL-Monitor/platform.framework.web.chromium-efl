...
public class ContentViewRenderView extends FrameLayout {
    ...
    public ContentViewRenderView(Context context) {
        ...
        // Disable ZOrderMediaOverlay enabled by ContentViewRenderView,
        // because SBrowser uses TextureView for video instead of SurfaceView.
        // Can not placed to terrace since the API must be called before view is added.
    ...
    public void onNativeLibraryLoaded(WindowAndroid rootWindow) {
        ...
        mSurfaceCallback = registerSurfaceCallback();
        mSurfaceView.getHolder().addCallback(mSurfaceCallback);
        mSurfaceView.setVisibility(VISIBLE);
    }

    public SurfaceHolder.Callback registerSurfaceCallback() {
        return new SurfaceHolder.Callback() {
            @Override
            public void surfaceChanged(SurfaceHolder holder, int format, int width, int height) {
                notifySurfaceChange(holder, format, width, height);
                assert mNativeContentViewRenderView != 0;
    ...
    protected void notifySurfaceChange(SurfaceHolder holder, int format,
            int width, int height) {
    }

    protected void notifySurfaceCreation(SurfaceHolder holder) {
    }

    protected void notifySurfaceDestroy(SurfaceHolder holder) {
    }
    ...
    /**
     * SAMSUNG CHANGE
     * This is called on swap buffers completed on GPU side.
     */
    @CalledByNative
    protected void onSwapBuffersCompleted() {}

    protected native long nativeInit(long rootWindowNativePointer);
    private native void nativeDestroy(long nativeContentViewRenderView);
    protected native void nativeSetCurrentWebContents(
    ...
    protected native void nativeSurfaceCreated(long nativeContentViewRenderView);
    protected native void nativeSurfaceDestroyed(long nativeContentViewRenderView);
    protected native void nativeSurfaceChanged(long nativeContentViewRenderView,
