...
public class ContentViewCore implements AccessibilityStateChangeListener, DisplayAndroidObserver,
    ...
    // TODO (Ankit): Revert to private after adding custom implementation of ImeAdapter
    ...
    @SuppressWarnings("unused")
    @CalledByNative
    protected void onSingleTapEventAck(boolean consumed) {
        for (mGestureStateListenersIterator.rewind();
                mGestureStateListenersIterator.hasNext();) {
    ...
    @SuppressWarnings("unused")
    @CalledByNative
    protected boolean filterTapOrPressEvent(int type, int x, int y) {
        ...
        if (!focusPreOSKViewportRect.isEmpty()) {
            ...
            if (!rect.equals(focusPreOSKViewportRect)) {
                ...
                if (rect.width() == focusPreOSKViewportRect.width()) {
                    assert mWebContents != null;
                    if (mWebContents != null) mWebContents.scrollFocusedEditableNodeIntoView();
                }
                cancelRequestToScrollFocusedEditableNodeIntoView();
            }
        }
    ...
    protected void setTextHandlesTemporarilyHidden(boolean hide) {
        if (mNativeContentViewCore == 0) return;
    ...
    @SuppressWarnings("unused")
    @CalledByNative
    protected void updateFrameInfo(float scrollOffsetX, float scrollOffsetY, float pageScaleFactor,
        ...
        // We need to call onTopControlsChanged every time on frame info change

        // FIXME: Commented code during dev/m60_3112 rebaseline.
        // Changes need to be re-looked by author.
    ...
    @SuppressWarnings("unused")
    @CalledByNative
    protected void showSelectPopup(View anchorView, long nativeSelectPopupSourceFrame, String[] items,
    ...
    @SuppressWarnings("unused")
    // SAMSUNG UX : No haptic feedback needed for normal selection
    // @CalledByNative
    ...
    @SuppressWarnings("unused")
    @CalledByNative
    protected void onRenderProcessChange() {
    ...
    private void onNativeFlingStopped() {
        ...
        // TODO(mahesh.ma): Remove the below line completely if there is no regression.
        // This is a failsafe operation as mentioned above. But, it is resetting the scroll progress
        // for next scroll sequence (if next scroll is started before current fling ends), which is
        // calling Floating Action Mode to show even when scroll is still in progress.
        /*setTouchScrollInProgress(false);*/
    }
    ...
    protected native void nativeSetFocus(long nativeContentViewCore, boolean focused);
    ...
    protected native void nativeScrollBegin(long nativeContentViewCore, long timeMs, float x, float y,
            float hintX, float hintY, boolean targetViewport, boolean fromGamepad);

    protected native void nativeScrollEnd(long nativeContentViewCore, long timeMs);

    protected native void nativeScrollBy(
    ...
    protected native void nativeSelectPopupMenuItems(
    ...
    protected native void nativeWasResized(long nativeContentViewCore);
