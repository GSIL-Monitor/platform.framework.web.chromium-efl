...
#error "Unsupported Linux platform."
  ...
  // On Exynos, uploading texture takes too long in GPU Raster mode.
  // Therefore, we need to consider the gl_renderer is Mali* or not to
  // reduce texture upload time.
  const std::string renderer_mali = "Mali";

  if (!gpu_init->gpu_info().gl_renderer.compare(
    0, renderer_mali.length(), renderer_mali)) {
