...
#if defined(OS_TIZEN)
void RenderWidgetCompositor::SetTileCoverAreaMultiplier(
    float cover_area_multiplier) {
  layer_tree_host_->SetTileCoverAreaMultiplier(cover_area_multiplier);
}

void RenderWidgetCompositor::DidChangeLoadState(bool is_loading) {
  layer_tree_host_->DidChangeLoadState(is_loading);
}
#endif
...
void RenderWidgetCompositor::SetVisible(bool visible) {
  ...
  // Pepper plugin may stucked during hide & show sequence.
  // DidCommitAndDrawFrame will trigger a SwapBuffersACK(PP_OK)
  // if there is a pending swap.
  if (visible)
...
void RenderWidgetCompositor::RegisterSelection(
  ...
  layer_tree_host_->RegisterSelection(
      ...
      gfx::Rect(selection_rect.x, selection_rect.y, selection_rect.width,
                selection_rect.height));
}

void RenderWidgetCompositor::ClearSelection() {
  cc::LayerSelection empty_selection;
  layer_tree_host_->RegisterSelection(empty_selection, gfx::Rect());
}
...
void RenderWidgetCompositor::setFastScrollerEnabled(bool enabled) {
  layer_tree_host_->SetFastScrollerEnabled(enabled);
}
#endif
...
#if defined(OS_TIZEN_TV_PRODUCT)
  delegate_->DidInitializeRenderer();
#endif
...
#if defined(OS_TIZEN_TV_PRODUCT)
void RenderWidgetCompositor::SetLayerInverted(bool inverted) {
  layer_tree_host_->SetLayerInverted(inverted);
}
#endif

#if defined(OS_TIZEN)
void RenderWidgetCompositor::SetClearTilesOnHide(bool enable) {
  layer_tree_host_->SetClearTilesOnHide(enable);
}
#endif
