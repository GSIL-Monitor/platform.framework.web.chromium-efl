...
// Controls a number of shared memory segments that will be allocated for the
// purposes of host => resource IPC audio sample delivery.
constexpr int kSharedMemorySegments = 2;
...
void PepperPlatformAudioInput::OnStreamCreated(
  ...
  if (base::ThreadTaskRunnerHandle::Get().get() != main_task_runner_.get()) {
    ...
    if (client_) {
      client_->StreamCreated(handle, handle.GetSize(), kSharedMemorySegments,
                             socket_handle);
    } else {
...
void PepperPlatformAudioInput::InitializeOnIOThread(int session_id) {
  ...
  ipc_->CreateStream(this, session_id, params_, false, kSharedMemorySegments);
}
