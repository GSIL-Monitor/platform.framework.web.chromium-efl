...
PP_Resource ResourceCreationImpl::CreateMediaPlayer(
    PP_Instance instance,
    PP_MediaPlayerMode player_mode,
    PP_BindToInstanceMode bind_mode) {
  return 0;  // Not supported in-process.
}
PP_Resource ResourceCreationImpl::CreateURLDataSource(PP_Instance instance,
                                                      const char* url) {
  return 0;  // Not supported in-process.
}
PP_Resource ResourceCreationImpl::CreateESDataSource(PP_Instance instance) {
  return 0;  // Not supported in-process.
}
#endif  // defined(TIZEN_PEPPER_PLAYER)

#if defined(TIZEN_PEPPER_TEEC)
PP_Resource ResourceCreationImpl::CreateTEECContext(PP_Instance instance) {
  return 0;  // Not supported in-process.
}

PP_Resource ResourceCreationImpl::CreateTEECSession(PP_Instance instance,
                                                    PP_Resource context) {
  return 0;  // Not supported in-process.
}
PP_Resource ResourceCreationImpl::CreateTEECSharedMemory(PP_Instance instance,
                                                         PP_Resource context,
                                                         uint32_t flags) {
  return 0;  // Not supported in-process.
}
