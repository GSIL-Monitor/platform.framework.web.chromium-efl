...
#if defined(TIZEN_LIBAV)
  if (media::LibavVideoDecoderTizen::IsCodecSupported(codec))
    return true;
#elif !defined(MEDIA_DISABLE_FFMPEG) && !defined(DISABLE_FFMPEG_VIDEO_DECODERS)
...
#if defined(TIZEN_LIBAV)
  {
    std::unique_ptr<media::LibavVideoDecoderTizen> libav_video_decoder_tizen(
        new media::LibavVideoDecoderTizen(base::ThreadTaskRunnerHandle::Get()));
    libav_video_decoder_tizen->set_decode_nalus(true);
    decoder_ = std::move(libav_video_decoder_tizen);
  }
#elif !defined(MEDIA_DISABLE_FFMPEG) && !defined(DISABLE_FFMPEG_VIDEO_DECODERS)
  {
...
void VideoDecoderShim::DecoderImpl::OnDecodeComplete(
  ...
  switch (status) {
    ...
    case media::DecodeStatus::DECODE_ERROR:
      LOG(ERROR) << "Decode Error occurred";
