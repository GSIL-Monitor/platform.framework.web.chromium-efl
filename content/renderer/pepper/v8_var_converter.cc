  ...
  *did_create = false;
  ...
  if (val->IsUndefined()) {
    ...
    *result = StringVar::StringToPPVar(std::string(*utf8, utf8.length()));
    ...
    // 2) If the object is an array buffer view, return an ArrayBufferVar
    //    (Note this will mean that object is not an ArrayBuffer).
    // 3) If object vars are allowed, return the object wrapped as a
    //    V8ObjectVar. This is to maintain backward compatibility with
    //    synchronous scripting in Flash.
    // 4) If the object is an array, return an ArrayVar.
    // 5) If the object can be converted to a resource, return the ResourceVar.
    // 6) Otherwise return a DictionaryVar.
    std::unique_ptr<blink::WebArrayBuffer> web_array_buffer(
        blink::WebArrayBufferConverter::CreateFromV8Value(val, isolate));
    std::unique_ptr<blink::WebArrayBufferView> web_array_buffer_view(
        blink::WebArrayBufferView::CreateFromV8Value(val));
    ...
    } else if (web_array_buffer_view.get()) {
      scoped_refptr<HostArrayBufferViewVar> buffer_var(
          new HostArrayBufferViewVar(*web_array_buffer_view));
      *result = buffer_var->GetPPVar();
