...
void LoseContext(PP_Instance instance) {
  PepperPluginInstanceImpl* plugin = host_globals->GetInstance(instance);
  if (!plugin)
    return;

  plugin->LoseGraphics3DContext();
}

const PPB_Testing_Samsung_1_0 samsung_testing_interface_1_0 = {
    &ReadImageData,
    &RunMessageLoop,
    &QuitMessageLoop,
    &GetLiveObjectsForInstance,
...
#if defined(TIZEN_PEPPER_EXTENSIONS) || defined(TIZEN_PEPPER_TEEC)
#include "ppapi/thunk/interfaces_ppb_samsung.h"
#endif  // defined(TIZEN_PEPPER_EXTENSIONS) || defined (TIZEN_PEPPER_TEEC)
...
#if defined(TIZEN_PEPPER_EXTENSIONS)
    if (strcmp(name, PPB_TESTING_SAMSUNG_INTERFACE_1_0) == 0)
      return &samsung_testing_interface_1_0;
#endif
