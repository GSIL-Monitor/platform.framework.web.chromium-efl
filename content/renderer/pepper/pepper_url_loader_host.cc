...
int32_t PepperURLLoaderHost::InternalOnHostMsgOpen(
  ...
  // again.
  if (main_document_loader_) {
    LOG(ERROR) << "Main document already opened - not allow to multiple opens";
  ...
  if (URLRequestRequiresUniversalAccess(filled_in_request_data) &&
    ...
    LOG(ERROR) << "The URL you're requesting is on a different security"
               << " origin than your plugin";
    return PP_ERROR_NOACCESS;
  }

  if (loader_.get()) {
    LOG(ERROR) << "Open already in progress";
    return PP_ERROR_INPROGRESS;
  }
  ...
  if (!loader_.get()) {
    LOG(ERROR) << "Unsuccessful creation of AssociatedURLLoader";
    return PP_ERROR_FAILED;
  }
...
int32_t PepperURLLoaderHost::OnHostMsgGrantUniversalAccess(
  ...
  if (!host()->permissions().HasPermission(ppapi::PERMISSION_PRIVATE)) {
    LOG(ERROR) << "Only plugins with private permission can bypass same origin";
    return PP_ERROR_FAILED;
  }
