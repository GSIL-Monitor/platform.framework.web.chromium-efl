...
#if defined(TIZEN_PEPPER_PLAYER)
  else if (bound_player_)
    bound_player_->ViewInitiatedPaint();
#endif
}
...
#if defined(TIZEN_PEPPER_PLAYER)
  for (auto* geometry_listener : geometry_listeners_) {
    geometry_listener->GeometryDidChange(view_data.rect);
  }
#endif
}
...
void PepperPluginInstanceImpl::UpdateLayer(bool force_creation) {
  ...
  bool want_video_layer = false;
#if defined(TIZEN_PEPPER_PLAYER)
  want_video_layer = !!bound_player_;
#endif
  ...
  if (!force_creation && (want_texture_layer == !!texture_layer_) &&
      ...
      (want_video_layer == !!video_layer_.get()) &&
      layer_bound_to_fullscreen_ == !!fullscreen_container_) {
    UpdateLayerTransform();
    return;
  }

  if (texture_layer_.get() || compositor_layer_.get() || video_layer_.get()) {
  ...
  } else if (want_video_layer) {
#if defined(TIZEN_PEPPER_PLAYER)
    video_layer_ = bound_player_->video_layer();
    web_layer_ = base::MakeUnique<cc_blink::WebLayerImpl>(video_layer_);
#endif
  }
...
#if defined(TIZEN_PEPPER_EXTENSIONS)
void PepperPluginInstanceImpl::LoseGraphics3DContext() {
  BindGraphics(pp_instance_, 0);
  base::ThreadTaskRunnerHandle::Get()->PostTask(
      FROM_HERE, base::Bind(&PepperPluginInstanceImpl::OnGraphics3DContextLost,
                            view_change_weak_ptr_factory_.GetWeakPtr()));
}

void PepperPluginInstanceImpl::OnGraphics3DContextLost() {
  if (!module_)
    return;

  const PPP_Graphics3D* ppp_graphics_3d = static_cast<const PPP_Graphics3D*>(
      module_->GetPluginInterface(PPP_GRAPHICS_3D_INTERFACE));
  if (ppp_graphics_3d)
    ppp_graphics_3d->Graphics3DContextLost(pp_instance_);
}
#endif
...
PP_Bool PepperPluginInstanceImpl::BindGraphics(PP_Instance instance,
...
#if defined(TIZEN_PEPPER_PLAYER)
  if (bound_player_) {
    bound_player_->BindToInstance(NULL);
    bound_player_ = NULL;
  }
#endif
...
#if defined(TIZEN_PEPPER_PLAYER)
  PepperMediaPlayerRendererHost* player = NULL;
#endif
  if (host) {
...
#if defined(TIZEN_PEPPER_PLAYER)
    } else if (host->IsMediaPlayerRendererHost()) {
      player = static_cast<PepperMediaPlayerRendererHost*>(host);
#endif
    } else {
      DLOG(ERROR) <<
  ...
  if (compositor) {
    ...
    if (player->BindToInstance(this)) {
      bound_player_ = player;
      UpdateLayer(true);
      return PP_TRUE;
    }
#endif
...
int32_t PepperPluginInstanceImpl::LockMouse(
  ...
  if (TrackedCallback::IsPending(lock_mouse_callback_)) {
    LOG(ERROR) << "Lock mouse callback is pending";
  ...
  if (!CanAccessMainFrame()) {
    LOG(ERROR) << "Cannot access main frame";
    return PP_ERROR_NOACCESS;
  }

  if (!IsProcessingUserGesture()) {
    LOG(ERROR) << "Not processing user gestrure";
  ...
  if (!FlashIsFullscreenOrPending() || flash_fullscreen_) {
    ...
    WebScopedUserGesture user_gesture(CurrentUserGestureToken());
    if (!LockMouse()) {
      LOG(ERROR) << "Locking mouse failed";
      return PP_ERROR_FAILED;
    }
  }
...
int32_t PepperPluginInstanceImpl::Navigate(
  ...
  if (!CreateWebURLRequest(
    ...
    LOG(ERROR) << "Cannot create WebURLRequest";
    return PP_ERROR_FAILED;
  }
  ...
  if (web_request.HttpMethod() != "GET" && web_request.HttpMethod() != "POST") {
    LOG(ERROR) << "Only GET and POST methods are supported, httpMethod: "
               << web_request.HttpMethod().Utf8();
...
void PepperPluginInstanceImpl::SendCrashEvent() {
  ppapi::PpapiGlobals::Get()->GetMainThreadMessageLoop()->PostTask(
      FROM_HERE,
      base::Bind(&PepperPluginInstanceImpl::SendCrashEventOnMainThread, this));
}
#endif  // defined(TIZEN_PEPPER_EXTENSIONS)

cc::Layer* PepperPluginInstanceImpl::ActiveLayer() {
  if (texture_layer_.get())
    return texture_layer_.get();

  if (compositor_layer_.get())
...
#if defined(TIZEN_PEPPER_PLAYER)
PepperPluginInstanceImpl::GeometryChangeListener::~GeometryChangeListener() =
    default;

void PepperPluginInstanceImpl::AddGeometryChangeListener(
    GeometryChangeListener* listener) {
  geometry_listeners_.push_back(listener);
}

void PepperPluginInstanceImpl::RemoveGeometryChangeListener(
    GeometryChangeListener* listener) {
  geometry_listeners_.erase(std::remove(geometry_listeners_.begin(),
                                        geometry_listeners_.end(), listener),
                            geometry_listeners_.end());
}
