...
int32_t PepperVideoDestinationHost::OnHostMsgOpen(
  ...
  if (!gurl.is_valid()) {
    LOG(ERROR) << "Invalid stream URL";
    return PP_ERROR_BADARGUMENT;
  }

  FrameWriterInterface* frame_writer = NULL;
  if (!PepperToVideoTrackAdapter::Open(NULL /* registry */, gurl.spec(),
                                       &frame_writer)) {
...
int32_t PepperVideoDestinationHost::OnHostMsgPutFrame(
  ...
  if (!PPB_ImageData_Impl::IsImageDataFormatSupported(
    ...
    LOG(ERROR) << "Image data format is unsupported, "
               << static_cast<int>(image_data_impl->format());
    return PP_ERROR_BADARGUMENT;
  }

  if (!frame_writer_.get()) {
    LOG(ERROR) << "Frame writer is NULL";
    return PP_ERROR_FAILED;
  }
