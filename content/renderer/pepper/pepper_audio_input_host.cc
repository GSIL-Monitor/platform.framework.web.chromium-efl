...
void PepperAudioInputHost::StreamCreated(
  ...
  OnOpenComplete(PP_OK, shared_memory_handle, shared_memory_size,
                 shared_memory_segment_count, socket);
}

void PepperAudioInputHost::StreamCreationFailed() {
  OnOpenComplete(PP_ERROR_FAILED, base::SharedMemoryHandle(), 0, 0,
...
void PepperAudioInputHost::OnOpenComplete(
  ...
  SendOpenReply(result, shared_memory_segment_count);
}
...
void PepperAudioInputHost::Close() {
  ...
  if (open_context_.is_valid())
    SendOpenReply(PP_ERROR_ABORTED, 0);
}
