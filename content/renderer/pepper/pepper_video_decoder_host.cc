...
int32_t PepperVideoDecoderHost::OnHostMsgInitialize(
...
#if defined(TIZEN_PEPPER_EXTENSIONS)
    PP_VideoLatencyMode latency_mode,
#endif
  ...
  if (min_picture_count > ppapi::proxy::kMaximumPictureCount) {
    LOG(ERROR) << "Min picture count: " << min_picture_count
               << "is larger than " << ppapi::proxy::kMaximumPictureCount;
  ...
  if (enter_graphics.failed()) {
    LOG(ERROR) << "Enter Graphics3D failed";
    return PP_ERROR_FAILED;
  }
  ...
  if (!command_buffer) {
    LOG(ERROR) << "Command buffer is NULL";
    return PP_ERROR_FAILED;
  }
  ...
  if (acceleration != PP_HARDWAREACCELERATION_NONE) {
...
#if defined(TIZEN_PEPPER_EXTENSIONS)
      VideoLatencyMode video_latency_mode;

      switch (latency_mode) {
        case PP_VIDEOLATENCYMODE_NORMAL:
          video_latency_mode = VideoLatencyMode::NORMAL;
          break;
        case PP_VIDEOLATENCYMODE_LOW:
          video_latency_mode = VideoLatencyMode::LOW;
          break;
        default:
          LOG(WARNING) << "Unknown mapping for latency mode: " << latency_mode
                       << ". Setting normal latency mode.";
          video_latency_mode = VideoLatencyMode::NORMAL;
          break;
      }

      vda_config.latency_mode = video_latency_mode;
#endif
    ...
    if (acceleration == PP_HARDWAREACCELERATION_ONLY) {
      LOG(ERROR) << "Cannot fallback to software video decoding";
      return PP_ERROR_NOTSUPPORTED;
    }
  }

#if defined(OS_ANDROID) || (defined(OS_TIZEN) && !defined(TIZEN_LIBAV))
  LOG(ERROR) << "Software video decoding is not available";
  return PP_ERROR_NOTSUPPORTED;
#else
  if (!TryFallbackToSoftwareDecoder()) {
    LOG(ERROR) << "Fallback to software decoder failed";
    return PP_ERROR_FAILED;
  }
  initialized_ = true;
  return PP_OK;
...
int32_t PepperVideoDecoderHost::OnHostMsgDecode(
  ...
  if (GetPendingDecodeById(decode_id) != pending_decodes_.end()) {
    LOG(ERROR) << "Rejecting non-unique decode_id values";
    return PP_ERROR_FAILED;
  }

  if (flush_reply_context_.is_valid() || reset_reply_context_.is_valid()) {
    LOG(ERROR) << "Flush reply context or reset reply context are valid "
...
int32_t PepperVideoDecoderHost::OnHostMsgAssignTextures(
  ...
  for (uint32_t i = 0; i < texture_ids.size(); i++) {
    if (picture_buffer_map_.find(texture_ids[i]) != picture_buffer_map_.end() ||
      ...
      LOG(ERROR) << "Cannot assign the same texture more than once";
...
int32_t PepperVideoDecoderHost::OnHostMsgRecyclePicture(
  ...
  if (it == picture_buffer_map_.end()) {
    LOG(ERROR) << "Couldn't find texture id in pictures map";
    return PP_ERROR_BADARGUMENT;
  }

  switch (it->second) {
    case PictureBufferState::ASSIGNED:
      LOG(ERROR) << "Picture already assigned";
      return PP_ERROR_BADARGUMENT;
