...
int32_t PepperVideoEncoderHost::OnHostMsgInitialize(
  ...
  if (initialized_) {
    LOG(ERROR) << "Encoder already initialized";
    return PP_ERROR_FAILED;
  }
  ...
  if (input_size.IsEmpty()) {
    LOG(ERROR) << "Input size is empty";
    return PP_ERROR_BADARGUMENT;
  }

  if (!IsInitializationValid(input_visible_size, output_profile,
                             acceleration)) {
    LOG(ERROR) << "Initialization argument set unsupported";
...
int32_t PepperVideoEncoderHost::OnHostMsgEncode(
  ...
  if (frame_id >= frame_count_) {
    LOG(ERROR) << "The frame ID = " << frame_id << " cannot be larger or "
               << "equal to " << frame_count_;
    return PP_ERROR_FAILED;
  }
...
int32_t PepperVideoEncoderHost::OnHostMsgRecycleBitstreamBuffer(
  ...
  if (buffer_id >= shm_buffers_.size() || shm_buffers_[buffer_id]->in_use) {
    LOG(ERROR) << "Wrong buffer ID = " << buffer_id << ", should be less than"
               << shm_buffers_.size() << " and not in use, buffer.in_use = "
               << shm_buffers_[buffer_id]->in_use;
    return PP_ERROR_FAILED;
  }
...
bool PepperVideoEncoderHost::EnsureGpuChannel() {
  ...
  command_buffer_ = gpu::CommandBufferProxyImpl::Create(
      ...
      ui::command_buffer_metrics::GPU_VIDEO_ACCELERATOR_CONTEXT);
#else
      GURL::EmptyGURL(), base::ThreadTaskRunnerHandle::Get());
#endif
  if (!command_buffer_) {
