...
RenderWidget::RenderWidget(int32_t widget_routing_id,
    ...
    : routing_id_(widget_routing_id),
      ...
      text_input_is_in_form_tag_(false),
#endif
...
bool RenderWidget::OnMessageReceived(const IPC::Message& message) {
  ...
  IPC_BEGIN_MESSAGE_MAP(RenderWidget, message)
    ...
    IPC_MESSAGE_HANDLER(ViewMsg_FastScrollerEnabled, OnFastScrollerEnabled)
#endif
...
#endif
#if defined(OS_TIZEN_TV_PRODUCT)
    IPC_MESSAGE_HANDLER(ViewMsg_SetLayerInverted, OnSetLayerInverted)
#endif
#if defined(USE_EFL)
    IPC_MESSAGE_HANDLER(ViewMsg_UpdateRotationDegrees, OnUpdateRotationDegrees)
#endif
    IPC_MESSAGE_HANDLER(ViewMsg_SetClearTilesOnHide, OnSetClearTilesOnHide)
    IPC_MESSAGE_UNHANDLED(handled = false)
    IPC_MESSAGE_HANDLER(ViewMsg_SetTileCoverAreaMultiplier,
                        OnSetTileCoverAreaMultiplier)
  IPC_END_MESSAGE_MAP()
  return handled;
}
...
#if defined(OS_TIZEN_TV_PRODUCT)
void RenderWidget::DidInitializeRenderer() {
  Send(new ViewHostMsg_DidInitializeRenderer(routing_id_));
}
#endif
...
#endif
    if (GetWebWidget())
      params.advanced_ime_options = GetWebWidget()->GetAdvancedIMEOptions();
    else
      params.advanced_ime_options = 0;
...
blink::WebLayerTreeView* RenderWidget::InitializeLayerTreeView() {
  ...
  LOG(INFO) << "Disabling compositor for non gpu model to improve performance";
  return nullptr;
#endif
...
#if defined(USE_EFL)
void RenderWidget::OnUpdateRotationDegrees(int rotation_degrees) {
...
#if defined(OS_TIZEN_TV_PRODUCT)
void RenderWidget::OnSetLayerInverted(bool inverted) {
  if (compositor_)
    compositor_->SetLayerInverted(inverted);
}
#endif

void RenderWidget::OnSetTileCoverAreaMultiplier(float cover_area_multiplier) {
#if defined(OS_TIZEN)
  if (compositor_)
    compositor_->SetTileCoverAreaMultiplier(cover_area_multiplier);
#endif
}

void RenderWidget::OnSetClearTilesOnHide(bool enable) {
#if defined(OS_TIZEN)
  if (compositor_) {
    compositor_->SetClearTilesOnHide(enable);
  }
#endif
}

#if defined(OS_TIZEN)
void RenderWidget::DidChangeLoadState(bool is_loading) {
  if (compositor_)
    compositor_->DidChangeLoadState(is_loading);
}
#endif
...
#if defined(OS_TIZEN_TV_PRODUCT)
  WebCursorInfo tv_cursor_info = cursor_info;
  bool is_web_browser = blink::IsWebBrowser();
  switch (cursor_info.type) {
    case WebCursorInfo::kTypePointer:
      tv_cursor_info.type = WebCursorInfo::kTypeTVPointer;
      if (is_web_browser) {
        CursorInfo last_cursor_info;
        current_cursor_.GetCursorInfo(&last_cursor_info);
        if (WebCursorInfo::kTypeWebCross == last_cursor_info.type)
          tv_cursor_info.type = WebCursorInfo::kTypeWebPNRPointer;
        else
          tv_cursor_info.type = WebCursorInfo::kTypeWebPointer;
      }
      break;
    case WebCursorInfo::kTypeCross:
      tv_cursor_info.type = is_web_browser ? WebCursorInfo::kTypeWebCross
                                           : WebCursorInfo::kTypeTVCross;
      break;
    case WebCursorInfo::kTypeHand:
      tv_cursor_info.type = is_web_browser ? WebCursorInfo::kTypeWebHand
                                           : WebCursorInfo::kTypeTVHand;
      break;
    case WebCursorInfo::kTypeIBeam:
      tv_cursor_info.type = is_web_browser ? WebCursorInfo::kTypeWebIBeam
                                           : WebCursorInfo::kTypeTVIBeam;
      break;
    case WebCursorInfo::kTypeNone:
      tv_cursor_info.type = WebCursorInfo::kTypeTVNone;
      break;
    default:
      tv_cursor_info.type = is_web_browser ? WebCursorInfo::kTypeWebDefault
                                           : WebCursorInfo::kTypeTVDefault;
      break;
  }
  InitializeCursorFromWebCursorInfo(&cursor, tv_cursor_info);
#else
...
void RenderWidget::OnDefersLoading(bool defers) {
  GetWebWidget()->pageNeedsToDeferLoading(defers);
}
...
#if defined(S_TERRACE_SUPPORT)
void RenderWidget::OnFastScrollerEnabled(bool enabled) {
  if (compositor_) {
    compositor_->setFastScrollerEnabled(enabled);
  }
}
#endif
...
void RenderWidget::UpdateSelectionBounds(bool is_last_touch_point) {
...
#if defined(OS_ANDROID)
  // With composited selection updates, is_multi_selection is not updated
  // hence sending explicit IPC msg
  if (is_last_touch_point
      && blink::WebRuntimeFeatures::IsCompositedSelectionUpdateEnabled())
    Send(new ViewHostMsg_MultiSelection_ACK(routing_id_));
#endif
  ...
  if (send_ipc) {
    ...
    if (selection_anchor_rect_ != params.anchor_rect ||
        selection_focus_rect_ != params.focus_rect || is_last_touch_point) {
      ...
      params.is_last_touch_point = is_last_touch_point;
...
#if defined(OS_TIZEN)
void RenderWidget::addEdgeEffectONSCROLLTizenUIF(bool top,
                                                 bool bottom,
                                                 bool right,
                                                 bool left) {
  Send(new ViewHostMsg_AddEdgeEffectONSCROLLTizenUIF(routing_id_, top, bottom,
                                                     right, left));
}
#endif
...
#if defined(USE_EFL)
  if (GetWebWidget() && GetTextInputType() != ui::TEXT_INPUT_TYPE_NONE) {
