...
#include "tizen_src/chromium_impl/content/renderer/media/tizen/renderer_media_player_manager_efl.h"
#endif
...
#if BUILDFLAG(USE_EXTERNAL_POPUP_MENU) || defined(USE_EFL)
...
bool RenderFrameImpl::OnMessageReceived(const IPC::Message& msg) {
  ...
  IPC_BEGIN_MESSAGE_MAP(RenderFrameImpl, msg)
    ...
    IPC_MESSAGE_HANDLER(InputMsg_SelectLinkText, OnSelectLinkText)
...
#if defined(S_TERRACE_SUPPORT)
    IPC_MESSAGE_HANDLER(FrameMsg_OpenWebLinkMatchInfos, OnOpenWebLinkMatchInfos)
#endif
#endif
...
void RenderFrameImpl::OnSelectLinkText() {
  base::AutoReset<bool> handling_select_range(&handling_select_range_, true);
  frame_->selectLinkText();
}
...
void RenderFrameImpl::OnMoveRangeSelectionExtent(const gfx::Point& point,
  ...
  frame_->MoveRangeSelectionExtent(
      render_view_->ConvertWindowPointToViewport(point), is_last_touch_point);
}
...
void RenderFrameImpl::OnCSSInsertRequest(const std::string& css) {
  frame_->GetDocument().InsertStyleSheet(WebString::FromUTF8(css));
}
// VRBrowser--
...
service_manager::BinderRegistry* RenderFrameImpl::Registry() {
  return &registry_;
}
...
#if !defined(OS_ANDROID)
#if defined(S_TERRACE_SUPPORT)
  if (!base::SamsungConfig::IsDexEnabled())
    return;
#endif
...
blink::WebMediaPlayer* RenderFrameImpl::CreateMediaPlayer(
  ...
  // https://codereview.chromium.org/2769153005
...
#if defined(S_TERRACE_SUPPORT)
void RenderFrameImpl::PasswordFormWasVisible(const blink::WebFormElement& form,
                                             const bool is_loading) {
  for (auto& observer : observers_)
    observer.PasswordFormWasVisible(form, is_loading);
}
#endif
...
// It Needs to be checked by author.
  if (is_main_frame_) {
    bool isNativePage
      = (document_loader->GetRequest().Url().GetString().Utf8().find("data:text/html")
        != std::string::npos) ? true : false;
    LOG(INFO) << "LoaderLog didStartProvisionalLoad isNativePage:"
              << isNativePage;
  }
...
#if defined(OS_TIZEN)
  if (frame_ == render_view_->webview()->MainFrame())
    GetRenderWidget()->DidChangeLoadState(true);
#endif
...
void RenderFrameImpl::DidFailProvisionalLoad(
  ...
  if (is_main_frame_)
    LOG(INFO) << "LoaderLog didFailProvisionalLoad error = "
              << error.reason;
...
void RenderFrameImpl::DidCommitProvisionalLoad(
  ...
  if (is_main_frame_)
    LOG(INFO) << "LoaderLog didCommitProvisionalLoad enter ";
...
#if defined(OS_TIZEN_TV_PRODUCT)
  if (render_view_->webview() && render_view_->webview()
                                     ->GetSettings()
...
#if defined(S_TERRACE_SUPPORT)
  bool is_back_forward_navigation = commit_type == blink::kWebBackForwardCommit;
#endif

  for (auto& observer : render_view_->observers_)
    observer.DidCommitProvisionalLoad(frame_, is_new_navigation);
#if defined(S_TERRACE_SUPPORT)
  if (!is_back_forward_navigation)
    for (auto& observer : observers_)
      observer.DidCommitRedirection();
#endif
  for (auto& observer : observers_) {
...
#if defined(OS_TIZEN_TV_PRODUCT)
  if (blink::IsWebBrowser())
    GetRenderWidget()->DidChangeCursor(
        blink::WebCursorInfo(blink::WebCursorInfo::kTypeWebDefault));
#endif
...
void RenderFrameImpl::DidFinishLoad() {
  ...
  if (is_main_frame_)
    LOG(INFO) << "LoaderLog didFinishLoad enter";
...
#if defined(OS_TIZEN)
  if (frame_ == render_view_->webview()->MainFrame())
    GetRenderWidget()->DidChangeLoadState(false);
#endif
...
void RenderFrameImpl::DidChangeSelection(bool is_empty_selection,
  ...
  GetRenderWidget()->UpdateTextInputState();
  SyncSelectionIfRequired(is_last_touch_point);
}
...
void RenderFrameImpl::WillSendRequest(blink::WebURLRequest& request) {
  ...
  if (GetContentClient()->renderer()->WillSendRequest(
          ...
          is_encrypted_file)) {
    request.SetEncryptedFileFlag(is_encrypted_file);
    request.SetParsedURL(WebURL(new_url));
  }
#else
  if (GetContentClient()->renderer()->WillSendRequest(
          frame_, transition_type, request.Url(), &throttles, &new_url)) {
...
#if defined(OS_TIZEN_TV_PRODUCT)
bool RenderFrameImpl::GetFileDecryptedDataBuffer(const blink::WebURL& url,
                                                 blink::WebData& data) {
  std::vector<char> file_data;
  if (GetContentClient() && GetContentClient()->renderer() &&
      GetContentClient()->renderer()->GetFileDecryptedDataBuffer(url,
                                                                 &file_data)) {
    data.Assign(file_data.data(), file_data.size());
    return true;
  }
  return false;
}
#endif
#if defined(USE_EFL)
bool RenderFrameImpl::GetWrtParsedUrl(const blink::WebURL& url,
                                      blink::WebURL& parsed_url) {
  GURL parsed_gurl;
  if (GetContentClient()->renderer() &&
      GetContentClient()->renderer()->GetWrtParsedUrl(url, parsed_gurl)) {
    parsed_url = parsed_gurl;
    return true;
  }
  return false;
}
...
#if defined(OS_TIZEN_TV_PRODUCT)
void RenderFrameImpl::CheckHLSSupport(const blink::WebString& url,
                                      bool& support) {
  media_factory_.CheckHLSSupport(url, support);
}
#endif

void RenderFrameImpl::registerContentHandler(const WebString& mime_type,
                                             const WebURL& base_weburl,
                                             const WebURL& url,
                                             const WebString& title) {
  bool user_gesture = WebUserGestureIndicator::IsProcessingUserGesture();
  GURL base_url(base_weburl);
  GURL absolute_url = base_url.Resolve(base::UTF16ToUTF8(url.GetString().Utf16()));
  Send(new FrameHostMsg_RegisterContentHandler(
      routing_id_, base::UTF16ToUTF8(mime_type.Utf16()), absolute_url,
      title.Utf16(), user_gesture));
}

void RenderFrameImpl::unregisterContentHandler(const WebString& mime_type,
                                               const WebURL& base_weburl,
                                               const WebURL& url) {
  bool user_gesture = WebUserGestureIndicator::IsProcessingUserGesture();
  GURL base_url(base_weburl);
  GURL absolute_url = base_url.Resolve(
      base::UTF16ToUTF8(base::StringPiece16(url.GetString().Utf16())));
  Send(new FrameHostMsg_UnregisterContentHandler(
      routing_id_, base::UTF16ToUTF8(mime_type.Utf16()), absolute_url,
      user_gesture));
}
...
void RenderFrameImpl::DidStopLoading() {
  TRACE_EVENT1("navigation,rail", "RenderFrameImpl::didStopLoading",
               "id", routing_id_);
  if (is_main_frame_)
    LOG(INFO) << "LoaderLog didStopLoading enter";
...
static base::string16 GetFocusedNodeId(const WebNode& node) {
  base::string16 node_id;
  if (!node.IsNull() && node.IsElementNode()) {
    const WebElement& element = node.ToConst<WebElement>();
    if (element.HasAttribute("name"))
     node_id = element.GetAttribute("name").Utf16();
    if (node_id.length() == 0) {
      if (element.HasAttribute("id"))
        node_id = element.GetAttribute("id").Utf16();
    }
  }
  if (node_id.length() == 0) {
    const int BUF_SIZE = 4 * sizeof(long);
    char buf[BUF_SIZE];
    snprintf(buf, BUF_SIZE, "%ld", labs(reinterpret_cast<long>(&node)));
    node_id = base::ASCIIToUTF16(buf);
  }
  return node_id;
}

static bool IsSelectNode(const WebNode& node) {
  if (node.IsNull() || !node.IsElementNode())
    return false;

  if (node.ToConst<WebElement>().HasHTMLTagName("select"))
    return true;
...
#if defined(OS_TIZEN_TV_PRODUCT)
static bool IsRadioOrCheckboxInputNode(const WebNode& node) {
  if (node.IsNull() || !node.IsElementNode())
    return false;

  WebElement element = const_cast<WebNode&>(node).To<WebElement>();
  if (!element.HasHTMLTagName("input") || !element.HasNonEmptyLayoutSize())
    return false;
...
static int PasswordInputNodeMinLength(const WebNode& node) {
  if (node.IsNull() || !node.IsElementNode())
    return -1;
...
static bool IsContentEditable(const WebNode& node) {
  if (node.IsNull())
    return false;
...
#if defined(OS_TIZEN_TV_PRODUCT)
  params.is_radio_or_checkbox_input_node = IsRadioOrCheckboxInputNode(node);
  params.password_input_minlength = PasswordInputNodeMinLength(node);
#endif
#if defined(USE_WAYLAND)
  params.is_content_editable = IsContentEditable(node);
#endif
#endif

  Send(new FrameHostMsg_FocusedNodeChanged(routing_id_, is_editable,
                                           node_bounds, IsSelectNode(node),
                                           GetFocusedNodeId(node)
#if defined(OS_TIZEN_TV_PRODUCT) || defined(USE_WAYLAND)
                                           , params
#endif
                                           ));
...
#if defined(S_TERRACE_SUPPORT)
void RenderFrameImpl::OnOpenWebLinkMatchInfos(int current_version) {
  std::vector<gfx::Point> match_points;
  std::vector<GURL> match_urls;

  int infos_version = frame_->FindMatchMarkersVersion();
  if (current_version != infos_version) {
    WebVector<WebPoint> points;
    points = frame_->FindMatchPoints();
    match_points.reserve(points.size());
    for (size_t i = 0; i < points.size(); ++i)
      match_points.push_back(points[i]);

    WebVector<WebURL> urls;
    urls = frame_->FindMatchUrls();
    match_urls.reserve(urls.size());
    for (size_t i = 0; i < urls.size(); ++i)
      match_urls.push_back(urls[i]);
  }

  Send(new FrameHostMsg_OpenWebLinkMatchInfos_Reply(routing_id_, infos_version,
                                                    match_points, match_urls));
}
#endif
#endif
...
#if defined(S_TERRACE_SUPPORT)
  if (canceled || ui::GetDeviceFormFactor() == ui::DEVICE_FORM_FACTOR_TABLET)
    external_popup_menu_.reset();
#else
  external_popup_menu_.reset();
#endif
...
void RenderFrameImpl::NavigateInternal(
  ...
  if (is_reload) {
    ...
    if (entry) {
      ...
      LOG(ERROR) << "LoaderLog NavigateInternal no HistoryEntry";
    }
  } else {
...
void RenderFrameImpl::SyncSelectionIfRequired(bool is_last_touch_point) {
  base::string16 text;
  size_t offset;
  gfx::Range range;
#if BUILDFLAG(ENABLE_PLUGINS)
  ...
  GetRenderWidget()->UpdateSelectionBounds(is_last_touch_point);
}
...
bool RenderFrameImpl::ShouldDisplayErrorPageForFailedLoad(
  ...
  if (base::VrModeUtil::isVrModeEnabled()) {
    // vrbrowser doesn't need to display error page when fail to load
    if((error_code != net::ERR_CONNECTION_TIMED_OUT)
        && (error_code != net::ERR_TIMED_OUT)
        && (error_code != net::ERR_CONNECTION_CLOSED)
        && (error_code != net::ERR_CONNECTION_RESET)
        && (error_code != net::ERR_CONNECTION_REFUSED)
        && (error_code != net::ERR_CONNECTION_ABORTED)
        && (error_code != net::ERR_CONNECTION_FAILED)
        && (error_code != net::ERR_SOCKET_NOT_CONNECTED)
        && (error_code != net::ERR_INTERNET_DISCONNECTED)
        && (error_code != net::ERR_ADDRESS_INVALID)
        && (error_code != net::ERR_ADDRESS_UNREACHABLE)
        && (error_code != net::ERR_NAME_NOT_RESOLVED)
        && (error_code != net::ERR_NAME_RESOLUTION_FAILED))
      return false;
  }
...
// FIXME: m60_3112 bringup, needs to be checked by author
// https://codereview.chromium.org/2769153005
/*blink::WebMediaPlayer* RenderFrameImpl::CreateAndroidWebMediaPlayer(
    WebMediaPlayerClient* client,
    WebMediaPlayerEncryptedMediaClient* encrypted_client,
  ...
  return TerraceContentBridge::Get().CreateWebMediaPlayer(
      ...
      enable_texture_copy, std::move(params));
}
}*/
#endif
#endif  // defined(OS_ANDROID)
...
// SBrowser Extension : Content Recommender
// For supporting Content Recommender Interface,
// update meta elements on document as JSON format
void RenderFrameImpl::UpdateMetadata() {
  ...
  std::string metadata = "[";
  for (WebNode node = head.FirstChild(); !node.IsNull()
      ; node = node.NextSibling()) {
    if (!node.IsElementNode())
      continue;
    WebElement element_node = node.To<WebElement>();

    std::string meta_info;
    if (element_node.HasHTMLTagName("meta")) {
      meta_info = "{";
      for (unsigned i = 0; i < element_node.AttributeCount(); ++i) {
        meta_info = meta_info + "\""
            + element_node.AttributeLocalName(i).Utf8()
            + "\":\"" + element_node.AttributeValue(i).Utf8() + "\",";
      }
      meta_info.replace(meta_info.size() - 1, 1, "},");
      metadata += meta_info;
    }
  }
  if (!metadata.compare("["))
  ...
  metadata.replace(metadata.size() - 1, 1, 1, ']');
  Send(new FrameHostMsg_DidGetMetadata(routing_id_, metadata));
}
