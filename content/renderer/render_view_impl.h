...
class CONTENT_EXPORT RenderViewImpl : public RenderWidget,
 ...
 public:
...
#if defined(OS_TIZEN)
  void addEdgeEffectONSCROLLTizenUIF(bool, bool, bool, bool) override;
#endif
  ...
  bool IsDeviceFormFactorTablet() const override;
 ...
 private:
  ...
  void OnSelectLinkText(const gfx::Point&);
  ...
  void OnHandleSelectionDragDrop(int x,
                                 int y,
                                 const base::string16& text,
                                 const base::string16& html_text,
                                 bool is_drop);
  void OnGetSelectionBitmap();
#if defined(USE_EFL)
  void OnSelectClosestWord(int x, int y);
#endif
  void OnClearTextSelection();
#if defined(OS_TIZEN)
  void OnDragStateEnter();
#endif
  void OnGetSelectionMarkup();
  void OnGetCurrentSelectionRect();
  void OnLongPressOnFocused(const blink::WebInputEvent* longpress_event);
  void OnGetBitmapFromCachedResource(const std::string& image_url,
                                     const int request_id);
  ...
  void OnUpdateBrowserControlsState(bool enable_hiding,
                                    bool enable_showing,
                                    bool animate);
  void OnGetSelectionMarkupWithBounds();
  void OnMoveFocusToPrevNextInputElement(bool is_next);
#elif defined(OS_MACOSX)
  void OnGetRenderedText();
#endif

#if defined(USE_EFL)
  void ScrollFocusedNode();
  void OnUpdateBrowserControlsState(BrowserControlsState constraints,
                                    BrowserControlsState current,
                                    bool animate);
  void OnSelectFocusedLink();
  void OnRequestSelectionRect();
#endif

  void OnRecognizeArticle(int mode);
  void OnRequestNumberOfBlockedElements();
  void OnEnableNightMode(bool nightMode);
  ...
  void PaintContentRectToBitmap(const gfx::Rect& src_subrect,
                                const float& page_scale_factor,
                                SkBitmap* snapshot);
...
// VRBrowser ++
  void OnSetNeedDesktopUA(bool enabled);
  void OnSetWebGLEnabled(bool enabled);
// VRBrowser --

#if defined(S_TERRACE_SUPPORT)
  void OnSearchCandidateImage(int resolution, double aspect_ratio);
#endif
