...
int RendererMain(const MainFunctionParams& parameters) {
  ...
  // Ecore has been integrated with glib, so main message loop can support
  // both ecore and glib events.
  std::unique_ptr<base::MessagePumpEcore> pump(new base::MessagePumpEcore());
  std::unique_ptr<base::MessageLoop> main_message_loop(
      new base::MessageLoop(std::move(pump)));
#else
