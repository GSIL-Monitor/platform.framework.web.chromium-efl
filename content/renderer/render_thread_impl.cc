...
RenderThreadImpl::CreateSyntheticBeginFrameSource() {
  ...
  if (max_refresh_rate_ >= 1 && max_refresh_rate_ <= 60) {
    auto timer_source = base::MakeUnique<viz::DelayBasedTimeSource>(
        compositor_impl_side_task_runner);
    const base::TimeDelta interval = base::TimeDelta::FromMicroseconds(
        base::Time::kMicrosecondsPerSecond / max_refresh_rate_);
    timer_source->SetTimebaseAndInterval(base::TimeTicks(), interval);
    return base::MakeUnique<viz::DelayBasedBeginFrameSource>(
        std::move(timer_source));
  }
#endif
...
#if defined(TIZEN_MULTIMEDIA_SUPPORT)
    renderer_demuxer_ = CreateRendererDemuxerEfl();
