...
#include "third_party/WebKit/public/web/WebContentBlockElementHiding.h"
...
#include "content/renderer/linkify/address_detector.h"
#include "content/renderer/linkify/email_detector.h"
#include "content/renderer/linkify/phone_number_detector.h"
#endif
...
//#include "base/night_mode_util.h"
...
void RenderViewImpl::Initialize(
  ...
  if (!was_created_by_renderer) {
    // Initialize device display info from browser.
    display::DeviceDisplayInfoEfl display_info;
    display_info.Update(params.display_width, params.display_height,
                        params.dip_scale, params.rotation_degrees);
  }
#endif
...
#if defined(S_TERRACE_SUPPORT)
  // RenderViewImpl::OnUpdateWebPreferences is called after the RenderViewImpl
  // is initialized. Set the font scale factor before the frame is created.
  webview()->GetSettings()->SetAccessibilityFontScaleFactor(
      webkit_preferences_.font_scale_factor);
#endif
...
#if defined(OS_TIZEN)
  RenderThreadImpl::current()->SetMaxRefreshRate(
      webkit_preferences_.max_refresh_rate);
#endif
}
...
void RenderView::ApplyWebPreferences(const WebPreferences& prefs,
  ...
  // FIXME: Commented the code during dev/m63_3239 rebase.
  // It needs to be re-;looked by Author.
  /* if (VrModeUtil::isWebGLEnabled()) {
  ...
  settings->SetTizenVersion(
      prefs.tizen_version.major,
      prefs.tizen_version.minor,
      prefs.tizen_version.release);
#endif
  ...
  settings->SetContentBlockerEnabled(prefs.content_blocker_enabled);
  settings->SetContentBlockerStatisticsEnabled(
      prefs.content_blocker_statistics_enabled);
  settings->SetDefaultContentBlockerEnabled(
      prefs.default_content_blocker_enabled);
  blink::WebContentBlockElementHiding::SetEnabled(
      false, prefs.content_blocker_enabled);
  blink::WebContentBlockElementHiding::SetEnabled(
      true, prefs.default_content_blocker_enabled);

#if defined(OS_WIN)
  WebRuntimeFeatures::EnableMiddleClickAutoscroll(true);
#endif

  if (VrModeUtil::isDayDreamModeEnabled() != prefs.daydream_mode_enabled) {
    VrModeUtil::setDayDreamModeEnabled(prefs.daydream_mode_enabled);
  }

  if (VrModeUtil::isGearVrModeEnabled() != prefs.gear_vr_mode_enabled) {
    VrModeUtil::setGearVrModeEnabled(prefs.gear_vr_mode_enabled);
  }

#if defined(OS_TIZEN_TV_PRODUCT)
  settings->SetMediaPlaybackNotificationEnabled(
      prefs.media_playback_notification_enabled);
  settings->SetMediaSubtitleNotificationEnabled(
      prefs.media_subtitle_notification_enabled);
  settings->SetUseScrollbarThumbFocusNotifications(
      prefs.use_scrollbar_thumb_focus_notifications);
#endif

#if defined(S_TERRACE_SUPPORT)
  if (VrModeUtil::isVrModeEnabled()) {
...
bool RenderViewImpl::OnMessageReceived(const IPC::Message& message) {
  ...
  IPC_BEGIN_MESSAGE_MAP(RenderViewImpl, message)
    ...
    IPC_MESSAGE_HANDLER(ViewMsg_HandleSelectionDragDrop,
                        OnHandleSelectionDragDrop)
...
#endif
#if defined(OS_TIZEN)
    IPC_MESSAGE_HANDLER(ViewMsg_DragStateEnter, OnDragStateEnter)
#endif
    ...
    IPC_MESSAGE_HANDLER(ViewMsg_RequestNumberOfBlockedElements,
                        OnRequestNumberOfBlockedElements)
    IPC_MESSAGE_HANDLER(ViewMsg_EnableNightMode, OnEnableNightMode)
    ...
    IPC_MESSAGE_HANDLER(ViewMsg_UpdateBrowserControlsState,
                        OnUpdateBrowserControlsState)
    IPC_MESSAGE_HANDLER(ViewMsg_GetSelectionMarkupWithBounds,
                        OnGetSelectionMarkupWithBounds)
    IPC_MESSAGE_HANDLER(ViewMsg_MoveFocusToPrevNextInputElement,
                        OnMoveFocusToPrevNextInputElement)
#elif defined(OS_MACOSX)
...
// VRBrowser ++
#if defined(S_TERRACE_SUPPORT)
    IPC_MESSAGE_HANDLER(ViewMsg_SetNeedDesktopUA, OnSetNeedDesktopUA)
    IPC_MESSAGE_HANDLER(ViewMsg_SetWebGLEnabled, OnSetWebGLEnabled)
#endif
// VRBrowser --

#if defined(S_TERRACE_SUPPORT)
    IPC_MESSAGE_HANDLER(ViewMsg_SearchCandidateImage,
                        OnSearchCandidateImage)
#endif
#if defined(USE_EFL)
    IPC_MESSAGE_HANDLER(ViewMsg_SelectFocusedLink, OnSelectFocusedLink)
    IPC_MESSAGE_HANDLER(ViewMsg_RequestSelectionRect, OnRequestSelectionRect)
#endif
...
void RenderViewImpl::OnSelectLinkText(const gfx::Point& point) {
  ...
  webview()->FocusedFrame()->selectLinkText();
}

void RenderViewImpl::OnGetBitmapFromCachedResource(
    const std::string& image_url, const int request_id) {
  if (!webview()) {
    Send(new ViewHostMsg_OnGetBitmapFromCachedResource(routing_id_,
                                                       SkBitmap(),
                                                       request_id));
    return;
  }
...
void RenderViewImpl::NotifyUrlForPlayingVideo(const WebURL& url) {
  Send(new ViewHostMsg_NotifyUrlForPlayingVideo(routing_id_, GURL(url)));
}
...
#if defined(USE_EFL)
void RenderViewImpl::DidChangeContents() {
  Send(new ViewHostMsg_EditableContentChanged(routing_id_));
}
#endif
...
#if defined(OS_TIZEN_TV_PRODUCT)
bool RenderViewImpl::HasFloatingVideoWindowOn() const {
  if (GetContentClient()->renderer() &&
      GetContentClient()->renderer()->FloatingVideoWindowOn())
    return true;

  return false;
}

void RenderViewImpl::ScrollbarThumbPartFocusChanged(
    const blink::WebScrollbar::Orientation orientation,
    bool focused) {
  GetContentClient()->renderer()->ScrollbarThumbPartFocusChanged(
      routing_id_, orientation, focused);
}
#endif

#if defined(TIZEN_ATK_FEATURE_VD)
void RenderViewImpl::MoveFocusToBrowser(int direction) {
  for (auto& observer : observers_)
    observer.MoveFocusToBrowser(direction);
}
#endif

#if defined(OS_TIZEN_TV_PRODUCT)
void RenderViewImpl::RunArrowScroll() {
  GetContentClient()->renderer()->RunArrowScroll(routing_id_);
}
#endif
...
bool RenderViewImpl::IsDeviceFormFactorTablet() const {
...
#if defined(OS_TIZEN)
void RenderViewImpl::addEdgeEffectONSCROLLTizenUIF(bool top,
                                                   bool bottom,
                                                   bool right,
                                                   bool left) {
  RenderWidget::addEdgeEffectONSCROLLTizenUIF(top, bottom, right, left);
}
#endif
...
#if defined(S_TERRACE_SUPPORT)
  bool textZoomFactorChanged = false;
  if (webkit_preferences_.font_scale_factor != prefs.font_scale_factor)
    textZoomFactorChanged = true;
#endif
  webkit_preferences_ = prefs;
  ApplyWebPreferencesInternal(webkit_preferences_, webview(), compositor_deps_);

#if defined(S_TERRACE_SUPPORT)
  if (webview() && textZoomFactorChanged) {
    // Hide selection and autofill popups.
    webview()->HidePopups();
    webview()->SetTextZoomFactor(webkit_preferences_.font_scale_factor);
  }
#endif
}
...
void RenderViewImpl::OnDisableAutoResize(const gfx::Size& new_size) {
  ...
  if (!new_size.IsEmpty()) {
    ...
    resize_params.bottom_controls_height = bottom_controls_height_;
...
#if defined(USE_EFL)
    LOG(INFO) << "FocusRingColor controlled by LayoutThemeChromiumTizen.";
#else
...
#if defined(USE_EFL)
void RenderViewImpl::ScrollFocusedNode() {
  if (webview() && webview()->MainFrame() &&
      webview()->MainFrame()->IsWebLocalFrame()) {
    blink::WebInputMethodController* controller =
        webview()->MainFrame()->ToWebLocalFrame()->GetInputMethodController();
    if (controller &&
...
#if defined(USE_EFL)
  // We want to scroll focused editable node into view after rotation/resize,
  // but due to reflow caused by resize we have to wait until reflow
  // is finished. Here is the place where we can be sure of that.
  base::ThreadTaskRunnerHandle::Get()->PostDelayedTask(
      FROM_HERE, base::Bind(&RenderViewImpl::ScrollFocusedNode, this),
      base::TimeDelta::FromMilliseconds(100));

  Send(new ViewHostMsg_DidResize(routing_id_));
#endif
}
...
void RenderViewImpl::didChangePageScaleFactor() {
  for (auto& observer : observers_)
    observer.DidChangePageScaleFactor();
}
...
WebURL RenderViewImpl::DetectContentIntentAt(
  ...
  DCHECK(touch_hit.GetNode().IsTextNode());
...
bool RenderViewImpl::DidTapMultipleTargets(
  ...
  float new_total_scale =
      ...
      DisambiguationPopupHelper::ComputeZoomAreaAndScaleFactor(
          ...
          device_scale_factor_ * webview()->PageScaleFactor(),
          device_scale_factor_, &zoom_rect);
#else
      DisambiguationPopupHelper::ComputeZoomAreaAndScaleFactor(
          touch_rect, target_rects, GetSize(),
          gfx::Rect(webview()->MainFrame()->VisibleContentRect()).size(),
...
void RenderViewImpl::OnUpdateBrowserControlsState(
    BrowserControlsState constraints,
    BrowserControlsState current,
    bool animate) {
  if (webview()) {
    webview()->UpdateBrowserControlsState(
        static_cast<blink::WebBrowserControlsState>(constraints),
        static_cast<blink::WebBrowserControlsState>(current), animate);
  }
}
#endif
...
#if defined(OS_TIZEN)
void RenderViewImpl::OnDragStateEnter() {
  if (webview() && webview()->MainFrame())
    webview()->MainFrame()->EnterDragState();
}
#endif

void RenderViewImpl::OnHandleSelectionDragDrop(int x,
                                               int y,
                                               const base::string16& text,
                                               const base::string16& html_text,
                                               bool is_drop) {
  if (!webview())
    return;

  webview()->onHandleSelectionDragDrop(x, y, WebString::FromUTF16(text),
                                       WebString::FromUTF16(html_text),
                                       is_drop);
}
...
void RenderViewImpl::OnGetSelectionMarkup() {
  ...
  if (main_frame->IsWebLocalFrame())
    markup = main_frame->ToWebLocalFrame()->SelectionAsMarkup().Utf16();
...
void RenderViewImpl::OnGetSelectionMarkupWithBounds() {
  ...
  if (is_selected) {
    selection_start_content_rect = gfx::Rect(anchor_webrect.x,
        anchor_webrect.y,
        anchor_webrect.width,
        anchor_webrect.height);
  }
  Send(new ViewHostMsg_SelectedMarkupWithStartContentRect(routing_id_,
          markup,
          selection_start_content_rect));
}

void RenderViewImpl::OnMoveFocusToPrevNextInputElement(bool is_next) {
  if (is_next)
...
void RenderViewImpl::OnRecognizeArticle(int mode) {
  ...
  if (main_frame && main_frame->IsWebLocalFrame()) {
    is_article = main_frame->ToWebLocalFrame()->
        recognizeArticle(static_cast<WebFrame::ReaderMode>(mode), page_url);
  }
  ...
  base::string16 wrapped_page_url = page_url.Utf16();
  Send(new ViewHostMsg_OnRecognizeArticleResult(routing_id_,
                                                is_article, wrapped_page_url));
}

void RenderViewImpl::OnRequestNumberOfBlockedElements() {
  int blocked_count =
      blink::WebContentBlockElementHiding::GetNumberOfBlockedElements();
  Send(new ViewHostMsg_OnNumberOfBlockedElements(routing_id_, blocked_count));
}

void RenderViewImpl::OnEnableNightMode(bool nightMode) {
  webview()->enableNightMode(nightMode);
}
...
void RenderViewImpl::OnLongPressOnFocused(
  ...
  if (!WebInputEvent::IsGestureEventType(longpress_event->GetType()))
    return;

  WebElement element = GetFocusedElement();
  if (element.IsNull() || element.IsEditable() ||
      element.HasHTMLTagName("select"))
    return;

  WebRect box_rect = element.BoundsInViewport();
  if (box_rect.IsEmpty())
    return;

  WebGestureEvent* gesture_longpress_event = const_cast<WebGestureEvent*>(
      static_cast<const WebGestureEvent*>(longpress_event));

  float page_scale = webview()->PageScaleFactor();
  gesture_longpress_event->x = (box_rect.x + box_rect.width / 2) / page_scale;
  // For elements, whose bounding box is not a fully qualified rectangle,
  // (bounding box falls on more than one line and is of polygon shape),
  // taking middle position is not correct because, point will be on the edge
  // or miss out, eventually misses correct hit test, hence keeping the point
  // above the half of height.
  gesture_longpress_event->y = (box_rect.y + box_rect.height / 5) / page_scale;
  OnHandleInputEvent(
      gesture_longpress_event, std::vector<const blink::WebInputEvent*>(),
...
/*  WebFrame* focused_frame = webview()->FocusedFrame();
  if (focused_frame) {
    WebDocument doc = focused_frame->GetDocument();
   // FIXME: Commented the code during dev/m60_3112 rebaseline.
   // It needs a re-look by author.
    if (!doc.IsNull())
...
void RenderViewImpl::OnCaptureRendererContentSnapShot(
  ...
  PaintContentRectToBitmap(src_subrect, page_scale_factor, &content_snapshot);
  Send(new ViewHostMsg_Snapshot(routing_id(), content_snapshot, request_id));
}

void RenderViewImpl::PaintContentRectToBitmap(const gfx::Rect& src_subrect,
                                              const float& page_scale_factor,
                                              SkBitmap* snapshot) {
   if (!snapshot) {
    LOG(ERROR) << "SoftBitmap:Invalid SkBitmap Passed as input!";
    return;
  }
  WebView* view = webview();
  if (!view || !view->MainFrame()) {
    LOG(ERROR) << "SoftBitmap:Invalid view";
    return;
  }
  ...
  if (view->Size().IsEmpty())
    return;
  int width = src_subrect.size().width();
  int height = src_subrect.size().height();
  snapshot->allocPixels(SkImageInfo::MakeN32Premul(width, height));
  SkCanvas canvas(*snapshot);
  view->paintSoftBitmap(
      &canvas, WebRect(src_subrect.x(), src_subrect.y(), src_subrect.width(),
                       src_subrect.height()));
  return;
}
...
#if defined(S_TERRACE_SUPPORT)
void RenderViewImpl::OnSearchCandidateImage(int resolution,
                                            double aspect_ratio) {
  if (!webview()) {
    Send(new ViewHostMsg_OnReceiveCandidateImage(routing_id_, SkBitmap()));
    return;
  }
  SkBitmap bitmap =
      webview()->SearchCandidateImage(resolution, aspect_ratio).GetSkBitmap();;
  Send(new ViewHostMsg_OnReceiveCandidateImage(routing_id_, bitmap));
}

// VRBrowser ++
void RenderViewImpl::OnSetNeedDesktopUA(bool enabled) {
  GetContentClient()->SetNeedDesktopUA(enabled);
}
void RenderViewImpl::OnSetWebGLEnabled(bool enabled) {
  webview()->setWebGLEnabled(enabled);
  if (VrModeUtil::isVrModeEnabled()) {
    VrModeUtil::setWebGLEnabled(enabled);
  }
}
// VRBrowser --
#endif
