...
#if defined(OS_TIZEN_TV_PRODUCT)
  if (blink::IsWebBrowser() && wp_tv_create()) {
    wp_tv_create_feature(FEATURE_NETWORK);
    wp_tv_create_feature(FEATURE_WEBGL);
  }
#endif
}
...
void RendererMainPlatformDelegate::PlatformUninitialize() {
  ...
  if (blink::IsWebBrowser()) {
    wp_tv_destroy_feature(FEATURE_NETWORK);
    wp_tv_destroy_feature(FEATURE_WEBGL);
    wp_tv_destroy();
  }
#endif
}
