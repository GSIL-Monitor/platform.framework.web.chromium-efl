...
class CONTENT_EXPORT RenderFrameImpl
 ...
 public:
  ...
  void PasswordFormWasVisible(const blink::WebFormElement& form,
                              const bool is_loading) override;
#endif
  ...
  void DidChangeSelection(bool is_empty_selection,
                          bool is_last_point_bounds = false) override;
  ...
  bool GetFileDecryptedDataBuffer(const blink::WebURL& url,
  ...
  void UnregisterProtocolHandler(const blink::WebString& scheme,
                                 const blink::WebURL& url) override;
  void registerContentHandler(const blink::WebString& mime_type,
                              const blink::WebURL& base_weburl,
                              const blink::WebURL& url,
                              const blink::WebString& title) override;
  void unregisterContentHandler(const blink::WebString& mime_type,
                                const blink::WebURL& base_weburl,
                                const blink::WebURL& url) override;
...
#if defined(OS_TIZEN_TV_PRODUCT)
  void CheckHLSSupport(const blink::WebString&, bool&);
#endif
  ...
  void SyncSelectionIfRequired(bool is_last_touch_point = false);
...
#if defined(USE_EFL)
  bool GetWrtParsedUrl(const blink::WebURL& url,
                       blink::WebURL& parsed_url) override;
#endif
 ...
 private:
  ...
  void OnMoveRangeSelectionExtent(const gfx::Point& point,
                                  bool is_last_touch_point = false);
...
#if defined(S_TERRACE_SUPPORT)
  void OnOpenWebLinkMatchInfos(int current_version);
#endif
#endif
