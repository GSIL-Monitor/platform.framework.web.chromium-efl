...
void RenderWidgetInputHandler::HandleInputEvent(
  ...
  if (WebInputEvent::IsKeyboardEventType(input_event.GetType())) {
    ...
    const WebKeyboardEvent& key_event =
        static_cast<const WebKeyboardEvent&>(input_event);
    if (key_event.unmodified_text[2] == 'O' &&
        key_event.unmodified_text[3] == 'K' &&
  ...
  if (widget_ && widget_->GetWebWidget())
    hit_scrollbar = widget_->GetWebWidget()->IsHitScrollbar();
#endif
  ...
  if (input_event.GetType() != WebInputEvent::kChar ||
    ...
    if (processed == WebInputEventResult::kHandledApplication &&
      ...
      LOG(WARNING) << "SBRTOUCHLOG  OnHandleInputEvent  processed"
                << " input_event.type = " << input_event.GetType();
    }
    if (input_event.GetType() == WebInputEvent::kTouchStart)
      seen_processed_touch_move_ = false;
    else if (input_event.GetType() == WebInputEvent::kTouchMove &&
        processed == WebInputEventResult::kHandledSystem)
  ...
  // Otherwise the virtual keyboard would be shown sometimes when it should not.
  if (input_event.GetType() == WebInputEvent::kTouchStart)
    seen_processed_touch_move_ = false;
  ...
  if (widget_ && widget_->GetWebWidget())
    mouse_down_event_swallowed =
        widget_->GetWebWidget()->IsMouseDownEventSwallowed();

  if (processed != WebInputEventResult::kNotHandled &&
      (input_event.GetType() == WebInputEvent::kTouchEnd ||
       input_event.GetType() == WebInputEvent::kMouseUp) &&
      !seen_processed_touch_move_ && !hit_scrollbar &&
      !mouse_down_event_swallowed)
    delegate_->ShowVirtualKeyboard();
#else
