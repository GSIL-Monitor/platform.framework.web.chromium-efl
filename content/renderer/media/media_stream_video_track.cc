...
class MediaStreamVideoTrack::FrameDeliverer
 ...
 public:
  ...
  void AddCallbackDecoderType(VideoSinkId id,
                              const VideoCaptureDecoderDataCB& callback);

  void AddCallbackSuspendingDecoder(const SuspendingDecoderCB& callback);
  ...
  void DeliverDecoderTypeOnIO(
      const scoped_refptr<media::MediaStreamVideoDecoderData>& decoder_data);
  void DeliverSuspendingDecoderOnIO(
      const scoped_refptr<media::SuspendingDecoder>& suspending_decoder);
 ...
 private:
  ...
  void AddCallbackDecoderTypeOnIO(VideoSinkId id,
                                  const VideoCaptureDecoderDataCB& callback);
  void AddCallbackSuspendingDecoderOnIO(const SuspendingDecoderCB& callback);
...
void MediaStreamVideoTrack::FrameDeliverer::AddCallbackDecoderType(
    VideoSinkId id,
    const VideoCaptureDecoderDataCB& callback) {
  DCHECK(main_render_thread_checker_.CalledOnValidThread());
  io_task_runner_->PostTask(
      FROM_HERE, base::Bind(&FrameDeliverer::AddCallbackDecoderTypeOnIO, this,
                            id, callback));
}

void MediaStreamVideoTrack::FrameDeliverer::AddCallbackSuspendingDecoder(
    const SuspendingDecoderCB& callback) {
  DCHECK(main_render_thread_checker_.CalledOnValidThread());
  io_task_runner_->PostTask(
      FROM_HERE, base::Bind(&FrameDeliverer::AddCallbackSuspendingDecoderOnIO,
                            this, callback));
}
...
void MediaStreamVideoTrack::FrameDeliverer::AddCallbackDecoderTypeOnIO(
    VideoSinkId id,
    const VideoCaptureDecoderDataCB& callback) {
  DCHECK(io_task_runner_->BelongsToCurrentThread());
  decoder_data_cb_ = callback;
}

void MediaStreamVideoTrack::FrameDeliverer::AddCallbackSuspendingDecoderOnIO(
    const SuspendingDecoderCB& callback) {
  DCHECK(io_task_runner_->BelongsToCurrentThread());
  suspending_decoder_cb_ = callback;
}
...
void MediaStreamVideoTrack::FrameDeliverer::DeliverDecoderTypeOnIO(
    const scoped_refptr<media::MediaStreamVideoDecoderData>& decoder_data) {
  DCHECK(io_task_runner_->BelongsToCurrentThread());
  decoder_data_cb_.Run(decoder_data);
}

void MediaStreamVideoTrack::FrameDeliverer::DeliverSuspendingDecoderOnIO(
    const scoped_refptr<media::SuspendingDecoder>& suspending_decoder) {
  DCHECK(io_task_runner_->BelongsToCurrentThread());
  suspending_decoder_cb_.Run(suspending_decoder);
}
...
MediaStreamVideoTrack::MediaStreamVideoTrack(
  ...
  source->AddTrack(
      ...
      base::Bind(&MediaStreamVideoTrack::FrameDeliverer::DeliverFrameOnIO,
                 frame_deliverer_),
      base::Bind(&MediaStreamVideoTrack::FrameDeliverer::DeliverDecoderTypeOnIO,
                 frame_deliverer_),
      base::Bind(
          &MediaStreamVideoTrack::FrameDeliverer::DeliverSuspendingDecoderOnIO,
          frame_deliverer_),
      callback);
}
...
void MediaStreamVideoTrack::AddSink(
  ...
  sinks_.push_back(sink);
  frame_deliverer_->AddCallback(sink, callback);
  frame_deliverer_->AddCallbackDecoderType(sink, decoder_data_cb);
  frame_deliverer_->AddCallbackSuspendingDecoder(suspending_decoder_cb);
  secure_tracker_.Add(sink, is_sink_secure);
  // Request source to deliver a frame because a new sink is added.
  if (!source_)
    return;
