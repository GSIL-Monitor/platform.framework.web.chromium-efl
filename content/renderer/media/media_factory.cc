...
#if defined(OS_TIZEN_TV_PRODUCT)
void MediaFactory::CheckHLSSupport(const blink::WebString& url, bool& support) {
  GetMediaPlayerManager()->CheckHLSSupport(url, support);
}
#endif
...
#if defined(TIZEN_MULTIMEDIA_SUPPORT)
  return CreateWebMediaPlayerEfl(web_frame, GetMediaPlayerManager(), client,
                                 encrypted_client,
                                 GetWebMediaPlayerDelegate()->AsWeakPtr(),
                                 std::move(params), is_video_hole);
#endif
...
#endif
      render_thread->GetIOTaskRunner(), compositor_task_runner,
...
RendererMediaPlayerManager* MediaFactory::GetMediaPlayerManager() {
  if (!media_player_manager_)
    media_player_manager_ = CreateRendererMediaPlayerManager(render_frame_);
  return media_player_manager_;
}
#endif
