...
webrtc::VideoDecoderData RTCVideoDecoder::GetDecoderData() const {
  return {webrtc::VideoDecoderType::TEXTURE, {}};
}
...
// To convert timestamp from 90KHz to ms.
const uint64_t kFactorKHztoMs = (1000 / 90);

static void RenderFrameFinished(std::unique_ptr<base::SharedMemory> shm) {
  // Let the shm fall out of scope
}

void RTCVideoDecoder::NotifyDecodeDone(base::SharedMemoryHandle buffer_handle,
                                       uint32_t bitstream_buffer_id,
                                       uint32_t buffer_size,
                                       gfx::Rect frame_size) {
  uint32_t timestamp = 0, width = 0, height = 0;
  gfx::Rect visible_rect;
  GetBufferData(bitstream_buffer_id, &timestamp, &visible_rect);
  width = frame_size.width();
  height = frame_size.height();
  gfx::Size size_frame(width, height);
  base::TimeDelta timestamp_ms = base::TimeDelta::FromInternalValue(
      base::checked_cast<uint64_t>(timestamp) * kFactorKHztoMs);
  std::unique_ptr<base::SharedMemory> shm(
      new base::SharedMemory(buffer_handle, true));
  if (!shm->Map(buffer_size)) {
    LOG(ERROR) << "RTCVideoDecoder::NotifyDecodeDone(): "
  ...
  scoped_refptr<media::VideoFrame> video_frame =
      media::VideoFrame::WrapExternalSharedMemory(
          media::PIXEL_FORMAT_I420, size_frame, gfx::Rect(size_frame),
          size_frame, shm_memory, buffer_size, buffer_handle, 0, timestamp_ms);
  if (!video_frame.get()) {
    LOG(ERROR) << "RTCVideoDecoder::NotifyDecodeDone(): "
  ...
  video_frame->AddDestructionObserver(
      base::Bind(content::RenderFrameFinished, base::Passed(&shm)));

  // Create a WebRTC video frame.
  webrtc::VideoFrame decoded_image(
      new rtc::RefCountedObject<WebRtcVideoFrameAdapter>(
          video_frame, WebRtcVideoFrameAdapter::CopyTextureFrameCallback()),
  ...
  // Invoke decode callback. WebRTC expects no callback after Reset or Release.
  {
    ...
    if (IsBufferAfterReset(bitstream_buffer_id, reset_bitstream_buffer_id_)) {
