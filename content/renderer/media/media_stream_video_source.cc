  ...
  switch (state_) {
    case NEW: {
      ...
      StartSourceImpl(
          ...
          decoder_data_cb_, suspending_decoder_cb);
      break;
    }
    case STARTING:
...
void MediaStreamVideoSource::StartSourceImpl(
    const VideoCaptureDeliverFrameCB& frame_callback,
    const VideoCaptureDecoderDataCB&,
    const SuspendingDecoderCB&) {
  StartSourceImpl(frame_callback);
}
