...
class MediaStreamVideoRendererSink::FrameDeliverer {
 public:
  FrameDeliverer(const RepaintCB& repaint_cb,
                 const DecoderDataCB& decoder_data_cb,
  ...
  base::WeakPtrFactory<FrameDeliverer>& GetWeakFactory() {
 ...
 private:
  ...
  const DecoderDataCB decoder_data_cb_;
  const SuspendingDecoderCB suspending_decoder_cb_;
  State state_;
...
void MediaStreamVideoRendererSink::Start() {
  ...
  frame_deliverer_.reset(new MediaStreamVideoRendererSink::FrameDeliverer(
      repaint_cb_, decoder_data_cb_, suspending_decoder_cb_, media_task_runner_,
  ...
  MediaStreamVideoSink::ConnectToTrack(
      ...
      base::Bind(&FrameDeliverer::OnDecoderTypeAvailable,
                 frame_deliverer_->GetWeakFactory().GetWeakPtr()),
      base::Bind(&FrameDeliverer::OnSuspendingDecoder,
                 frame_deliverer_->GetWeakFactory().GetWeakPtr()),
      true);
...
void MediaStreamVideoRendererSink::OnDecoderType(
    const scoped_refptr<media::MediaStreamVideoDecoderData>& decoder_data) {
  DCHECK(task_runner_->BelongsToCurrentThread());
  decoder_data_cb_.Run(decoder_data);
}

void MediaStreamVideoRendererSink::OnSuspendingDecoder(
    const scoped_refptr<media::SuspendingDecoder>& suspending_decoder) {
  DCHECK(task_runner_->BelongsToCurrentThread());
  suspending_decoder_cb_.Run(suspending_decoder);
}
