...
void CacheStorageContextImpl::UpdatePath(
    const base::FilePath& user_data_directory) {
  if (BrowserThread::CurrentlyOn(BrowserThread::IO)) {
    is_incognito_ = user_data_directory.empty();
    if (cache_manager_.get())
      cache_manager_->UpdatePath(user_data_directory);

    return;
  }

  BrowserThread::PostTask(BrowserThread::IO, FROM_HERE,
                          base::Bind(&CacheStorageContextImpl::UpdatePath, this,
                                     user_data_directory));
}
#endif
