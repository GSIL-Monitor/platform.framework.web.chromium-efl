...
bool RenderWidgetHostImpl::OnMessageReceived(const IPC::Message &msg) {
  ...
  IPC_BEGIN_MESSAGE_MAP(RenderWidgetHostImpl, msg)
    ...
    IPC_MESSAGE_HANDLER(ViewHostMsg_Snapshot, OnSoftwareBitmapReceived)
...
#if defined(OS_TIZEN_TV_PRODUCT)
  bool enter_fullscreen = true, exit_fullscreen = true;

  // ReSize procedure was called 2 times when browser enter/exit fullscreen
  // mode. One is called before the browser UI(top control) fades out. One is
  // called when webview becames fullscreen. It causes performance issue.
  // The first one is not necessary. Here we filter the first one.
  if (content::IsWebBrowser()) {
    enter_fullscreen = resize_params->is_fullscreen_granted &&
                       (resize_params->new_size ==
                        resize_params->screen_info.available_rect.size());
    exit_fullscreen = !resize_params->is_fullscreen_granted &&
                      (resize_params->new_size !=
                       resize_params->screen_info.available_rect.size());
  }
#endif
...
void RenderWidgetHostImpl::WasResized() {
  ...
  if (Send(new ViewMsg_Resize(routing_id_, *params))) {
    ...
    // Several resizes can happen while hidden, and the resizes happen
    // only after WasShown() is called - resulting in unnecessary resizes
    // We can set to the last size while hidden without waiting for ack.
    resize_ack_pending_ &= !is_hidden_;
...
void RenderWidgetHostImpl::ForwardKeyboardEventWithCommands(
  ...
  LOG(INFO) << "type:" << key_event.GetType()
            << ", windowsKeyCode:" << key_event.windows_key_code;
  if (owner_delegate_ &&
...
void RenderWidgetHostImpl::ClearDisplayedGraphics() {
  ...
  if (view_) {
    ...
    LOG(INFO) << "LoaderLog  RenderWidgetHostImpl::ClearDisplayedGraphics";
  }
}
...
void RenderWidgetHostImpl::OnResizeOrRepaintACK(
  ...
  if (is_resize_ack) {
    // Disabled DCHECK because of not waiting for ack when hidden.
    //DCHECK(!g_check_for_pending_resize_ack || resize_ack_pending_);
    resize_ack_pending_ = false;
    if (view_)
      view_->DidReceiveResizeAck(current_size_);
  }
...
void RenderWidgetHostImpl::NotifyScrollbarState(bool disable, bool visible) {
  if (view_)
    view_->NotifyScrollbarState(disable, visible);
}
#endif
...
void RenderWidgetHostImpl::OnSoftwareBitmapReceived(const SkBitmap& bitmap,
