...
bool RenderViewHostImpl::CreateRenderView(
  ...
  params->display_width = display_info.GetDisplayWidth();
  params->display_height = display_info.GetDisplayHeight();
  params->dip_scale = display_info.GetDIPScale();
  params->rotation_degrees = display_info.GetRotationDegrees();
#endif
...
WebPreferences RenderViewHostImpl::ComputeWebkitPrefs() {
  ...
  prefs.daydream_mode_enabled = base::VrModeUtil::isDayDreamModeEnabled();
  prefs.gear_vr_mode_enabled = base::VrModeUtil::isGearVrModeEnabled();
  // FIXME: m60_3112 bringup, needs to be checked by author
  /*prefs.user_gesture_required_for_media_playback =
      !(base::VrModeUtil::isVrModeEnabled() ||
        base::SamsungConfig::IsDexEnabled());*/
#endif
...
// crbug.com/758042
  ...
  // Adding enable_acclerated_2d_canvas feature after UpdateRendererWebPrefs
  // as enabling switch will forcefully enable the feature.
  prefs.accelerated_2d_canvas_enabled =
      GpuProcessHost::gpu_enabled() &&
      command_line.HasSwitch(switches::kForceAccelerated2dCanvas);
#endif
  ...
  // Disallow media subtitle notification by default.
  prefs.media_subtitle_notification_enabled =
      command_line.HasSwitch(switches::kEnableMediaSubtitleNotification);
#endif
  ...
  if (command_line.HasSwitch(switches::kMaxRefreshRate)) {
    ...
    base::StringToInt(string_value, &refresh_rate);
...
bool RenderViewHostImpl::OnMessageReceived(const IPC::Message& msg) {
  ...
  IPC_BEGIN_MESSAGE_MAP(RenderViewHostImpl, msg)
...
#if defined(S_TERRACE_SUPPORT)
    IPC_MESSAGE_HANDLER(ViewHostMsg_OnReceiveCandidateImage,
                        OnReceiveCandidateImage)
#endif
#endif
    ...
    IPC_MESSAGE_HANDLER(ViewHostMsg_OnNumberOfBlockedElements,
                        OnNumberOfBlockedElements)
...
void RenderViewHostImpl::GetSelectionBitmap() {
  Send(new ViewMsg_GetSelectionBitmap(GetRoutingID()));
}
...
void RenderViewHostImpl::HandleSelectionDragDrop(int x,
                                                 int y,
                                                 base::string16& text,
                                                 base::string16& html_text,
                                                 bool is_drop) {
  Send(new ViewMsg_HandleSelectionDragDrop(GetRoutingID(), x, y, text,
                                           html_text, is_drop));
}
...
void RenderViewHostImpl::OnRecognizeArticleResult(
    bool is_article, const base::string16& page_url) {
  if (GetWidget()->GetView())
    GetWidget()->GetView()->OnRecognizeArticleResult(is_article, page_url);
}

void RenderViewHostImpl::OnNumberOfBlockedElements(int count) {
  if (GetWidget()->GetView())
    GetWidget()->GetView()->OnNumberOfBlockedElements(count);
}

void RenderViewHostImpl::MoveFocusToPrevNextInput(bool is_next) {
#if defined(OS_ANDROID)
  Send(new ViewMsg_MoveFocusToPrevNextInputElement(GetRoutingID(), is_next));
#endif
}
...
void RenderViewHostImpl::OnReceiveBitmapFromCache(const SkBitmap& bitmap,
  ...
  if (!delegate_)
    return;
  DCHECK(delegate_->GetAsWebContents());
  if (!delegate_->GetAsWebContents())
    return;
  WebContentsImpl* webContentsImpl =
      static_cast<WebContentsImpl*>(delegate_->GetAsWebContents());
  DCHECK(webContentsImpl->GetDelegate());
  if (!webContentsImpl->GetDelegate())
    return;
  webContentsImpl->GetDelegate()->ReceivedBitmapFromCache(bitmap, request_id);
}

#if defined(OS_ANDROID)
void RenderViewHostImpl::OnSelectedMarkupWithStartContentRect(
    const base::string16& markup, const gfx::Rect& start_rect) {
  RenderViewHostDelegateView* view = delegate_->GetDelegateView();
  if (!view)
    return;
  view->SelectedMarkupWithStartContentRect(markup, start_rect);
}
...
#if defined(S_TERRACE_SUPPORT)
void RenderViewHostImpl::OnReceiveCandidateImage(const SkBitmap& bitmap) {
  DCHECK(delegate_);
  if (!delegate_)
    return;
  DCHECK(delegate_->GetAsWebContents());
  ...
  if (!webContentsImpl->GetDelegate())
    return;
  webContentsImpl->GetDelegate()->ReceivedCandidateImage(bitmap);
}
#endif
#endif
