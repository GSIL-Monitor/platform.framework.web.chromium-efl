...
// If window width is bigger than 960(FHD landscape width 1920 / density 2),
// consider the device is high resolution but low density. In this case treat
// a page as mobile-optimized only when the contents width and window width
// are same.
const float kHighResolutionLogDensityCriterion = 960.f;
#endif
...
#if defined(S_TERRACE_SUPPORT)
  if (window_width_dip > kHighResolutionLogDensityCriterion) {
    return std::abs(window_width_dip - content_width_css)
