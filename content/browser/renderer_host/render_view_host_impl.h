...
class CONTENT_EXPORT RenderViewHostImpl : public RenderViewHost,
 ...
 public:
  ...
  void OnReceiveBitmapFromCache(const SkBitmap& bitmap,
                                const int request_id) override;

  void OnWebkitPreferencesChanged() override;
  void SelectWordAroundCaret() override;

#if defined(OS_ANDROID)
  void GetSelectionMarkupWithBounds() override;
  // FIXME: Commented the api during dev/m60_3112 rebaseline
  // Author needs to re-visit the implementation.
  // void OnHoverHitTestResult(const blink::WebHoverContentType contentType) override;
  ...
  void OnReceiveCandidateImage(const SkBitmap& bitmap);
#endif
#endif
  ...
  void RetrieveWebState();
  void RestoreWebState(const std::string& web_state);
  void HandleSelectionDragDrop(int x,
                               int y,
                               base::string16& text,
                               base::string16& html_text,
                               bool is_drop);
  void GetSelectionMarkup();
 ...
 protected:
  ...
  void OnSelectedMarkup(const base::string16& markup) override;
  void OnSelectedBitmap(const SkBitmap& bitmap) override;
  void OnRecognizeArticleResult(
      bool is_article, const base::string16& page_url);
  void OnNumberOfBlockedElements(int count);
  void MoveFocusToPrevNextInput(bool is_next) override;
