...
#endif
#if !defined(USE_EFL) || defined(TIZEN_WEB_SPEECH_RECOGNITION)
...
#if defined(TIZEN_MULTIMEDIA_SUPPORT)
  browser_demuxer_efl_ = CreateBrowserDemuxerEfl();
  AddFilter(browser_demuxer_efl_.get());

#if defined(TIZEN_TBM_SUPPORT)
  browser_mediapacket_manager_ = CreateBrowserMediapacketManager();
  AddFilter(browser_mediapacket_manager_.get());
#endif
#endif
...
void RenderProcessHostImpl::AppendRendererCommandLine(
  ...
  command_line->AppendSwitchASCII(switches::kRendererClientId,
                                  std::to_string(GetID()));
#if defined(S_TERRACE_SUPPORT)
  if (browser_command_line.HasSwitch(switches::kCscFeatureCustomErrorPage)) {
    command_line->AppendSwitchASCII(switches::kCscFeatureCustomErrorPage,
                                    browser_command_line.GetSwitchValueASCII(
                                        switches::kCscFeatureCustomErrorPage));
  }
  if (browser_command_line.HasSwitch(switches::kUseGEDFeature)) {
    command_line->AppendSwitch(switches::kUseGEDFeature);
  }
#endif
}
...
void RenderProcessHostImpl::PropagateBrowserCommandLineToRenderer(
  ...
  static const char* const kSwitchNames[] = {
    ...
    switches::kUseSamsungMediaControls,
    ...
    cc::switches::kEnableDirectCanvas,
...
#if defined(S_TERRACE_SUPPORT)
// static
void RenderProcessHost::RunNotifyRendererDiedOnUIBrowserThread(int render_process_id) {
  BrowserThread::PostTask(
      BrowserThread::UI,
      FROM_HERE,
      base::Bind(
          &RenderProcessHostImpl::NotifyRendererDiedWithProcessID,
          render_process_id));
}

// static
void RenderProcessHost::NotifyRendererDiedWithProcessID(int render_process_id) {
  int renderer_id = 0;
  RenderProcessHost::iterator it = RenderProcessHost::AllHostsIterator();
  while (!it.IsAtEnd()) {
    if (base::GetProcId(it.GetCurrentValue()->GetHandle()) == render_process_id) {
      renderer_id = it.GetCurrentValue()->GetID();
      break;
    }
    it.Advance();
  }
  LOG(ERROR) << "[Renderer][Exit] ChildProcessLauncher::OnChildProcessDied "
               "called through JNI pid = " << render_process_id
            << ", renderer_id =" << renderer_id;
  if (!renderer_id)
    return;
  RenderProcessHost* process_host = RenderProcessHost::FromID(renderer_id);
  if (process_host) {
    process_host->NotifyRendererDied();
    TerraceContentBridge::Get().OnChildProcessDied(renderer_id);
  }
}
#endif
...
#if defined(S_TERRACE_SUPPORT)
void RenderProcessHostImpl::NotifyRendererDied() {
  ProcessDied(true /* already_dead */, nullptr);
}
#endif
