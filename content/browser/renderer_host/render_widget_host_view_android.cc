...
#include "swebview/content/native/browser/android/swim_content_view_core_impl.h"
#include "swebview/content/native/browser/android/sbr/sbr_content_view_core_impl.h"
#endif
namespace content {
...
RenderWidgetHostViewAndroid::RenderWidgetHostViewAndroid(
    ...
    : host_(widget_host),
      ...
      is_spen_event_(false),
      is_dex_enabled_(false),
      current_content_offset_(gfx::Vector2dF()),
...
#if defined(S_TERRACE_SUPPORT)
    IPC_MESSAGE_HANDLER(ViewHostMsg_MultiSelection_ACK,
                        OnMultiSelectionAck)
#endif
...
bool RenderWidgetHostViewAndroid::OnTouchEvent(
  ...
  if (!host_ || !host_->delegate()) {
    LOG(ERROR) << "SBRTOUCHLOG  RWHVA::OnTouchEvent, host_ is null";
    return false;
  }
  ...
  if (view_.GetLayer() && 
      web_event.GetType() == blink::WebInputEvent::kTouchMove &&
...
void RenderWidgetHostViewAndroid::CopyFromSurface(
  ...
  if (src_subrect_in_pixel.width() % 2 != 0) {
    src_subrect_in_pixel.set_width(src_subrect_in_pixel.width() - 1);
  }
  if (dst_size_in_pixel.width() % 2 != 0) {
    dst_size_in_pixel.set_width(dst_size_in_pixel.width() - 1);
  }

#if defined(S_TERRACE_SUPPORT)
  int dst_size_mod = dst_size_in_pixel.width() % 64;
  int src_subrect_mod = src_subrect_in_pixel.width() % 64;
  if (dst_size_mod != 0) {
    dst_size_in_pixel.set_width(
        dst_size_in_pixel.width() + 64 - dst_size_mod);
  }
  if (src_subrect_mod != 0) {
    src_subrect_in_pixel.set_width(
        src_subrect_in_pixel.width() + 64 - src_subrect_mod);
  }
#endif
...
void RenderWidgetHostViewAndroid::SubmitCompositorFrame(
  ...
  current_content_offset_ = gfx::Vector2dF(0.0f,
      frame.metadata.top_controls_height
      * frame.metadata.top_controls_shown_ratio);
  current_content_offset_.Scale(frame.metadata.device_scale_factor);
  ...
  bool top_controls_bitmap_composition_enabled =
      base::CommandLine::ForCurrentProcess()->HasSwitch(
          cc::switches::kEnableTopControlsUiComposition);

  // Offset root layer position if top controls ui composition enabled.
  if (top_controls_bitmap_composition_enabled) {
    if (content_view_core_ &&
        content_view_core_->GetViewAndroid()->GetLayer()) {
      content_view_core_->GetViewAndroid()->GetLayer()->SetPosition(
          gfx::PointAtOffsetFromOrigin(current_content_offset_));
    } else if (view_.GetLayer() && view_.GetLayer()->RootLayer()) {
      cc::Layer* root_layer = view_.GetLayer()->RootLayer();
      if (root_layer)
        root_layer->SetPosition(
            gfx::PointAtOffsetFromOrigin(current_content_offset_));
    }
  }
...
void RenderWidgetHostViewAndroid::OnFrameMetadataUpdated(
  ...
  if (top_changed) {
    // To make Terrace.onOffsetsForFullscreenChanged API compatible with 5.x
    // We need to prevent duplicate callback call hence comment out here
    // Please look at CVC#updateFrameInfo for the replacement
...
void RenderWidgetHostViewAndroid::HideInternal() {
  ...
  if (stop_observing_root_window) {
    ...
    LOG(WARNING) << "TAB-STATUS observation kept while hiding";
  }
#endif
...
#if defined(S_TERRACE_SUPPORT)
  if (is_showing_)
    LOG(ERROR) << "TAB-STATUS it is still showing";
#endif
...
void RenderWidgetHostViewAndroid::HandleSelectionDrag() {
#if defined(S_NATIVE_SUPPORT)
  if (selection_controller_)
    selection_controller_->ResetLongPress();
#endif
}
...
#if defined(S_TERRACE_SUPPORT)
void RenderWidgetHostViewAndroid::NotifyScrollbarState(
    bool disable, bool visible) {
  return;
}
#endif
...
void RenderWidgetHostViewAndroid::InitSelectionController() {
  touch_selection_controller_ = CreateSelectionController(this, content_view_core_);
}
...
int RenderWidgetHostViewAndroid::GetStartHandleStateForTest() const {
#if defined(S_NATIVE_SUPPORT)
  if (selection_controller_)
    return selection_controller_->GetStartHandleStateForTest();
#endif
  return 3;
}

int RenderWidgetHostViewAndroid::GetEndHandleStateForTest() const {
#if defined(S_NATIVE_SUPPORT)
  if (selection_controller_)
    return selection_controller_->GetEndHandleStateForTest();
#endif
  return 3;
}

void RenderWidgetHostViewAndroid::SetEdgeEffectProperties(
    const SkColor theme_color,
    const SkScalar edge_intensity_factor) {
  if (overscroll_controller_)
    overscroll_controller_->SetEdgeEffectProperties(theme_color,
