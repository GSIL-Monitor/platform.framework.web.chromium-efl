...
void GestureEventQueue::QueueAndForwardIfNecessary(
  ...
  // Sometimes, gesture event ack does not come(P151202-06533)
  // It may result in a touch lockup issue.
  // Check whether lots of gesture events are queued or not.
  else if(coalesced_gesture_events_.size() > 5){
    GestureEventWithLatencyInfo& first_gesture_event =
      coalesced_gesture_events_.front();
    LOG(WARNING)<< "SBRTOUCHLOG  Awaiting a gesture ACK"
      << " first_gesture_event = " << first_gesture_event.event.GetType()
      << " coalesced_gesture_size = " << coalesced_gesture_events_.size();
  }
#endif
}
