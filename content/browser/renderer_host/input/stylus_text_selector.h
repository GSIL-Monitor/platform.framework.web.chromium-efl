...
class CONTENT_EXPORT StylusTextSelector : public ui::SimpleGestureListener {
 ...
 protected:
  // Getters and setters for Selection triggered
  bool IsTextSelectionTriggered();
  void SetTextSelectionTriggered(bool triggered);

  // Getters and setters for Secondary button state
 public:
 ...
 protected:
  ...
  float GetAnchorX();
  float GetAnchorY();
  void SetAnchorX(float x_pos);
  void SetAnchorY(float y_pos);

  bool ShouldStartStylusTextSelection(const ui::MotionEvent& event);
