...
int32_t PepperTCPSocketMessageFilter::OnMsgBind(
  ...
  if (version_ != ppapi::TCP_SOCKET_VERSION_1_1_OR_ABOVE) {
    LOG(ERROR) << "Supported only by PPB_TCPSocket v1.1 or above";
    NOTREACHED();
  ...
  if (!pepper_socket_utils::CanUseSocketAPIs(external_plugin_,
                                             ...
                                             render_process_id_,
                                             render_frame_id_)) {
    LOG(ERROR) << "Cannot use Socket APIs";
    return PP_ERROR_NOACCESS;
  }
...
int32_t PepperTCPSocketMessageFilter::OnMsgConnect(
  ...
  if (!IsPrivateAPI()) {
    NOTREACHED();
    LOG(ERROR) << "Supported only by PPB_TCPSocket_Private";
    return PP_ERROR_NOACCESS;
  }
  ...
  if (!render_process_host) {
    LOG(ERROR) << "Cannot create RenderProcessHost from process id "
               << render_process_id_;
    return PP_ERROR_FAILED;
  }
...
int32_t PepperTCPSocketMessageFilter::OnMsgConnectWithNetAddress(
  ...
  if (!pepper_socket_utils::CanUseSocketAPIs(external_plugin_,
                                             ...
                                             render_process_id_,
                                             render_frame_id_)) {
    LOG(ERROR) << "Insufficient permissions: Cannot connect with NetAddress";
    return PP_ERROR_NOACCESS;
  }
...
int32_t PepperTCPSocketMessageFilter::OnMsgSSLHandshake(
  ...
  if (!state_.IsValidTransition(TCPSocketState::SSL_CONNECT) ||
    ...
    LOG(ERROR) << "Socket not connected or pending read or write";
    return PP_ERROR_FAILED;
  }

  // TODO(raymes,rsleevi): Use trusted/untrusted certificates when connecting.
  net::IPEndPoint peer_address;
  auto error = socket_->GetPeerAddress(&peer_address);
  if (error != net::OK) {
    LOG(ERROR) << "Socket peer address: " << static_cast<int>(error);
    return PP_ERROR_FAILED;
  }
...
int32_t PepperTCPSocketMessageFilter::OnMsgRead(
  ...
  if (!state_.IsConnected() || end_of_file_reached_) {
    LOG(ERROR) << "Not connected or reached EOF";
    return PP_ERROR_FAILED;
  }
  if (read_buffer_.get()) {
    LOG(ERROR) << "Invalid read buffer";
    return PP_ERROR_INPROGRESS;
  }
  if (bytes_to_read <= 0 ||
      bytes_to_read > TCPSocketResourceConstants::kMaxReadSize) {
    LOG(ERROR) << "Incorrect number of bytes to read - " << bytes_to_read
               << " Should be <= " << TCPSocketResourceConstants::kMaxReadSize
               << " and > 0";
...
int32_t PepperTCPSocketMessageFilter::OnMsgWrite(
  ...
  if (data_size == 0 ||
    ...
    LOG(ERROR) << "Invalid data size - " << data_size << " should be <= "
               << static_cast<size_t>(TCPSocketResourceConstants::kMaxWriteSize)
               << " and != 0";
    return PP_ERROR_BADARGUMENT;
  }
...
int32_t PepperTCPSocketMessageFilter::OnMsgAccept(
  ...
  if (pending_accept_) {
    LOG(WARNING) << "Pending accept";
    return PP_ERROR_INPROGRESS;
  }
  if (state_.state() != TCPSocketState::LISTENING) {
    LOG(ERROR) << "Invalid socket state: " << static_cast<int>(state_.state());
    return PP_ERROR_FAILED;
  }
  ...
  switch (name) {
    case PP_TCPSOCKET_OPTION_NO_DELAY: {
      ...
      if (!value.GetBool(&boolean_value)) {
        LOG(ERROR) << "Invalid socket option";
        return PP_ERROR_BADARGUMENT;
      }
    ...
    case PP_TCPSOCKET_OPTION_SEND_BUFFER_SIZE: {
      ...
      if (!value.GetInt32(&integer_value) || integer_value <= 0 ||
        ...
        LOG(ERROR) << "Invalid send buffer size value - " << integer_value
                   << " should be <= "
                   << TCPSocketResourceConstants::kMaxSendBufferSize
                   << " and > 0";
        return PP_ERROR_BADARGUMENT;
      }
    ...
    case PP_TCPSOCKET_OPTION_RECV_BUFFER_SIZE: {
      ...
      if (!value.GetInt32(&integer_value) || integer_value <= 0 ||
        ...
        LOG(ERROR) << "Invalid receive buffer size - " << integer_value
                   << " should be <= "
                   << TCPSocketResourceConstants::kMaxReceiveBufferSize
                   << " and > 0";
        return PP_ERROR_BADARGUMENT;
      }
...
void PepperTCPSocketMessageFilter::OnReadCompleted(
  ...
  if (net_result > 0) {
    SendReadReply(context, PP_OK,
                  ppapi::DataView(read_buffer_->data(),
                                  static_cast<int32_t>(net_result)));
  } else if (net_result == 0) {
    end_of_file_reached_ = true;
    SendReadReply(context, PP_OK, ppapi::DataView());
  } else {
...
void PepperTCPSocketMessageFilter::SendReadError(
    const ppapi::host::ReplyMessageContext& context,
    int32_t pp_error) {
  SendReadReply(context, pp_error, ppapi::DataView());
}
