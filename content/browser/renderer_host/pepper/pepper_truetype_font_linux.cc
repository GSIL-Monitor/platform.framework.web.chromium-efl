...
int32_t PepperTrueTypeFontLinux::GetTableTags(std::vector<uint32_t>* tags) {
  if (!fd_.is_valid()) {
    LOG(ERROR) << "Invalid filedescriptor";
  ...
  if (!GetFontTable(fd_.get(), 0 /* tag */, 4 /* offset */,
                    reinterpret_cast<uint8_t*>(&num_tables_buf),
                    &output_length)) {
    LOG(ERROR) << "Cannot get Font Table";
    return PP_ERROR_FAILED;
  }
  ...
  if (!GetFontTable(fd_.get(), 0 /* tag */, kFontHeaderSize /* offset */,
                    table_entries.get(), &output_length)) {
    LOG(ERROR) << "Cannot get Font Table";
    return PP_ERROR_FAILED;
  }
...
int32_t PepperTrueTypeFontLinux::GetTable(uint32_t table_tag,
  ...
  if (!GetFontTable(fd_.get(), table_tag, offset,
                    reinterpret_cast<uint8_t*>(&(*data)[0]), &table_size)) {
    LOG(ERROR) << "Cannot get Font Table";
    return PP_ERROR_FAILED;
  }
