...
PepperUDPSocketMessageFilter::OverrideTaskRunnerForMessage(
  ...
  switch (message.type()) {
    ...
    case PpapiHostMsg_UDPSocket_GetOption::ID:
#endif
...
int32_t PepperUDPSocketMessageFilter::OnResourceMessageReceived(
  ...
  PPAPI_BEGIN_MESSAGE_MAP(PepperUDPSocketMessageFilter, msg)
    ...
    PPAPI_DISPATCH_HOST_RESOURCE_CALL(PpapiHostMsg_UDPSocket_LeaveGroup,
                                      OnMsgLeaveGroup)
#if defined(TIZEN_PEPPER_EXTENSIONS)
    PPAPI_DISPATCH_HOST_RESOURCE_CALL(PpapiHostMsg_UDPSocket_GetOption,
                                      OnMsgGetOption)
#endif
  ...
  switch (name) {
    case PP_UDPSOCKET_OPTION_ADDRESS_REUSE: {
      if (socket_.get()) {
        ...
        LOG(ERROR) << "Socket already bound";
        return PP_ERROR_FAILED;
      }
    ...
    case PP_UDPSOCKET_OPTION_SEND_BUFFER_SIZE: {
      ...
      if (!value.GetInt32(&integer_value) || integer_value <= 0 ||
        ...
        LOG(ERROR) << "Wrong send buffer size: " << integer_value;
        return PP_ERROR_BADARGUMENT;
      }
    ...
    case PP_UDPSOCKET_OPTION_RECV_BUFFER_SIZE: {
      ...
      if (!value.GetInt32(&integer_value) || integer_value <= 0 ||
        ...
        LOG(ERROR) << "Wrong receive buffer size: " << integer_value;
        return PP_ERROR_BADARGUMENT;
      }
    ...
    case PP_UDPSOCKET_OPTION_MULTICAST_TTL: {
      ...
      if (!value.GetInt32(&integer_value) || integer_value < 0 ||
          integer_value > 255) {
        LOG(ERROR) << "Wrong TTL Value: " << integer_value;
        return PP_ERROR_BADARGUMENT;
      }
    ...
    default: {
      LOG(ERROR) << "Unresolved socket option";
      NOTREACHED();
      return PP_ERROR_BADARGUMENT;
    }
  }
...
int32_t PepperUDPSocketMessageFilter::OnMsgJoinGroup(
  ...
  if (!NetAddressPrivateImpl::NetAddressToIPEndPoint(addr, &group, &port)) {
    LOG(ERROR) << "Improperly formed address";
...
void PepperUDPSocketMessageFilter::DoBind(
  ...
  if (!(socket_options_ & SOCKET_OPTION_MULTICAST_LOOP)) {
    ...
    int net_result = socket->SetMulticastLoopbackMode(false);
    if (net_result != net::OK) {
      SendBindError(context, NetErrorToPepperError(net_result));
      return;
    }
...
#if defined(TIZEN_PEPPER_EXTENSIONS)
int32_t PepperUDPSocketMessageFilter::OnMsgGetOption(
    const ppapi::host::HostMessageContext* context,
  ...
  if (!socket_.get()) {
    LOG(ERROR) << "Socket is not bound";
    return PP_ERROR_FAILED;
  }

  ppapi::SocketOptionData return_value;
  int32_t pp_result = PP_ERROR_FAILED;
  switch (name) {
    case PP_UDPSOCKET_OPTION_ADDRESS_REUSE: {
      bool boolean_value = false;
      pp_result = NetErrorToPepperError(
          socket_->GetAllowAddressReuse(&boolean_value));
      return_value.SetBool(boolean_value);
      break;
    }
    case PP_UDPSOCKET_OPTION_BROADCAST: {
      bool boolean_value = false;
      pp_result = NetErrorToPepperError(
          socket_->GetBroadcast(&boolean_value));
      return_value.SetBool(boolean_value);
      break;
    }
    case PP_UDPSOCKET_OPTION_SEND_BUFFER_SIZE: {
      int32_t int_value = 0;
      pp_result = NetErrorToPepperError(
          socket_->GetSendBufferSize(&int_value));
      return_value.SetInt32(int_value);
      break;
    }
    case PP_UDPSOCKET_OPTION_RECV_BUFFER_SIZE: {
      int32_t int_value = 0;
      pp_result = NetErrorToPepperError(
          socket_->GetReceiveBufferSize(&int_value));
      return_value.SetInt32(int_value);
      break;
    }
    case PP_UDPSOCKET_OPTION_MULTICAST_LOOP: {
      bool boolean_value = false;
      pp_result = NetErrorToPepperError(
          socket_->GetMulticastLoopbackMode(&boolean_value));
      return_value.SetBool(boolean_value);
      break;
    }
    case PP_UDPSOCKET_OPTION_MULTICAST_TTL: {
      int32_t int_value = 0;
      pp_result = NetErrorToPepperError(
          socket_->GetMulticastTimeToLive(&int_value));
      return_value.SetInt32(int_value);
      break;
    }
    default: {
  ...
  SendGetOptionReply(context->MakeReplyMessageContext(), pp_result,
                     return_value);
  return PP_OK_COMPLETIONPENDING;
}

void PepperUDPSocketMessageFilter::SendGetOptionReply(
    const ppapi::host::ReplyMessageContext& context,
    int32_t result,
    const ppapi::SocketOptionData& value) {
  ppapi::host::ReplyMessageContext reply_context(context);
  reply_context.params.set_result(result);
  SendReply(reply_context, PpapiPluginMsg_UDPSocket_GetOptionReply(value));
}
#endif
