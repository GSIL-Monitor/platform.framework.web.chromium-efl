...
WebContentsImpl::WebContentsImpl(BrowserContext* browser_context)
  ...
  media_web_contents_observer_.reset(
      TerraceContentBridge::Get().CreateMediaWebContentsObserver(this));
#else
...
bool WebContentsImpl::OnMessageReceived(RenderViewHostImpl* render_view_host,
  ...
  IPC_BEGIN_MESSAGE_MAP_WITH_PARAM(WebContentsImpl, message, render_view_host)
    ...
    IPC_MESSAGE_HANDLER(ViewHostMsg_NotifyUrlForPlayingVideo,
                        OnNotifyUrlForPlayingVideo)
...
bool WebContentsImpl::OnMessageReceived(RenderFrameHostImpl* render_frame_host,
  ...
  IPC_BEGIN_MESSAGE_MAP_WITH_PARAM(WebContentsImpl, message, render_frame_host)
    ...
    IPC_MESSAGE_HANDLER(FrameHostMsg_RegisterContentHandler,
                        OnRegisterContentHandler)
    IPC_MESSAGE_HANDLER(FrameHostMsg_UnregisterContentHandler,
                        OnUnregisterContentHandler)
...
#if defined(S_TERRACE_SUPPORT)
    IPC_MESSAGE_HANDLER(FrameHostMsg_OpenWebLinkMatchInfos_Reply,
                        OnOpenWebLinkMatchInfosReply)
#endif
#endif
#if defined(OS_ANDROID) || defined(USE_EFL)
#endif
    // SBrowser Extension : Content Recommender
    IPC_MESSAGE_HANDLER(FrameHostMsg_DidGetMetadata, OnDidGetMetadata)
    IPC_MESSAGE_UNHANDLED(handled = false)
  IPC_END_MESSAGE_MAP()
...
#if defined(S_TERRACE_SUPPORT)
  if (capturer_count_ != 0) {
    LOG(WARNING) << "TAB-STATUS not hidden"
      << " due to " << capturer_count_ << " capturer(s)";
  }
#endif
...
void WebContentsImpl::ExitFullscreenMode(bool will_cause_resize) {
...
#if defined(OS_ANDROID) && !defined(S_TERRACE_SUPPORT)
  ...
  if (media_web_contents_observer_)
    media_web_contents_observer_->OnExitFullscreenMode();
#endif
...
void WebContentsImpl::OnNotifyUrlForPlayingVideo(const GURL& url) {
  if (delegate_)
    delegate_->NotifyUrlForPlayingVideo(url);
}
...
void WebContentsImpl::PopulateSoftwareBitmapFinished(const SkBitmap& bitmap,
                                                     const int request_id) {
  if (delegate_)
    delegate_->PopulateSoftwareBitmapFinished(bitmap, request_id);
}
...
void WebContentsImpl::MoveRangeSelectionExtent(const gfx::Point& extent,
  ...
  focused_frame->GetFrameInputHandler()->MoveRangeSelectionExtent(extent, is_last_touch_point);
}
...
void WebContentsImpl::SelectLinkText() {
  RenderFrameHost* focused_frame = GetFocusedFrame();
  if (!focused_frame)
    return;

  focused_frame->Send(
      new InputMsg_SelectLinkText(focused_frame->GetRoutingID()));
}
...
void WebContentsImpl::DidReplaceDateTime(const std::string& value) {
  Send(new ViewMsg_ReplaceDateTime_String(GetRenderViewHost()->GetRoutingID(),
                                          value));
}

void WebContentsImpl::DidCancelDialog() {
#if defined(OS_ANDROID) || defined(USE_EFL)
  Send(new ViewMsg_CancelDateTimeDialog(GetRenderViewHost()->GetRoutingID()));
#endif
}
...
// VRBrowser++
void WebContentsImpl::InsertCSSIncludingIFrame(const std::string& css) {
  // In VR mode, CSS should be inserted into iframe.
  FrameTreeNode* rootNode = frame_tree_.root();
  if (rootNode && rootNode->child_count()) {
    for (unsigned int i = 0; i < rootNode->child_count(); i++) {
      FrameTreeNode* childNode = rootNode->child_at(i);
      if (childNode && childNode->current_frame_host()) {
        childNode->current_frame_host()->Send(new FrameMsg_CSSInsertRequest(
            childNode->current_frame_host()->GetRoutingID(), css));
      }
    }
  }
}
...
#if defined(S_TERRACE_SUPPORT)
  // For detect whether theme color is set ColorTRANSPARENT or not.
  LOG(INFO) << "WebContentsImpl::DidNavigateMainFramePostCommit "
            << "theme_color is set: " << theme_color_;
#endif
...
#if defined(S_TERRACE_SUPPORT)
  // For detect whether theme color is properly changed or not.
  LOG(INFO) << "WebContentsImpl::OnThemeColorChanged "
            << "theme_color: " << theme_color_
            << " last_sent_theme_color: " << last_sent_theme_color_;
#endif
}
...
void WebContentsImpl::OnDidFinishLoad(RenderFrameHostImpl* source,
  ...
  if (theme_color_ != last_sent_theme_color_) {
    // Theme color should be updated if it was not.
    for (auto& observer : observers_)
      observer.DidChangeThemeColor(theme_color_);
    last_sent_theme_color_ = theme_color_;
  }
...
#if defined(S_TERRACE_SUPPORT)
  // For detect whether theme color is properly changed.
  LOG(INFO) << "WebContentsImpl::OnDidFinishLoad "
            << "theme_color: " << theme_color_
            << " last_sent_theme_color: " << last_sent_theme_color_;
#endif
...
void WebContentsImpl::OnRegisterContentHandler(const std::string& mime_type,
                                               const GURL& url,
                                               const base::string16& title,
                                               bool user_gesture) {
  if (!delegate_)
  ...
  delegate_->RegisterContentHandler(this, mime_type, url, user_gesture);
}

void WebContentsImpl::OnUnregisterContentHandler(const std::string& mime_type,
                                                 const GURL& url,
                                                 bool user_gesture) {
  if (!delegate_)
    return;

  delegate_->UnregisterContentHandler(this, mime_type, url, user_gesture);
}
...
// SBrowser Extension : Content Recommender
void WebContentsImpl::OnDidGetMetadata(RenderFrameHostImpl* source,
                                       const std::string& metadata) {
  for (auto& observer : observers_)
    observer.DidGetMeta(source, metadata);
}
...
#if defined(S_TERRACE_SUPPORT)
void WebContentsImpl::OnOpenWebLinkMatchInfosReply(
    RenderFrameHostImpl* source,
    int version,
    const std::vector<gfx::Point>& points,
    const std::vector<GURL> urls) {
  GetOrCreateFindRequestManager()->OnOpenWebLinkMatchInfosReply(source, version,
                                                                points, urls);
}
#endif
...
void WebContentsImpl::OnOpenDateTimeDialog(
  ...
  if (base::VrModeUtil::isDayDreamModeEnabled()) {
    delegate_->OpenDateTimeDialog(value.dialog_type, value.dialog_value,
                                  value.minimum, value.maximum, value.step,
                                  value.suggestions);
    DidCancelDialog();
    return;
  }
...
void WebContentsImpl::OnOpenColorChooser(
  ...
  ColorChooser* new_color_chooser = delegate_ ?
      ...
      NULL;
  if (!new_color_chooser && !base::VrModeUtil::isDayDreamModeEnabled())
    return;
  if (color_chooser_info_.get())
    color_chooser_info_->chooser->End();
...
#if defined(S_TERRACE_SUPPORT)
  // For detect whether theme color is properly changed.
  LOG(INFO) << "WebContentsImpl::OnFirstVisuallyNonEmptyPaint "
            << "theme_color: " << theme_color_
            << " last_sent_theme_color: " << last_sent_theme_color_;
#endif
}
...
#if defined(S_TERRACE_SUPPORT)
void WebContentsImpl::SetLastSentThemeColor(SkColor theme_color) {
  last_sent_theme_color_ = theme_color;
}

SkColor WebContentsImpl::GetLastSentThemeColor() const {
  return last_sent_theme_color_;
}
#endif
...
void WebContentsImpl::OnFocusedElementChangedInFrame(
  ...
  root_view->FocusedNodeChanged(frame->has_focused_editable_element(),
                                bounds_in_screen, is_select_node, node_id);
#else
  root_view->FocusedNodeChanged(frame->has_focused_editable_element(),
                                bounds_in_screen
#if defined(OS_TIZEN_TV_PRODUCT)
...
#if defined(S_TERRACE_SUPPORT)
void WebContentsImpl::RequestOpenWebLinkMatchInfos(int current_version) {
  GetOrCreateFindRequestManager()->RequestOpenWebLinkMatchInfos(
      current_version);
}
#endif
...
void WebContentsImpl::SetIMERecommendedWords(
    const std::string& recommended_words) {}

void WebContentsImpl::SetIMERecommendedWordsType(bool should_enable) {}

void WebContentsImpl::SetUserInputURLNeedNotify(bool is_url_need_notify) {
  is_url_need_notify_ = is_url_need_notify;
}

bool WebContentsImpl::GetUserInputURLNeedNotify() const {
  return is_url_need_notify_;
}
#endif
...
#if defined(OS_TIZEN_TV_PRODUCT)
  if (dialog_manager_)
    dialog_manager_->OnDialogClosed(this);
#endif
...
#if defined(S_TERRACE_SUPPORT)
void WebContentsImpl::NotifyOpenWebLinkMatchInfosReply(
    int version,
    const std::vector<gfx::Point>& points,
    const std::vector<GURL> urls) {
  if (delegate_)
    delegate_->OpenWebLinkMatchInfosReply(this, version, points, urls);
}
#endif
#endif
