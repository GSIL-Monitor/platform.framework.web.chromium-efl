...
class CONTENT_EXPORT WebContentsImpl : public WebContents,
 ...
 public:
  ...
  // VRBrowser++
  void InsertCSSIncludingIFrame(const std::string& css) override;
  // VRBrowser--
  ...
  void RequestOpenWebLinkMatchInfos(int current_version) override;
#endif
  ...
  void MoveRangeSelectionExtent(const gfx::Point& extent,
                                bool is_last_touch_point = false) override;
  ...
  void PopulateSoftwareBitmapFinished(const SkBitmap& bitmap,
...
#if defined(S_TERRACE_SUPPORT)
  void NotifyOpenWebLinkMatchInfosReply(int version,
 ...
 private:
...
#if defined(S_TERRACE_SUPPORT)
  void SetLastSentThemeColor(SkColor theme_color) override;
  SkColor GetLastSentThemeColor() const override;
#endif
  ...
  void OnUnregisterProtocolHandler(RenderFrameHostImpl* source,
                                   ...
                                   const GURL& url,
                                   bool user_gesture);
  void OnRegisterContentHandler(const std::string& mime_type,
                                const GURL& url,
                                const base::string16& title,
                                bool user_gesture);
  void OnUnregisterContentHandler(const std::string& mime_type,
                                  const GURL& url,
                                  bool user_gesture);
  ...
  void OnGetNearestFindResultReply(RenderFrameHostImpl* source,
                                   ...
                                   float distance);
#if defined(S_TERRACE_SUPPORT)
  void OnOpenWebLinkMatchInfosReply(RenderFrameHostImpl* source,
                                    int version,
                                    const std::vector<gfx::Point>& points,
                                    const std::vector<GURL> urls);
#endif
  ...
  void OnDidGetMetadata(RenderFrameHostImpl* source,
