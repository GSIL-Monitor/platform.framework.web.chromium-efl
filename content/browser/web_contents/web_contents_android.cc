  ...
  if (web_contents_->ShowingInterstitialPage()) {
    content::RenderViewHost* interstitial_view_host =
        web_contents_->GetInterstitialPage()
            ->GetMainFrame()
            ->GetRenderViewHost();
    interstitial_view_host->Send(new ViewMsg_UpdateBrowserControlsState(
        interstitial_view_host->GetRoutingID(), enable_hiding, enable_showing,
        animate));
  }
}
...
// VRBrowser++
void WebContentsAndroid::InsertCSSIncludingIFrame(
    JNIEnv* env,
    const JavaParamRef<jobject>& jobj,
    const JavaParamRef<jstring>& jcss) {
  web_contents_->InsertCSSIncludingIFrame(
      base::android::ConvertJavaStringToUTF8(env, jcss));
}
// VRBrowser--
