...
RenderWidgetHostViewBase* WebContentsViewAndroid::CreateViewForWidget(
  ...
  RenderWidgetHostViewAndroid* rwhv =
      TerraceContentBridge::Get().CreateRenderWidgetHostViewAndroid(
...
RenderWidgetHostViewBase* WebContentsViewAndroid::CreateViewForPopupWidget(
  ...
  RenderWidgetHostImpl* rwhi = RenderWidgetHostImpl::From(render_widget_host);
#if defined(S_TERRACE_SUPPORT)
  return TerraceContentBridge::Get().CreateRenderWidgetHostViewAndroid(
      rwhi, nullptr);
#else
...
#if defined(S_TERRACE_SUPPORT)
  if (!content_view_core_ || drop_data.text.is_null()) {
    OnDragEnded();
    return;
  }
  RenderWidgetHostViewAndroid* rwhv =
      content_view_core_->GetRenderWidgetHostViewAndroid();

  if (!rwhv) {
    OnDragEnded();
    return;
  }
  if (rwhv->SelectionController()->GetRectBetweenBounds().Contains(
          event_info.event_location.x(), event_info.event_location.y())) {
    // Text Selection Drag
    web_contents_->GetRenderViewHost()->GetSelectionMarkup();
    web_contents_->GetRenderViewHost()->GetSelectionBitmap();
  } else {
    // Explicit Link Drag
    SelectedMarkup(drop_data.text.string());
...
void WebContentsViewAndroid::SelectedMarkup(const base::string16& markup) {
  if (content_view_core_) content_view_core_->SelectedMarkup(markup);
}

void WebContentsViewAndroid::SelectedMarkupWithStartContentRect(
    const base::string16& markup, const gfx::Rect& start_rect) {
  if (content_view_core_) {
    content_view_core_->SelectedMarkupWithStartContentRect(markup, start_rect);
  }
}

void WebContentsViewAndroid::SelectedBitmap(const SkBitmap& bitmap) {
  if (content_view_core_) content_view_core_->SelectedBitmap(bitmap);
}
