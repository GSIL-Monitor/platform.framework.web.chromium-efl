...
void MimeSniffingResourceHandler::OnResponseStarted(
  ...
  if (!(response_->head.headers.get() &&
    ...
    // override new mime type
    std::string new_mime_type;
    if (host_->delegate() && host_->delegate()->ShouldOverrideMimeType(
                                 request()->url(), new_mime_type)) {
      if (blink::IsSupportedMimeType(new_mime_type))
        response_->head.mime_type.assign(new_mime_type);
    }
  }
