...
#if defined(S_TERRACE_SUPPORT)
    // Samsung sound policy allows only one media in same time.
    // Actually it was handled by MediaPlayerListener before M51.
    // The upstream has been changed to using MediaSession for all media.
...
#if defined(S_TERRACE_SUPPORT)
  // Need to set state to INACTIVE and abandonAudioFocus if content is paused.
  Stop(SuspendType::CONTENT);
#else
...
#if defined(S_TERRACE_SUPPORT)
  delegate_ = TerraceContentBridge::Get().CreateAudioFocusDelegate(this);
#else
