...
base::FilePath SavePackage::GetSuggestedNameForSaveAs(
  ...
  t.LocalExplode(&tex);
  return base::FilePath(base::StringPrintf("%02d%02d%02d%02d%02d",
...
void SavePackage::FinalizeDownloadEntry() {
  ...
  base::FilePath::StringType fileName = download_->GetFullPath().value();
  DCHECK(web_contents());
  if (web_contents() && web_contents()->GetDelegate())
    web_contents()->GetDelegate()->PageSavedAs(fileName);
  download_ = nullptr;
