...
void DOMStorageContextWrapper::UpdatePath(const base::FilePath& path) {
  DCHECK(context_.get());
  context_->UpdatePath(path.AppendASCII(kLocalStorageDirectory),
                       path.AppendASCII(kSessionStorageDirectory));
  mojo_state_->UpdatePath(path.AppendASCII(kLocalStorageDirectory));
}
#endif
