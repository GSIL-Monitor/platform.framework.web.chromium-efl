...
#if defined(TIZEN_ATK_SUPPORT)
  if (IsMobileProfile()) {
    if (manager_->GetRoot() == this) {
      gfx::Vector2dF vector2df =
          manager_->delegate()->AccessibilityGetScrollOffset();
      bounds.Offset(-ToFlooredVector2d(vector2df));
    }
    float page_scale_factor =
        manager_->delegate()->AccessibilityGetPageScaleFactor();
    bounds = ConvertRectToPixel(page_scale_factor, bounds);
  }
#endif
