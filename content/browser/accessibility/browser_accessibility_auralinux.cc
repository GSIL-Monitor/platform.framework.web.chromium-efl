...
static AtkObject* browser_accessibility_accessible_at_point(
  ...
  if (IsMobileProfile()) {
    ...
    point = ConvertPointToDIP(device_scale_factor, point);
    result = ToBrowserAccessibilityEfl(obj)->GetChildAtPoint(point);
  }
#endif
  if (!result)
    return NULL;
...
#if defined(TIZEN_ATK_SUPPORT)
  if (IsMobileProfile())
    bounds = ToBrowserAccessibilityEfl(obj)->GetScaledGlobalBoundsRect();
#endif
  if (x)
    *x = bounds.x();
  if (y)
...
#if defined(TIZEN_ATK_SUPPORT)
static gboolean browser_accessibility_grab_highlight(
    AtkComponent* atk_component) {
  if (!IsMobileProfile())
    return FALSE;
  ...
  auto obj = ToBrowserAccessibilityAuraLinux(atk_component);
  if (!obj)
    return FALSE;
  ...
  auto manager_efl = obj->manager()->ToBrowserAccessibilityManagerEfl();
  manager_efl->SetHighlighted(obj);
  return TRUE;
}

static gboolean browser_accessibility_clear_highlight(
    AtkComponent* atk_component) {
  if (!IsMobileProfile())
    return FALSE;
  ...
  auto manager_efl = obj->manager()->ToBrowserAccessibilityManagerEfl();
  manager_efl->MaybeInvalidateHighlighted(obj);
  return TRUE;
}
#endif
...
#if defined(TIZEN_ATK_SUPPORT)
  if (IsMobileProfile()) {
    iface->grab_highlight = browser_accessibility_grab_highlight;
    iface->clear_highlight = browser_accessibility_clear_highlight;
  }
#endif
}
...
#if defined(TIZEN_ATK_SUPPORT)
  if (IsMobileProfile())
    img_pos_size = ToBrowserAccessibilityEfl(obj)->GetScaledGlobalBoundsRect();
#endif
...
static AtkRole browser_accessibility_get_role(AtkObject* atk_object) {
  ...
  std::string text;
  if (obj->GetHtmlAttribute("role", &text) && text.empty() &&
      obj->HasStringAttribute(ui::AX_ATTR_ROLE))
    return ATK_ROLE_INVALID;
#endif
...
#if defined(TIZEN_ATK_SUPPORT)
  if (IsMobileProfile()) {
    if (ToBrowserAccessibilityEfl(obj)->is_highlightable())
      atk_state_set_add_state(state_set, ATK_STATE_HIGHLIGHTABLE);
    atk_state_set_add_state(state_set, ATK_STATE_SHOWING);
    atk_state_set_add_state(state_set, ATK_STATE_VISIBLE);
    if (obj->manager()) {
      auto manager_efl = obj->manager()->ToBrowserAccessibilityManagerEfl();
      if (manager_efl->IsHighlighted(obj))
        atk_state_set_add_state(state_set, ATK_STATE_HIGHLIGHTED);
    }
  }
#endif
...
// it's best to leave this out rather than break people's builds:
  ...
  // Support checked state. It's the same as in the function of
  // AXPlatformNodeAuraLinux::GetAtkState()
  const auto checked_state = static_cast<ui::AXCheckedState>(
      obj->GetIntAttribute(ui::AX_ATTR_CHECKED_STATE));
  switch (checked_state) {
    case ui::AX_CHECKED_STATE_MIXED:
      atk_state_set_add_state(state_set, ATK_STATE_INDETERMINATE);
      break;
    case ui::AX_CHECKED_STATE_TRUE:
      atk_state_set_add_state(state_set,
                              obj->GetRole() == ui::AX_ROLE_TOGGLE_BUTTON
                                  ? ATK_STATE_PRESSED
                                  : ATK_STATE_CHECKED);
      break;
    default:
...
#if defined(TIZEN_ATK_SUPPORT)
  ToBrowserAccessibilityEfl(obj)->SetInterfaceMaskFromObject(interface_mask);
#endif
...
#if defined(TIZEN_ATK_SUPPORT)
  if (interface_mask & (1 << ATK_EDITABLE_TEXT_INTERFACE))
    g_type_add_interface_static(type, ATK_TYPE_EDITABLE_TEXT, &ActionInfo);

  ToBrowserAccessibilityEfl(obj)->SetInterfaceFromObject(type, interface_mask);
#endif
...
void BrowserAccessibilityAuraLinux::OnDataChanged() {
...
#if defined(TIZEN_ATK_SUPPORT)
  if (IsMobileProfile())
    ToBrowserAccessibilityEfl(this)->RecalculateHighlightable(true);
#endif
  ...
  if (!atk_object_) {
    ...
    if (this->PlatformGetParent()) {
      ...
      if (root && (this == ToBrowserAccessibilityAuraLinux(root))) {
        auto manager_auralinux =
            manager()->ToBrowserAccessibilityManagerAuraLinux();
        if (manager_auralinux->parent_object()) {
          atk_object_set_parent(atk_object_,
                                manager_auralinux->parent_object());
        }
      }
    }
#endif
