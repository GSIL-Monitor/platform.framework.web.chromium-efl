...
void BrowserAccessibilityManager::OnAccessibilityEvents(
  ...
  for (uint32_t index = 0; index < details.size(); ++index) {
    ...
    // For blur event, last_focused_node_ should be set to root or nullptr
    ...
    if (event_type == ui::AX_EVENT_BLUR && last_focused_node_)
...
#if defined(TIZEN_ATK_FEATURE_VD)
    // Tizen screen reader can't response the change for descendants of live
    // region, so need to propagate live region change event for them
    if (object && object->HasStringAttribute(ui::AX_ATTR_CONTAINER_LIVE_STATUS))
      tree_events_[object->GetId()].insert(ui::AX_EVENT_LIVE_REGION_CHANGED);
#endif
  }
}
