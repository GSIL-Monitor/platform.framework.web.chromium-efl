...
StoragePartitionImpl::~StoragePartitionImpl() {
  ...
  for (auto& observer : observers_)
    observer.StoragePartitionWillExit(this);
#endif
...
#if defined(OS_TIZEN_TV_PRODUCT)
  partition->quota_manager_->StoragePartitionReady(partition.get());
  partition->filesystem_context_->StoragePartitionReady(partition.get());
  partition->database_tracker_->StoragePartitionReady(partition.get());
  partition->dom_storage_context_->StoragePartitionReady(partition.get());
  partition->indexed_db_context_->StoragePartitionReady(partition.get());
  partition->cache_storage_context_->StoragePartitionReady(partition.get());
  partition->service_worker_context_->StoragePartitionReady(partition.get());
  partition->appcache_service_->StoragePartitionReady(partition.get());
  partition->platform_notification_context_->StoragePartitionReady(
      partition.get());
#endif
...
#if defined(OS_TIZEN_TV_PRODUCT)
void StoragePartitionImpl::AddObserver(StoragePartitionPathObserver* observer) {
  observers_.AddObserver(observer);
}

void StoragePartitionImpl::RemoveObserver(
    StoragePartitionPathObserver* observer) {
  observers_.RemoveObserver(observer);
}

void StoragePartitionImpl::UpdatePath(const base::FilePath& path) {
  partition_path_ = path;

  file::AssociateServiceUserIdWithUserDir(
      BrowserContext::GetServiceUserIdFor(browser_context_), path);
