...
void ZygoteCommunication::AppendCommandLineSwitches(
    const std::vector<std::string>& v) {
  DCHECK(init_);
  base::Pickle pickle;

  pickle.WriteInt(kZygoteCommandAppendCommandLineSwitches);
  pickle.WriteUInt32(v.size());
  for (size_t i = 0; i < v.size(); ++i)
    pickle.WriteString(v[i]);
  if (!SendMessage(pickle, NULL))
    LOG(ERROR) << "Failed to send AppendCommandLineSwitches message to zygote";
}
