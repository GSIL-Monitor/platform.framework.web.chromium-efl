...
pid_t ZygoteHostImpl::LaunchZygote(base::CommandLine* cmd_line,
  ...
  // To enable setuid sandbox for NaCl under --no-sandbox flag,
  // chrome-sandbox binary needs to be run with suid permission. If zygote sets
  // prctl(PR_SET_NO_NEW_PRIVS, 1, 0, 0, 0)
  // then it won't be possible to run chrome-sandbox with suid permission.
  //
  // It should be enabled only if namespace sandbox is disabled.
  const base::CommandLine& command_line =
      *base::CommandLine::ForCurrentProcess();
  if (!command_line.HasSwitch(switches::kDisableSetuidSandbox) &&
      (!sandbox::Credentials::CanCreateProcessInNewUserNS() ||
