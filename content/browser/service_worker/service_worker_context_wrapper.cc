...
void ServiceWorkerContextWrapper::UpdatePath(const base::FilePath& path) {
  if (!BrowserThread::CurrentlyOn(BrowserThread::IO)) {
    BrowserThread::PostTask(
        BrowserThread::IO, FROM_HERE,
        base::Bind(&ServiceWorkerContextWrapper::UpdatePath, this, path));
    return;
  }

  is_incognito_ = path.empty();
  if (context_core_.get())
    context_core_->UpdatePath(path);
}
#endif
