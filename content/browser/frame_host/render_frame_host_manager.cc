...
bool RenderFrameHostManager::ShouldTransitionCrossSite() {
  ...
  // False when limited to use one render process, which does not support
  // cross-process
  // navigations or OOPIFs.
  bool single_renderer_mode =
      (base::CommandLine::ForCurrentProcess()->GetSwitchValueASCII(
           switches::kRendererProcessLimit) == "1") &&
      base::CommandLine::ForCurrentProcess()->HasSwitch(
          switches::kProcessPerTab);

  return !single_process_mode && !single_renderer_mode;
#endif

  return !single_process_mode;
}
