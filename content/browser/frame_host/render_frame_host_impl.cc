...
RenderFrameHostImpl::RenderFrameHostImpl(SiteInstance* site_instance,
    ...
    : render_view_host_(render_view_host),
      ...
      select_dialog_advanced_ime_options_(0),
...
gfx::Rect RenderFrameHostImpl::AccessibilityGetViewBounds() const {
  RenderWidgetHostView* view = render_view_host_->GetWidget()->GetView();
  // TODO(djmix.kim@samsung.com) : TIZEN_ATK_SUPPORT
  // Need to change from view->GetViewBounds() to
  // view_efl->GetVisibleViewBounds().
  // https://review.tizen.org/gerrit/#/c/108505/
  if (view)
...
void RenderFrameHostImpl::OnShowPopup(
  ...
  select_dialog_advanced_ime_options_ = params.advanced_ime_options;
...
#if defined(USE_TTRACE)
  traceBegin(TTRACE_TAG_WEB, "RenderFrameHostImpl::Navigate url: %s",
             common_params.url.spec().c_str());
#endif

  UpdatePermissionsForNavigation(common_params, request_params);
  LOG(INFO) << "LoaderLog Navigate navigations_suspended = "
...
void RenderFrameHostImpl::SendNavigateMessage(
  ...
  LOG(INFO) << "LoaderLog send FrameMsg_Navigate";
...
#if defined(S_TERRACE_SUPPORT)
void RenderFrameHostImpl::CreateFullscreenVideoContextMenu(GURL url) {
  ContextMenuParams video_params;
  video_params.src_url = url;
  video_params.media_type = blink::WebContextMenuData::kMediaTypeVideo;
  OnContextMenu(video_params);
}
#endif
...
#if defined(TIZEN_ATK_SUPPORT)
float RenderFrameHostImpl::AccessibilityGetPageScaleFactor() const {
  EWebView* web_view = web_contents_utils::WebViewFromWebContents(
      WebContents::FromRenderViewHost(render_view_host_));
  return web_view->GetScale();
}

gfx::Vector2dF RenderFrameHostImpl::AccessibilityGetScrollOffset() {
  return GetViewForAccessibility()->GetLastScrollOffset();
}
#endif
