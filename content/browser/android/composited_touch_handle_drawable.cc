...
const float kHandleRightHorizontalPaddingRatio = 0.25f;
const float kHandleLeftHorizontalPaddingRatio = 0.75f;
// Max value for handler scale up
const float kMaxHandlerScaleSize = 1.5f;
#endif
...
CompositedTouchHandleDrawable::~CompositedTouchHandleDrawable() {
  g_selection_resources.Get().ResetResources();
  DetachLayer();
}
...
#if defined(S_TERRACE_SUPPORT)
void CompositedTouchHandleDrawable::SetScale(float scale) {
  DCHECK(layer_->parent());
...
void CompositedTouchHandleDrawable::SetOriginForScale() {
  DCHECK(layer_->parent());
  float layer_width = layer_->bounds().width();
  if (orientation_ == ui::TouchHandleOrientation::RIGHT /*||
      orientation_ == ui::TouchHandleOrientation::LEFT_FLIPPED*/) {
    layer_width *= kHandleRightHorizontalPaddingRatio;
  } else {
    layer_width *= kHandleLeftHorizontalPaddingRatio;
  }
  layer_->SetTransformOrigin(gfx::Point3F(layer_width, 0.f, 0.f));
}
#endif
