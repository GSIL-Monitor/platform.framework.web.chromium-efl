...
OverscrollControllerAndroid::OverscrollControllerAndroid(
    ...
    : compositor_(compositor),
      ...
      block_pull_to_refresh_effect_(false),
#endif
...
void OverscrollControllerAndroid::OnGestureEventAck(
  ...
  if (event.GetType() == blink::WebInputEvent::kGestureScrollUpdate &&
    ...
    // https://codereview.chromium.org/2884423003/patch/780001/790012
    // if (block_pull_to_refresh_effect_)
    //   consumed = true;
#endif
...
#if defined(S_TERRACE_SUPPORT)
void OverscrollControllerAndroid::BlockPullToRefreshEffect() {
  if (!refresh_effect_)
    return;
  block_pull_to_refresh_effect_ = true;
}
#endif

void OverscrollControllerAndroid::SetEdgeEffectProperties(
    SkColor theme_color,
    SkScalar edge_intensity_factor) {
  DCHECK(glow_effect_.get());
  glow_effect_->SetEdgeEffectProperties(theme_color, edge_intensity_factor);
}
