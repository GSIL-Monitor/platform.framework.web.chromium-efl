...
void WebContentsObserverProxy::DidFailLoadInternal(
  ...
  Java_WebContentsObserverProxy_didFailLoad(
      env, java_observer_, is_provisional_load, is_main_frame, error_code,
...
void WebContentsObserverProxy::DidFailLoad(
  ...
  Java_WebContentsObserverProxy_didFailLoad(
      env, java_observer_, false, !render_frame_host->GetParent(), error_code,
      jstring_error_description, jstring_url);
}
...
void WebContentsObserverProxy::DidGetMeta(
    RenderFrameHost* render_frame_host,
    const std::string& meta_elements) {
  JNIEnv* env = AttachCurrentThread();
  ScopedJavaLocalRef<jobject> obj(java_observer_);
  ScopedJavaLocalRef<jstring> jstring_meta_elements(
      ConvertUTF8ToJavaString(env, meta_elements));

  Java_WebContentsObserverProxy_didGetMeta(
      env, obj, render_frame_host->GetRoutingID(),
      !render_frame_host->GetParent(),
      jstring_meta_elements);
}
