...
void ContentViewCore::OnGestureEventAck(const blink::WebGestureEvent& event,
  ...
  switch (event.GetType()) {
    ...
    case WebInputEvent::kGestureLongPress:
      ...
      // S_TERRACE_SUPPORT flag can't be used as opensource build will fail
      /*if (ack_result == INPUT_EVENT_ACK_STATE_CONSUMED)
...
void ContentViewCore::SelectPopupMenuItems(
  ...
  // Fix monkey test crash (P160725-01313)
  // The renderFrameHost associated with selectPopupSourceFrame is destroyed.
  // As the select popup menu destruction flow is not synchronous, before the
  // popup is actually hidden a click is performed and control comes here.
#if defined(S_TERRACE_SUPPORT)
  if (rfhi == NULL)
    return;
#else
...
#if defined(S_TERRACE_SUPPORT)
  ContentViewCore* view = TerraceContentBridge::Get().CreateContentViewCore(
#else
  ContentViewCore* view = new ContentViewCore(
#endif
    env, obj, web_contents, dip_scale);
