...
class CONTENT_EXPORT FindRequestManager {
 public:
  ...
  // Requests the infos of the current find matches from the renderer process.
  void RequestOpenWebLinkMatchInfos(int current_version);

  // Called when a reply is received from a frame in response to a request for
  // find match infos.
  void OnOpenWebLinkMatchInfosReply(RenderFrameHost* rfh,
                                    int version,
...
#if defined(S_TERRACE_SUPPORT)
  // Called when a find match infos reply is no longer pending for a frame.
  void RemoveOpenWebLinkMatchInfosPendingReply(RenderFrameHost* rfh);
#endif
...
#if defined(S_TERRACE_SUPPORT)
    // The points contained in a single frame.
    std::vector<gfx::Point> points;

    // The urls contained in a single frame.
...
#if defined(S_TERRACE_SUPPORT)
    FrameRects(const std::vector<gfx::Point>& points,
               const std::vector<GURL> urls,
               int version);
#endif
    ~FrameRects();
