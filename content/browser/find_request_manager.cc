...
FindRequestManager::FrameRects::FrameRects(
    ...
    int version)
    : points(points), urls(urls), version(version) {}
#endif
FindRequestManager::FrameRects::~FrameRects() {}
...
#if defined(S_TERRACE_SUPPORT)
  RemoveOpenWebLinkMatchInfosPendingReply(rfh);
#endif
#endif
...
#if defined(S_TERRACE_SUPPORT)
void FindRequestManager::RequestOpenWebLinkMatchInfos(int current_version) {
  match_rects_.pending_replies.clear();
  match_rects_.request_version = current_version;
  ...
  for (FrameTreeNode* node : contents_->GetFrameTree()->Nodes()) {
    ...
    int version = (it != match_rects_.frame_rects.end()) ? it->second.version
                                                         : kInvalidId;
    rfh->Send(new FrameMsg_OpenWebLinkMatchInfos(rfh->GetRoutingID(), version));
  }
}

void FindRequestManager::OnOpenWebLinkMatchInfosReply(
    RenderFrameHost* rfh,
    int version,
    const std::vector<gfx::Point>& points,
    const std::vector<GURL> urls) {
  auto it = match_rects_.frame_rects.find(rfh);
  if (it == match_rects_.frame_rects.end() || it->second.version != version) {
    // New version of infos has been received, so update the data.
    match_rects_.frame_rects[rfh] = FrameRects(points, urls, version);
    ++match_rects_.known_version;
  }
  RemoveOpenWebLinkMatchInfosPendingReply(rfh);
}
...
#if defined(S_TERRACE_SUPPORT)
void FindRequestManager::RemoveOpenWebLinkMatchInfosPendingReply(
    RenderFrameHost* rfh) {
  auto it = match_rects_.pending_replies.find(rfh);
  if (it == match_rects_.pending_replies.end())
    return;
  ...
  if (match_rects_.pending_replies.empty()) {
    // All replies are in.
    std::vector<gfx::Point> aggregate_points;
    std::vector<GURL> aggregate_urls;
    if (match_rects_.request_version != match_rects_.known_version) {
      for (RenderFrameHost* frame = GetInitialFrame(true /* forward */); frame;
        ...
        std::vector<gfx::Point>& frame_points = it->second.points;
        aggregate_points.insert(aggregate_points.end(), frame_points.begin(),
                                frame_points.end());
        std::vector<GURL>& frame_rects_urls = it->second.urls;
        aggregate_urls.insert(aggregate_urls.end(), frame_rects_urls.begin(),
                              frame_rects_urls.end());
      }
    }

    contents_->NotifyOpenWebLinkMatchInfosReply(
        match_rects_.known_version, aggregate_points, aggregate_urls);
  }
}
#endif
#endif
