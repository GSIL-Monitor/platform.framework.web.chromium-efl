...
WeakPtr<AutofillPopupControllerImpl> AutofillPopupControllerImpl::GetOrCreate(
  ...
  if (previous.get()) {
    previous->Hide();
    // FIXME: TinAutofillPopupControllerImpl is a kind of singleton.
    previous->DestroyPopupControllerImpl();
  }
...
void AutofillPopupControllerImpl::Show(
  ...
  if (just_created) {
    view_->UseCreditCardHelper(use_credit_card_helper_);
    view_->UseUpiHelper(use_upi_helper_);
    view_->Show();
  } else {
...
void AutofillPopupControllerImpl::Hide() {
  ...
  // FIXME: SBrowser / AutofillPopupControllerImpl is permanent
  // this is deleted when page is destroyed by autofill_client
  // delete this;
  view_ = NULL;
  is_above_anchor_ = false;
  ClearState();
}
...
void AutofillPopupControllerImpl::AcceptSuggestion(int index) {
  if (index == -1) {
    ...
    suggestion.frontend_id = POPUP_ITEM_ID_POPUP_HELPER;
