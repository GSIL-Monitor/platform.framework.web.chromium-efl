...
bool Zygote::HandleRequestFromBrowser(int fd) {
  ...
  if (iter.ReadInt(&kind)) {
    switch (kind) {
      ...
      case kZygoteCommandAppendCommandLineSwitches:
        HandleAppendCommandLineSwitches(iter);
        return false;
#endif
#if defined(OS_TIZEN_TV_PRODUCT)
...
#if defined(TIZEN_PEPPER_EXTENSIONS) && defined(OS_TIZEN_TV_PRODUCT)
void Zygote::HandleAppendCommandLineSwitches(base::PickleIterator iter) {
  std::string command_line_switch;
  uint32_t size;
  if (!iter.ReadUInt32(&size))
    return;
  for (uint32_t i = 0; i < size; ++i) {
    if (!iter.ReadString(&command_line_switch))
      return;
    base::CommandLine::ForCurrentProcess()->AppendSwitch(command_line_switch);
  }
}
#endif
