...
#if !defined(TIZEN_PEPPER_EXTENSIONS)  // Don't preload plugins on Tizen TVs
  PreloadPepperPlugins();
#else
  bool running_with_sandbox =
      !base::CommandLine::ForCurrentProcess()->HasSwitch(switches::kNoSandbox);
  LOG_IF(ERROR, running_with_sandbox)
      << "Running with sandbox enabled. Tizen Pepper Plugins extensions won't "
         "be loaded properly";
#endif
#endif
#if BUILDFLAG(ENABLE_WEBRTC)
