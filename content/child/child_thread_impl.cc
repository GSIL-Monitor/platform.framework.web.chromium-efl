...
// plugins), PluginThread has EnsureTerminateMessageFilter.
    ...
    defined(UNDEFINED_SANITIZER) || defined(USE_EFL)
...
class SuicideOnChannelErrorFilter : public IPC::MessageFilter {
 public:
  ...
  void OnChannelError() override {
    ...
    defined(UNDEFINED_SANITIZER) || defined(USE_EFL)
#if defined(USE_EFL)
    // On Tizen, DynamicPlugin::stopSession has to be called before
    // renderer process is terminate, so that WRT can shutdown properly.
    // Instead of exiting the process immediately after getting detached from
    // the Browser Process, wait an extra 60 second in order to get the
    // renderer process destruction chain finalized
    if (!CreateWaitAndExitThread(base::TimeDelta::FromSeconds(60))) {
      LOG(ERROR) << "Failed to create wait and exit thread";
      _exit(0);
    }
#else
