...
void CheckSchemeForReferrerPolicy(const ResourceRequest& request) {
  if ((request.referrer_policy == blink::kWebReferrerPolicyDefault ||
    ...
    LOG(WARNING) << "Trying to send secure referrer for insecure request "
...
#if defined(S_TERRACE_SUPPORT)
  if (message.type() == ResourceMsg_SetElementHidingRules::ID ||
      message.type() == ResourceMsg_SetGlobalElementHidingRules::ID) {
    // FIXME: maybe we should go through the same path as other messages?
...
#if defined(S_TERRACE_SUPPORT)
    IPC_MESSAGE_HANDLER(ResourceMsg_SetElementHidingRules,
                        OnSetElementHidingRules)
    IPC_MESSAGE_HANDLER(ResourceMsg_SetGlobalElementHidingRules,
                        OnSetGlobalElementHidingRules)
#endif
  IPC_END_MESSAGE_MAP()
}
...
#if defined(USE_TTRACE)
  traceBegin(TTRACE_TAG_WEB, "ResourceDispatcher::StartSync url: %s",
             request->url.spec().substr(0, 128).c_str());
#endif
...
#if defined(USE_TTRACE)
  traceBegin(TTRACE_TAG_WEB, "ResourceDispatcher::StartAsync url: %s",
             request->url.spec().substr(0, 128).c_str());
#endif
...
#if defined(S_TERRACE_SUPPORT)
void ResourceDispatcher::OnSetElementHidingRules(
    const bool is_default,
    const bool enabled,
    const std::vector<std::string>& selectors,
    const std::vector<std::string>& whitelist_selectors) {
  blink::WebContentBlockElementHiding::SetEnabled(is_default, enabled);
  if (enabled) {
    blink::WebContentBlockElementHiding::SetRulesForCurrentPage(
        is_default, selectors, whitelist_selectors);
  }
}

void ResourceDispatcher::OnSetGlobalElementHidingRules(
    const bool is_default,
    const std::vector<std::string>& selectors) {
  blink::WebContentBlockElementHiding::SetGlobalRules(is_default, selectors);
}
#endif
